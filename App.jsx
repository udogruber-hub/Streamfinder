  ...
  useEffect(()=>{
    ...
    async function loadImages(){
      ...
      const genreMovies=[
        ...
        {genreId:53,  movieId:6977,   type:"movie"},  // Thriller: No Country for Old Men
            ...
            {STIMMUNGEN.map(m=>{
              ...
              return(
                <button key={m.id} onClick={()=>{if(m.id===activeStimmung){setActiveStimmung(null);setStimmungRecs([]);return;}setActiveStimmung(m.id);activeStimmungRef.current=m.id;try{localStorage.setItem('sf_active_stimmung',m.id);}catch{}loadStimmungRecs(m);}}
                  ...
                  <img key={genreImages[m.genres[0]]||m.id} src={genreImages[m.genres[0]]||""} alt="" style={{position:"absolute",inset:0,width:"100%",height:"100%",objectFit:"cover",objectPosition:"center 20%",opacity:active?0.7:0.4,transition:"opacity 0.25s"}} onError={e=>{e.target.style.display="none";}}/>
      ...
      <div style={{height:"100vh",maxHeight:"-webkit-fill-available",background:"#09090f",display:"flex",flexDirection:"column",position:"relative",overflow:"hidden"}}>
        ...
        <div style={{position:"relative",zIndex:4,padding:"54px 0 0",display:"flex",justifyContent:"center"}}>
          <div style={{background:"rgba(0,0,0,0.35)",backdropFilter:"blur(16px)",WebkitBackdropFilter:"blur(16px)",border:"1px solid rgba(255,255,255,0.1)",borderRadius:20,padding:"10px 20px"}}>
            <StreamFinderLogo size={38} showText={true}/>
          </div>
        </div>
        {/* Content unten */}
  ...
  if(step===1){
    return(
      <div style={{minHeight:"100vh",background:"linear-gradient(180deg,#09090f,#0d0d1a)",display:"flex",flexDirection:"column"}}>
        ...
        <div style={{flex:1,padding:"0 20px",overflowY:"hidden",display:"flex",flexDirection:"column"}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:8}}>
            {STIMMUNGEN.map(m=>{
              ...
              return(<button key={m.id} onClick={()=>toggleStimmung(m.id)} style={{position:"relative",borderRadius:16,overflow:"hidden",cursor:"pointer",border:"2px solid "+(sel?m.color:"transparent"),transition:"all 0.2s",height:90,background:"#0a0a12",boxShadow:sel?"0 0 24px "+m.color+"50":"none",padding:0}}>
                <img key={genreImages[m.genres[0]]||m.id} src={genreImages[m.genres[0]]||""} alt="" style={{position:"absolute",inset:0,width:"100%",height:"100%",objectFit:"cover",objectPosition:"center 20%",opacity:sel?0.75:0.35,transition:"opacity 0.2s"}} onError={e=>{e.target.style.display="none";}}/>
...
function StreamFinderLogo({size=40,showText=true}){
  ...
  return(
    <div style={{display:"flex",alignItems:"center",gap:showText?10:0}}>
      {/* Auge + Linse SVG */}
      <svg width={s} height={s*0.65} viewBox="0 0 60 39" fill="none" xmlns="http://www.w3.org/2000/svg">
        {/* Äußere Augenform */}
        <path d="M2 19.5C2 19.5 13 2 30 2C47 2 58 19.5 58 19.5C58 19.5 47 37 30 37C13 37 2 19.5 2 19.5Z"
          stroke="url(#eyeGrad)" strokeWidth="2.5" fill="none" strokeLinecap="round"/>
        {/* Äußerer Linsenring */}
        <circle cx="30" cy="19.5" r="10.5" stroke="url(#eyeGrad)" strokeWidth="2" fill="none"/>
        {/* Innerer Linsenring */}
        <circle cx="30" cy="19.5" r="5.5" fill="url(#eyeGrad)" opacity="0.9"/>
        {/* Lichtreflex */}
        <circle cx="33.5" cy="16" r="2" fill="white" opacity="0.6"/>
        {/* Linsen-Markierungen */}
        <path d="M19.5 19.5 H21.5" stroke="url(#eyeGrad)" strokeWidth="1.5" strokeLinecap="round" opacity="0.6"/>
        <path d="M38.5 19.5 H40.5" stroke="url(#eyeGrad)" strokeWidth="1.5" strokeLinecap="round" opacity="0.6"/>
        <path d="M30 9 V11" stroke="url(#eyeGrad)" strokeWidth="1.5" strokeLinecap="round" opacity="0.6"/>
        <path d="M30 28 V30" stroke="url(#eyeGrad)" strokeWidth="1.5" strokeLinecap="round" opacity="0.6"/>
        <defs>
          <linearGradient id="eyeGrad" x1="2" y1="2" x2="58" y2="37" gradientUnits="userSpaceOnUse">
            <stop offset="0%" stopColor="#c4a960"/>
            <stop offset="50%" stopColor="#e84393"/>
            <stop offset="100%" stopColor="#7c3aed"/>
          </linearGradient>
        </defs>
      </svg>
      {showText&&<span style={{fontFamily:"'Instrument Serif',serif",fontSize:s*0.55,background:"linear-gradient(135deg,#c4a960,#f5e090)",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent",letterSpacing:"-0.5px",lineHeight:1}}>StreamFinder</span>}
    </div>
  );
}
  ...
  return(
    <div style={{maxWidth:820,margin:"0 auto",minHeight:"100vh",background:"#09090f",position:"relative",fontFamily:"'DM Sans',sans-serif"}}>
      ...
      <div style={{position:"sticky",top:0,zIndex:100,background:"rgba(9,9,15,0.92)",backdropFilter:"blur(20px)",borderBottom:"1px solid rgba(255,255,255,0.05)",padding:"10px 18px",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
        <StreamFinderLogo size={32} showText={true}/>
