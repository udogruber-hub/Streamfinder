...
function buildCtx(profile){
  ...
  const watchlist=(profile.liked_items||[]).map(it=>it.title||it.name||"").slice(0,5).join(", ");
  return{platforms,top,low,blocked,watched,topGenres,watchlist};
}
...
function PlatformCard({platform,profile,onSelect,onBlock,onLike,onRate,browseType}){
  ...
  const [loading,setLoading]=useState(false);
  const [open,setOpen]=useState(false); // Standard: eingeklappt
  const lastTypeRef=useRef(null);

  // Lade KI-Empfehlungen aus localStorage + fülle auf 5 mit TMDB auf
  async function loadKiRecs(passedRecs){
    const kiRecs=passedRecs||JSON.parse(localStorage.getItem("sf_ki_recs")||"[]");
    if(!kiRecs.length)return false;
    const platRecs=kiRecs.filter(r=>{
      const rPlat=(r.platform||"").toLowerCase();
      return rPlat.includes(platform.name.toLowerCase())||platform.name.toLowerCase().includes(rPlat);
    });
    if(!platRecs.length)return false;
    setLoading(true);setRecs(null);
    try{
      const enriched=await Promise.all(platRecs.map(r=>enrichWithTMDB(r,profile)));
      let valid=enriched.filter(Boolean);
      // Auffüllen auf 5 wenn KI weniger geliefert hat
      if(valid.length>0&&valid.length<5){
        const tmdbExtra=await getTMDBTop5(profile,platform,browseType||"serie");
        const existingIds=new Set(valid.map(it=>it.id));
        const fill=tmdbExtra.filter(it=>!existingIds.has(it.id)).slice(0,5-valid.length);
        valid=[...valid,...fill];
      }
      if(valid.length>0){
        setRecs(valid.map(it=>({
          ...
          reason:platRecs.find(r=>titleKey(r.title)===titleKey(it.title||it.name||""))?.reason||
                 (it.overview?it.overview.substring(0,120)+(it.overview?.length>120?"…":""):""),
  ...
  function loadTop5Recs(){
    ...
    // Sofort TMDB laden — kein KI-Warten, Overview direkt als Text
  ...
  // Reset wenn browseType wechselt — neu laden wenn gerade offen
  useEffect(()=>{
    lastTypeRef.current=null;
    setRecs(null);
    if(open)loadTop5Recs();
  },[browseType]);

  // Listen for watched/blocked events to remove from Top5 immediately
  useEffect(()=>{
    function onWatched(e){
      setRecs(prev=>prev?prev.filter(r=>r._tmdbItem?.id!==e.detail.id&&titleKey(r.title)!==titleKey(e.detail.title)):prev);
    }
    function onBlocked(e){
      setRecs(prev=>prev?prev.filter(r=>titleKey(r.title)!==titleKey(e.detail.title)):prev);
    }
    // Nach Universal Swipe → KI-Empfehlungen laden + anzeigen
    async function onSwipeDone(e){
      setOpen(true);
      await loadKiRecs(e.detail?.kiRecs);
    }
  ...
  return(
    <div style={{background:"#12121f",borderRadius:18,overflow:"hidden",border:"1px solid #1e1e30",marginBottom:10}}>
      ...
      <div style={{padding:"14px 16px",display:"flex",alignItems:"center",gap:12,cursor:"pointer"}} onClick={async()=>{
        const next=!open;
        setOpen(next);
        if(next){
          // Prüfe zuerst ob KI-Recs vorhanden
          const hasKiRecs=await loadKiRecs();
          if(!hasKiRecs&&!recs)loadTop5Recs();
        }
      }}>
      ...
      {open&&mode==="swipe"&&(
        <div style={{padding:"0 18px 48px"}}>
          ...
          <PlatformSwipe platform={platform} profile={profile} browseType={browseType} onBlock={t=>onBlock(t)} onLike={it=>onLike(it)} onSelect={onSelect} onRate={onRate||((t,s,g)=>{})} onDone={async(freshProfile)=>{
            ...
            lastTypeRef.current=null;
            setLoading(true);setRecs(null);
            // Kurze Pause damit Profil-Updates gespeichert sind
            ...
            try{
              const system=`Du bist ein Streaming-Experte. Antworte NUR mit JSON-Array, keine Backticks. Format: [{"title":"...","year":"..."}]`;
              const msg=`Empfehle genau 5 ${typeStr} auf ${platform.name} Deutschland.

Swipe-Auswertung:
- GEMOCHT ❤️: ${liked||"noch nichts"} → Diese Genres und Stimmungen bevorzugen
- ABGELEHNT ✕: ${disliked||"nichts"} → Diese Art von Titeln NICHT empfehlen
- Gesperrt: ${blocked||"nichts"}
- Top-Genres: ${topGenres||"gemischt"}

Regeln:
1. Orientiere dich STARK an den gemochten Titeln
2. Wenn viel abgelehnt: völlig andere Richtung einschlagen
3. Alle 5 Titel müssen auf ${platform.name} DE verfügbar sein
4. Nur hochwertige Titel (mind. 7★)`;
              const text=await callAI([{role:"user",content:msg}],system);
              ...
              if(valid.length>0){
                // Erst anzeigen, dann KI-Texte nachladen (1 Call)
                const initial=valid.map(it=>({
                  title:it.title||it.name||"",
                  year:(it.release_date||it.first_air_date||"").substring(0,4),
                  reason:"…",
                  emoji:it.media_type==="tv"?"📺":"🎬",
                ...
                setRecs(initial);
                setLoading(false);
                // 1 KI-Call für deutsche Begründungen
                try{
                  const titles=valid.map(it=>it.title||it.name||"").join(", ");
                  const sys2=`Antworte NUR mit JSON-Array, kein Markdown. Format: [{"title":"...","reason":"..."}]`;
                  const msg2=`Schreibe für diese Titel auf ${platform.name} je einen witzigen deutschen Satz (max 15 Wörter) warum sie zu diesem Nutzer passen.
Titel: ${titles}
Nutzer mochte: ${liked||"noch nichts"}
Sei direkt, witzig, auf Deutsch.`;
                  const txt2=await callAI([{role:"user",content:msg2}],sys2);
                  const reasons2=JSON.parse(txt2.replace(/```json|```/g,"").trim());
                  setRecs(prev=>prev?prev.map((r,i)=>{
                    const match=reasons2.find(x=>titleKey(x.title||"")===titleKey(r.title))||reasons2[i];
                    return match?.reason?{...r,reason:match.reason}:r;
                  }):prev);
                }catch(e){}
                return;
              }
            }catch(e){console.error("KI nach Swipe fehlgeschlagen, Similar Fallback:",e);}

            // Fallback: /similar + TMDB Discover
            try{
              const fp2=freshProfile||profile;
              const topLiked2=Object.entries(fp2.ratings||{})
                .filter(([,v])=>v>=4).map(([k])=>k).slice(0,2);
              const likedItems2=(fp2.watched||[]).filter(w=>topLiked2.includes(titleKey(w.title||"")));
              const simResults2=await Promise.all(
                likedItems2.slice(0,2).map(w=>getSimilarTitles(w.media_type||"movie",w.id,platform.tmdbIds,fp2))
              );
              const simFlat2=simResults2.flat();
              let combined2=simFlat2;
              if(combined2.length<5){
                const extra2=await getTMDBTop5(fp2,platform,browseType||"serie");
                const existIds2=new Set(combined2.map(r=>r.id));
                combined2=[...combined2,...extra2.filter(r=>!existIds2.has(r.id))];
              }
              if(combined2.length>0){
                setRecs(combined2.slice(0,5).map((it,i)=>{
                  const srcTitle=likedItems2[Math.floor(i/2)]?.title||likedItems2[0]?.title||"";
                  return{
                    title:it.title||it.name||"",
                    year:(it.release_date||it.first_air_date||"").substring(0,4),
                    reason:srcTitle?similarReason(srcTitle,it):(it.overview?it.overview.substring(0,120)+"…":""),
                    emoji:it.media_type==="tv"?"📺":"🎬",
                    _tmdbItem:it,
                  };
                }));
            ...
            // Final fallback
            getTMDBTop5(freshProfile||profile,platform,browseType||"serie")
              .then(tmdbItems=>{
                if(tmdbItems.length>0){
                  setRecs(tmdbItems.map(it=>({
                    title:it.title||it.name||"",
...
function LikedTab({profile,cardProps}){
  // Use stored liked_items directly — no TMDB fetch needed, no wrong title bug
  const items=profile.liked_items||[];
  return(
    <div style={{padding:"0 18px"}}>
      <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:6}}>
        <h3 style={{fontSize:20,fontWeight:800,margin:0}}>Watchlist</h3>
        {items.length>0&&<span style={{fontSize:12,color:"#b0a8b8",background:"#1a1a2e",borderRadius:20,padding:"2px 10px"}}>{items.length}</span>}
      </div>
      <p style={{fontSize:12,color:"#b0a8b8",marginBottom:14}}>Titel die du noch sehen möchtest</p>
      {items.length===0&&<div style={{textAlign:"center",padding:40,color:"#b0a8b8"}}><div style={{fontSize:40,marginBottom:10}}>🤍</div><p>Noch nichts gespeichert.</p><p style={{fontSize:11,marginTop:6}}>Tippe bei einem Titel auf das Herz-Icon.</p></div>}
...
function UniversalSwipe({profile,cardProps,onSelect,onDone}){
  ...
  useEffect(()=>{
    ...
    const genreStr=topGenres.length>0?topGenres.join("|"):null;
    const rp=Math.floor(Math.random()*5)+1;
    Promise.all([
      genreStr?discoverTitles("serie",allIds,genreStr,1,"vote_average.desc",langFilter):Promise.resolve({results:[]}),
      discoverTitles("serie",allIds,null,rp,"popularity.desc",langFilter),
      genreStr?discoverTitles("film",allIds,genreStr,1,"vote_average.desc",langFilter):Promise.resolve({results:[]}),
      discoverTitles("film",allIds,null,rp,"popularity.desc",langFilter),
    ]).then(([s1,s2,f1,f2])=>{
      const all=[
      ...
      setItems(filtered.slice(0,25));
      setLoading(false);
    }).catch(()=>setLoading(false));
  },[]);

  // Bug-Fix: Capture current item at time of swipe
  function handleRight(item){
  ...
  async function handleDone(){
    ...
    // 1 KI-Call für alle Anbieter — wählt Titel + schreibt Begründungen
    let kiRecs=null;
    if(liked_sw.length>0||disliked_sw.length>0){
      try{
        const ctx=buildCtx(profile);
        const userPlats=PLATFORMS.filter(p=>profile.platforms.includes(p.id));
        const platNames=userPlats.map(p=>p.name).join(", ");
        const system=`Du bist ein Streaming-Experte. Antworte NUR mit JSON-Array, kein Markdown. Format: [{"title":"...","platform":"...","reason":"..."}]. Plattform muss exakt so heißen wie angegeben.`;
        const msg=`Empfehle GENAU 5 Titel pro Plattform basierend auf diesen Swipes:
...
Regeln:
- Orientiere dich STARK an den gemochten Titeln
- Vermeide Ähnliches zu den abgelehnten
- Titel müssen auf der jeweiligen Plattform verfügbar sein
- reason = witziger deutscher Satz max 12 Wörter`;
        const text=await callAI([{role:"user",content:msg}],system);
        kiRecs=JSON.parse(text.replace(/```json|```/g,"").trim());
        if(Array.isArray(kiRecs)&&kiRecs.length>0){
          localStorage.setItem("sf_ki_recs",JSON.stringify(kiRecs));
        }
      }catch(e){
        // KI nicht verfügbar → /similar + Discover Fallback
        if(liked_sw.length>0){
          try{
            const userPlats=PLATFORMS.filter(p=>profile.platforms.includes(p.id));
            const allIds=userPlats.flatMap(p=>p.tmdbIds);
            // Top 2 gemochte Titel → /similar
            ...
            // Genre-Boost aus Swipes → Discover
            ...
            const discRes=genreStr
              ?await discoverTitles("serie",allIds,genreStr,1,"vote_average.desc",profile.languages||null).then(r=>(r.results||[]).map(it=>({...it,media_type:"tv"})))
              :[];
            const combined=[...simResults.flat(),...discRes];
            const seen2=new Set();
            const allDeduped=combined.filter(it=>{if(!it.id||seen2.has(it.id))return false;seen2.add(it.id);return true;});
            // Pro Plattform SEPARAT filtern mit eigenen TMDB-IDs
            const platResults=await Promise.all(userPlats.map(async plat=>{
              // Discover für diese Plattform mit Swipe-Genres
              const platDisc=genreStr
                ...
                :[];
              const platDisc2=await discoverTitles("film",plat.tmdbIds,genreStr||null,1,"vote_average.desc",profile.languages||null).then(r=>(r.results||[]).map(it=>({...it,media_type:"movie"})));
              // /similar die auf dieser Plattform verfügbar sind
              const platSim=allDeduped.filter(it=>{
                const providers=it.watch?.flatMap(p=>p.flatrate||[])||[];
                return providers.length===0||true; // TMDB /similar hat keine Provider-Info → nehme alle
              });
              const platCombined=[...platSim,...platDisc,...platDisc2];
              const seenPlat=new Set();
              const platItems=platCombined.filter(it=>{
                if(!it.id||seenPlat.has(it.id))return false;
                seenPlat.add(it.id);
                return true;
              }).slice(0,5);
              return platItems.map(it=>({
                title:it.title||it.name||"",
                platform:plat.name,
                reason:it._sourceTitle?similarReason(it._sourceTitle,it):(it.overview?it.overview.substring(0,100)+"…":""),
              }));
            }));
            kiRecs=platResults.flat();
            if(kiRecs.length>0)localStorage.setItem("sf_ki_recs",JSON.stringify(kiRecs));
          }catch(e2){}
        }
      }
    }

    // Event → alle Anbieter laden neu mit KI-Daten
    setTimeout(()=>{
      window.dispatchEvent(new CustomEvent("sf_swipe_done",{
        detail:{liked:liked_sw,disliked:disliked_sw,kiRecs}
      }));
      onDone();
    },400);
  }
    ...
    <div style={{textAlign:"center",padding:40}}>
      {processing?(
        <>
          ...
          <p style={{fontSize:14,color:"#b0a8b8"}}>Empfehlungen werden angepasst…</p>
        </>
      ):(
        <>
          ...
          <p style={{fontSize:12,color:"#b0a8b8",marginBottom:20}}>
            {Object.entries(localRatings.current).filter(([,v])=>v.stars>=4).length} gemerkt · {Object.entries(localRatings.current).filter(([,v])=>v.stars<=2).length} abgelehnt
          </p>
          <button onClick={()=>{setProcessing(true);handleDone();}} style={{background:"linear-gradient(135deg,#ff6b35,#e84393)",border:"none",borderRadius:12,padding:"12px 24px",color:"#fff",cursor:"pointer",fontFamily:"'DM Sans'",fontWeight:700,fontSize:14}}>
  ...
  return(
    <div>
      ...
      {/* Card — key auf idx damit React neu rendert */}
...
function TourModal({onClose}){
  ...
  const slides=[
    ...
    },
    {
      ...
      tip:"Tipp: Sterne-Bewertungen verbessern deine Empfehlungen am meisten.",
    },
    {
      icon:"🔍",
      title:"Entdecken",
    ...
    },
    {
      ...
      text:"Was dich interessiert wandert in die Merkliste. Was du gesehen hast in den Verlauf. Daraus berechnet sich dein Geschmacksprofil.",
      tip:"Bereit? Dann los.",
    },
  ];
...
function TabHintBanner({tab,onDismiss}){
  const hints={
    home:{icon:"✨",text:"Hier sind deine Top-Picks. Wische links zum Ausblenden, tippe einen Titel für Details."},
    browse:{icon:"📺",text:"Tippe einen Anbieter an um seine Top 5 zu sehen, oder starte den Swipe-Modus."},
    fun:{icon:"🎲",text:"Lass die KI für dich entscheiden — Würfel, Orakel oder dein Persönlichkeitstyp."},
    liked:{icon:"❤️",text:"Hier sammeln sich alle Titel die du gemerkt hast."},
    history:{icon:"📋",text:"Dein Verlauf und Geschmacksprofil — je voller, desto besser die Empfehlungen."},
  };
  const h=hints[tab];
  if(!h)return null;
  return(
...
function MainApp({profile:initProfile,onReset}){
  ...
  function handleRate(itemTitle,stars,directGenreIds){
    ...
    if(stars<=2){
      ...
      if(replacement){
        ...
        setFeedItems(prev=>[...removeIfBad(prev),{...replacement,_isNew:true}]);
        setBaseFeedItems(prev=>[...removeIfBad(prev),{...replacement,_isNew:true}]);
        // Reserve fast leer → im Hintergrund auffüllen
        if(reserveItems.current.length<3){
          getSmartRecommendations(profile).then(items=>{
            const existIds=new Set([...heroItems,...feedItems].map(it=>it.id));
  ...
  function handleBlock(itemTitle){
    ...
    if(replacement){
      reserveItems.current=reserve.filter(r=>r.id!==replacement.id);
      setFeedItems(prev=>[...filterOut(prev),{...replacement,_isNew:true}]);
      setBaseFeedItems(prev=>[...filterOut(prev),{...replacement,_isNew:true}]);
    }else{
      setFeedItems(prev=>filterOut(prev));
      setBaseFeedItems(prev=>filterOut(prev));
      // Reserve leer → direkt neuen Titel nachladen und einfügen
      getSmartRecommendations(profile).then(newItems=>{
        ...
        if(fresh.length>0){
          ...
          setFeedItems(prev=>[...filterOut(prev),toShow]);
          setBaseFeedItems(prev=>[...filterOut(prev),toShow]);
  ...
  function handleWatched(item){
    ...
    if(replacement){
      reserveItems.current=reserve.filter(r=>r.id!==replacement.id);
      setFeedItems(prev=>[...removeFromFeed(prev),{...replacement,_isNew:true}]);
      ...
      getSmartRecommendations(profile).then(newItems=>{
        ...
        if(fresh.length>0){
          const toShow={...fresh[0],_isNew:true};
          setFeedItems(prev=>[...removeFromFeed(prev),toShow]);
          setBaseFeedItems(prev=>[...removeFromFeed(prev),toShow]);
          reserveItems.current=fresh.slice(1,11);
        }
      }).catch(()=>{});
    }
