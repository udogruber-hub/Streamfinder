...
function PlatformCard({platform,profile,onSelect,onBlock,onLike,onRate,browseType}){
  ...
  async function loadTMDBRecs(){
    ...
    try{
      ...
      // Unterschiedliche Seiten pro Plattform für Abwechslung
      const platSeed=platform.id%7; // 0-6 je nach Plattform
      const page1=(platSeed%3)+1;
      const page2=((platSeed+2)%4)+1;
      const [r1,r2,r3]=await Promise.all([
        discoverTitles(mt,platform.tmdbIds,genreStr,page1,"vote_average.desc",langFilter),
        discoverTitles(mt,platform.tmdbIds,null,page2,"popularity.desc",langFilter),
        discoverTitles(mt,platform.tmdbIds,genreStr,page1+1,"vote_count.desc",langFilter),
      ]);
  ...
  async function replaceRec(removedTitle){
    ...
    setRecs(prev=>{
      ...
      // Reserve leer → nur entfernen, KI-Recs bleiben unangetastet
      const next=filtered.slice(0,5);
      storeRecs(next);
      // Stil im Hintergrund einen einzelnen TMDB-Titel nachholen
      const bl=new Set(profile.blocked_titles||[]);
      const wt=new Set((profile.watched||[]).map(w=>w.id));
      discoverTitles(browseType||"serie",platform.tmdbIds,null,Math.floor(Math.random()*5)+1,"vote_average.desc",null)
        .then(r=>{
          const candidates=(r.results||[])
            .map(it=>({...it,media_type:(browseType||"serie")==="serie"?"tv":"movie"}))
            .filter(it=>{
              const t=titleKey(it.title||it.name||"");
              return !existIds.has(it.id)&&!existTitles.has(t)&&!bl.has(t)&&!wt.has(it.id)&&(it.vote_average||0)>=7;
            });
          if(candidates.length>0){
            reserveRef.current=[...reserveRef.current,...candidates.slice(0,5)];
          }
        }).catch(()=>{});
      return next;
    });
...
function UniversalSwipe({profile,cardProps,onSelect,onDone,browseType}){
  const [items,setItems]=useState([]);
  const [idx,setIdx]=useState(0);
  const [loading,setLoading]=useState(true);
  const [done,setDone]=useState(false);
  ...
  useEffect(()=>{
    ...
    // Nur den aktuell gewählten Typ laden
    const swipeType=browseType||"serie";
    const mediaType=swipeType==="serie"?"tv":"movie";
    Promise.all([
      genreStr?discoverTitles(swipeType,allIds,genreStr,rp,"vote_average.desc",langFilter):Promise.resolve({results:[]}),
      discoverTitles(swipeType,allIds,null,rp2,"popularity.desc",langFilter),
      discoverTitles(swipeType,allIds,null,rp+1,"vote_count.desc",langFilter),
    ]).then(([s1,s2,s3])=>{
      const all=[
        ...(s1.results||[]).map(r=>({...r,media_type:mediaType})),
        ...(s2.results||[]).map(r=>({...r,media_type:mediaType})),
        ...(s3.results||[]).map(r=>({...r,media_type:mediaType})),
      ];
      const seen=new Set();
      const deduped=all.filter(r=>{if(seen.has(r.id))return false;seen.add(r.id);return true;});
      const filtered=filterItems(deduped);
  ...
  async function handleDone(){
    ...
    const userPlats=PLATFORMS.filter(p=>profile.platforms.includes(p.id));
    const swType=browseType||"serie";
    const mediaType=swType==="serie"?"tv":"movie";
    const ctx=buildCtx(profile);
    const langFilter=profile.languages&&profile.languages.length>0?profile.languages:null;
    const watched=new Set((profile.watched||[]).map(w=>w.id));
    const blocked=new Set(profile.blocked_titles||[]);
    ...
    try{
      // 1. Pro Plattform TMDB-Kandidaten laden — garantiert verfügbar, verschiedene Seiten
      const platCandidates=await Promise.all(userPlats.map(async(plat,pi)=>{
        const page1=(pi%4)+1;
        const page2=((pi+2)%4)+1;
        const [r1,r2,r3]=await Promise.all([
          discoverTitles(swType,plat.tmdbIds,genreStr,page1,"vote_average.desc",langFilter),
          discoverTitles(swType,plat.tmdbIds,null,page2,"popularity.desc",langFilter),
          discoverTitles(swType,plat.tmdbIds,genreStr,page1+1,"vote_count.desc",langFilter),
        ]);
        const all=[...(r1.results||[]),...(r2.results||[]),...(r3.results||[])].map(r=>({...r,media_type:mediaType}));
        const seen=new Set();
      ...
      // 2. Pro Plattform: KI wählt die 5 besten aus den Kandidaten
      await Promise.all(platCandidates.map(async({plat,candidates})=>{
        if(!candidates.length)return;
        const titles=candidates.slice(0,12).map(it=>it.title||it.name||"").join(", ");
        const system=`Du bist ein Streaming-Experte. Antworte NUR mit JSON-Array, kein Markdown. Format: [{"title":"...","reason":"..."}]`;
        const msg=`Wähle die 5 besten ${swType==="serie"?"Serien":"Filme"} für diesen Nutzer aus dieser Liste:
...
reason = witziger persönlicher Satz Deutsch 15-20 Wörter`;
        ...
        try{
          ...
          const result=picks.map(p=>{
            ...
            return{id:match.id,title:match.title||match.name||"",year:(match.release_date||match.first_air_date||"").substring(0,4),reason:p.reason||"",emoji:mediaType==="tv"?"📺":"🎬",_tmdbItem:match};
          }).filter(Boolean).slice(0,5);
          // Auffüllen mit Top-Kandidaten wenn nötig
          if(result.length<5){
            const existIds=new Set(result.map(r=>r.id));
            const fill=candidates.filter(c=>!existIds.has(c.id)).slice(0,5-result.length).map(c=>({
              ...
              emoji:mediaType==="tv"?"📺":"🎬",_tmdbItem:c,
            }));
            result.push(...fill);
          }
          if(result.length>0)localStorage.setItem("sf_plat_"+plat.id+"_"+swType,JSON.stringify(result));
        }catch(e){
          // KI fehlgeschlagen → Top-Kandidaten direkt
          const fallback=candidates.slice(0,5).map(c=>({
            id:c.id,title:c.title||c.name||"",year:(c.release_date||c.first_air_date||"").substring(0,4),
            reason:c.overview?c.overview.substring(0,160)+(c.overview.length>160?"…":""):"",
            emoji:mediaType==="tv"?"📺":"🎬",_tmdbItem:c,
          }));
          if(fallback.length>0)localStorage.setItem("sf_plat_"+plat.id+"_"+swType,JSON.stringify(fallback));
        }
      }));
    }catch(e){
      // Komplett-Fallback: TMDB direkt pro Plattform
      await Promise.all(userPlats.map(async(plat,pi)=>{
        try{
          const r=await discoverTitles(swType,plat.tmdbIds,genreStr,(pi%4)+1,"vote_average.desc",langFilter);
          const items=(r.results||[]).filter(it=>!watched.has(it.id)&&(it.vote_average||0)>=7).slice(0,5).map(it=>({
            id:it.id,title:it.title||it.name||"",year:(it.release_date||it.first_air_date||"").substring(0,4),
            reason:it.overview?it.overview.substring(0,160)+"…":"",emoji:mediaType==="tv"?"📺":"🎬",_tmdbItem:{...it,media_type:mediaType},
          }));
          if(items.length>0)localStorage.setItem("sf_plat_"+plat.id+"_"+swType,JSON.stringify(items));
        }catch(e2){}
      }));
    }
...
function BrowseTab({profile,cardProps,onSelect}){
  ...
  const [swipeType,setSwipeType]=useState("serie"); // "serie" oder "film"
      ...
      {searchQuery?(
        ...
        <div>
          ...
          <div style={{background:"linear-gradient(135deg,#1a1525,#0f0e1a)",borderRadius:18,padding:"16px",marginBottom:16,border:"1px solid #2a1f3d"}}>
            ...
            {/* 2 Swipe Buttons — kompakt */}
            <div style={{display:"flex",gap:8,marginBottom:14}}>
              {[{type:"serie",label:"📺 Serien swipen",color:"#e84393"},{type:"film",label:"🎬 Filme swipen",color:"#ff6b35"}].map(btn=>(
                <button key={btn.type} onClick={()=>{setSwipeType(btn.type);setShowUniversalSwipe(true);}}
                  style={{flex:1,background:"rgba(255,255,255,0.04)",border:"1px solid "+btn.color+"55",borderRadius:12,padding:"11px 10px",cursor:"pointer",textAlign:"center"}}>
                  <div style={{fontSize:13,fontWeight:700,color:"#f0ece4"}}>{btn.label}</div>
                  <div style={{fontSize:10,color:"#b0a8b8",marginTop:3}}>Für personalisierte Picks</div>
                </button>
              ))}
            </div>
            ...
            <div style={{position:"fixed",inset:0,background:"#09090f",zIndex:200,overflow:"auto"}}>
              <div style={{padding:"16px 18px",display:"flex",alignItems:"center",gap:12,borderBottom:"1px solid #1e1e30"}}>
                ...
                <span style={{fontSize:15,fontWeight:800,color:"#f0ece4"}}>{swipeType==="serie"?"📺 Serien":"🎬 Filme"} entdecken</span>
              </div>
              <div style={{padding:"16px 18px 4px",background:"rgba(232,67,147,0.06)",borderBottom:"1px solid #1e1e30"}}>
                <p style={{fontSize:12,color:"#b0a8b8",margin:0,lineHeight:1.5}}>Wische rechts ❤️ für Titel die dich interessieren, links ✕ für alles andere. Danach passen sich deine {swipeType==="serie"?"Serien":"Film"}-Empfehlungen an.</p>
              </div>
              <div style={{padding:"0 18px",paddingBottom:120}}>
                <UniversalSwipe profile={profile} cardProps={cardProps} onSelect={cardProps.onSelect||onSelect} onDone={()=>setShowUniversalSwipe(false)} browseType={swipeType}/>
              </div>
            </div>
          )}
          {showPlatforms&&(
