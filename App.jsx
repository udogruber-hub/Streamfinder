          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:8}}>
            {STIMMUNGEN.map(m=>{
              ...
              return(<button key={m.id} onClick={()=>toggleStimmung(m.id)} style={{position:"relative",borderRadius:16,overflow:"hidden",cursor:"pointer",border:"2px solid "+(sel?m.color:"transparent"),transition:"all 0.2s",height:90,background:"#0a0a12",boxShadow:sel?"0 0 24px "+m.color+"50":"none",padding:0}}>
                <img key={GENRE_IMAGES[m.genres[0]]||m.id} src={GENRE_IMAGES[m.genres[0]]||""} alt="" style={{position:"absolute",inset:0,width:"100%",height:"100%",objectFit:"cover",objectPosition:"center 20%",opacity:sel?0.75:0.35,transition:"opacity 0.2s"}} onError={e=>{e.target.style.display="none";}}/>
