...
function HelpModal({onClose,activeTab}){
  ...
  const HELP={
    ...
    browse:{
      title:"🔍 Erkunden",
      intro:"Deine Anbieter auf einen Blick — mit persönlichen Top 10 und Swipe-Modus.",
      items:[
        {icon:"📺",title:"Top 10 pro Anbieter",text:"Tippe einen Anbieter an — die Top 10 Titel für dich laden sofort. Sortiert nach deiner persönlichen Übereinstimmung. Der Balken links zeigt wie gut der Titel zu dir passt."},
        {icon:"✨",title:"Persönliche Empfehlungen holen",text:"Tippe 'Persönliche Empfehlungen holen' — swipe durch Titel, rechts für Interesse, links für nicht mein Ding. Danach berechnet die KI deine Top 10 pro Anbieter neu."},
        {icon:"✓",title:"Gesehen-Button",text:"Direkt in der Top 10 Liste: '✓ Gesehen' tippt einen Titel als gesehen ab — er verschwindet sofort und wird durch einen neuen ersetzt."},
        {icon:"🔴🟠🟢",title:"Direkt bewerten",text:"Die LED-Punkte sind direkt in der Top 10 Liste tippbar — kein Öffnen der Detailansicht nötig."},
        {icon:"💡",title:"Schnellsuche",text:"Oben in Erkunden: Suche nach jedem Titel direkt. Ergebnisse erscheinen sofort mit allen Infos."},
      ]
    },
      ...
      if(top5.length>0){
        ...
        const formatted=top5.slice(0,10).map(it=>{
          ...
          return{
            ...
            reason:it.overview?it.overview.substring(0,120)+(it.overview.length>120?"…":""):"",
...
function SpotlightCard({item,profile,onSelect,onRate,onLike,onWatched,onBlock}){
  ...
  return(
    <div style={{borderRadius:20,overflow:"hidden",marginBottom:12,position:"relative",background:"#0a0a12"}}>
      ...
      {/* Gradient overlay */}
      <div style={{position:"absolute",top:0,left:0,right:0,height:220,background:"linear-gradient(to bottom,rgba(0,0,0,0.1) 0%,rgba(0,0,0,0.8) 100%)"}}/>
      {/* Score Badge */}
      ...
      {/* Info unten auf Bild */}
      <div style={{position:"absolute",bottom:0,left:0,right:0,padding:"12px 14px 0"}}>
        ...
        {aiReason&&<p style={{fontSize:12,color:"rgba(255,255,255,0.75)",margin:"0 0 8px",fontStyle:"italic",lineHeight:1.4,textShadow:"0 1px 4px rgba(0,0,0,0.8)"}}>{aiReason}</p>}
      </div>
      {/* Actions */}
      <div onClick={e=>e.stopPropagation()} style={{padding:"10px 14px 14px",background:"#0d0d1a",display:"flex",alignItems:"center",gap:8}}>
        {/* LED Dots */}
        <div style={{display:"flex",gap:4,alignItems:"center"}}>
          {[{v:1,c:"#ef4444",g:"rgba(239,68,68,0.6)",l:"Nicht mein Ding"},{v:3,c:"#f59e0b",g:"rgba(245,158,11,0.6)",l:"Ok"},{v:5,c:"#4ade80",g:"rgba(74,222,128,0.6)",l:"Top"}].map(dot=>{
            ...
            return(
              <button key={dot.v} onClick={()=>onRate(title,myRating===dot.v?0:dot.v,item.genre_ids||[])}
          ...
          {myRating>0&&<span style={{fontSize:9,color:ratingColors[myRating],fontFamily:"'DM Sans'",fontWeight:700}}>{ratingLabels[myRating]}</span>}
        </div>
        <div style={{flex:1}}/>
        {/* Gesehen */}
        ...
        <button onClick={()=>onSelect(item)}
          style={{width:34,height:34,borderRadius:8,background:"rgba(255,255,255,0.05)",border:"1px solid rgba(255,255,255,0.1)",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}}>
          <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="#888" strokeWidth="2" strokeLinecap="round"><polyline points="9 18 15 12 9 6"/></svg>
        </button>
      </div>
    </div>
  );
...
function SpotlightTab({profile,cardProps,onSelect}){
  ...
  useEffect(()=>{
    const queries=["Breaking Bad","Inception","Peaky Blinders"];
  ...
  return(
    <div style={{display:"flex",flexDirection:"column",height:"calc(100vh - 120px)"}}>
      ...
      {/* Hero Mosaic */}
      <div style={{flex:1,display:"flex",flexDirection:"column",gap:8,padding:"0 12px 8px",minHeight:0}}>
...
function MainApp({profile:initProfile,onReset}){
  ...
  function handleBlock(itemTitle){
    const key=titleKey(itemTitle);
    // Profil updaten — nur blocken, nie entsperren
    updateProfile(p=>{
      const blocked=p.blocked_titles||[];
      if(blocked.includes(key))return p; // Bereits geblockt
    ...
    if(replacement){
      ...
      getSmartRecommendations(profile).then(newItems=>{
        // Verwende immer frischen State für Dedup
        setFeedItems(prev=>{
          const heroIds=new Set(heroItemsRef.current.map(it=>it.id));
          const feedIds=new Set(prev.map(it=>it.id));
          const allIds=new Set([...heroIds,...feedIds]);
          const fresh=newItems.filter(it=>
            !allIds.has(it.id)&&
            titleKey(it.title||it.name||"")!==key
          );
          if(fresh.length===0)return prev.filter(it=>titleKey(it.title||it.name||"")!==key);
          ...
          // Sync baseFeed
          setBaseFeedItems(bp=>{
            const bf=bp.filter(it=>titleKey(it.title||it.name||"")!==key);
            return bf.some(it=>it.id===toShow.id)?bf:[...bf,toShow];
          });
          return n;
        });
      }).catch(()=>{});
    }
    // Kein sf_blocked Event — PlatformCard hört selbst auf Block-Events
  }
  ...
  function handleWatched(item){
    ...
    if(replacement){
      ...
      setFeedItems(prev=>{
        const heroIds=new Set(heroItemsRef.current.map(it=>it.id));
        if(heroIds.has(replacement.id)||prev.some(it=>it.id===replacement.id&&it.id!==item.id))
          return prev.filter(it=>it.id!==item.id);
        const n=[...prev.filter(it=>it.id!==item.id),{...replacement,_isNew:true}];
        feedItemsRef.current=n;return n;
      });
      setBaseFeedItems(prev=>{
        if(prev.some(it=>it.id===replacement.id&&it.id!==item.id))return prev.filter(it=>it.id!==item.id);
        return[...removeFromFeed(prev),{...replacement,_isNew:true}];
      });
    }else{
        ...
        {tab==="home"&&(
          <div>
            ...
            {feedLoading?(
              ...
              <div style={{padding:"0 18px"}}>
                <AssistentBanner profile={profile} onTabChange={onTabChange} onDismiss={()=>{}}/>
                <GeschmacksScore profile={profile}/>
                {heroItems.length>0&&(
                ...
                {feedItems.length>0&&(
                  <div>
                    ...
                    {(()=>{
                      const heroIds=new Set(heroItems.map(it=>it.id));
                      const seen=new Set();
                      return feedItems.filter(it=>{
                        if(heroIds.has(it.id)||seen.has(it.id))return false;
                        seen.add(it.id);return true;
                      }).map(it=><SwipeToBlock key={it.id} onBlock={()=>cardProps.onBlock(it.title||it.name||"")}><TitleCard item={it} {...cardProps}/></SwipeToBlock>);
                    })()}
                  </div>
                )}
              </div>
...
// ── Smarter Assistent-Banner ──
const BANNER_TYPES={
  onboarding:{id:"onboarding",emoji:"👋",title:"Willkommen bei StreamFinder!",text:"Starte mit 3 Bewertungen — dann kennen wir deinen Geschmack.",action:"Los gehts",tab:null},
  first_rate:{id:"first_rate",emoji:"🎯",title:"Gut gemacht!",text:"Weiter bewerten — nach 5 Titeln schalten sich % Übereinstimmungen frei.",action:null,tab:null},
  try_swipe:{id:"try_swipe",emoji:"🔄",title:"Tipp: Swipe nutzen",text:"Wisch Titel in Erkunden für schnelle Bewertungen — das trainiert deinen Algorithmus.",action:"Zu Erkunden",tab:"browse"},
  try_spotlight:{id:"try_spotlight",emoji:"✨",title:"Hast du Spotlight schon probiert?",text:"Sag mir was du magst — ich finde was dazu passt. Oder frag den Popcorn-Guru!",action:"Zu Spotlight",tab:"spotlight"},
  try_guru:{id:"try_guru",emoji:"🍿",title:"Frag den Popcorn-Guru!",text:"Stell jede Frage — er findet den perfekten Film für heute Abend.",action:"Zum Guru",tab:"spotlight"},
  evening:{id:"evening",emoji:"🌙",title:"Was schaust du heute Abend?",text:"Der Guru hat eine Idee — oder lass dich einfach überraschen.",action:"Überrasch mich",tab:"spotlight"},
  level_up:{id:"level_up",emoji:"⭐",title:"Neues Level erreicht!",text:"Dein Profil wird stärker — deine Empfehlungen werden persönlicher.",action:null,tab:null},
  try_oracle:{id:"try_oracle",emoji:"🔮",title:"Das Orakel wartet auf dich",text:"Mystische Prophezeiung gefällig? Das Orakel kennt deinen nächsten Film.",action:"Orakel befragen",tab:"spotlight"},
};

function AssistentBanner({profile,onTabChange,onDismiss}){
  const [banner,setBanner]=useState(null);

  useEffect(()=>{
    const shown=JSON.parse(localStorage.getItem("sf_banners_shown")||"{}");
    const now=Date.now();
    const today=new Date().toDateString();
    const lastShown=localStorage.getItem("sf_banner_last_day");
    // Max 1 Banner pro Tag
    ...
    // Priorität: zuerst wichtigstes Banner wählen
    let chosen=null;

    if(!shown.onboarding&&total===0){
      chosen=BANNER_TYPES.onboarding;
    }else if(!shown.first_rate&&total>=1&&total<5){
      chosen=BANNER_TYPES.first_rate;
    }else if(!shown.try_swipe&&total>=5){
      chosen=BANNER_TYPES.try_swipe;
    }else if(!shown.try_spotlight&&total>=3){
      chosen=BANNER_TYPES.try_spotlight;
    }else if(!shown.try_guru&&total>=5){
      chosen=BANNER_TYPES.try_guru;
    }else if(!shown.try_oracle&&total>=8){
      chosen=BANNER_TYPES.try_oracle;
    }else if(hour>=18&&hour<=23&&!shown["evening_"+today]){
      chosen=BANNER_TYPES.evening;
    }

    if(chosen){
      setTimeout(()=>setBanner(chosen),1500);
    }
  },[]);
  ...
  function handleDismiss(){
    const shown=JSON.parse(localStorage.getItem("sf_banners_shown")||"{}");
    shown[banner.id]=Date.now();
    if(banner.id==="evening")shown["evening_"+new Date().toDateString()]=1;
    localStorage.setItem("sf_banners_shown",JSON.stringify(shown));
    localStorage.setItem("sf_banner_last_day",new Date().toDateString());
    setBanner(null);
    if(onDismiss)onDismiss();
  }

  function handleAction(){
    if(banner.tab&&onTabChange)onTabChange(banner.tab);
    handleDismiss();
  }

  return(
    <div style={{margin:"0 0 14px",animation:"fadeIn 0.4s ease"}}>
      <div style={{background:"linear-gradient(135deg,rgba(196,169,96,0.1),rgba(255,107,53,0.06))",border:"1px solid rgba(196,169,96,0.25)",borderRadius:16,padding:"14px 16px",position:"relative"}}>
        <button onClick={handleDismiss} style={{position:"absolute",top:10,right:12,background:"none",border:"none",color:"#4a4060",cursor:"pointer",fontSize:16,padding:0,lineHeight:1}}>✕</button>
        <div style={{display:"flex",alignItems:"flex-start",gap:12,paddingRight:20}}>
          <div style={{fontSize:24,lineHeight:1,flexShrink:0}}>{banner.emoji}</div>
          <div style={{flex:1}}>
            <div style={{fontSize:13,fontWeight:800,color:"#f0ece4",marginBottom:4,fontFamily:"'DM Sans'"}}>{banner.title}</div>
            <div style={{fontSize:12,color:"#9a94aa",lineHeight:1.5,marginBottom:banner.action?10:0}}>{banner.text}</div>
            {banner.action&&(
              <button onClick={handleAction}
                style={{background:"linear-gradient(135deg,#c4a960,#ff6b35)",border:"none",borderRadius:10,padding:"7px 16px",color:"#fff",cursor:"pointer",fontSize:12,fontFamily:"'DM Sans'",fontWeight:700}}>
                {banner.action} →
              </button>
            )}
          </div>
...
function GeschmacksScore({profile}){
  ...
  const milestones=[
    {min:0,   max:4,   emoji:"🌱", label:"Neuling",               sub:"Bewerte deine ersten Titel — wir lernen dich kennen"},
    {min:5,   max:14,  emoji:"🎬", label:"Film-Entdecker",         sub:"Gut! Deine Empfehlungen werden schon persönlicher"},
    {min:15,  max:29,  emoji:"🎯", label:"Geschmackskenner",       sub:"StreamFinder versteht was du magst — und was nicht"},
    {min:30,  max:59,  emoji:"🔥", label:"Streaming-Profi",        sub:"Deine Top 10 sind jetzt wirklich auf dich zugeschnitten"},
    {min:60,  max:99,  emoji:"⭐", label:"Kurator",                sub:"Du hast einen der stärksten Geschmacks-Profile"},
    {min:100, max:199, emoji:"🏆", label:"Geschmacks-Experte",     sub:"Kaum jemand kennt seinen Geschmack so gut wie du"},
    {min:200, max:999, emoji:"👑", label:"StreamFinder Legende",   sub:"Du bist die Referenz — dein Profil ist unschlagbar"},
  ];
