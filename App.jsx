...
function UniversalSwipe({profile,cardProps,onSelect,onDone}){
  ...
  async function handleDone(){
    ...
    await Promise.all(platCandidates.map(async({plat,candidates})=>{
      ...
      const system=`Du bist Streaming-Kurator. Nur JSON-Array. Format: [{"title":"...","reason":"...","score":85}]. reason = 1 Satz mit du, warum GENAU dieser Titel jetzt. score = wie gut passt Titel zum Nutzer (70-99 wenn gut passend, unter 70 nur wenn schwache Übereinstimmung).`;
      ...
      try{
        ...
        // Fallback-Titel: Score basierend auf vote_average + Genre-Match mit Profil
        const profileGenres=mergedGenres||{};
        const maxG=Math.max(...Object.values(profileGenres).filter(v=>v>0),1);
        const fill=unique.filter(c=>!mappedIds.has(c.id)).map(c=>{
          ...
          const calcScore=Math.min(94,Math.max(30,Math.round(30+gm*42+rb*22)));
          return{id:c.id,title:c.title||c.name||"",year:(c.release_date||c.first_air_date||"").substring(0,4),reason:"",score:calcScore,emoji:c.media_type==="tv"?"📺":"🎬",_tmdbItem:c};
        });
        ...
        // KI-Fehler Fallback: Score trotzdem korrekt berechnen
        const profileGenres2=mergedGenres||{};
        const maxG2=Math.max(...Object.values(profileGenres2).filter(v=>v>0),1);
        const fallback=unique.slice(0,30).map(c=>{
          const tg=c.genre_ids||[];
          const boost=tg.reduce((s,g)=>s+(profileGenres2[g]||0),0);
    ...
    try{
      ...
      if(spResult.length>0){try{localStorage.setItem("sf_spotlight_recs",JSON.stringify(spResult));}catch{}}
    }catch{}
    ...
    try{
      ...
      if(cached){
        ...
        if(recs&&recs.length>0){
          ...
          // KI im Hintergrund aktualisieren
          setTimeout(()=>loadFirstRecs(bestMatch),2000);
          return;
        }
      }
    }catch{}
