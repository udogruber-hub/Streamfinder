...
function PlatformCard({platform,profile,onSelect,onBlock,onLike,onRate,browseType,onWatched}){
  ...
  async function loadTMDBRecs(){
    ...
    try{
      ...
      const kidGenresF=[16,10762,10751];
      const profileHasKidsF=(profile.genres||{})[16]>2||(profile.genres||{})[10762]>2;
      const filteredNoKids=filtered.filter(it=>profileHasKidsF||(it.genre_ids||[]).every(g=>!kidGenresF.includes(g)));
      const top5=filteredNoKids.slice(0,10);
      ...
      <div style={{padding:"14px 16px",display:"flex",alignItems:"center",gap:12,cursor:"pointer"}} onClick={handleOpen}>
        ...
        <div style={{flex:1}}>
          ...
          <div style={{display:"flex",alignItems:"center",gap:4,marginTop:2}}>
            <span style={{fontSize:9,color:"#4a4458"}}>▌% = Übereinstimmung</span>
            <button onClick={e=>{e.stopPropagation();setShowMatchInfo(true);setTimeout(()=>setShowMatchInfo(false),4000);}}
              style={{background:"none",border:"none",padding:0,cursor:"pointer",fontSize:9,color:"#3a3350",lineHeight:1}}>ℹ</button>
          </div>
          {showMatchInfo&&(
            <div style={{position:"absolute",bottom:44,left:8,right:8,background:"linear-gradient(135deg,#1a1520,#12101e)",border:"1px solid rgba(196,169,96,0.3)",borderRadius:10,padding:"8px 12px",zIndex:10,boxShadow:"0 4px 16px rgba(0,0,0,0.5)"}}>
              <p style={{fontSize:11,color:"#c4b8a8",margin:0,lineHeight:1.5}}>📊 Der Balken zeigt wie gut dieser Titel zu <strong>deinem Geschmack</strong> passt — basierend auf deinen Bewertungen und Lieblingsgenres.</p>
            </div>
          )}
        </div>
...
Plattformen: ${ctx.platforms}`;
      ...
      // Extrahiere alle Titel aus Antwort — Anführungszeichen, Nummerierung, Fettdruck
      const q1=text.match(/[\u201c\u201d\u201e"„]([^\u201c\u201d\u201e"„]{2,50})[\u201c\u201d"]/g)||[];
      const q2=(text.match(/\*\*([^*\n]{2,50})\*\*/g)||[]);
      const q3=(text.match(/(?:^|\n)\s*\d+[\.\.)]\s+\*?\*?([^\*\n:,]{2,50})\*?\*?/gm)||[]);
      const q4=(text.match(/(?:^|\n)[^\n]*?:\s*[A-ZÄÖÜ][^\n"*,]{2,40}(?:\s*\(|\s*[–-])/gm)||[]);
      const allMatches=[...q1,...q2,...q3,...q4];
      const titles=[...new Set(allMatches.map(t=>t.replace(/[\u201c\u201d\u201e"„*]/g,"").replace(/^\s*\d+[\.]\s+/,"").replace(/^\s*[^:]*:\s*/,"").replace(/\s*[\(\-–].*$/,"").trim()).filter(t=>t.length>2&&t.length<55&&/[a-zA-ZäöüÄÖÜ]/.test(t)))].slice(0,6);
    }catch(e){
      const isLimit=e.message?.includes("429")||e.message?.includes("limit");const msg=isLimit?"__RATE_LIMIT__":"Fehler — bitte nochmal versuchen.";
        ...
        {botMessages.map((m,i)=>(
          <div key={i} style={{display:"flex",flexDirection:"column",alignItems:m.role==="user"?"flex-end":"flex-start",width:"100%",gap:8}}>
            {m.text==="__RATE_LIMIT__"
              ...
              :<>
                <div style={{background:m.role==="user"?"linear-gradient(135deg,#e84393,#ff6b35)":"rgba(255,255,255,0.06)",borderRadius:12,padding:"10px 14px",maxWidth:"85%",fontSize:13,color:"#f0ece4",lineHeight:1.6,border:m.role==="user"?"none":"1px solid rgba(255,255,255,0.08)"}}>{m.text}</div>
