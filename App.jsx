      ...
      {searchQuery?(
        ...
        <div>
          ...
          <button onClick={()=>setShowPlatforms(!showPlatforms)}
            style={{width:"100%",background:showPlatforms?"linear-gradient(135deg,#ff6b3522,#e8439318)":"#12121f",border:"1px solid "+(showPlatforms?"#ff6b3544":"#1e1e30"),borderRadius:14,padding:"12px 16px",cursor:"pointer",display:"flex",alignItems:"center",gap:10,marginBottom:12,transition:"all 0.2s"}}>
            <span style={{fontSize:18}}>📺</span>
            <div style={{flex:1,textAlign:"left"}}>
              <div style={{fontSize:13,fontWeight:700,color:"#f0ece4"}}>Anbieter durchstöbern</div>
              <div style={{fontSize:11,color:"#b0a8b8"}}>Top 5 + Swipe pro Plattform</div>
            </div>
            <div style={{display:"flex",gap:6}}>
              {["serie","film"].map(t=>(
                <button key={t} onClick={e=>{e.stopPropagation();setBrowseType(t);setShowPlatforms(true);}}
                  style={{background:browseType===t?"#ff6b35":"#1a1a2e",border:"none",borderRadius:8,padding:"4px 10px",color:browseType===t?"#fff":"#888",cursor:"pointer",fontSize:10,fontFamily:"'DM Sans'",fontWeight:700}}>
                  {t==="serie"?"📺":"🎬"}
                </button>
              ))}
            </div>
            <div style={{color:"#3a3344",fontSize:14,transition:"transform 0.3s",transform:showPlatforms?"rotate(180deg)":"rotate(0)"}}>▾</div>
          </button>
