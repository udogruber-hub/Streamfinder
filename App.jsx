...
function LedStrip({itemTitle,profile,onRate,genre_ids}){
  ...
  const current=(profile.ratings||{})[key]||0;
  const dots=[
    {v:1,color:"#ef4444",glow:"rgba(239,68,68,0.6)",label:"Geht gar nicht"},
    {v:3,color:"#f59e0b",glow:"rgba(245,158,11,0.6)",label:"Ok"},
    {v:5,color:"#a3e635",glow:"rgba(163,230,53,0.6)",label:"Top"},
  ];
  const activeLabel=dots.find(d=>d.v===current)?.label||"";
  const activeColor=dots.find(d=>d.v===current)?.color||"";
  return(
    <div style={{display:"flex",flexDirection:"column",alignItems:"flex-end",gap:4}}>
          ...
          return(
            <button key={dot.v}
              ...
              <div style={{
                ...
                background:active?dot.color:"rgba(255,255,255,0.1)",
                border:"1.5px solid "+(active?dot.color:"rgba(255,255,255,0.15)"),
                boxShadow:active?"0 0 10px 3px "+dot.glow:"none",
      ...
      <div style={{display:"flex",gap:10,paddingRight:4}}>
        {dots.map(dot=>(
          <span key={dot.v} style={{
            width:44,textAlign:"center",
            fontSize:8,fontWeight:600,
            color:current===dot.v?dot.color:"rgba(255,255,255,0.2)",
            fontFamily:"'DM Sans'",
    ...
    <div style={{background:"#12121f",borderRadius:18,overflow:"hidden",border:"1px solid #1e1e30",marginBottom:10}}>
      <div style={{padding:"14px 16px",display:"flex",alignItems:"center",gap:12,cursor:"pointer"}} onClick={handleOpen}>
        ...
        <div style={{flex:1}}>
          ...
          <div style={{fontSize:9,color:"#4a4458",marginTop:2}}>▌= KI-Match · Top/Gut/Ok/Naja</div>
        </div>
      ...
      {open&&(
        <div style={{padding:"0 14px 16px"}}>
          ...
          {!loading&&recs&&recs.map((rec,i)=>{
            ...
            const maxGenre=Math.max(...Object.values(profile.genres||{0:1}).filter(v=>v>0),1);
            // KI-Score wenn vorhanden, sonst relative Berechnung
            // KI-Score hat Vorrang, TMDB-Fallback max 72%
        const matchPct=rec.score||Math.min(72,Math.max(30,Math.round(30+(genreScore/maxGenre)*35+(((score||7)-6)/4)*12)));
            const matchColor=matchPct>=80?"#4ade80":matchPct>=60?"#c4a960":matchPct>=45?"#f97316":"#b0a8b8";
            const matchLabel=matchPct>=80?"Top":matchPct>=60?"Gut":matchPct>=45?"Ok":"Naja";
            return(
                ...
                <div style={{borderRadius:16,overflow:"hidden",position:"relative",background:"#0d0d18",border:"1px solid #1e1e30"}}>
                  <div onClick={()=>onSelect(tmdb||{title:rec.title,name:rec.title,overview:rec.reason,vote_average:0,genre_ids:[],poster_path:null,media_type:"movie"})}
                    ...
                    <div style={{position:"absolute",inset:0,display:"flex",alignItems:"center",gap:12,padding:"0 14px"}}>
                      ...
                      <div style={{display:"flex",flexDirection:"column",alignItems:"center",flexShrink:0,width:22,gap:2}}>
                        <div style={{width:5,height:40,borderRadius:3,background:"#1e1e30",overflow:"hidden",position:"relative"}}>
                          <div style={{position:"absolute",bottom:0,left:0,right:0,height:matchPct+"%",background:matchColor,borderRadius:3,boxShadow:"0 0 6px "+matchColor+"88"}}/>
                        </div>
                        <span style={{fontSize:7,color:matchColor,fontWeight:800,fontFamily:"'DM Sans'"}}>{matchLabel}</span>
                      </div>
                  ...
                  <div onClick={e=>e.stopPropagation()} style={{padding:"6px 12px 10px",display:"flex",gap:8,alignItems:"center"}}>
                    ...
                    <button onClick={e=>{e.stopPropagation();const item=rec._tmdbItem||{id:rec.id,title:rec.title,genre_ids:tmdb?.genre_ids||[],media_type:rec.emoji==="📺"?"tv":"movie"};if(onWatched)onWatched(item);replaceRec(rec.title);}}
                      style={{background:"rgba(74,222,128,0.08)",border:"1px solid rgba(74,222,128,0.2)",borderRadius:8,padding:"4px 10px",cursor:"pointer",fontSize:11,color:"#4ade80",fontFamily:"'DM Sans'",fontWeight:600,flexShrink:0,whiteSpace:"nowrap"}}>
                      ✓ Gesehen
                    </button>
                  </div>
                </div>
...
reason = witziger Satz Deutsch.`;
      ...
      if(e.message?.includes("429")||e.message?.includes("limit"))setDDResults([{_rateLimit:true}]);
    }
    setDDLoading(false);
  }
...
reason = witziger Satz Deutsch.`;
      ...
      if(e.message?.includes("429")||e.message?.includes("limit"))setVibeResults([{_rateLimit:true}]);
    }
  ...
  async function handleBot(){
    ...
    try{
      const ctx=buildCtx(profile);
      const system=`Du bist StreamBot, ein freundlicher Streaming-Experte. Antworte auf Deutsch, max 3 Sätze + 2-3 Titel-Empfehlungen. Nutze das Nutzerprofil:
${ctx.taste||"gemischt"}
Plattformen: ${ctx.platforms}`;
      ...
      const msg=e.message?.includes("429")||e.message?.includes("limit")?"🌙 Heute ausgebucht — morgen wieder verfügbar.":"Fehler — bitte nochmal versuchen.";
  ...
  if(mode==="deepdive")return(
    <div>
      <div style={{display:"flex",gap:8,marginBottom:12}}>
        <input ref={ddRef} value={ddInput} onChange={e=>{setDDInput(e.target.value);handleDDSearch(e.target.value);}}
      ...
      {ddResults.length>0&&(
        <div style={{display:"flex",flexDirection:"column",gap:8}}>
          {ddResults[0]?._rateLimit
            ?<p style={{color:"#b0a8b8",fontSize:13,textAlign:"center",padding:12}}>🌙 Heute ausgebucht — morgen wieder verfügbar.</p>
      ...
      {[{label:"Action & Spannung",val:heavy,set:setHeavy},{label:"Leichtigkeit",val:100-trash,set:v=>setTrash(100-v)},{label:"Dunkel & Ernst",val:dark,set:setDark}].map(s=>(
        <div key={s.label} style={{marginBottom:14}}>
          <div style={{display:"flex",justifyContent:"space-between",marginBottom:6}}>
            <span style={{fontSize:12,color:"#b0a8b8",fontFamily:"'DM Sans'"}}>{s.label}</span>
  ...
  if(mode==="bot")return(
    <div>
      <div style={{minHeight:100,maxHeight:300,overflow:"auto",marginBottom:12,display:"flex",flexDirection:"column",gap:8}}>
        {botMessages.length===0&&<p style={{fontSize:13,color:"#555",fontStyle:"italic",textAlign:"center",padding:20}}>Frag mich alles über Serien und Filme…</p>}
        {botMessages.map((m,i)=>(
          <div key={i} style={{display:"flex",justifyContent:m.role==="user"?"flex-end":"flex-start"}}>
            <div style={{background:m.role==="user"?"linear-gradient(135deg,#e84393,#ff6b35)":"#1a1a2e",borderRadius:12,padding:"10px 14px",maxWidth:"85%",fontSize:13,color:"#f0ece4",lineHeight:1.5}}>
              {m.text}
            </div>
          </div>
        ))}
        {botLoading&&<div style={{display:"flex",justifyContent:"flex-start"}}><div style={{background:"#1a1a2e",borderRadius:12,padding:"10px 14px",fontSize:13,color:"#555"}}>…</div></div>}
      </div>
...
function SpotlightTab({profile,cardProps,onSelect}){
  ...
  const tiles=[
    {
      id:"deepdive",
      title:"Sag mir was mir gefällt…",
      desc:"Starte mit einem Lieblingstitel — wir finden was dazu passt",
      color:"#c4a960",
      gradient:"linear-gradient(145deg,#1a1a10,#0f1020)",
      border:"rgba(196,169,96,0.2)",
      size:"large", // nimmt mehr Platz
    },
    {
      id:"vibe",
      title:"Vibe-Meter",
      desc:"Stimmung einstellen → perfekter Titel",
      color:"#e84393",
      gradient:"linear-gradient(145deg,#1a1020,#130818)",
      border:"rgba(232,67,147,0.2)",
      size:"small",
    },
    {
      id:"bot",
      title:"Frag den Popcorn-Guru",
      desc:"Dein persönlicher Experte antwortet",
      color:"#4ade80",
      gradient:"linear-gradient(145deg,#0f1a1a,#0d1f1a)",
  ...
  const extras=[
    {id:"surprise",title:"Überrasch mich",desc:"1 perfekter Titel",color:"#c4a960"},
    {id:"oracle",title:"Orakel",desc:"Mystische Prophezeiung",color:"#a78bfa"},
    {id:"personality",title:"Welcher Filmtyp bin ich?",desc:"Dein Streaming-Profil",color:"#e84393"},
  ];

  const iconMap={
    deepdive:"deepdive",vibe:"vibe",bot:"bot",
    surprise:"surprise",oracle:"oracle",personality:"personality",
  };

  if(activeFeature&&['deepdive','vibe','bot'].includes(activeFeature)){
    const tile=tiles.find(t=>t.id===activeFeature);
    return(
      <div style={{padding:"0 18px 24px",position:"relative",minHeight:"85vh"}}>
        <div style={{position:"absolute",inset:0,background:"radial-gradient(ellipse at 30% 20%,"+tile.color+"12,transparent 60%)",pointerEvents:"none",zIndex:0}}/>
        <div style={{position:"relative",zIndex:1}}>
          <button onClick={()=>setActiveFeature(null)}
            style={{background:"rgba(255,255,255,0.05)",border:"1px solid rgba(255,255,255,0.1)",borderRadius:10,padding:"8px 14px",cursor:"pointer",color:"#b0a8b8",fontSize:13,fontFamily:"'DM Sans'",fontWeight:600,marginBottom:18,display:"flex",alignItems:"center",gap:6}}>
            ← Zurück
          </button>
          <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:20}}>
            <div style={{width:44,height:44,borderRadius:14,background:tile.color+"20",border:"1px solid "+tile.color+"40",display:"flex",alignItems:"center",justifyContent:"center",boxShadow:"0 0 20px "+tile.color+"30"}}>
              {SpotlightIcons[iconMap[activeFeature]]?.(tile.color,24)}
            </div>
            <h3 style={{fontFamily:"'Instrument Serif',serif",fontSize:20,margin:0,color:"#f0ece4"}}>{tile.title}</h3>
          </div>
  ...
  if(activeFeature&&['surprise','oracle','personality'].includes(activeFeature)){
    const extra=extras.find(e=>e.id===activeFeature);
    return(
      <div style={{padding:"0 18px 24px",position:"relative",minHeight:"85vh"}}>
        <div style={{position:"absolute",inset:0,background:"radial-gradient(ellipse at 50% 20%,"+extra.color+"10,transparent 60%)",pointerEvents:"none",zIndex:0}}/>
        <div style={{position:"relative",zIndex:1}}>
          <button onClick={()=>setActiveFeature(null)}
            style={{background:"rgba(255,255,255,0.05)",border:"1px solid rgba(255,255,255,0.1)",borderRadius:10,padding:"8px 14px",cursor:"pointer",color:"#b0a8b8",fontSize:13,fontFamily:"'DM Sans'",fontWeight:600,marginBottom:18,display:"flex",alignItems:"center",gap:6}}>
            ← Zurück
          ...
          <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:20}}>
            <div style={{width:44,height:44,borderRadius:14,background:extra.color+"20",border:"1px solid "+extra.color+"40",display:"flex",alignItems:"center",justifyContent:"center",boxShadow:"0 0 20px "+extra.color+"30"}}>
              {SpotlightIcons[iconMap[activeFeature]]?.(extra.color,24)}
            </div>
            <h3 style={{fontFamily:"'Instrument Serif',serif",fontSize:20,margin:0,color:"#f0ece4"}}>{extra.title}</h3>
          </div>
          <FunFeature mode={activeFeature} profile={profile} onSelect={onSelect}/>
        </div>
      </div>
    );
  }
  ...
  return(
    <div style={{position:"relative",minHeight:"85vh",overflow:"hidden"}}>
      ...
      <div style={{position:"absolute",inset:0,pointerEvents:"none",zIndex:0}}>
        <div style={{position:"absolute",top:"5%",left:"5%",width:260,height:260,background:"radial-gradient(circle,rgba(196,169,96,0.1),transparent 70%)",borderRadius:"50%",animation:"orbFloat1 8s ease-in-out infinite"}}/>
        <div style={{position:"absolute",top:"15%",right:"-5%",width:200,height:200,background:"radial-gradient(circle,rgba(232,67,147,0.08),transparent 70%)",borderRadius:"50%",animation:"orbFloat2 11s ease-in-out infinite"}}/>
        <div style={{position:"absolute",bottom:"25%",left:"15%",width:160,height:160,background:"radial-gradient(circle,rgba(74,222,128,0.07),transparent 70%)",borderRadius:"50%",animation:"orbFloat3 9s ease-in-out infinite"}}/>
        <div style={{position:"absolute",bottom:"5%",right:"10%",width:140,height:140,background:"radial-gradient(circle,rgba(167,139,250,0.08),transparent 70%)",borderRadius:"50%",animation:"orbFloat1 13s ease-in-out infinite reverse"}}/>
      </div>

      {/* Header */}
      <div style={{position:"relative",zIndex:1,padding:"20px 18px 16px"}}>
        <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:6}}>
          <div style={{width:48,height:48,borderRadius:14,background:"linear-gradient(135deg,#c4a960,#ff6b35)",display:"flex",alignItems:"center",justifyContent:"center",boxShadow:"0 4px 20px rgba(196,169,96,0.4)",flexShrink:0}}>
            {SpotlightIcons.surprise("#fff",24)}
          </div>
          <div>
            <h2 style={{fontFamily:"'Instrument Serif',serif",fontSize:32,margin:"0 0 1px",background:"linear-gradient(135deg,#c4a960,#f5e090)",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent",lineHeight:1}}>Spotlight</h2>
            <p style={{fontSize:10,color:"#6a6478",margin:0,letterSpacing:"1px",textTransform:"uppercase"}}>KI-Assistent · Persönlich · Präzise</p>
          </div>
        </div>
      </div>

      {/* Mosaic Grid */}
      <div style={{position:"relative",zIndex:1,padding:"0 14px",display:"grid",gridTemplateColumns:"1fr 1fr",gridTemplateRows:"auto auto auto",gap:10,marginBottom:10}}>

        {/* Deep Dive — große Kachel oben links, 2 Zeilen */}
        <button onClick={()=>setActiveFeature("deepdive")}
          style={{gridColumn:"1",gridRow:"1 / 3",background:tiles[0].gradient,border:"1px solid "+tiles[0].border,borderRadius:20,padding:"22px 18px",cursor:"pointer",textAlign:"left",position:"relative",overflow:"hidden",minHeight:200}}>
          <div style={{position:"absolute",top:-30,right:-30,width:120,height:120,background:"radial-gradient(circle,"+tiles[0].color+"25,transparent 70%)",pointerEvents:"none"}}/>
          <div style={{position:"absolute",bottom:-20,left:-10,width:100,height:100,background:"radial-gradient(circle,"+tiles[0].color+"15,transparent 70%)",pointerEvents:"none"}}/>
          <div style={{width:52,height:52,borderRadius:15,background:tiles[0].color+"18",border:"1px solid "+tiles[0].color+"35",display:"flex",alignItems:"center",justifyContent:"center",marginBottom:16,boxShadow:"0 0 20px "+tiles[0].color+"25"}}>
            {SpotlightIcons.deepdive(tiles[0].color,28)}
          </div>
          <div style={{fontSize:15,fontWeight:800,color:"#f0ece4",marginBottom:8,lineHeight:1.3}}>{tiles[0].title}</div>
          <div style={{fontSize:11,color:"#8a849a",lineHeight:1.5}}>{tiles[0].desc}</div>
          <div style={{position:"absolute",bottom:16,right:16,width:28,height:28,borderRadius:8,background:tiles[0].color+"18",border:"1px solid "+tiles[0].color+"30",display:"flex",alignItems:"center",justifyContent:"center"}}>
            <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke={tiles[0].color} strokeWidth="2.5" strokeLinecap="round"><polyline points="9 18 15 12 9 6"/></svg>
          </div>
        </button>

        {/* Vibe-Meter — kleine Kachel rechts oben */}
        <button onClick={()=>setActiveFeature("vibe")}
          style={{gridColumn:"2",gridRow:"1",background:tiles[1].gradient,border:"1px solid "+tiles[1].border,borderRadius:20,padding:"18px 16px",cursor:"pointer",textAlign:"left",position:"relative",overflow:"hidden"}}>
          <div style={{position:"absolute",top:-20,right:-20,width:80,height:80,background:"radial-gradient(circle,"+tiles[1].color+"25,transparent 70%)",pointerEvents:"none"}}/>
          <div style={{width:42,height:42,borderRadius:12,background:tiles[1].color+"18",border:"1px solid "+tiles[1].color+"35",display:"flex",alignItems:"center",justifyContent:"center",marginBottom:10,boxShadow:"0 0 16px "+tiles[1].color+"25"}}>
            {SpotlightIcons.vibe(tiles[1].color,22)}
          </div>
          <div style={{fontSize:13,fontWeight:800,color:"#f0ece4",marginBottom:5,lineHeight:1.3}}>{tiles[1].title}</div>
          <div style={{fontSize:10,color:"#8a849a",lineHeight:1.4}}>{tiles[1].desc}</div>
        </button>

        {/* Popcorn-Guru — kleine Kachel rechts unten */}
        <button onClick={()=>setActiveFeature("bot")}
          style={{gridColumn:"2",gridRow:"2",background:tiles[2].gradient,border:"1px solid "+tiles[2].border,borderRadius:20,padding:"18px 16px",cursor:"pointer",textAlign:"left",position:"relative",overflow:"hidden"}}>
          <div style={{position:"absolute",top:-20,right:-20,width:80,height:80,background:"radial-gradient(circle,"+tiles[2].color+"25,transparent 70%)",pointerEvents:"none"}}/>
          <div style={{width:42,height:42,borderRadius:12,background:tiles[2].color+"18",border:"1px solid "+tiles[2].color+"35",display:"flex",alignItems:"center",justifyContent:"center",marginBottom:10,boxShadow:"0 0 16px "+tiles[2].color+"25"}}>
            {SpotlightIcons.bot(tiles[2].color,22)}
          </div>
          <div style={{fontSize:13,fontWeight:800,color:"#f0ece4",marginBottom:5,lineHeight:1.3}}>{tiles[2].title}</div>
          <div style={{fontSize:10,color:"#8a849a",lineHeight:1.4}}>{tiles[2].desc}</div>
        </button>

        {/* Extras — 3 kleine Kacheln unten */}
        {extras.map((e,i)=>(
          <button key={e.id} onClick={()=>setActiveFeature(e.id)}
            style={{gridColumn:i===0?"1":i===1?"1":"2",gridRow:"3",background:"rgba(255,255,255,0.03)",border:"1px solid rgba(255,255,255,0.07)",borderRadius:16,padding:"14px 10px",cursor:"pointer",textAlign:"center",position:"relative",overflow:"hidden"}}>
            <div style={{width:36,height:36,borderRadius:10,background:e.color+"15",border:"1px solid "+e.color+"30",display:"flex",alignItems:"center",justifyContent:"center",margin:"0 auto 8px",boxShadow:"0 0 12px "+e.color+"20"}}>
              {SpotlightIcons[iconMap[e.id]]?.(e.color,20)}
            </div>
            <div style={{fontSize:10,fontWeight:800,color:"#d0c8d8",lineHeight:1.3}}>{e.title}</div>
          </button>
        ))}
      </div>
    </div>
  ...
  return(
    <div style={{minHeight:"100vh",background:"#09090f",color:"#f0ece4",fontFamily:"'DM Sans',sans-serif",paddingBottom:90}}>
      ...
      <div style={{paddingTop:14}}>
        ...
        {/* BROWSE */}
        {tab==="spotlight"&&(
          <SpotlightTab profile={profile} cardProps={cardProps} onSelect={setSelectedItem}/>
        )}
        {tab==="browse"&&( // Erkunden
          <div style={{padding:"0 18px"}}>
