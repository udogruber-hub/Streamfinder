...
// ── Platform Card ──
function PlatformCard({platform,profile,onSelect,browseType}){
  ...
  function handleToggle(){
    ...
    if(next&&lastTypeRef.current!==browseType){
    ...
    <div style={{background:"#12121f",borderRadius:18,overflow:"hidden",border:"1px solid #1e1e30",marginBottom:10}}>
      <div style={{padding:"14px 16px",display:"flex",alignItems:"center",gap:12}}>
        ...
        <div style={{flex:1}}>
          ...
          <div style={{fontSize:11,color:"#b0a8b8"}}>Top 3 nur für dich</div>
        </div>
        <button onClick={handleToggle} style={{background:open?"#ff6b3522":"#1a1a2e",border:open?"1px solid #ff6b3544":"1px solid #2a2340",borderRadius:10,padding:"7px 12px",color:open?"#ff6b35":"#b0a8b8",cursor:"pointer",fontSize:11,fontFamily:"'DM Sans'",fontWeight:700}}>{open?"▲":"🎯 Top 3"}</button>
      </div>
      {open&&(
        <div style={{padding:"0 16px 14px"}}>
          <div style={{height:1,background:"#1e1e30",marginBottom:12}}/>
          {loading&&<div style={{textAlign:"center",padding:12}}><div style={{fontSize:20,animation:"spin 1.5s linear infinite",display:"inline-block"}}>✨</div><p style={{fontSize:12,color:"#b0a8b8",marginTop:6}}>KI analysiert deinen Geschmack…</p></div>}
          {!loading&&recs&&recs.length===0&&<p style={{fontSize:12,color:"#b0a8b8",textAlign:"center"}}>Noch zu wenig Daten — bewerte ein paar Titel!</p>}
          {!loading&&recs&&recs.map((rec,i)=>(
            <div key={i} style={{display:"flex",gap:10,padding:"10px 0",borderBottom:i<recs.length-1?"1px solid #1e1e30":"none",alignItems:"flex-start",cursor:"pointer"}} onClick={()=>onSelect&&onSelect({title:rec.title,name:rec.title,overview:rec.reason,vote_average:0,genre_ids:[],poster_path:null,media_type:browseType==="serie"?"tv":"movie",release_date:rec.year||"",_aiOnly:true})}>
              ...
              <div style={{fontSize:14,fontWeight:900,color:"#3a3344",flexShrink:0}}>#{i+1}</div>
            </div>
          ))}
        </div>
      )}
...
function MainApp({profile:initProfile,onReset}){
  ...
  async function doAiRefresh(){
    ...
    const msg=`Empfehle 6 frische Titel.\nPlattformen: ${ctx.platforms}.\nLIEBT: ${ctx.top||"nichts"}.\nHASST: ${ctx.low||"nichts"}.\nGESEHEN (NICHT empfehlen!): ${ctx.watched||"nichts"}.\nBLOCKIERT (NICHT empfehlen!): ${ctx.blocked||"nichts"}.\nGenres: ${ctx.topGenres||"gemischt"}.\nMix Serien + Filme. reason = 1 witziger Satz Deutsch.`;
    try{
      const text=await callAI([{role:"user",content:msg}],system);
          ...
          <div style={{padding:"0 18px"}}>
            ...
            {searchQuery?(
              ...
              <div>
                ...
                {userPlatforms.map(p=><PlatformCard key={p.id} platform={p} profile={profile} onSelect={setSelectedItem} browseType={browseType}/>)}
