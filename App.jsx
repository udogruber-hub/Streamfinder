                ...
                <SwipeToBlock onBlock={()=>{
                  ...
                  Promise.all([
                    ...
                    setRecs(cur=>{
                      ...
                      if(replacement){
                        const ctx=buildCtx(profile);
                        // KI Begründung für Ersatztitel
                        const system=`Antworte NUR mit einem JSON-Objekt: {"reason":"..."}. Kein Markdown.`;
                        const msg=`Schreibe einen witzigen Satz warum "${replacement.title||replacement.name}" perfekt für diesen Nutzer ist.\nNutzer liebt: ${ctx.top||"gute Serien"}.\nMin 10 Wörter, persönlich und kreativ.`;
                        callAI([{role:"user",content:msg}],system).then(text=>{
                          try{
                            const r=JSON.parse(text.replace(/```json|```/g,"").trim());
                            setRecs(c=>c?c.map(x=>x.title===(replacement.title||replacement.name)?{...x,reason:r.reason}:x):c);
                          }catch(e){}
                        }).catch(()=>{});
                        return[...cur,{
