...
function LedStrip({itemTitle,profile,onRate,genre_ids}){
  ...
  const current=(profile.ratings||{})[key]||0;
  const segments=[
    {v:1,color:"#ef4444",glow:"rgba(239,68,68,0.5)",label:"Geht gar nicht"},
    {v:3,color:"#f59e0b",glow:"rgba(245,158,11,0.5)",label:"Ok"},
    {v:5,color:"#a3e635",glow:"rgba(163,230,53,0.5)",label:"Top"},
  ];
  return(
    <div style={{display:"flex",alignItems:"center",gap:4}}>
      <div style={{display:"flex",flex:1,height:14,borderRadius:20,overflow:"hidden",gap:1,background:"rgba(255,255,255,0.03)"}}>
        {segments.map((seg,i)=>{
          const active=current===seg.v;
          return(
            <button key={seg.v}
              onClick={e=>{e.stopPropagation();e.preventDefault();onRate(itemTitle,current===seg.v?0:seg.v,genre_ids||[]);}}
              style={{
                flex:1,
                background:active?"linear-gradient(90deg,"+seg.color+"99,"+seg.color+"cc)":"rgba(255,255,255,0.04)",
                border:"none",
                cursor:"pointer",
                transition:"all 0.2s ease",
                boxShadow:active?"0 0 10px 2px "+seg.glow+", inset 0 0 6px rgba(255,255,255,0.1)":"none",
                borderRadius:i===0?"20px 0 0 20px":i===2?"0 20px 20px 0":"0",
                position:"relative",
                overflow:"hidden",
              }}>
              {active&&<div style={{position:"absolute",inset:0,background:"linear-gradient(90deg,transparent,rgba(255,255,255,0.12),transparent)",pointerEvents:"none"}}/>}
              <span style={{
                position:"absolute",inset:0,display:"flex",alignItems:"center",justifyContent:"center",
                fontSize:8,fontWeight:700,
                color:active?"rgba(255,255,255,0.9)":"rgba(255,255,255,0.15)",
                fontFamily:"'DM Sans'",pointerEvents:"none",letterSpacing:"0.3px",
                transition:"color 0.2s",
              }}>{active?seg.label:""}</span>
            </button>
          );
        })}
      </div>
...
function TitleCard({item,profile,onRate,onLike,onWatched,onBlock,onSelect}){
  ...
  return(
    <div style={{background:"#12121f",borderRadius:18,overflow:"hidden",border:"1px solid #1e1e30",marginBottom:8,position:"relative"}}>
      ...
      {/* LED Streifen */}
      <div onClick={e=>e.stopPropagation()} style={{padding:"0 14px 12px"}}>
  ...
  return(
    <div style={{padding:"0 18px 24px"}}>
      ...
      <div style={{marginBottom:22,position:"relative"}}>
        <div style={{position:"absolute",top:-20,left:"50%",transform:"translateX(-50%)",width:200,height:200,background:"radial-gradient(circle,rgba(196,169,96,0.12) 0%,transparent 70%)",pointerEvents:"none"}}/>
        <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:6}}>
          <div style={{width:36,height:36,borderRadius:10,background:"linear-gradient(135deg,#c4a960,#ff6b35)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:18}}>✨</div>
          <h2 style={{fontFamily:"'Instrument Serif',serif",fontSize:28,margin:0,background:"linear-gradient(135deg,#c4a960,#ff6b35)",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent"}}>Spotlight</h2>
        </div>
        <p style={{fontSize:12,color:"#b0a8b8",margin:0,lineHeight:1.5}}>Dein persönlicher KI-Assistent — findet genau was du suchst</p>
      </div>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:12,marginBottom:24}}>
          {mainFeatures.map((f,i)=>(
            <button key={f.id} onClick={()=>setActiveFeature(f.id)}
              style={{background:f.gradient,border:"1px solid "+f.border,borderRadius:20,padding:"18px 20px",cursor:"pointer",textAlign:"left",width:"100%",position:"relative",overflow:"hidden"}}>
              {/* Glow accent */}
              <div style={{position:"absolute",top:-30,right:-30,width:100,height:100,background:"radial-gradient(circle,"+f.color+"22 0%,transparent 70%)",pointerEvents:"none"}}/>
              <div style={{display:"flex",alignItems:"center",gap:14}}>
                <div style={{width:52,height:52,borderRadius:15,background:f.color+"18",border:"1px solid "+f.color+"33",display:"flex",alignItems:"center",justifyContent:"center",fontSize:26,flexShrink:0,boxShadow:"0 0 20px "+f.color+"22"}}>{f.icon}</div>
                <div style={{flex:1}}>
                  <div style={{fontSize:16,fontWeight:800,color:"#f0ece4",marginBottom:5}}>{f.title}</div>
                  <div style={{fontSize:12,color:"#8a849a",lineHeight:1.5}}>{f.desc}</div>
                </div>
      ...
      {/* Divider */}
      <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:14,padding:"0 18px"}}>
        <div style={{flex:1,height:1,background:"#1e1e30"}}/>
        <span style={{fontSize:10,color:"#3a3344",fontWeight:600,letterSpacing:"1px",textTransform:"uppercase"}}>Extras</span>
        <div style={{flex:1,height:1,background:"#1e1e30"}}/>
      </div>
      ...
      <div style={{display:"flex",gap:8,marginBottom:16,padding:"0 18px"}}>
        {extras.map(e=>(
          <button key={e.id} onClick={()=>setActiveFeature(activeFeature===e.id?null:e.id)}
            style={{flex:1,background:activeFeature===e.id?"rgba(196,169,96,0.1)":"rgba(255,255,255,0.03)",border:"1px solid "+(activeFeature===e.id?"rgba(196,169,96,0.3)":"rgba(255,255,255,0.07)"),borderRadius:12,padding:"12px 8px",cursor:"pointer",textAlign:"center"}}>
            <div style={{fontSize:20,marginBottom:4}}>{e.icon}</div>
            <div style={{fontSize:11,fontWeight:700,color:activeFeature===e.id?"#c4a960":"#f0ece4",marginBottom:2}}>{e.title}</div>
            <div style={{fontSize:9,color:"#555"}}>{e.desc}</div>
          </button>
        ))}
      </div>
