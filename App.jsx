  ...
  async function loadFirstRecs(stimmung){
    ...
    try{
      ...
      try{
        ...
        // Kein Provider-Filter im Fallback — sonst zu wenig Ergebnisse
        const [rs,rf]=await Promise.all([
          fetch(TMDB_BASE+"/discover/tv?api_key="+TMDB_API_KEY+"&language=de-DE&sort_by=vote_average.desc&vote_count.gte=100&vote_average.gte=6.5&with_genres="+gStr+"&page=1").then(r=>r.json()),
          fetch(TMDB_BASE+"/discover/movie?api_key="+TMDB_API_KEY+"&language=de-DE&sort_by=vote_average.desc&vote_count.gte=200&vote_average.gte=6.5&with_genres="+gStr+"&page=1").then(r=>r.json()),
        ]);
        const combined=[
          ...(rs.results||[]).map(it=>({...it,media_type:"tv"})),
          ...(rf.results||[]).map(it=>({...it,media_type:"movie"})),
        ].sort((a,b)=>(b.vote_average||0)-(a.vote_average||0))
         .filter(it=>!shownTitles.current.has(it.id));
        combined.forEach(it=>shownTitles.current.add(it.id));
        recReserve.current=combined.slice(3);
        setStimmungRecs(combined.slice(0,3));
      }catch{}
    }
...
Kombiniere Stimmung und persönlichen Geschmack.`;
      ...
      try{
        const gStr=stimmung.genres.join("|");
        const [rs2,rf2]=await Promise.all([
          fetch(TMDB_BASE+"/discover/tv?api_key="+TMDB_API_KEY+"&language=de-DE&sort_by=vote_average.desc&vote_count.gte=100&vote_average.gte=6.5&with_genres="+gStr+"&page=1").then(r=>r.json()),
          fetch(TMDB_BASE+"/discover/movie?api_key="+TMDB_API_KEY+"&language=de-DE&sort_by=vote_average.desc&vote_count.gte=200&vote_average.gte=6.5&with_genres="+gStr+"&page=1").then(r=>r.json()),
        ]);
        const combined2=[
          ...(rs2.results||[]).map(it=>({...it,media_type:"tv"})),
          ...(rf2.results||[]).map(it=>({...it,media_type:"movie"})),
        ].sort((a,b)=>(b.vote_average||0)-(a.vote_average||0));
        setStimmungRecs(combined2.slice(0,3));
        setStimmungReserve(combined2.slice(3));
      }catch{}
    }
    setStimmungLoading(false);
  }
