  ...
  async function loadFirstRecs(stimmung){
    ...
    try{
      ...
      try{
        const allIds=(profile.platforms||[]).flatMap(id=>PLATFORMS.find(p=>p.id===id)?.tmdbIds||[]);
        const topGenres=Object.entries(profile.genres||{}).filter(([,v])=>v>0).sort(([,a],[,b])=>b-a).slice(0,3).map(([g])=>g);
        const gStr=stimmung.genres.join("|"); // Immer Stimmungs-Genres im Fallback
        const r=await discoverTitles("serie",allIds,gStr,1,"vote_average.desc",null);
        const items=(r.results||[]).filter(it=>(it.vote_average||0)>=6.5&&!shownTitles.current.has(it.id));
        items.forEach(it=>shownTitles.current.add(it.id));
        recReserve.current=items.slice(3).map(it=>({...it,media_type:"tv"}));
        setStimmungRecs(items.slice(0,3).map(it=>({...it,media_type:"tv"})));
      }catch{}
    }
