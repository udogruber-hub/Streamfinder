...
function getAIRecommendations(profile, mood, type) {
  var liked = (profile.watched || []).map(function(w) { return w.title; }).join(", ");
  var thumbsUpGenres = [];
  ...
  if (mood) {
    ...
    if (m) moodText = "Aktuelle Stimmung: " + m.name + " (" + m.desc + "). ";
  }
  ...
  var prompt = "Du bist ein Streaming-Experte. Empfehle genau 8 " + (type === "serie" ? "Serien" : type === "film" ? "Filme" : "Filme und Serien") + " die aktuell auf diesen Streaming-Diensten in Deutschland verfügbar sind: " + platforms + ". " +
    moodText +
    typeText +
    (liked ? "Der User hat zuletzt geschaut: " + liked + ". " : "") +
    (thumbsUpGenres.length > 0 ? "Bevorzugte Genres: " + thumbsUpGenres.join(", ") + ". " : "") +
    "Antworte NUR mit einem JSON-Array. Jedes Element hat: title (exakter Titel), year (Erscheinungsjahr als Zahl), type (\"movie\" oder \"tv\"), reason (1 Satz auf Deutsch warum der User das mögen wird). Kein anderer Text, nur das JSON-Array.";
...
function getAISurprise(profile, mood) {
  var liked = (profile.watched || []).map(function(w) { return w.title; }).join(", ");
  var thumbsUpGenres = [];
  Object.entries(profile.genres || {}).forEach(function(e) {
    if (e[1] > 2) {
  ...
  if (mood) {
    var m = MOODS.find(function(x) { return x.id === mood; });
    if (m) moodText = "Stimmung: " + m.name + ". ";
  }

  var prompt = "Du bist ein witziger Streaming-Berater. Der User kann sich nicht entscheiden was er schauen soll. " +
    moodText +
    "Verfügbare Dienste: " + platforms + ". " +
    (liked ? "Zuletzt geschaut: " + liked + ". " : "") +
    (thumbsUpGenres.length > 0 ? "Mag: " + thumbsUpGenres.join(", ") + ". " : "") +
    "Empfehle EINEN einzigen Titel der perfekt passt. Antworte NUR mit JSON: {\"title\": \"...\", \"year\": 2024, \"type\": \"movie\" oder \"tv\", \"reason\": \"lustiger kurzer Spruch auf Deutsch warum genau DAS heute Abend geschaut werden muss (2-3 Sätze, locker und witzig)\", \"tagline\": \"kurzer witziger Einzeiler wie z.B. Dein Sofa hat gesprochen!\"}. Nur JSON, kein anderer Text.";
      ...
      <div style={{ width: "100%", maxWidth: 600 }}>
        ...
        {step === 0 ? (
          <div style={{ animation: "fadeUp 0.4s" }}>
            <h1 style={{ fontFamily: "'Instrument Serif', serif", fontSize: 34, margin: "0 0 8px", background: "linear-gradient(135deg, #ff6b35, #e84393)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>Deine Plattformen</h1>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", marginTop: 28 }}>
          ...
          <button onClick={function() { if (step < 2) setStep(step + 1); else finish(); }} disabled={!canNext()} style={{
            background: canNext() ? "linear-gradient(135deg, #ff6b35, #e84393)" : "#1a1a2e",
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
            <div style={{ background: scoreColor + "18", padding: "3px 10px", borderRadius: 8, display: "flex", alignItems: "center", gap: 4 }}>
              <span style={{ color: "#f5c518", fontSize: 11, fontWeight: 900 }}>★</span>
        ...
        <div style={{ padding: "0 16px 18px", animation: "fadeUp 0.3s ease" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10 }}>
            <div style={{ background: scoreColor + "18", border: "1px solid " + scoreColor + "33", borderRadius: 12, padding: "6px 14px", display: "flex", alignItems: "center", gap: 6 }}>
              <span style={{ color: "#f5c518", fontSize: 14, fontWeight: 900 }}>TMDB</span>
...
function MainApp(props) {
  ...
  function loadData() {
    ...
    var aiPromises = [
      getAIRecommendations(profile, mood, null),      // For You (mixed)
      getAIRecommendations(profile, mood, "serie"),    // Series
      getAIRecommendations(profile, mood, "film"),     // Films
    ];
  ...
  function handleSurprise() {
    ...
    getAISurprise(profile, mood).then(function(aiResult) {
            ...
            {MOODS.map(function(m) {
              return (
                <button key={m.id} onClick={function() { changeMood(m.id); }} style={{
                  background: mood === m.id ? "linear-gradient(135deg, #ff6b3520, #e8439320)" : "linear-gradient(135deg, #1a1525, #1e1830)",
                  border: mood === m.id ? "1px solid #ff6b3555" : "1px solid #2a2340",
                  borderRadius: 14, padding: "10px 16px", cursor: "pointer",
                  color: mood === m.id ? "#f0ece4" : "#8a7e6e",
                  fontFamily: "'DM Sans'", fontSize: 14, fontWeight: 700, whiteSpace: "nowrap", flexShrink: 0,
