...
function UniversalSwipe({profile,cardProps,onSelect,onDone,browseType}){
  ...
  async function handleDone(){
    ...
    try{
      ...
      const system=`Du bist ein Streaming-Experte der Geschmack tief versteht. Antworte NUR mit JSON-Array, kein Markdown. Format: [{"title":"...","platform":"...","type":"Serie oder Film","reason":"..."}]. type MUSS entweder genau "Serie" oder genau "Film" sein - niemals mischen. Pro Plattform: 3 Serien UND 2 Filme oder 2 Serien UND 3 Filme. Plattform exakt so wie angegeben.`;
      ...
      if(Array.isArray(parsed)&&parsed.length>0){
        ...
        await Promise.all(userPlats.map(async plat=>{
          ...
          if(platItems.length>0){
            // Nur den aktuell geswipen Typ speichern
            const swType=browseType||"serie";
            await Promise.all([swType].map(async bt=>{
              try{
                const enriched=await Promise.all(platItems.map(r=>enrichWithTMDB({title:r.title,type:swType==="serie"?"Serie":"Film"},profile)));
      ...
      {searchQuery?(
        ...
        <div>
          ...
          <div style={{background:"linear-gradient(135deg,#1a1525,#0f0e1a)",borderRadius:18,padding:"16px",marginBottom:16,border:"1px solid #2a1f3d"}}>
            ...
            {/* 2 Swipe Buttons — Serien & Filme getrennt */}
            <div style={{display:"flex",gap:10,marginBottom:14}}>
              {[{type:"serie",label:"Serien entdecken",icon:"📺",color:"#e84393"},{type:"film",label:"Filme entdecken",icon:"🎬",color:"#ff6b35"}].map(btn=>(
                <button key={btn.type} onClick={()=>{setSwipeType(btn.type);setShowUniversalSwipe(true);}}
                  style={{flex:1,background:"linear-gradient(135deg,rgba(232,67,147,0.08),rgba(255,107,53,0.05))",border:"1px solid "+btn.color+"44",borderRadius:14,padding:"14px 12px",cursor:"pointer",display:"flex",flexDirection:"column",alignItems:"center",gap:6}}>
                  <div style={{width:38,height:38,borderRadius:10,background:btn.color+"22",border:"1px solid "+btn.color+"44",display:"flex",alignItems:"center",justifyContent:"center",fontSize:18}}>{btn.icon}</div>
                  <div style={{fontSize:12,fontWeight:800,color:"#f0ece4"}}>{btn.label}</div>
                  <div style={{fontSize:10,color:"#b0a8b8"}}>Rechts ❤️ · Links ✕</div>
                </button>
              ))}
            </div>
