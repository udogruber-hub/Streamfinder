...
function PlatformCard({platform,profile,onSelect,onBlock,onLike,onRate,browseType}){
  ...
  // Lade TMDB Top5 als Fallback
  async function loadTMDBRecs(){
    setLoading(true);
    try{
      const items=await getTMDBTop5(profile,platform,browseType||"serie");
      if(items.length>0){
        ...
        reserveRef.current=items.slice(5);
        setRecs(formatted);
        storeRecs(formatted);
      }else{setRecs([]);}
    }catch(e){setRecs([]);}
  ...
  // Ersetze Titel durch Ersatz aus Reserve oder lade neu
  async function replaceRec(removedTitle){
    ...
    if(replacement){
      reserveRef.current=reserve.filter(r=>r.id!==replacement.id);
      const newRec={
        id:replacement.id,
        title:replacement.title||replacement.name||"",
        year:(replacement.release_date||replacement.first_air_date||"").substring(0,4),
        reason:replacement.overview?replacement.overview.substring(0,140)+(replacement.overview.length>140?"…":""):"",
        emoji:replacement.media_type==="tv"?"📺":"🎬",
        _tmdbItem:replacement,
        _isNew:true,
      };
      setRecs(prev=>{
        const next=prev?[...prev.filter(r=>titleKey(r.title)!==key),newRec]:null;
        if(next)storeRecs(next);
        return next;
      });
    }else{
      // Reserve leer → TMDB nachladen
      setRecs(prev=>{
        const next=prev?prev.filter(r=>titleKey(r.title)!==key):null;
        if(next)storeRecs(next);
        return next;
      });
      const fresh=await getTMDBTop5(profile,platform,browseType||"serie");
      const currentTitles=new Set((recs||[]).map(r=>titleKey(r.title)));
      const newItems=fresh.filter(it=>!currentTitles.has(titleKey(it.title||it.name||""))&&titleKey(it.title||it.name||"")!==key);
      if(newItems.length>0){
        const newRec={
          id:newItems[0].id,
          title:newItems[0].title||newItems[0].name||"",
          year:(newItems[0].release_date||newItems[0].first_air_date||"").substring(0,4),
          reason:newItems[0].overview?newItems[0].overview.substring(0,140)+"…":"",
          emoji:newItems[0].media_type==="tv"?"📺":"🎬",
          _tmdbItem:newItems[0],
          _isNew:true,
        };
        reserveRef.current=newItems.slice(1);
        setRecs(prev=>{
          const next=prev?[...prev,newRec]:null;
          if(next)storeRecs(next);
          return next;
        });
      }
  ...
  // browseType wechsel → neu laden
  useEffect(()=>{
    if(open){setRecs(null);loadTMDBRecs();}
  },[browseType]);

  // Events: watched/blocked entfernen + ersetzen
  useEffect(()=>{
    function onWatched(e){
      setRecs(prev=>{
        if(!prev)return prev;
        const has=prev.find(r=>r._tmdbItem?.id===e.detail.id||titleKey(r.title)===titleKey(e.detail.title));
        if(has)replaceRec(has.title);
        return prev; // replaceRec setzt neuen State
      });
    }
    function onBlocked(e){
      setRecs(prev=>{
        if(!prev)return prev;
        const has=prev.find(r=>titleKey(r.title)===titleKey(e.detail.title));
        if(has)replaceRec(has.title);
        return prev;
      });
    }
    // Nach Swipe: KI-Recs aus localStorage laden (pro Plattform gespeichert)
...
function UniversalSwipe({profile,cardProps,onSelect,onDone}){
  ...
  async function handleDone(){
    ...
    try{
      ...
      const system=`Du bist ein Streaming-Experte. Antworte NUR mit JSON-Array, kein Markdown. Format: [{"title":"...","platform":"...","reason":"..."}]. Plattform exakt so wie angegeben.`;
      const msg=`Empfehle GENAU 5 Titel pro Plattform:\n❤️ GEMOCHT: ${liked_sw.join(", ")||"nichts"}\n✕ ABGELEHNT: ${disliked_sw.join(", ")||"nichts"}\nWatchlist: ${ctx.watchlist||"leer"}\nPlattformen: ${platNames}\n\n- Orientiere dich STARK an gemochten Titeln\n- Vermeide Ähnliches zu abgelehnten\n- Nur auf der jeweiligen Plattform verfügbar\n- reason = witziger deutscher Satz max 12 Wörter`;
      const text=await callAI([{role:"user",content:msg}],system);
      const parsed=JSON.parse(text.replace(/```json|```/g,"").trim());
      if(Array.isArray(parsed)&&parsed.length>0){
        // Pro Plattform getrennt speichern
        userPlats.forEach(plat=>{
          ...
          if(platItems.length>0){
            ["serie","film"].forEach(bt=>{
              try{localStorage.setItem("sf_plat_"+plat.id+"_"+bt,JSON.stringify(platItems.map(r=>({
                id:null,title:r.title,year:"",reason:r.reason||"",
                emoji:"🎬",_tmdbItem:null,_needsEnrich:true,
              }))));}catch(e){}
            });
          }
        });
...
function MainApp({profile:initProfile,onReset}){
  ...
  function handleRate(itemTitle,stars,directGenreIds){
    ...
    if(stars<=2){
      ...
      if(replacement){
        ...
        setFeedItems(prev=>{const n=[...prev.filter(it=>titleKey(it.title||it.name||'')!==key),{...replacement,_isNew:true}];feedItemsRef.current=n;return n;});
    ...
    if(replacement){
      ...
      getSmartRecommendations(profile).then(newItems=>{
        ...
        if(fresh.length>0){
          ...
          setFeedItems(prev=>{const n=[...prev.filter(it=>titleKey(it.title||it.name||"")!==key),toShow];feedItemsRef.current=n;return n;});
          setBaseFeedItems(prev=>[...prev.filter(it=>titleKey(it.title||it.name||"")!==key),toShow]);
  ...
  async function loadHomeFeed(){
    ...
    try{
      ...
      setFeedItems(feed);feedItemsRef.current=feed;
      setBaseHeroItems(hero);
      setBaseFeedItems(feed);
      reserveItems.current=items.slice(20);
    }catch(e){}
...
function Onboarding({onComplete}){
  ...
  const STEPS=[
    ...
    {
      title:"Was schaust du gerne?",
      subtitle:"Wähle deine Lieblingsgenres",
      type:"genres",
    },
  ];
  ...
  function handleNext(){
    if(step===0){setStep(1);setSelected([]);}
      ...
      // Store platforms temp
      window._ob_plats=plats;
    }else{
      const genres={};
      selected.forEach(g=>{genres[g]=3;});
      const profile={
        platforms:window._ob_plats||[],
        genres,
        liked:[],liked_items:[],liked_titles:[],
        blocked_titles:[],watched:[],ratings:{},
        languages:["en","de"],
      };
      onComplete(profile);
    }
      ...
      <div style={{maxWidth:420,width:"100%"}}>
        ...
        {cur.type==="genres"&&(
          <div style={{display:"flex",flexWrap:"wrap",gap:8,marginBottom:24,justifyContent:"center"}}>
            {Object.entries(GENRES_TMDB).filter(([id])=>![10752,37,10759,10765].includes(Number(id))).map(([id,name])=>{
              const active=selected.includes(id);
              const m=MOODS.find(x=>x.genres.includes(Number(id)));
              const color=m?.color||"#c4a960";
              return(
                <button key={id} onClick={()=>handleGenreToggle(id)}
                  style={{background:active?color+"22":"transparent",border:"1px solid "+(active?color+"88":color+"33"),borderRadius:20,padding:"8px 16px",cursor:"pointer",color:active?color:"#888",fontFamily:"'DM Sans'",fontWeight:active?700:400,fontSize:12,transition:"all 0.2s"}}>
                  {name}
                </button>
              );
            })}
