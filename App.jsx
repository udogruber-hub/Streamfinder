    ...
    <div style={{background:"#12121f",borderRadius:18,overflow:"hidden",border:"1px solid #1e1e30",marginBottom:10}}>
      ...
      {open&&mode==="swipe"&&(
        <div style={{padding:"0 18px 48px"}}>
          ...
          <PlatformSwipe platform={platform} profile={profile} browseType={browseType} onBlock={t=>onBlock(t)} onLike={it=>onLike(it)} onSelect={onSelect} onRate={onRate||((t,s,g)=>{})} onDone={(freshProfile)=>{
            setMode("top5");
            lastTypeRef.current=null;
            // Berechne Top5 mit frischem Profil das Swipe-Ergebnisse enthält
            setLoading(true);setRecs(null);
            getTMDBTop5(freshProfile||profile,platform,browseType||"serie")
              .then(tmdbItems=>{
                if(tmdbItems.length>0){
                  ...
                  setRecs(initial);setLoading(false);
                  // KI Begründungen
                  const ctx=buildCtx(freshProfile||profile);
                  const titles=tmdbItems.map(it=>it.title||it.name||"").join(", ");
                  const typeStr=browseType==="serie"?"Serien":"Filme";
                  const system=`Du bist ein witziger Streaming-Experte. Antworte NUR mit JSON-Array, keine Backticks. Format: [{"title":"...","reason":"..."}]`;
                  ...
                  callAI([{role:"user",content:msg}],system).then(text=>{
                    try{
                      const aiR=JSON.parse(text.replace(/```json|```/g,"").trim());
                      setRecs(prev=>prev?prev.map((rec,i)=>{
                        const match=aiR.find(a=>titleKey(a.title||"")===titleKey(rec.title))||aiR[i];
                        return match?.reason?{...rec,reason:match.reason}:rec;
                      }):prev);
                    }catch(e){}
                  }).catch(()=>{});
                }else{setRecs([]);setLoading(false);}
              }).catch(()=>{setRecs([]);setLoading(false);});
          }}/>
        </div>
      )}
