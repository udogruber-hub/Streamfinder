    ...
    <div style={{background:"#12121f",borderRadius:18,overflow:"hidden",border:"1px solid #1e1e30",marginBottom:10}}>
      ...
      {open&&(
        <div style={{padding:"0 14px 16px"}}>
          ...
          {!loading&&recs&&[...recs].sort((a,b)=>(b.score||b._tmdbItem?.vote_average||0)-(a.score||a._tmdbItem?.vote_average||0)).map((rec,i)=>{
...
function SpotlightTab({profile,cardProps,onSelect}){
  ...
  // Lade hochwertige Filmbilder von TMDB
  useEffect(()=>{
  ...
  const tiles=[
    {id:"deepdive",title:"Sag mir was mir gefällt…",hint:"Nenn einen Lieblingstitel → wir bauen dein Universum",color:"#c4a960",accent:"rgba(196,169,96,0.9)"},
    {id:"vibe",title:"Vibe-Meter",hint:"Stimmung einstellen → perfekter Titel",color:"#e84393",accent:"rgba(232,67,147,0.9)"},
    {id:"bot",title:"Frag den Popcorn-Guru",hint:"Dein persönlicher Experte",color:"#4ade80",accent:"rgba(74,222,128,0.9)"},
  ];

  const extras=[
    {id:"surprise",title:"Überrasch mich",hint:"1 perfekter Titel",color:"#c4a960",icon:"surprise"},
    {id:"oracle",title:"Orakel",hint:"Mystische Prophezeiung",color:"#a78bfa",icon:"oracle"},
    {id:"personality",title:"Welcher Filmtyp bin ich?",hint:"Dein Profil",color:"#e84393",icon:"personality"},
  ];
  ...
  if(activeFeature){
    ...
    return(
      <div style={{position:"relative",minHeight:"85vh",overflow:"hidden"}}>
        {bg&&<img src={bg} alt="" style={{position:"fixed",top:0,left:0,width:"100%",height:"100%",objectFit:"cover",opacity:0.08,zIndex:0,pointerEvents:"none"}}/>}
  ...
  return(
    <div style={{display:"flex",flexDirection:"column",height:"calc(100vh - 120px)"}}>
      ...
      <div style={{flexShrink:0,padding:"16px 18px 12px",display:"flex",alignItems:"center",gap:12}}>
        <div style={{width:46,height:46,borderRadius:14,background:"linear-gradient(135deg,#c4a960,#ff6b35)",display:"flex",alignItems:"center",justifyContent:"center",boxShadow:"0 4px 20px rgba(196,169,96,0.45)",flexShrink:0}}>
      ...
      <div style={{flex:1,display:"flex",flexDirection:"column",gap:8,padding:"0 12px 8px",minHeight:0}}>
        ...
        {/* Deep Dive — Hauptkachel */}
        <button onClick={()=>setActiveFeature("deepdive")}
          style={{flex:2.2,background:"#0a0a12",borderRadius:22,cursor:"pointer",textAlign:"left",position:"relative",overflow:"hidden",border:"none",minHeight:0}}>
          {bgImgs[0]&&<img src={bgImgs[0]} alt="" style={{position:"absolute",inset:0,width:"100%",height:"100%",objectFit:"cover",opacity:0.5}}/>}
          {/* Gradient overlay — bottom strong, top light */}
          ...
          <div style={{position:"absolute",top:0,left:0,right:0,height:2,background:"linear-gradient(90deg,transparent,#c4a960,transparent)"}}/>
          <div style={{position:"relative",padding:"16px 18px",height:"100%",display:"flex",flexDirection:"column",justifyContent:"space-between"}}>
            {/* Top: Icon */}
            <div style={{width:44,height:44,borderRadius:13,background:"rgba(196,169,96,0.2)",border:"1.5px solid rgba(196,169,96,0.5)",display:"flex",alignItems:"center",justifyContent:"center",backdropFilter:"blur(8px)",boxShadow:"0 0 20px rgba(196,169,96,0.3)"}}>
              {SpotlightIcons.deepdive("#c4a960",24)}
            </div>
            {/* Bottom: Text */}
            <div>
              <div style={{fontFamily:"'Instrument Serif',serif",fontSize:28,fontWeight:400,color:"#ffffff",marginBottom:8,letterSpacing:"-0.3px",textShadow:"0 2px 12px rgba(0,0,0,0.7)",lineHeight:1.1}}>{tiles[0].title}</div>
              <div style={{fontSize:14,color:"rgba(255,255,255,0.75)",lineHeight:1.5,textShadow:"0 1px 4px rgba(0,0,0,0.8)",fontWeight:500}}>{tiles[0].hint}</div>
            </div>
          </div>
          {/* Arrow */}
          <div style={{position:"absolute",bottom:16,right:16,width:34,height:34,borderRadius:10,background:"rgba(196,169,96,0.25)",border:"1.5px solid rgba(196,169,96,0.5)",display:"flex",alignItems:"center",justifyContent:"center",backdropFilter:"blur(8px)"}}>
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#c4a960" strokeWidth="2.5" strokeLinecap="round"><polyline points="9 18 15 12 9 6"/></svg>
          </div>
        </button>
        ...
        <div style={{flex:1.6,display:"flex",gap:8,minHeight:0}}>
          {tiles.slice(1).map((t,ri)=>(
            <button key={t.id} onClick={()=>setActiveFeature(t.id)}
              style={{flex:1,background:"#0a0a12",borderRadius:20,cursor:"pointer",textAlign:"left",position:"relative",overflow:"hidden",border:"none",minHeight:0}}>
              {bgImgs[ri+1]&&<img src={bgImgs[ri+1]} alt="" style={{position:"absolute",inset:0,width:"100%",height:"100%",objectFit:"cover",opacity:0.4}}/>}
              ...
              <div style={{position:"relative",padding:"14px 12px",height:"100%",display:"flex",flexDirection:"column",justifyContent:"space-between"}}>
                <div style={{width:38,height:38,borderRadius:11,background:t.color+"22",border:"1.5px solid "+t.color+"55",display:"flex",alignItems:"center",justifyContent:"center",backdropFilter:"blur(8px)",boxShadow:"0 0 16px "+t.color+"30"}}>
                  {SpotlightIcons[t.id]?.(t.color,20)}
                </div>
                <div>
                  <div style={{fontFamily:"'Instrument Serif',serif",fontSize:19,color:"#ffffff",marginBottom:5,textShadow:"0 2px 8px rgba(0,0,0,0.7)",lineHeight:1.15}}>{t.title}</div>
                  <div style={{fontSize:11,color:"rgba(255,255,255,0.65)",lineHeight:1.4,textShadow:"0 1px 4px rgba(0,0,0,0.8)",fontWeight:500}}>{t.hint}</div>
                </div>
              </div>
            </button>
          ))}
        ...
        <div style={{flex:1,display:"flex",gap:8,minHeight:0}}>
          {extras.map((e,ei)=>{
            const extraBg=["rgba(196,169,96,0.06)","rgba(167,139,250,0.06)","rgba(232,67,147,0.06)"][ei];
            const extraBorder=["rgba(196,169,96,0.2)","rgba(167,139,250,0.2)","rgba(232,67,147,0.2)"][ei];
            return(
              <button key={e.id} onClick={()=>setActiveFeature(e.id)}
                style={{flex:1,background:extraBg,border:"1px solid "+extraBorder,borderRadius:18,cursor:"pointer",textAlign:"center",position:"relative",overflow:"hidden",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:8,padding:"10px 6px"}}>
                <div style={{position:"absolute",top:-20,left:"50%",transform:"translateX(-50%)",width:80,height:80,background:"radial-gradient(circle,"+e.color+"15,transparent 70%)",pointerEvents:"none"}}/>
                <div style={{width:40,height:40,borderRadius:12,background:e.color+"18",border:"1.5px solid "+e.color+"35",display:"flex",alignItems:"center",justifyContent:"center",boxShadow:"0 0 16px "+e.color+"25"}}>
                  {SpotlightIcons[e.icon]?.(e.color,21)}
                </div>
                <div>
                  <div style={{fontFamily:"'Instrument Serif',serif",fontSize:13,color:"#f0ece4",lineHeight:1.3,marginBottom:3}}>{e.title}</div>
                  <div style={{fontSize:9,color:"rgba(255,255,255,0.4)",lineHeight:1.3,fontWeight:500}}>{e.hint}</div>
                </div>
              </button>
            );
          })}
