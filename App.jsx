...
function PlatformSwipe({platform,profile,browseType,onBlock,onLike,onSelect,onDone,onRate}){
  ...
  return(
    <div>
      ...
      <div style={{position:"relative",height:"calc(100vh - 340px)",maxHeight:400,margin:"0 -18px"}}>
...
function UniversalSwipe({profile,cardProps,onSelect,onDone}){
  ...
  async function handleDone(){
    ...
    // ── TMDB-Kandidaten für alle Anbieter parallel holen ──
    const platCandidates=await Promise.all(userPlats.map(async(plat,pi)=>{
      try{
        ...
        const calls=pages.flatMap(([pg,sort])=>[
          ...
          discoverTitles("serie",plat.tmdbIds,null,pg,"popularity.desc",langFilter),
          discoverTitles("film",plat.tmdbIds,null,pg,"popularity.desc",langFilter),
        ]);
    ...
    // ── KI pro Anbieter: 30 Titel (10 aktiv + 20 Reserve) ──
    ...
    await Promise.all(platCandidates.map(async({plat,candidates})=>{
      ...
      const titleList=unique.slice(0,30).map(it=>`${it.title||it.name||""} (${it.media_type==="tv"?"S":"F"}, ${Math.round((it.vote_average||0)*10)/10}★)`).join(", ");
      const system=`Du bist Streaming-Kurator. Nur JSON-Array. Format: [{"title":"...","reason":"...","score":85}]. reason = 1 Satz mit du, persönlich. score 50-99.`;
      const msg=`Top 30 für ${plat.name}. Kandidaten: ${titleList}. Gerade geliked: ${swipedLiked||"nichts"}. Abgelehnt: ${swipedDisliked||"nichts"}. Geschmack: ${ctx.taste||"gemischt"}. Besten zuerst.`;
      try{
        const text=await callAI([{role:"user",content:msg}],system);
    ...
    // ── Spotlight-Empfehlungen: 3 beste Titel anhand Swipe + Bewertungen ──
    try{
      ...
      const spSys=`Nur JSON-Array. Format: [{"title":"...","reason":"..."}]. reason mit du, persönlich.`;
      const spMsg=`3 perfekte Titel. Kandidaten: ${spList}. Geliked: ${swipedLiked||"nichts"}. Geschmack: ${ctx.taste||"gemischt"}.`;
