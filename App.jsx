...
function LedStrip({itemTitle,profile,onRate,genre_ids}){
  ...
  function getValueFromX(x){
    ...
    if(pct<0.33)return 1; // Rot
    if(pct<0.66)return 3; // Orange
    return 5;              // Grün
  }

  function handleStart(x){setDragging(true);}
  function handleMove(x){
    if(!dragging)return;
    const val=getValueFromX(x);
    if(val!==current)onRate(itemTitle,val,genre_ids||[]);
  }
  function handleEnd(x){
    if(dragging){
      const val=getValueFromX(x);
      onRate(itemTitle,current===val?0:val,genre_ids||[]);
    }
    setDragging(false);
  }

  const segments=[
    {v:1,color:"#ef4444",glow:"rgba(239,68,68,0.8)",label:"Nope"},
    {v:3,color:"#f97316",glow:"rgba(249,115,22,0.8)",label:"Okay"},
    {v:5,color:"#4ade80",glow:"rgba(74,222,128,0.8)",label:"Feuer"},
  ];

  return(
    <div style={{display:"flex",gap:6,borderRadius:12,overflow:"hidden",height:32}}>
      {segments.map(seg=>{
        ...
        return(
          <button key={seg.v}
            ...
            style={{
              ...
              border:"1px solid "+(active?seg.color:"rgba(255,255,255,0.08)"),
              borderRadius:10,
              cursor:"pointer",
              display:"flex",alignItems:"center",justifyContent:"center",
              boxShadow:active?"0 0 14px 2px "+seg.glow+", inset 0 0 8px rgba(255,255,255,0.15)":"none",
              transition:"all 0.2s ease",
              transform:active?"scale(1.04)":"scale(1)",
            }}>
            <span style={{
              fontSize:11,fontWeight:800,
              color:active?"rgba(0,0,0,0.75)":"rgba(255,255,255,0.25)",
              letterSpacing:"0.3px",
              fontFamily:"'DM Sans'",
              pointerEvents:"none",
              ...
              <div key={rec.id||rec.title+i} style={{animation:rec._isNew?"fadeIn 0.5s ease":"none",marginBottom:10}}>
                ...
                <div style={{borderRadius:16,overflow:"hidden",position:"relative",background:"#0d0d18",border:"1px solid #1e1e30"}}>
                  ...
                  <div onClick={e=>e.stopPropagation()} style={{padding:"6px 12px 10px",display:"flex",gap:8,alignItems:"center"}}>
                    ...
                    <button onClick={()=>onWatched(rec._tmdbItem||{id:rec.id,title:rec.title,genre_ids:tmdb?.genre_ids||[],media_type:"movie"})}
  ...
  return(
    <div style={{padding:"0 18px 24px"}}>
      ...
      <div style={{marginBottom:20}}>
        ...
        <p style={{fontSize:12,color:"#b0a8b8",margin:0}}>Dein persönlicher KI-Assistent — präzise, persönlich, schnell</p>
      </div>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:10,marginBottom:20}}>
          {mainFeatures.map(f=>(
            <button key={f.id} onClick={()=>setActiveFeature(f.id)}
              style={{background:f.gradient,border:"1px solid "+f.border,borderRadius:18,padding:"16px 18px",cursor:"pointer",textAlign:"left",width:"100%"}}>
              <div style={{display:"flex",alignItems:"center",gap:14}}>
                <div style={{width:50,height:50,borderRadius:14,background:f.color+"22",border:"1px solid "+f.border,display:"flex",alignItems:"center",justifyContent:"center",fontSize:24,flexShrink:0}}>{f.icon}</div>
                <div style={{flex:1}}>
                  <div style={{fontSize:16,fontWeight:800,color:"#f0ece4",marginBottom:4}}>{f.title}</div>
                  <div style={{fontSize:12,color:"#b0a8b8",lineHeight:1.5}}>{f.desc}</div>
                </div>
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke={f.color} strokeWidth="2" strokeLinecap="round"><polyline points="9 18 15 12 9 6"/></svg>
              </div>
            </button>
          ))}
        </div>
