...
const GROQ_API_KEY = "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";
...
const MOODS = [
  { id:"action",  label:"Action & Spannung", emoji:"💥", genres:[28,53,80,12] },
  { id:"laugh",   label:"Lachen & Leicht",   emoji:"😂", genres:[35,16,10751] },
  { id:"drama",   label:"Tief & Bewegend",   emoji:"🎭", genres:[18,10749,36] },
  { id:"scifi",   label:"Sci-Fi & Fantasy",  emoji:"🚀", genres:[878,14,10765] },
  { id:"horror",  label:"Gruseln",           emoji:"👻", genres:[27,9648,53] },
  { id:"doku",    label:"Entdecken & Lernen",emoji:"🌍", genres:[99,36] },
];
...
function getDetails(mediaType,id){
  return tmdbFetch("/"+mediaType+"/"+id,{append_to_response:"credits,watch/providers"});
}
...
function getTrending(mediaType){
  return fetch(TMDB_BASE+"/trending/"+mediaType+"/week?api_key="+TMDB_API_KEY+"&language=de-DE").then(r=>r.json());
}
...
function buildCtx(profile){
  ...
  return{platforms,topGenres,watched,liked};
}

async function getSmartRecs(profile,prompt){
  const ctx=buildCtx(profile);
  const system=`Du bist ein Streaming-Experte. Antworte auf Deutsch, enthusiastisch, konkret. Max 180 Wörter. Nenne 3-5 spezifische Titel mit kurzem Grund.`;
  const msg=`Plattformen: ${ctx.platforms||"diverse"}. Lieblingsgenres: ${ctx.topGenres||"gemischt"}. Gesehen: ${ctx.watched||"nichts"}. Gemerkt: ${ctx.liked||"nichts"}.\n\n${prompt}`;
  return callAI([{role:"user",content:msg}],system);
}

async function getAutoRecs(profile,platformIds){
  const ctx=buildCtx(profile);
  const platformNames=platformIds.map(id=>PLATFORMS.find(p=>p.id===id)?.name||id).join(", ");
  ...
  const msg=`Empfehle 6 Titel von diesen Plattformen: ${platformNames}.\nNutzerprofil: Genres: ${ctx.topGenres||"gemischt"}. Gesehen: ${ctx.watched||"nichts"}. Gemerkt: ${ctx.liked||"nichts"}.\nVerteile die Empfehlungen auf verschiedene Plattformen. reason = 1 kurzer Satz Deutsch.`;
...
function Onboarding({onComplete}){
  const [step,setStep]=useState(0); // 0=platforms, 1=quiz
  const [plats,setPlats]=useState([]);
  const [ratings,setRatings]=useState({});

  function togglePlat(id){setPlats(p=>p.includes(id)?p.filter(x=>x!==id):[...p,id]);}
  ...
  function finish(){
    ...
    onComplete({platforms:plats,genres,liked:[],disliked:[],watched:[],liked_titles:[],thumbsUp:[]});
  }

  const canNext0=plats.length>0;
  const rated=Object.keys(ratings).length;

  return(
    <div style={{minHeight:"100vh",background:"#09090f",color:"#f0ece4",fontFamily:"'DM Sans',sans-serif",display:"flex",flexDirection:"column",overflow:"hidden"}}>
      ...
      {/* Background glow */}
      <div style={{position:"fixed",top:"-20%",left:"50%",transform:"translateX(-50%)",width:600,height:600,background:"radial-gradient(circle,#ff6b3520 0%,transparent 70%)",pointerEvents:"none",zIndex:0}}/>

      <div style={{flex:1,display:"flex",flexDirection:"column",padding:"40px 24px 32px",position:"relative",zIndex:1,maxWidth:560,margin:"0 auto",width:"100%"}}>

        {/* Progress */}
        <div style={{display:"flex",gap:6,marginBottom:40}}>
          {[0,1].map(i=>(
            <div key={i} style={{flex:1,height:3,borderRadius:2,background:i<step?"linear-gradient(90deg,#ff6b35,#e84393)":i===step?"linear-gradient(90deg,#ff6b35,#e84393)":"#1e1830",opacity:i>step?0.3:1,transition:"all 0.4s"}}/>
          ))}
        </div>
        ...
        {step===0&&(
          <div style={{animation:"fadeUp 0.4s ease"}}>
            ...
            <h1 style={{fontFamily:"'Instrument Serif',serif",fontSize:36,textAlign:"center",margin:"0 0 8px",lineHeight:1.1}}>
              <span style={{background:"linear-gradient(135deg,#ff6b35,#e84393)",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent"}}>Dein persönlicher</span><br/>
              <span style={{color:"#f0ece4"}}>Streaming-Guide</span>
            </h1>
            <p style={{textAlign:"center",color:"#6a5e70",marginBottom:36,fontSize:15}}>Welche Dienste hast du?</p>

            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:16}}>
              {PLATFORMS.map(p=>{
                const sel=plats.includes(p.id);
                return(
                  <button key={p.id} onClick={()=>togglePlat(p.id)} style={{background:sel?"linear-gradient(135deg,"+p.color+"33,"+p.color+"11)":"#12121f",border:"1px solid "+(sel?p.color+"88":"#1e1e30"),borderRadius:16,padding:"14px 16px",cursor:"pointer",display:"flex",alignItems:"center",gap:12,transition:"all 0.2s",boxShadow:sel?"0 0 20px "+p.color+"22":"none"}}>
                    <div style={{width:36,height:36,borderRadius:10,background:sel?p.color+"33":"#1a1a2e",border:"1px solid "+(sel?p.color+"55":"#2a2a3e"),display:"flex",alignItems:"center",justifyContent:"center",fontSize:16,fontWeight:900,color:sel?p.color:"#555",flexShrink:0}}>{p.icon}</div>
                    <span style={{fontSize:13,fontWeight:700,color:sel?"#f0ece4":"#6a5e70",textAlign:"left"}}>{p.name}</span>
            ...
            {/* Mediatheken toggle */}
            <button onClick={()=>togglePlat("mediatheken")} style={{width:"100%",background:plats.includes("mediatheken")?"linear-gradient(135deg,#004E8A33,#FA7D1922)":"#12121f",border:"1px solid "+(plats.includes("mediatheken")?"#004E8A88":"#1e1e30"),borderRadius:16,padding:"14px 16px",cursor:"pointer",display:"flex",alignItems:"center",gap:12,marginBottom:32,transition:"all 0.2s"}}>
              <div style={{width:36,height:36,borderRadius:10,background:"#1a1a2e",display:"flex",alignItems:"center",justifyContent:"center",fontSize:18}}>📡</div>
              <div style={{textAlign:"left"}}>
                <div style={{fontSize:13,fontWeight:700,color:plats.includes("mediatheken")?"#f0ece4":"#6a5e70"}}>Öffentlich-Rechtliche</div>
                <div style={{fontSize:11,color:"#555"}}>ARD · ZDF · Arte · BR · mehr</div>
              </div>
              {plats.includes("mediatheken")&&<span style={{marginLeft:"auto",fontSize:16}}>✓</span>}
            </button>

            <button onClick={()=>canNext0&&setStep(1)} style={{width:"100%",background:canNext0?"linear-gradient(135deg,#ff6b35,#e84393)":"#1a1525",border:"none",borderRadius:18,padding:"18px",color:canNext0?"#fff":"#444",cursor:canNext0?"pointer":"default",fontFamily:"'DM Sans'",fontWeight:800,fontSize:17,boxShadow:canNext0?"0 8px 32px #ff6b3544":"none",transition:"all 0.3s"}}>
              {canNext0?"Weiter →":"Bitte mindestens einen Dienst wählen"}
            </button>
          </div>
        )}
        ...
        {step===1&&(
          <div style={{animation:"fadeUp 0.4s ease"}}>
            <h2 style={{fontFamily:"'Instrument Serif',serif",fontSize:30,margin:"0 0 6px"}}>
              <span style={{background:"linear-gradient(135deg,#ff6b35,#e84393)",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent"}}>Was schaust du gerne?</span>
            </h2>
            <p style={{color:"#6a5e70",fontSize:13,marginBottom:20}}>Bewerte ein paar Titel — optional aber hilfreich</p>
            <p style={{color:"#555",fontSize:12,marginBottom:16}}>❤️ Liebe ich &nbsp;·&nbsp; 👎 Nicht meins &nbsp;·&nbsp; ❓ Kenn ich nicht</p>

            <div style={{display:"flex",flexDirection:"column",gap:8,marginBottom:24}}>
              {REF_TITLES.map(rt=>{
                const r=ratings[rt.title];
                return(
                  <div key={rt.title} style={{background:r==="love"?"linear-gradient(135deg,#ff6b3515,#e8439310)":r==="nope"?"#ef444410":"#12121f",borderRadius:14,padding:"12px 14px",display:"flex",alignItems:"center",justifyContent:"space-between",border:"1px solid "+(r==="love"?"#ff6b3533":r==="nope"?"#ef444433":"#1e1e30"),transition:"all 0.2s"}}>
                    <div style={{display:"flex",alignItems:"center",gap:10}}>
                      <span style={{fontSize:22}}>{rt.emoji}</span>
                      <span style={{fontSize:14,fontWeight:700,color:"#f0ece4"}}>{rt.title}</span>
                    </div>
                    <div style={{display:"flex",gap:5}}>
                      {[{v:"love",l:"❤️"},{v:"nope",l:"👎"},{v:"unknown",l:"❓"}].map(o=>(
                        <button key={o.v} onClick={()=>setRatings({...ratings,[rt.title]:o.v})} style={{width:40,height:40,borderRadius:10,background:r===o.v?"#2a2340":"transparent",border:r===o.v?"2px solid #ff6b3566":"2px solid transparent",cursor:"pointer",fontSize:18,display:"flex",alignItems:"center",justifyContent:"center",transform:r===o.v?"scale(1.15)":"scale(1)",transition:"all 0.15s"}}>{o.l}</button>
                      ))}
                    </div>
                  </div>
                );
            ...
            <div style={{display:"flex",gap:10}}>
              <button onClick={()=>setStep(0)} style={{background:"transparent",border:"1px solid #2a2340",borderRadius:14,padding:"14px 20px",color:"#6a5e70",cursor:"pointer",fontFamily:"'DM Sans'",fontWeight:600}}>← Zurück</button>
              <button onClick={finish} style={{flex:1,background:"linear-gradient(135deg,#ff6b35,#e84393)",border:"none",borderRadius:14,padding:"14px",color:"#fff",cursor:"pointer",fontFamily:"'DM Sans'",fontWeight:800,fontSize:16,boxShadow:"0 8px 32px #ff6b3544"}}>
                {rated>=3?"Los geht's 🍿":"Überspringen →"}
              </button>
            </div>
          </div>
        )}
...
// ── Hero Card (big featured title) ──
function HeroCard({item,profile,onLike,onThumbsUp,onDislike,onWatched,onSimilar}){
  ...
  const isThumbedUp=(profile.thumbsUp||[]).includes(item.id);
  ...
  return(
    <div style={{borderRadius:24,overflow:"hidden",border:"1px solid #2a2340",marginBottom:14,cursor:"pointer"}} onClick={handleExpand}>
      {/* Backdrop */}
      <div style={{position:"relative",height:200,background:"#0d0d18"}}>
        {backdrop&&<img src={backdrop} alt="" style={{width:"100%",height:"100%",objectFit:"cover",opacity:0.6}}/>}
        <div style={{position:"absolute",inset:0,background:"linear-gradient(to bottom,transparent 30%,#0d0d18 100%)"}}/>
        {/* Score badge */}
        <div style={{position:"absolute",top:12,right:12,background:"rgba(0,0,0,0.7)",backdropFilter:"blur(10px)",borderRadius:10,padding:"6px 10px",display:"flex",alignItems:"center",gap:5}}>
          <span style={{color:"#f5c518",fontSize:12,fontWeight:900}}>★</span>
          <span style={{color:scoreColor,fontSize:14,fontWeight:900}}>{score}</span>
        </div>
        {/* Type badge */}
        <div style={{position:"absolute",top:12,left:12,background:"rgba(0,0,0,0.7)",backdropFilter:"blur(10px)",borderRadius:10,padding:"6px 12px"}}>
          <span style={{fontSize:12,fontWeight:700,color:"#f0ece4"}}>{mediaType==="tv"?"📺 Serie":"🎬 Film"}</span>
        </div>
      </div>
      ...
      <div style={{background:"linear-gradient(145deg,#13121f,#0f0e1a)",padding:"14px 16px"}}>
        <div style={{display:"flex",gap:12,alignItems:"flex-start"}}>
          {poster&&<img src={poster} alt="" style={{width:52,height:78,borderRadius:10,objectFit:"cover",flexShrink:0,marginTop:-40,border:"2px solid #2a2340",boxShadow:"0 8px 24px #00000088"}}/>}
          <div style={{flex:1,minWidth:0}}>
            <h3 style={{margin:"0 0 4px",fontSize:18,fontWeight:800,color:"#f0ece4",letterSpacing:"-0.3px"}}>{title}</h3>
            <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:6}}>
              {year&&<span style={{fontSize:11,color:"#6a5e70"}}>{year}</span>}
              {genreIds.slice(0,3).map(gid=>GENRES_TMDB[gid]?<span key={gid} style={{fontSize:11,color:"#6a5e70"}}>{GENRE_EMOJI[gid]} {GENRES_TMDB[gid]}</span>:null)}
            </div>
            {item.overview&&<p style={{margin:0,fontSize:13,color:"#8a7e90",lineHeight:1.5,display:"-webkit-box",WebkitLineClamp:2,WebkitBoxOrient:"vertical",overflow:"hidden"}}>{item.overview}</p>}
          </div>
        </div>

        {/* Actions */}
        <div style={{display:"flex",gap:8,marginTop:12}}>
          <button onClick={e=>{e.stopPropagation();onLike(item);}} style={{flex:1,padding:"10px",borderRadius:12,background:isLiked?"#E5091422":"#1a1a2e",border:isLiked?"1px solid #E5091455":"1px solid #2a2340",color:isLiked?"#E50914":"#888",cursor:"pointer",fontWeight:700,fontSize:12,fontFamily:"'DM Sans'"}}>{isLiked?"❤️ Gemerkt":"🤍 Merken"}</button>
          <button onClick={e=>{e.stopPropagation();onThumbsUp(item);}} style={{padding:"10px 14px",borderRadius:12,background:isThumbedUp?"#4ade8022":"#1a1a2e",border:isThumbedUp?"1px solid #4ade8055":"1px solid #2a2340",color:isThumbedUp?"#4ade80":"#888",cursor:"pointer",fontSize:16}}>👍</button>
          <button onClick={e=>{e.stopPropagation();onDislike(item.id);}} style={{padding:"10px 14px",borderRadius:12,background:"#1a1a2e",border:"1px solid #2a2340",color:"#555",cursor:"pointer",fontSize:16}}>👎</button>
          <button onClick={e=>{e.stopPropagation();onWatched(item);}} style={{padding:"10px 14px",borderRadius:12,background:isWatched?"#3b82f622":"#1a1a2e",border:isWatched?"1px solid #3b82f655":"1px solid #2a2340",color:isWatched?"#3b82f6":"#888",cursor:"pointer",fontSize:13,fontFamily:"'DM Sans'",fontWeight:700}}>{isWatched?"✅":"👁"}</button>
          <button onClick={e=>{e.stopPropagation();onSimilar(item);}} style={{padding:"10px 14px",borderRadius:12,background:"#1a1a2e",border:"1px solid #2a2340",color:"#888",cursor:"pointer",fontSize:16}}>🔮</button>
        </div>
...
// ── Compact Title Card ──
function TitleCard({item,profile,onLike,onThumbsUp,onDislike,onWatched,onSimilar}){
  const [expanded,setExpanded]=useState(false);
  const [details,setDetails]=useState(null);
  const title=item.title||item.name||"";
  const year=(item.release_date||item.first_air_date||"").substring(0,4);
    ...
    <div style={{background:"#12121f",borderRadius:18,overflow:"hidden",border:expanded?"1px solid #ff6b3530":"1px solid #1e1e30",marginBottom:8,transition:"border 0.2s"}}>
      <div onClick={handleExpand} style={{padding:14,display:"flex",gap:12,cursor:"pointer",alignItems:"flex-start"}}>
        ...
        <div style={{flex:1,minWidth:0}}>
          <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:4}}>
            ...
            {isThumbedUp&&<span style={{fontSize:11,flexShrink:0}}>👍</span>}
          </div>
          ...
          {item.overview&&<p style={{margin:"0 0 4px",fontSize:12,color:"#6a5e70",lineHeight:1.4,display:"-webkit-box",WebkitLineClamp:2,WebkitBoxOrient:"vertical",overflow:"hidden"}}>{item.overview}</p>}
      ...
      {expanded&&(
        <div style={{padding:"0 14px 14px",animation:"fadeUp 0.3s ease"}}>
          {item.overview&&<p style={{fontSize:13,color:"#8a7e90",lineHeight:1.6,marginBottom:12}}>{item.overview}</p>}
          ...
          <div style={{display:"flex",gap:6}}>
            ...
            <button onClick={e=>{e.stopPropagation();onThumbsUp(item);}} style={{padding:"9px 12px",borderRadius:10,background:isThumbedUp?"#4ade8022":"#1a1a2e",border:isThumbedUp?"1px solid #4ade8055":"1px solid #2a2340",color:isThumbedUp?"#4ade80":"#666",cursor:"pointer",fontSize:14}}>👍</button>
            <button onClick={e=>{e.stopPropagation();onDislike(item.id);}} style={{padding:"9px 12px",borderRadius:10,background:"#1a1a2e",border:"1px solid #2a2340",color:"#555",cursor:"pointer",fontSize:14}}>👎</button>
    ...
    <div style={{background:"#12121f",borderRadius:18,overflow:"hidden",border:"1px solid #1e1e30",marginBottom:16}}>
      <div onClick={()=>setOpen(!open)} style={{padding:14,display:"flex",alignItems:"center",gap:12,cursor:"pointer"}}>
        ...
        <div style={{flex:1}}>
          ...
          <div style={{fontSize:11,color:"#555"}}>ARD · ZDF · Arte · BR · 3sat · funk</div>
        </div>
...
function MainApp({profile:initProfile,onReset}){
  ...
  if(!profile.thumbsUp)profile.thumbsUp=[];
  ...
  function handleThumbsUp(item){
    updateProfile(p=>{
      const id=item.id,isUp=(p.thumbsUp||[]).includes(id);
      const thumbsUp=isUp?(p.thumbsUp||[]).filter(x=>x!==id):[...(p.thumbsUp||[]),id];
      const genres={...p.genres};
      if(!isUp)(item.genre_ids||[]).forEach(g=>{genres[g]=(genres[g]||0)+3;});
      return{...p,thumbsUp,genres};
    });
  }

  function handleDislike(id){
    updateProfile(p=>{
      const item=[...heroItems,...feedItems,...searchResults,...similarItems].find(i=>i.id===id);
      const genres={...p.genres};
      if(item&&!(p.disliked||[]).includes(id))(item.genre_ids||[]).forEach(g=>{genres[g]=(genres[g]||0)-2;});
      return{...p,disliked:[...(p.disliked||[]),id],liked:(p.liked||[]).filter(x=>x!==id),thumbsUp:(p.thumbsUp||[]).filter(x=>x!==id),genres};
    });
  }
  ...
  const cardProps={profile,onLike:handleLike,onThumbsUp:handleThumbsUp,onDislike:handleDislike,onWatched:handleWatched,onSimilar:handleSimilar};

  // Load home feed
  useEffect(()=>{
    const userPlats=PLATFORMS.filter(p=>profile.platforms.includes(p.id));
    if(userPlats.length===0){setFeedLoading(false);setAiLoading(false);return;}
    ...
    Promise.all([
      ...
      deduped.forEach(item=>{
        let s=(item.vote_average||0)*2;
        (item.genre_ids||[]).forEach(g=>{s+=(profile.genres[g]||0)*0.5;});
        item._score=s;
      });
    ...
    // AI recs
    getAutoRecs(profile,userPlats.map(p=>p.id)).then(recs=>{
      setAiRecs(recs);setAiLoading(false);
  ...
  function applyMood(mood){
    ...
    setFeedLoading(true);
    Promise.all([
      discoverTitles("serie",allIds,genreStr,1,"popularity.desc"),
      discoverTitles("film",allIds,genreStr,1,"popularity.desc"),
    ]).then(([ser,fil])=>{
  ...
  // Browse platform
  function loadPlatform(pl){
    setBrowsePlatform(pl);setBrowseLoading(true);setBrowseItems([]);
    const profileGenres=Object.entries(profile.genres||{}).filter(([,v])=>v>0).sort(([,a],[,b])=>b-a).slice(0,3).map(([gid])=>gid);
    const genreStr=profileGenres.length>0?profileGenres.join("|"):null;
    Promise.all([
      discoverTitles("serie",pl.tmdbIds,genreStr,1,"popularity.desc"),
      discoverTitles("film",pl.tmdbIds,genreStr,1,"popularity.desc"),
    ]).then(([ser,fil])=>{
      const all=[...(ser.results||[]).map(r=>({...r,media_type:"tv"})),...(fil.results||[]).map(r=>({...r,media_type:"movie"}))];
      const seen=new Set();const deduped=[];
      all.forEach(item=>{if(!seen.has(item.id)){seen.add(item.id);deduped.push(item);}});
      deduped.sort((a,b)=>(b.vote_average||0)-(a.vote_average||0));
      setBrowseItems(deduped.slice(0,25));setBrowseLoading(false);
  ...
  function handleSearch(q){
    ...
    searchRef.current=setTimeout(()=>{
      setSearching(true);
      searchTitles(q).then(data=>{
        setSR((data?.results||[]).filter(r=>r.media_type==="movie"||r.media_type==="tv").slice(0,15));
        setSearching(false);
  ...
  return(
    <div style={{minHeight:"100vh",background:"#09090f",color:"#f0ece4",fontFamily:"'DM Sans',sans-serif",paddingBottom:90}}>
      ...
      <div style={{padding:"20px 18px 0",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
        <div>
          <h1 style={{fontFamily:"'Instrument Serif',serif",fontSize:26,margin:0,background:"linear-gradient(135deg,#ff6b35,#e84393)",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent"}}>🍿 StreamFinder</h1>
        </div>
      ...
      {showSettings&&(
        <div style={{margin:"10px 18px",padding:14,background:"#12121f",borderRadius:14,border:"1px solid #1e1e30"}}>
          <p style={{fontSize:12,color:"#777",margin:"0 0 10px"}}>Dienste neu wählen & Profil zurücksetzen?</p>
      ...
      <div style={{paddingTop:12}}>
        ...
        {tab==="home"&&(
          <div>
            ...
            <div style={{padding:"0 18px",marginBottom:16}}>
              <p style={{fontSize:11,color:"#555",fontWeight:700,textTransform:"uppercase",letterSpacing:0.8,marginBottom:10}}>Heute bin ich in der Stimmung für…</p>
              <div style={{display:"flex",gap:8,overflowX:"auto",paddingBottom:4}}>
                {MOODS.map(m=>{
                  const active=activeMood===m.id;
                  return(
                    <button key={m.id} onClick={()=>applyMood(m)} style={{background:active?"linear-gradient(135deg,#ff6b35,#e84393)":"#12121f",border:"1px solid "+(active?"transparent":"#1e1e30"),borderRadius:20,padding:"8px 16px",cursor:"pointer",color:active?"#fff":"#8a7e90",fontFamily:"'DM Sans'",fontWeight:700,fontSize:12,whiteSpace:"nowrap",flexShrink:0,transition:"all 0.2s"}}>
                      {m.emoji} {m.label}
                    </button>
                  );
                })}
            ...
            {(aiLoading||aiRecs.length>0)&&(
              <div style={{padding:"0 18px",marginBottom:20}}>
                <p style={{fontSize:11,color:"#555",fontWeight:700,textTransform:"uppercase",letterSpacing:0.8,marginBottom:10}}>Ausgewählt für dich</p>
                {aiLoading?(
                  <div style={{display:"flex",gap:10,overflowX:"auto"}}>
                    {[1,2,3].map(i=><div key={i} style={{width:140,height:90,borderRadius:14,background:"#12121f",flexShrink:0,animation:"pulse 1.5s infinite"}}/>)}
                  </div>
                ):(
                  <div style={{display:"flex",gap:10,overflowX:"auto",paddingBottom:4}}>
                    {aiRecs.map((rec,i)=>{
                      ...
                      return(
                        <div key={i} style={{flexShrink:0,width:160,background:"linear-gradient(145deg,#1a1525,#130f1e)",border:"1px solid "+(pl?pl.color+"33":"#2a2340"),borderRadius:16,padding:14,transition:"transform 0.2s"}}>
                          ...
                          <div style={{display:"flex",gap:4,marginBottom:6}}>
                            ...
                            <span style={{fontSize:10,color:"#555",background:"#0d0d18",padding:"2px 6px",borderRadius:5}}>{rec.type}</span>
                          </div>
                          {pl&&<div style={{fontSize:10,color:pl.color,fontWeight:700}}>{pl.icon} {pl.name}</div>}
                          <p style={{fontSize:11,color:"#6a5e70",margin:"6px 0 0",lineHeight:1.4,fontStyle:"italic"}}>"{rec.reason}"</p>
                        </div>
                      );
                    })}
                  </div>
            ...
            {/* Hero items */}
            {feedLoading?(
              <div style={{padding:"0 18px",textAlign:"center",paddingTop:20}}>
                <div style={{fontSize:28,animation:"spin 2s linear infinite",display:"inline-block"}}>🍿</div>
                <p style={{fontSize:13,color:"#555",marginTop:8}}>Lade Empfehlungen…</p>
              </div>
            ):(
              <div style={{padding:"0 18px"}}>
                {heroItems.length>0&&(
                  <div style={{marginBottom:8}}>
                    <p style={{fontSize:11,color:"#555",fontWeight:700,textTransform:"uppercase",letterSpacing:0.8,marginBottom:10}}>Top-Picks</p>
                    {heroItems.map(item=><HeroCard key={item.id} item={item} {...cardProps}/>)}
                  </div>
                )}
                {feedItems.length>0&&(
                  <div>
                    <p style={{fontSize:11,color:"#555",fontWeight:700,textTransform:"uppercase",letterSpacing:0.8,marginBottom:10}}>Weitere Empfehlungen</p>
                    {feedItems.map(item=><TitleCard key={item.id} item={item} {...cardProps}/>)}
                  </div>
                )}
              </div>
            )}
            ...
            {searchQuery?(
              ...
              <div>
                ...
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:20}}>
                  {userPlatforms.map(p=>(
                    <button key={p.id} onClick={()=>loadPlatform(p)} style={{background:browsePlatform?.id===p.id?"linear-gradient(135deg,"+p.color+"33,"+p.color+"11)":"#12121f",border:"1px solid "+(browsePlatform?.id===p.id?p.color+"66":"#1e1e30"),borderRadius:16,padding:"16px",cursor:"pointer",display:"flex",alignItems:"center",gap:12,transition:"all 0.2s"}}>
                      <div style={{width:36,height:36,borderRadius:10,background:p.color+"22",display:"flex",alignItems:"center",justifyContent:"center",fontSize:18,fontWeight:900,color:p.color}}>{p.icon}</div>
                      <span style={{fontSize:13,fontWeight:700,color:browsePlatform?.id===p.id?"#f0ece4":"#8a7e90"}}>{p.name}</span>
                    </button>
                  ))}
                </div>
        ...
        {tab==="history"&&(
          <div style={{padding:"0 18px"}}>
            ...
            <p style={{fontSize:12,color:"#555",marginBottom:16}}>Je mehr du markierst, desto besser deine Empfehlungen.</p>
            {(profile.watched||[]).length===0
              ?<div style={{textAlign:"center",padding:40,color:"#444"}}><div style={{fontSize:40,marginBottom:10}}>📋</div><p>Noch keine Titel gesehen.</p></div>
      ...
      <style>{"@keyframes fadeUp{from{opacity:0;transform:translateY(14px)}to{opacity:1;transform:translateY(0)}} @keyframes spin{from{transform:rotate(0deg)}to{transform:rotate(360deg)}} @keyframes pulse{0%,100%{opacity:0.5}50%{opacity:1}} *{box-sizing:border-box}"}</style>
    </div>
  );
}
