...
function Onboarding({onComplete}){
  ...
  if(step===0)return(
    <div style={{minHeight:"100vh",background:"#09090f",display:"flex",flexDirection:"column",fontFamily:"'DM Sans',sans-serif",position:"relative",overflow:"hidden"}}>
      ...
      <div style={{position:"absolute",inset:0,background:"radial-gradient(ellipse at 30% 20%,rgba(196,169,96,0.12),transparent 60%), radial-gradient(ellipse at 80% 80%,rgba(232,67,147,0.08),transparent 50%)",pointerEvents:"none"}}/>
      <div style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"40px 28px",position:"relative",zIndex:1}}>
        {/* Logo */}
        <div style={{width:80,height:80,borderRadius:24,background:"linear-gradient(135deg,#c4a960,#ff6b35)",display:"flex",alignItems:"center",justifyContent:"center",marginBottom:32,boxShadow:"0 8px 40px rgba(196,169,96,0.5)"}}>
        ...
        <h1 style={{fontFamily:"'Instrument Serif',serif",fontSize:36,color:"#f0ece4",marginBottom:16,textAlign:"center",lineHeight:1.15}}>Schluss mit 20 Minuten Scrollen.</h1>
        <p style={{fontSize:16,color:"#c4b8c8",lineHeight:1.7,textAlign:"center",marginBottom:40,maxWidth:340}}>StreamFinder kennt deinen Geschmack — und findet in Sekunden was du heute Abend wirklich schauen willst.</p>
        <div style={{display:"flex",flexDirection:"column",gap:12,width:"100%",maxWidth:360,marginBottom:40}}>
          {[
            {icon:"⚡",text:"Sofort personalisiert — nach 10 Titeln"},
            {icon:"🎯",text:"Über alle deine Anbieter hinweg"},
            {icon:"✨",text:"KI die deinen Geschmack versteht"},
          ].map(({icon,text})=>(
            <div key={text} style={{display:"flex",gap:14,alignItems:"center",background:"rgba(255,255,255,0.04)",borderRadius:14,padding:"13px 16px",border:"1px solid rgba(255,255,255,0.07)"}}>
              <span style={{fontSize:20,flexShrink:0}}>{icon}</span>
              <span style={{fontSize:14,color:"#c4b8c8",fontWeight:500}}>{text}</span>
            </div>
          ))}
        </div>
        <button onClick={()=>setStep(1)} style={{width:"100%",maxWidth:360,background:"linear-gradient(135deg,#c4a960,#ff6b35)",border:"none",borderRadius:16,padding:"18px",color:"#fff",cursor:"pointer",fontFamily:"'DM Sans'",fontWeight:800,fontSize:16,boxShadow:"0 8px 32px rgba(196,169,96,0.35)"}}>
          Jetzt starten →
        </button>
        <p style={{fontSize:12,color:"#4a4060",marginTop:16,textAlign:"center"}}>Kostenlos · Keine Registrierung</p>
      </div>
    </div>
  );
  ...
  if(step===3){
    ...
    return(
      <div style={{minHeight:"100vh",background:"#09090f",fontFamily:"'DM Sans',sans-serif",display:"flex",flexDirection:"column"}}>
        ...
        {!swipeLoading&&cur&&(
          <div style={{flex:1,display:"flex",flexDirection:"column",padding:"16px 24px 24px",maxWidth:440,margin:"0 auto",width:"100%"}}>
            {/* Poster */}
            <div style={{flex:1,borderRadius:20,overflow:"hidden",position:"relative",background:"#0a0a12",minHeight:280}}>
              {cur.poster_path
                ?<img src={"https://image.tmdb.org/t/p/w500"+cur.poster_path} alt="" style={{width:"100%",height:"100%",objectFit:"cover",display:"block"}}/>
                :<div style={{width:"100%",height:"100%",display:"flex",alignItems:"center",justifyContent:"center",fontSize:60}}>🎬</div>}
              <div style={{position:"absolute",inset:0,background:"linear-gradient(to bottom,transparent 50%,rgba(0,0,0,0.95) 100%)"}}/>
              <div style={{position:"absolute",bottom:0,left:0,right:0,padding:"16px 18px"}}>
                <h3 style={{fontFamily:"'Instrument Serif',serif",fontSize:22,color:"#fff",margin:"0 0 4px",textShadow:"0 2px 8px rgba(0,0,0,0.8)"}}>{cur.title||cur.name}</h3>
            ...
            <div style={{display:"flex",gap:10,marginTop:14,flexShrink:0}}>
              <button onClick={()=>handleSwipe(cur,false)}
                style={{flex:1,background:"rgba(239,68,68,0.1)",border:"2px solid rgba(239,68,68,0.3)",borderRadius:14,padding:"16px",cursor:"pointer",fontFamily:"'DM Sans'",fontWeight:800,fontSize:14,color:"#ef4444",transition:"all 0.2s"}}>
                👎 Nein
              </button>
              <button onClick={()=>{setSwipeIdx(i=>i+1);if(swipeIdx>=swipeItems.length-1)finishOnboarding();}}
                style={{flex:0.7,background:"rgba(255,255,255,0.05)",border:"2px solid #1e1e30",borderRadius:14,padding:"16px",cursor:"pointer",fontFamily:"'DM Sans'",fontWeight:600,fontSize:13,color:"#555",transition:"all 0.2s"}}>
                Kenne ich nicht
              </button>
              <button onClick={()=>handleSwipe(cur,true)}
                style={{flex:1,background:"rgba(74,222,128,0.1)",border:"2px solid rgba(74,222,128,0.3)",borderRadius:14,padding:"16px",cursor:"pointer",fontFamily:"'DM Sans'",fontWeight:800,fontSize:14,color:"#4ade80",transition:"all 0.2s"}}>
                👍 Ja
              </button>
            </div>
            <button onClick={()=>finishOnboarding()}
              style={{marginTop:12,background:"transparent",border:"none",color:"#4a4060",cursor:"pointer",fontFamily:"'DM Sans'",fontSize:12,padding:"8px",textAlign:"center"}}>
