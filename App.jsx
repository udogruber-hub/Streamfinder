            ...
            return(
              <div key={i} style={{borderRadius:16,overflow:"hidden",marginBottom:10,position:"relative",background:"#0d0d18",border:"1px solid #1e1e30"}}>
                ...
                <button onClick={e=>{e.stopPropagation();onBlock(rec.title);setRecs(prev=>prev.filter((_,j)=>j!==i));}}
                  style={{width:"100%",background:"transparent",border:"none",borderTop:"1px solid rgba(255,255,255,0.04)",color:"#555",cursor:"pointer",fontFamily:"'DM Sans'",fontSize:10,padding:"6px",display:"flex",alignItems:"center",justifyContent:"center",gap:4}}>
                  👻 Nicht mein Ding
                </button>
              </div>
            );
...
function MediathekenSection({profile,onSelect,onBlock,onLike,browseType}){
  ...
  return(
    <div style={{marginBottom:10}}>
      <button onClick={()=>setOpen(!open)} style={{width:"100%",background:"linear-gradient(135deg,rgba(0,78,138,0.13),rgba(250,125,25,0.13))",border:"1px solid rgba(0,78,138,0.27)",borderRadius:18,padding:"14px 16px",cursor:"pointer",display:"flex",alignItems:"center",gap:12,marginBottom:open?0:0}}>
