  ...
  const tiles=[
    {
      ...
      hint:"Nenn uns einen Lieblingstitel → wir bauen dein persönliches Universum",
      color:"#c4a960",
      gradient:"linear-gradient(160deg,rgba(20,18,10,0.95),rgba(15,14,20,0.85))",
      border:"rgba(196,169,96,0.25)",
    },
    {
      id:"vibe",
      title:"Vibe-Meter",
      hint:"Düster & intensiv? Leicht & lustig? Wir finden den Titel für deine Stimmung",
      color:"#e84393",
      gradient:"linear-gradient(160deg,rgba(20,10,18,0.95),rgba(15,8,20,0.85))",
      border:"rgba(232,67,147,0.25)",
    },
    {
      id:"bot",
      title:"Frag den Popcorn-Guru",
      hint:"Stell jede Frage — dein persönlicher Streaming-Experte antwortet",
      color:"#4ade80",
      gradient:"linear-gradient(160deg,rgba(10,20,15,0.95),rgba(8,15,12,0.85))",
      border:"rgba(74,222,128,0.25)",
    },
  ];

  const extras=[
    {id:"surprise",title:"Überrasch mich",hint:"1 perfekter Titel — kein Scrollen",color:"#c4a960",icon:"surprise"},
    {id:"oracle",title:"Orakel",hint:"Mystische Prophezeiung",color:"#a78bfa",icon:"oracle"},
    {id:"personality",title:"Welcher Filmtyp bin ich?",hint:"Dein Streaming-Profil",color:"#e84393",icon:"personality"},
  ];

  // Get backdrop images from watched history
  const watchedWithImg=(profile.watched||[]).filter(w=>w.backdrop_path||w.poster_path);
  const bgImgs=[0,1,2].map(i=>watchedWithImg[i]?(TMDB_IMG+(watchedWithImg[i].backdrop_path||watchedWithImg[i].poster_path)):null);
  ...
  if(activeFeature){
    ...
    return(
      <div style={{padding:"0 18px 24px",position:"relative",minHeight:"85vh"}}>
        <div style={{position:"absolute",inset:0,background:"radial-gradient(ellipse at 30% 10%,"+(active?.color||"#c4a960")+"10,transparent 60%)",pointerEvents:"none",zIndex:0}}/>
        <div style={{position:"relative",zIndex:1}}>
          ...
          <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:20}}>
            <div style={{width:44,height:44,borderRadius:14,background:(active?.color||"#c4a960")+"20",border:"1px solid "+(active?.color||"#c4a960")+"40",display:"flex",alignItems:"center",justifyContent:"center",boxShadow:"0 0 20px "+(active?.color||"#c4a960")+"30"}}>
              {SpotlightIcons[activeFeature]?.(active?.color||"#c4a960",24)}
            </div>
            <div>
              <h3 style={{fontFamily:"'Instrument Serif',serif",fontSize:20,margin:"0 0 3px",color:"#f0ece4"}}>{active?.title}</h3>
              <p style={{fontSize:11,color:"#8a849a",margin:0}}>{active?.hint}</p>
            </div>
          </div>
          {isMain
  ...
  return(
    <div style={{position:"relative",overflow:"hidden",display:"flex",flexDirection:"column"}}>
      ...
      <div style={{position:"fixed",inset:0,pointerEvents:"none",zIndex:0,overflow:"hidden"}}>
        <div style={{position:"absolute",top:"10%",left:"5%",width:250,height:250,background:"radial-gradient(circle,rgba(196,169,96,0.08),transparent 70%)",borderRadius:"50%",animation:"orbFloat1 9s ease-in-out infinite"}}/>
        <div style={{position:"absolute",top:"30%",right:"-5%",width:200,height:200,background:"radial-gradient(circle,rgba(232,67,147,0.07),transparent 70%)",borderRadius:"50%",animation:"orbFloat2 12s ease-in-out infinite"}}/>
        <div style={{position:"absolute",bottom:"20%",left:"20%",width:160,height:160,background:"radial-gradient(circle,rgba(74,222,128,0.06),transparent 70%)",borderRadius:"50%",animation:"orbFloat3 10s ease-in-out infinite"}}/>
      </div>

      {/* Header */}
      <div style={{position:"relative",zIndex:1,padding:"18px 18px 12px",display:"flex",alignItems:"center",gap:12}}>
        <div style={{width:46,height:46,borderRadius:14,background:"linear-gradient(135deg,#c4a960,#ff6b35)",display:"flex",alignItems:"center",justifyContent:"center",boxShadow:"0 4px 20px rgba(196,169,96,0.4)",flexShrink:0}}>
          {SpotlightIcons.surprise("#fff",22)}
        </div>
        <div>
          <h2 style={{fontFamily:"'Instrument Serif',serif",fontSize:30,margin:"0 0 1px",background:"linear-gradient(135deg,#c4a960,#f5e090)",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent",lineHeight:1}}>Spotlight</h2>
          <p style={{fontSize:10,color:"#6a6478",margin:0,letterSpacing:"0.8px",textTransform:"uppercase"}}>KI-Assistent · Persönlich · Präzise</p>
        </div>
      </div>

      {/* MOSAIC GRID — füllt den ganzen Bildschirm */}
      <div style={{position:"relative",zIndex:1,padding:"0 12px",flex:1,display:"flex",flexDirection:"column",gap:10,minHeight:"calc(100vh - 200px)"}}>

        {/* ROW 1: Deep Dive — große Kachel volle Breite */}
        <button onClick={()=>setActiveFeature("deepdive")}
          style={{background:tiles[0].gradient,border:"1px solid "+tiles[0].border,borderRadius:22,cursor:"pointer",textAlign:"left",position:"relative",overflow:"hidden",flex:"0 0 auto",height:180}}>
          {bgImgs[0]&&<img src={bgImgs[0]} alt="" style={{position:"absolute",inset:0,width:"100%",height:"100%",objectFit:"cover",opacity:0.22}}/>}
          <div style={{position:"absolute",inset:0,background:"linear-gradient(135deg,rgba(0,0,0,0.85) 0%,rgba(0,0,0,0.4) 100%)"}}/>
          <div style={{position:"absolute",top:-40,right:-40,width:150,height:150,background:"radial-gradient(circle,"+tiles[0].color+"25,transparent 70%)",pointerEvents:"none"}}/>
          <div style={{position:"relative",padding:"20px 20px 16px",height:"100%",display:"flex",flexDirection:"column",justifyContent:"space-between"}}>
            <div style={{width:48,height:48,borderRadius:14,background:tiles[0].color+"20",border:"1px solid "+tiles[0].color+"40",display:"flex",alignItems:"center",justifyContent:"center",boxShadow:"0 0 20px "+tiles[0].color+"30"}}>
              {SpotlightIcons.deepdive(tiles[0].color,26)}
            </div>
            <div>
              <div style={{fontSize:18,fontWeight:800,color:"#f0ece4",marginBottom:6,letterSpacing:"-0.3px"}}>{tiles[0].title}</div>
              <div style={{fontSize:12,color:"rgba(255,255,255,0.6)",lineHeight:1.5}}>{tiles[0].hint}</div>
            </div>
          </div>
          <div style={{position:"absolute",bottom:18,right:18,width:32,height:32,borderRadius:10,background:tiles[0].color+"20",border:"1px solid "+tiles[0].color+"40",display:"flex",alignItems:"center",justifyContent:"center"}}>
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke={tiles[0].color} strokeWidth="2.5" strokeLinecap="round"><polyline points="9 18 15 12 9 6"/></svg>
          </div>
        </button>

        {/* ROW 2: Vibe + Popcorn-Guru — zwei gleich große Kacheln */}
        <div style={{display:"flex",gap:10,flex:"0 0 auto"}}>
          {tiles.slice(1).map((t,ri)=>(
            <button key={t.id} onClick={()=>setActiveFeature(t.id)}
              style={{flex:1,background:t.gradient,border:"1px solid "+t.border,borderRadius:20,cursor:"pointer",textAlign:"left",position:"relative",overflow:"hidden",height:160}}>
              {bgImgs[ri+1]&&<img src={bgImgs[ri+1]} alt="" style={{position:"absolute",inset:0,width:"100%",height:"100%",objectFit:"cover",opacity:0.18}}/>}
              <div style={{position:"absolute",inset:0,background:"linear-gradient(135deg,rgba(0,0,0,0.88),rgba(0,0,0,0.5))"}}/>
              <div style={{position:"absolute",top:-20,right:-20,width:90,height:90,background:"radial-gradient(circle,"+t.color+"28,transparent 70%)",pointerEvents:"none"}}/>
              <div style={{position:"relative",padding:"16px 14px",height:"100%",display:"flex",flexDirection:"column",justifyContent:"space-between"}}>
                <div style={{width:42,height:42,borderRadius:12,background:t.color+"22",border:"1px solid "+t.color+"40",display:"flex",alignItems:"center",justifyContent:"center",boxShadow:"0 0 16px "+t.color+"28"}}>
                  {SpotlightIcons[t.id]?.(t.color,22)}
                </div>
                <div>
                  <div style={{fontSize:14,fontWeight:800,color:"#f0ece4",marginBottom:5,lineHeight:1.2}}>{t.title}</div>
                  <div style={{fontSize:10,color:"rgba(255,255,255,0.5)",lineHeight:1.4}}>{t.hint}</div>
                </div>
              </div>
            </button>
          ))}
        ...
        {/* ROW 3: Extras — 3 gleiche Kacheln */}
        <div style={{display:"flex",gap:10,flex:"0 0 auto"}}>
          {extras.map(e=>(
            <button key={e.id} onClick={()=>setActiveFeature(e.id)}
              style={{flex:1,background:"rgba(255,255,255,0.03)",border:"1px solid rgba(255,255,255,0.08)",borderRadius:18,cursor:"pointer",textAlign:"center",position:"relative",overflow:"hidden",padding:"16px 8px 14px"}}>
              <div style={{position:"absolute",top:-15,left:"50%",transform:"translateX(-50%)",width:70,height:70,background:"radial-gradient(circle,"+e.color+"15,transparent 70%)",pointerEvents:"none"}}/>
              <div style={{width:40,height:40,borderRadius:12,background:e.color+"15",border:"1px solid "+e.color+"30",display:"flex",alignItems:"center",justifyContent:"center",margin:"0 auto 10px",boxShadow:"0 0 14px "+e.color+"20"}}>
                {SpotlightIcons[e.icon]?.(e.color,22)}
              </div>
              <div style={{fontSize:11,fontWeight:800,color:"#d0c8d8",marginBottom:4,lineHeight:1.2}}>{e.title}</div>
              <div style={{fontSize:9,color:"rgba(255,255,255,0.3)",lineHeight:1.3}}>{e.hint}</div>
            </button>
          ))}
        </div>
