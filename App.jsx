...
function MainApp(props) {
  ...
  function loadData() {
    ...
    var promises = [
      ...
      discoverTitles(apiKey, "serie", providerIds, genreParam ? [genreParam] : [], 1, "popularity.desc"),
      discoverTitles(apiKey, "film", providerIds, genreParam ? [genreParam] : [], 1, "popularity.desc"),
      // Top rated series
    ...
    Promise.all(promises).then(function(results) {
      ...
      var forYouAll = dedup(forYouSeries.concat(forYouFilms).concat(popSeries).concat(popFilms));
      forYouAll = forYouAll.filter(function(i) { return !profile.disliked.includes(i.id); });
      setForYou(forYouAll.slice(0, 20));

      setSeries(dedup(popSeries).filter(function(i) { return !profile.disliked.includes(i.id); }).slice(0, 20));
      setFilms(dedup(popFilms).filter(function(i) { return !profile.disliked.includes(i.id); }).slice(0, 20));
