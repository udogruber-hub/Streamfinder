...
function Onboarding({onDone}){
      ...
      <div style={{minHeight:"100vh",background:"linear-gradient(180deg,#09090f,#0d0d1a)",display:"flex",flexDirection:"column"}}>
        <div style={{padding:"24px 24px 0",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
          <div style={{display:"flex",alignItems:"center",gap:8}}>
            <div style={{width:36,height:36,borderRadius:10,background:"linear-gradient(135deg,#7c3aed,#e84393)",display:"flex",alignItems:"center",justifyContent:"center"}}>
        ...
        <div style={{padding:"16px 20px 8px"}}>
          ...
          <p style={{fontSize:13,color:"#7a7488",margin:0}}>Swipe kurz durch ein paar Titel — StreamFinder lernt deinen Geschmack sofort.</p>
        </div>
        <div style={{flex:1,padding:"0 18px",overflowY:"auto"}}>
          <UniversalSwipe
            profile={profile}
            cardProps={{onRate:()=>{},onLike:()=>{},onWatched:()=>{},onBlock:()=>{}}}
            onSelect={()=>{}}
            onDone={()=>{sSet("sf_profile",profile);onDone(profile);}}
          />
        </div>
      </div>
    );
  ...
  const steps=[
    {title:"Hallo! 👋",sub:"Dein persönlicher Streaming-Assistent"},
    {title:"Deine Anbieter",sub:"Welche Streaming-Dienste nutzt du?"},
    {title:"Dein Geschmack",sub:"Was schaust du am liebsten?"},
    {title:"Fast fertig!",sub:"Noch ein letzter Schritt"},
  ];
  const s=steps[step];
  ...
  return(
    <div style={{minHeight:"100vh",background:"linear-gradient(180deg,#09090f,#0d0d1a)",display:"flex",flexDirection:"column"}}>
      <div style={{padding:"48px 24px 24px",textAlign:"center"}}>
        <div style={{width:64,height:64,borderRadius:20,background:"linear-gradient(135deg,#7c3aed,#e84393)",display:"flex",alignItems:"center",justifyContent:"center",margin:"0 auto 16px",boxShadow:"0 8px 32px rgba(124,58,237,0.5)"}}>
          <svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><polygon points="23 7 16 12 23 17 23 7"/><rect x="1" y="5" width="15" height="14" rx="2" ry="2"/></svg>
        </div>
        <h1 style={{fontFamily:"'Instrument Serif',serif",fontSize:36,margin:"0 0 6px",background:"linear-gradient(135deg,#c4a960,#f5e090)",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent"}}>StreamFinder</h1>
        <p style={{fontSize:13,color:"#7a7488",margin:0}}>{s.sub}</p>
      </div>

      <div style={{flex:1,padding:"0 20px 24px",overflowY:"auto"}}>
        {step===0&&(
          <div style={{textAlign:"center",paddingTop:20}}>
            <h2 style={{fontFamily:"'Instrument Serif',serif",fontSize:28,color:"#f0ece4",marginBottom:16}}>{s.title}</h2>
            <div style={{display:"flex",flexDirection:"column",gap:12,marginBottom:24}}>
              {[
                {emoji:"✨",title:"KI-Empfehlungen",text:"Personalisiert für deinen Geschmack"},
                {emoji:"🎴",title:"Swipe durch Titel",text:"Wie du's von Dating-Apps kennst"},
                {emoji:"🎭",title:"Lerne deinen Typ kennen",text:"Welcher Filmtyp bist du?"},
                {emoji:"📺",title:"Alle Anbieter",text:"Netflix, Prime, Apple TV+ und mehr"},
              ].map((f,i)=>(
                <div key={i} style={{display:"flex",alignItems:"center",gap:14,background:"rgba(255,255,255,0.04)",borderRadius:16,padding:"14px 16px",border:"1px solid rgba(255,255,255,0.06)"}}>
                  <span style={{fontSize:24,flexShrink:0}}>{f.emoji}</span>
                  <div style={{textAlign:"left"}}>
                    <div style={{fontSize:14,fontWeight:700,color:"#f0ece4",marginBottom:2}}>{f.title}</div>
                    <div style={{fontSize:12,color:"#8a849a"}}>{f.text}</div>
                  </div>
                </div>
              ))}
            </div>
        ...
        {step===1&&(
          <div>
            <h2 style={{fontFamily:"'Instrument Serif',serif",fontSize:24,color:"#f0ece4",margin:"0 0 6px"}}>{s.title}</h2>
            <p style={{fontSize:13,color:"#7a7488",marginBottom:16}}>Wähle alle die du nutzt — du kannst das später ändern.</p>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
              {PLATFORMS.map(p=>{
                const sel=selPlats.includes(p.id);
                return(
                  <button key={p.id} onClick={()=>togglePlat(p.id)}
                    style={{background:sel?p.color+"22":"#12121f",border:"2px solid "+(sel?p.color:"#1e1e30"),borderRadius:16,padding:"16px 12px",cursor:"pointer",textAlign:"center",transition:"all 0.2s",boxShadow:sel?"0 0 20px "+p.color+"33":"none"}}>
                    <div style={{fontSize:22,fontWeight:900,color:p.color,marginBottom:6}}>{p.icon}</div>
                    <div style={{fontSize:13,fontWeight:700,color:sel?p.color:"#b0a8b8"}}>{p.name}</div>
                    {sel&&<div style={{fontSize:10,color:p.color,marginTop:2}}>✓</div>}
                  </button>
                );
              })}
            </div>
        ...
        {step===2&&(
          <div>
            <h2 style={{fontFamily:"'Instrument Serif',serif",fontSize:24,color:"#f0ece4",margin:"0 0 6px"}}>{s.title}</h2>
            <p style={{fontSize:13,color:"#7a7488",marginBottom:16}}>Wähle deine Lieblingsgenres — beeinflusst alle Empfehlungen.</p>
            <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:8}}>
              {STIMMUNGEN.map(m=>{
                const sel=selStimmungen.includes(m.id);
                const IconFn=StimmungIcons[m.id];
                return(
                  <button key={m.id} onClick={()=>toggleStimmung(m.id)}
                    style={{background:sel?m.color+"22":"#12121f",border:"2px solid "+(sel?m.color:"#1e1e30"),borderRadius:16,padding:"14px 8px",cursor:"pointer",textAlign:"center",transition:"all 0.2s",boxShadow:sel?"0 0 20px "+m.color+"33":"none",display:"flex",flexDirection:"column",alignItems:"center",gap:6}}>
                    {IconFn&&<div style={{opacity:sel?1:0.5,transition:"opacity 0.2s"}}>{IconFn(sel?m.color:"#6a6478",26)}</div>}
                    <span style={{fontSize:11,fontWeight:700,color:sel?m.color:"#8a849a",lineHeight:1.2}}>{m.label}</span>
                    {sel&&<div style={{fontSize:9,color:m.color}}>✓</div>}
                  </button>
                );
              })}
            </div>
        ...
        {step===3&&(
          <div style={{textAlign:"center",paddingTop:20}}>
            ...
            <h2 style={{fontFamily:"'Instrument Serif',serif",fontSize:28,color:"#f0ece4",marginBottom:12}}>Fast fertig!</h2>
            <div style={{background:"#12121f",borderRadius:16,padding:"14px 16px",border:"1px solid #1e1e30",marginBottom:16,textAlign:"left"}}>
              <p style={{fontSize:12,color:"#7a7488",fontWeight:700,textTransform:"uppercase",letterSpacing:0.5,margin:"0 0 8px"}}>Dein Profil</p>
              <div style={{fontSize:13,color:"#b0a8b8",lineHeight:1.8}}>
                <div>📺 {selPlats.length} Anbieter ausgewählt</div>
                <div>🎬 {selStimmungen.length} Lieblingsgenres</div>
                <div>✨ KI-Empfehlungen bereit</div>
              </div>
            </div>
            <p style={{fontSize:12,color:"#555",lineHeight:1.5,marginBottom:4}}>Tipp: Swipe kurz durch ein paar Titel —<br/>dann sind die ersten Empfehlungen noch besser.</p>
          </div>
        )}
      </div>

      <div style={{padding:"16px 20px 16px",display:"flex",gap:10}}>
        {step>0&&<button onClick={()=>setStep(s=>s-1)} style={{flex:1,background:"#12121f",border:"1px solid #1e1e30",borderRadius:14,padding:"14px",cursor:"pointer",color:"#b0a8b8",fontFamily:"'DM Sans'",fontWeight:700,fontSize:14}}>← Zurück</button>}
        {step<3&&<button onClick={()=>setStep(s=>s+1)}
          disabled={step===1&&selPlats.length===0}
          style={{flex:2,background:"linear-gradient(135deg,#ff6b35,#e84393)",border:"none",borderRadius:14,padding:"14px",cursor:"pointer",color:"#fff",fontFamily:"'DM Sans'",fontWeight:800,fontSize:15,opacity:step===1&&selPlats.length===0?0.4:1}}>
          Weiter →
        </button>}
        {step===3&&<>
          <button onClick={finish} style={{flex:1,background:"#12121f",border:"1px solid #1e1e30",borderRadius:14,padding:"14px",cursor:"pointer",color:"#b0a8b8",fontFamily:"'DM Sans'",fontWeight:700,fontSize:13}}>Direkt starten</button>
          <button onClick={goToSwipe} style={{flex:2,background:"linear-gradient(135deg,#c4a960,#ff6b35)",border:"none",borderRadius:14,padding:"14px",cursor:"pointer",color:"#fff",fontFamily:"'DM Sans'",fontWeight:800,fontSize:14}}>🎴 Titel swipe →</button>
        </>}
      </div>

      <div style={{display:"flex",gap:4,justifyContent:"center",paddingBottom:20}}>
        {steps.map((_,i)=><div key={i} style={{width:i===step?20:7,height:5,borderRadius:3,background:i===step?"#e84393":"#2a2340",transition:"all 0.3s"}}/>)}
      </div>
    </div>
  );
}
