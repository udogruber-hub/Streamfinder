...
function PlatformCard({platform,profile,onSelect,onBlock,onLike,onRate,browseType,onWatched,forceOpen}){
  ...
  useEffect(()=>{if(forceOpen&&!open){handleOpen();}},[forceOpen]);
  useEffect(()=>{
    ...
    try{
      ...
      if(stored){
        ...
        if(parsed&&parsed.active&&parsed.active.length>0){
          setRecs(parsed.active);recsRef.current=parsed.active;
          // Reserve in ref laden
          reserveRef.current=parsed.reserve||[];
          return true;
        }
        // Altes Format: direkt Array
        if(parsed&&Array.isArray(parsed)&&parsed.length>0){
          setRecs(parsed);recsRef.current=parsed;return true;
        }
      }
    }catch(e){}
    return false;
...
function UniversalSwipe({profile,cardProps,onSelect,onDone}){
  ...
  async function handleDone(){
    ...
    // Für jeden Anbieter: TMDB-Kandidaten holen (breiter Pool)
    const platCandidates=await Promise.all(userPlats.map(async(plat,pi)=>{
      try{
        const pages=[[1,"vote_average.desc"],[2,"popularity.desc"],[3,"vote_average.desc"],[4,"popularity.desc"]];
        const calls=pages.flatMap(([pg,sort])=>[
          ...
          discoverTitles("serie",plat.tmdbIds,null,pg,sort,langFilter),
          discoverTitles("film",plat.tmdbIds,null,pg,sort,langFilter),
        ]);
        const results=await Promise.all(calls);
        const all=results.flatMap((r,i)=>(r.results||[]).map(x=>({...x,media_type:i%2===0?"tv":"movie"})));
        const seen=new Set();
    ...
    // KI pro Anbieter: wählt 30 Titel (10 aktiv + 20 Reserve)
    await Promise.all(platCandidates.map(async({plat,candidates})=>{
      const unique=candidates.filter(c=>!globalUsedIds.has(c.id)).slice(0,40);
      if(!unique.length)return;
      const titleList=unique.slice(0,30).map(it=>(it.title||it.name||"")+" ("+(it.media_type==="tv"?"S":"F")+", "+Math.round(it.vote_average*10)/10+"★)").join(", ");
      const system=`Du bist Streaming-Kurator. Antworte NUR mit JSON-Array. Format: [{"title":"...","reason":"...","score":85}]. reason = 1 Satz mit du, persönlich. score = 50-99.`;
      const msg=`Wähle 30 Titel für ${ctx.taste||"diesen Nutzer"} auf ${plat.name}.
Kandidaten: ${titleList}
Top-Bewertungen: ${ctx.rated5||"keine"}. Mag nicht: ${ctx.rated2||"nichts"}.
Sortiere nach Passung. Wähle NUR aus den Kandidaten.`;
      try{
        const text=await callAI([{role:"user",content:msg}],system);
        const picks=JSON.parse(text.replace(/```json|```/g,"").trim());
        // Mappe KI-Picks zurück auf TMDB-Objekte
        const mapped=picks.map(p=>{
          const match=unique.find(c=>titleKey(c.title||c.name||"")===titleKey(p.title||""));
          if(!match)return null;
          return{id:match.id,title:match.title||match.name||"",year:(match.release_date||match.first_air_date||"").substring(0,4),reason:p.reason||"",score:p.score||70,emoji:match.media_type==="tv"?"📺":"🎬",_tmdbItem:match};
        }).filter(Boolean);
        // Auffüllen auf 30 falls KI weniger liefert
        const mappedIds=new Set(mapped.map(r=>r.id));
        const fill=unique.filter(c=>!mappedIds.has(c.id)).map(c=>({id:c.id,title:c.title||c.name||"",year:(c.release_date||c.first_air_date||"").substring(0,4),reason:"",score:60,emoji:c.media_type==="tv"?"📺":"🎬",_tmdbItem:c}));
        const full=[...mapped,...fill].slice(0,30);
        full.forEach(r=>globalUsedIds.add(r.id));
        // Speichern: top10 aktiv, rest als Reserve
        const toStore={active:full.slice(0,10),reserve:full.slice(10,30)};
        localStorage.setItem("sf_plat_"+plat.id,JSON.stringify(toStore));
      }catch(e){
        // KI-Fehler: TMDB-Fallback, kein Reserve
        const fallback=unique.slice(0,30).map(c=>({id:c.id,title:c.title||c.name||"",year:(c.release_date||c.first_air_date||"").substring(0,4),reason:"",score:60,emoji:c.media_type==="tv"?"📺":"🎬",_tmdbItem:c}));
    ...
    // KI: 3 neue Spotlight-Empfehlungen basierend auf frischem Profil
    try{
      const freshCtx=buildCtx({...profile,genres:mergedGenres});
      const allIds2=(profile.platforms||[]).flatMap(id=>PLATFORMS.find(p=>p.id===id)?.tmdbIds||[]);
      const topG=Object.entries(mergedGenres).filter(([,v])=>v>0).sort(([,a],[,b])=>b-a).slice(0,3).map(([g])=>g);
      const [sp1,sp2]=await Promise.all([
        discoverTitles("serie",allIds2,topG.join("|"),1,"vote_average.desc",null),
        discoverTitles("film",allIds2,topG.join("|"),1,"vote_average.desc",null),
      ]);
      const spCandidates=[...(sp1.results||[]).map(x=>({...x,media_type:"tv"})),...(sp2.results||[]).map(x=>({...x,media_type:"movie"}))]
        .filter(x=>x.poster_path&&!watched.has(x.id)&&!blocked.has(titleKey(x.title||x.name||"")))
        .sort((a,b)=>(b.vote_average||0)-(a.vote_average||0));
      const spList=spCandidates.slice(0,15).map(x=>(x.title||x.name||"")+" ("+(x.media_type==="tv"?"S":"F")+")").join(", ");
      const spSys=`Du bist Streaming-Kurator. Nur JSON-Array. Format: [{"title":"...","reason":"..."}]. reason = 1 Satz mit du.`;
      const spMsg=`Wähle 3 perfekte Titel. Kandidaten: ${spList}. Geschmack: ${freshCtx.taste||"gemischt"}. Top: ${freshCtx.rated5||"keine"}.`;
      ...
      if(spResult.length>0)localStorage.setItem("sf_spotlight_recs",JSON.stringify(spResult));
    }catch{}
    setTimeout(()=>{
      window.dispatchEvent(new CustomEvent("sf_swipe_done",{detail:{liked:liked_sw,disliked:disliked_sw,hasNewSpotlight:true}}));
      onDone();
    },400);
  }
    ...
    <div style={{textAlign:"center",padding:"32px 20px"}}>
      {processing?(
        <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:16}}>
          <div style={{width:64,height:64,borderRadius:20,background:"linear-gradient(135deg,#7c3aed,#e84393)",display:"flex",alignItems:"center",justifyContent:"center",animation:"pulse 1.5s ease infinite"}}>
            <svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="m12 3-1.912 5.813a2 2 0 0 1-1.275 1.275L3 12l5.813 1.912a2 2 0 0 1 1.275 1.275L12 21l1.912-5.813a2 2 0 0 1 1.275-1.275L21 12l-5.813-1.912a2 2 0 0 1-1.275-1.275L12 3z"/></svg>
          </div>
          <div>
            <p style={{fontSize:16,fontWeight:700,color:"#f0ece4",marginBottom:6}}>KI analysiert deinen Geschmack…</p>
            <p style={{fontSize:13,color:"#7a7488"}}>Deine persönlichen Top 10 werden berechnet</p>
          </div>
          <div style={{display:"flex",flexDirection:"column",gap:8,width:"100%",maxWidth:280}}>
            {PLATFORMS.filter(p=>profile.platforms.includes(p.id)).map((p,i)=>(
              <div key={p.id} style={{display:"flex",alignItems:"center",gap:10,background:"rgba(255,255,255,0.04)",borderRadius:10,padding:"8px 12px",animation:`fadeIn 0.4s ease ${i*0.15}s both`}}>
                <span style={{fontSize:14,color:p.color,fontWeight:700}}>{p.icon}</span>
                <span style={{fontSize:12,color:"#b0a8b8",flex:1,textAlign:"left"}}>{p.name}</span>
                <div style={{width:16,height:16,borderRadius:8,border:"2px solid #2a2340",borderTopColor:p.color,animation:"spin 0.8s linear infinite"}}/>
              </div>
            ))}
          </div>
        </div>
        ...
        <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:20,padding:"0 10px"}}>
          ...
          <div style={{textAlign:"center"}}>
            <h2 style={{fontFamily:"'Instrument Serif',serif",fontSize:30,color:"#f0ece4",margin:"0 0 10px",lineHeight:1.1}}>✨ Deine Top 10<br/>sind fertig!</h2>
            <p style={{fontSize:14,color:"#b0a8b8",lineHeight:1.6,margin:"0 0 6px"}}>
            ...
            <p style={{fontSize:13,color:"#7a7488",lineHeight:1.5,margin:0}}>Für jeden Anbieter wurde deine<br/>persönliche Top 10 zusammengestellt.</p>
          </div>
          {/* Anbieter-Preview */}
          ...
          <button onClick={()=>{
            // Tab zu Erkunden wechseln + alle aufklappen
            window.dispatchEvent(new CustomEvent("sf_swipe_done_goto_browse"));
            onDone();
          }} style={{width:"100%",maxWidth:300,background:"linear-gradient(135deg,#c4a960,#ff6b35)",border:"none",borderRadius:18,padding:"18px",color:"#fff",cursor:"pointer",fontFamily:"'DM Sans'",fontWeight:800,fontSize:17,boxShadow:"0 8px 32px rgba(196,169,96,0.4)"}}>
            Jetzt in Erkunden ansehen →
          </button>
          <p style={{fontSize:11,color:"#3a3344",fontFamily:"'DM Sans'",textAlign:"center"}}>Spotlight zeigt auch neue persönliche Picks</p>
        </div>
      )}
  );
  const current=items[idx];
...
function SpotlightTab({profile,cardProps,onSelect}){
  ...
  useEffect(()=>{
    function onSwipeDoneRefresh(){
      // Nach Swipe: Spotlight-Empfehlungen neu laden
      setPersonalRecs([]);
      setTimeout(()=>loadPersonalRecs(),500);
    }
    ...
    function onReloadSpotlight(){
      try{
        const stored=localStorage.getItem("sf_spotlight_recs");
  ...
  useEffect(()=>{
    ...
    // Persönliche KI-Empfehlungen parallel laden
    loadPersonalRecs();
  },[]);
  ...
  async function loadPersonalRecs(){
    ...
    try{
      ...
      const msg=`Wähle 3 perfekte Titel für diesen Nutzer.
Kandidaten: ${titleList}
Geschmack: ${ctx.taste||"unbekannt"}
Top-Bewertungen: ${ctx.rated5||"keine"}
Mag nicht: ${ctx.rated2||"nichts"}
Nur aus Kandidaten wählen!`;
      const text=await callAI([{role:"user",content:msg}],system);
      const picks=JSON.parse(text.replace(/```json|```/g,"").trim());
  ...
  return(
    <div style={{display:"flex",flexDirection:"column"}}>
      ...
      <div style={{display:"flex",flexDirection:"column",gap:8,padding:"0 12px 8px"}}>
        ...
        {/* Persönliche Empfehlungen — KI basierend auf Bewertungen */}
        {(personalLoading||personalRecs.length>0)&&(
          <div style={{padding:"0 0 8px"}}>
            <div style={{display:"flex",alignItems:"center",gap:10,padding:"4px 2px 10px"}}>
              ...
              <span style={{fontSize:10,color:"#a78bfa",fontWeight:700,letterSpacing:"1.5px",textTransform:"uppercase",fontFamily:"'DM Sans'"}}>Für dich heute</span>
            ...
            {personalLoading&&personalRecs.length===0&&(
              <div style={{textAlign:"center",padding:"16px 0"}}>
                ...
                <p style={{color:"#7a7488",fontSize:12,marginTop:6,fontFamily:"'DM Sans'"}}>Suche deine Titel…</p>
              </div>
            )}
            {personalRecs.slice(0,3).map((it,i)=>(
              <SwipeToBlock key={it.id||i} onBlock={()=>{
                setPersonalRecs(prev=>prev.filter((_,j)=>j!==i));
              }}>
            ...
            {!personalLoading&&personalRecs.length===0&&(
              <button onClick={loadPersonalRecs} style={{width:"100%",background:"rgba(124,58,237,0.08)",border:"1px solid rgba(124,58,237,0.2)",borderRadius:14,padding:12,cursor:"pointer",color:"#a78bfa",fontFamily:"'DM Sans'",fontWeight:700,fontSize:13}}>
                ✨ Neue Empfehlungen laden
              </button>
            )}
          </div>
        )}
...
// ── BrowseTab ──
function BrowseTab({profile,cardProps,onSelect,onSwipe,onWatched,allOpen,onAllOpenDone}){
  const [activeSwipe,setActiveSwipe]=useState(null);
  const [showUniversal,setShowUniversal]=useState(false);
  const [allOpen,setAllOpen]=useState(false);
  const effectiveAllOpen=allOpen||!!allOpen;
  ...
  useEffect(()=>{
    ...
    function onOpenAll(){setAllOpen(true);}
...
function Onboarding({onDone}){
  ...
  if(step===0){
    ...
    const [heroIdx,setHeroIdx]=useState(Math.floor(Math.random()*heroImgs.length));
    const heroImg=heroImgs[heroIdx];
    useEffect(()=>{
    ...
    return(
      <div style={{height:"100vh",maxHeight:"-webkit-fill-available",background:"#09090f",display:"flex",flexDirection:"column",position:"relative",overflow:"hidden"}}>
        ...
        <div style={{position:"relative",zIndex:4,padding:"44px 0 0",display:"flex",justifyContent:"center"}}>
          <div style={{display:"flex",alignItems:"center",gap:8,background:"rgba(0,0,0,0.3)",backdropFilter:"blur(12px)",WebkitBackdropFilter:"blur(12px)",border:"1px solid rgba(255,255,255,0.1)",borderRadius:16,padding:"8px 16px"}}>
            <div style={{width:28,height:28,borderRadius:8,background:"linear-gradient(135deg,#7c3aed,#e84393)",display:"flex",alignItems:"center",justifyContent:"center"}}>
            ...
            <span style={{fontFamily:"'Instrument Serif',serif",fontSize:16,color:"rgba(255,255,255,0.9)",letterSpacing:"0.3px"}}>StreamFinder</span>
          </div>
        </div>
        {/* Content unten */}
