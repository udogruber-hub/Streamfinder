...
async function enrichWithTMDB(rec){
  try{
...
function MainApp({profile:initProfile,onReset}){
  ...
  async function loadHomeFeed(){
    ...
    setFeedItems([]);
    try{
      // Step 1: KI generiert Empfehlungen
      const aiRecs=await getAIHomeFeed(profile);
      if(!aiRecs||aiRecs.length===0){setFeedLoading(false);return;}
      // Step 2: TMDB anreichern für Bilder/Details — parallel
      const enriched=await Promise.all(aiRecs.map(rec=>enrichWithTMDB(rec)));
  ...
  async function doAiRefresh(){
    ...
    try{
      ...
      if(aiRecs&&aiRecs.length>0){
        const enriched=await Promise.all(aiRecs.map(rec=>enrichWithTMDB(rec)));
        setHeroItems(enriched.slice(0,3));
        setFeedItems(enriched.slice(3));
      }
                ...
                {(top5Serie.length>0||top5Film.length>0)&&(
                  <div style={{background:"linear-gradient(135deg,#1a1525,#13101e)",borderRadius:18,padding:"14px 16px",marginBottom:16,border:"1px solid #2a1f3d"}}>
                    ...
                    {(browseType==="serie"?top5Serie:top5Film).map((rec,i)=>{
                      ...
                      return(
                        <div key={i} style={{display:"flex",gap:10,padding:"8px 0",borderBottom:i<4?"1px solid #1e1e30":"none",alignItems:"flex-start",cursor:"pointer"}} onClick={()=>setSelectedItem({title:rec.title,name:rec.title,overview:rec.reason,vote_average:0,genre_ids:[],poster_path:null,media_type:browseType==="serie"?"tv":"movie",release_date:rec.year||"",_aiOnly:true})}>
