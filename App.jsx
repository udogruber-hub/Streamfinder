  ...
  return(
    <div style={{position:"relative",overflow:"hidden",borderRadius:16,marginBottom:10}}>
      ...
      <div style={{position:"absolute",right:0,top:0,bottom:0,width:100,background:"linear-gradient(to left,#1a0a0a,transparent)",display:"flex",alignItems:"center",justifyContent:"flex-end",paddingRight:16,opacity:blockOpacity,pointerEvents:"none"}}>
        <span style={{fontSize:11,color:"#ef4444",fontWeight:600,fontFamily:"'DM Sans'"}}>Nicht mein Ding</span>
      </div>
      {/* Card */}
      <div
...
function MediathekChannelCard({ch,profile,onSelect,onBlock}){
  ...
  async function load(){
    ...
    try{
      ...
      const filtered=results.filter(r=>
        (r.duration||0)>1200&&
        !/(trailer|teaser|vorschau|clip)/i.test(r.title)
      ).slice(0,8);
      setItems(filtered);
    }catch(e){setItems([]);}
    setLoading(false);
  ...
  function handleOpen(){const next=!open;setOpen(next);if(next)load();}
  ...
  return(
    <div style={{marginBottom:8}}>
      {/* Channel Header */}
      <div onClick={handleOpen} style={{display:"flex",alignItems:"center",gap:10,padding:"10px 14px",background:"linear-gradient(135deg,"+ch.color+"22,"+ch.color+"11)",borderRadius:open?"14px 14px 0 0":"14px",border:"1px solid "+ch.color+"33",cursor:"pointer",borderBottom:open?"none":undefined}}>
        <div style={{width:36,height:36,borderRadius:8,background:ch.color,display:"flex",alignItems:"center",justifyContent:"center",fontSize:14,fontWeight:900,color:"#fff",flexShrink:0}}>{ch.icon}</div>
        <div style={{flex:1}}>
          <div style={{fontSize:13,fontWeight:800,color:"#f0ece4"}}>{ch.name}</div>
          <div style={{fontSize:10,color:"#b0a8b8"}}>Aktuelle Sendungen</div>
        </div>
        <div style={{color:ch.color,fontSize:12,transition:"transform 0.3s",transform:open?"rotate(180deg)":"none"}}>▾</div>
      </div>

      {/* TV Magazine Content */}
      {open&&(
        <div style={{background:"#0a0a14",border:"1px solid "+ch.color+"22",borderTop:"none",borderRadius:"0 0 14px 14px",overflow:"hidden"}}>
          {loading&&(
            <div style={{textAlign:"center",padding:16}}>
              <div style={{fontSize:18,animation:"spin 1.5s linear infinite",display:"inline-block"}}>📡</div>
              <p style={{fontSize:11,color:"#b0a8b8",marginTop:6}}>Lade Sendungen…</p>
            </div>
          )}
          {!loading&&items.length===0&&(
            <div style={{textAlign:"center",padding:16}}>
              <p style={{fontSize:12,color:"#b0a8b8",marginBottom:8}}>Keine aktuellen Sendungen</p>
              <a href={ch.url} target="_blank" rel="noopener noreferrer"
                style={{color:ch.color,fontSize:12,fontWeight:700,textDecoration:"none"}}>
                Zur {ch.name} →
              </a>
            </div>
          )}
          {!loading&&items.map((item,i)=>{
            ...
            const isNew=(item.timestamp&&(Date.now()/1000-item.timestamp)<86400);
            return(
              <div key={i}>
                <div onClick={()=>onSelect({
                  ...
                  style={{display:"flex",gap:0,cursor:"pointer",borderBottom:i<items.length-1?"1px solid rgba(255,255,255,0.04)":"none"}}>
                  {/* Zeit-Spalte */}
                  <div style={{width:54,flexShrink:0,background:"rgba(255,255,255,0.02)",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"10px 4px",borderRight:"1px solid rgba(255,255,255,0.04)"}}>
                    {time&&<div style={{fontSize:12,fontWeight:700,color:ch.color,lineHeight:1}}>{time}</div>}
                    {date&&<div style={{fontSize:9,color:"#555",marginTop:2}}>{date}</div>}
                    {dur&&<div style={{fontSize:9,color:"#444",marginTop:4}}>{dur}</div>}
                  </div>
                  {/* Thumbnail */}
                  {item.url_thumbnail?(
                    <img src={item.url_thumbnail} alt="" style={{width:90,height:58,objectFit:"cover",flexShrink:0}}/>
                  ):(
                    <div style={{width:90,height:58,background:ch.color+"11",flexShrink:0,display:"flex",alignItems:"center",justifyContent:"center",fontSize:22}}>{ch.icon}</div>
                  )}
                  {/* Info */}
                  <div style={{flex:1,padding:"8px 10px",minWidth:0}}>
                    <div style={{display:"flex",alignItems:"flex-start",gap:6,marginBottom:3}}>
                      <div style={{fontSize:12,fontWeight:700,color:"#f0ece4",lineHeight:1.3,flex:1}}>{item.title}</div>
                      {isNew&&<div style={{background:ch.color,borderRadius:4,padding:"1px 5px",fontSize:8,color:"#fff",fontWeight:700,flexShrink:0,marginTop:1}}>NEU</div>}
                    </div>
                    {item.topic&&<div style={{fontSize:10,color:ch.color,fontWeight:600,marginBottom:2}}>{item.topic}</div>}
                    {item.description&&<div style={{fontSize:10,color:"#666",lineHeight:1.3,display:"-webkit-box",WebkitLineClamp:2,WebkitBoxOrient:"vertical",overflow:"hidden"}}>{item.description}</div>}
                  </div>
                </div>
              </div>
            );
          ...
          {!loading&&items.length>0&&(
            <div style={{padding:"8px 14px",display:"flex",justifyContent:"flex-end"}}>
              <a href={ch.url} target="_blank" rel="noopener noreferrer"
                style={{color:ch.color,fontSize:10,fontWeight:700,textDecoration:"none",opacity:0.6}}>
                Mehr auf {ch.name} →
              </a>
            </div>
          )}
        ...
        {tab==="home"&&(
          <div>
            ...
            {feedLoading?(
              ...
              <div style={{padding:"0 18px"}}>
                {heroItems.length>0&&<div style={{marginBottom:10}}><p style={{fontSize:11,color:"#b0a8b8",fontWeight:700,textTransform:"uppercase",letterSpacing:0.8,marginBottom:10}}>Top-Picks</p>{heroItems.map(it=><SwipeToBlock key={it.id} onBlock={()=>cardProps.onBlock(it.title||it.name||"")}><HeroCard item={it} {...cardProps}/></SwipeToBlock>)}</div>}
                {feedItems.length>0&&<div><p style={{fontSize:11,color:"#b0a8b8",fontWeight:700,textTransform:"uppercase",letterSpacing:0.8,marginBottom:10}}>Weitere Empfehlungen</p>{feedItems.map(it=><SwipeToBlock key={it.id} onBlock={()=>cardProps.onBlock(it.title||it.name||"")}><TitleCard item={it} {...cardProps}/></SwipeToBlock>)}</div>}
              </div>
            )}
          </div>
        )}
