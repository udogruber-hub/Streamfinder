...
function MainApp(props) {
  ...
  function loadData() {
    ...
    var platformsToLoad = [];
    if (filterPlatform) {
      platformsToLoad = [filterPlatform];
    } else {
      platformsToLoad = profile.platforms.slice();
    }
    
    if (platformsToLoad.length === 0) { setLoading(false); return; }
    ...
    // Build promises: for each platform, fetch genre-filtered AND popular
    var promises = [];
    var promiseMap = []; // tracks what each promise is for
    
    platformsToLoad.forEach(function(platId) {
      var pids = PROVIDER_MAP[platId] || [];
      if (pids.length === 0) return;
      
      // "Für dich" - one call per mood/profile genre per platform
      if (forYouGenres.length > 0) {
        forYouGenres.slice(0, 2).forEach(function(gid) {
          promises.push(discoverTitles(apiKey, "serie", pids, String(gid), 1, "vote_average.desc"));
          promiseMap.push({ type: "foryou", platform: platId, media: "tv" });
          promises.push(discoverTitles(apiKey, "film", pids, String(gid), 1, "vote_average.desc"));
          promiseMap.push({ type: "foryou", platform: platId, media: "movie" });
        });
      }
      
      // Popular series for this platform
      promises.push(discoverTitles(apiKey, "serie", pids, null, 1, "popularity.desc"));
      promiseMap.push({ type: "series", platform: platId, media: "tv" });
      
      // Popular films for this platform
      promises.push(discoverTitles(apiKey, "film", pids, null, 1, "popularity.desc"));
      promiseMap.push({ type: "films", platform: platId, media: "movie" });
      
      // Top rated for this platform
      promises.push(discoverTitles(apiKey, "serie", pids, null, 1, "vote_average.desc", { "vote_count.gte": 50 }));
      promiseMap.push({ type: "top", platform: platId, media: "tv" });
      promises.push(discoverTitles(apiKey, "film", pids, null, 1, "vote_average.desc", { "vote_count.gte": 50 }));
      promiseMap.push({ type: "top", platform: platId, media: "movie" });
    });
    ...
    Promise.all(promises).then(function(results) {
      ...
      results.forEach(function(res, idx) {
        ...
        var info = promiseMap[idx];
        
        // Tag each item with media_type and platform
        items.forEach(function(item) {
          item.media_type = info.media;
          if (!item._platform) item._platform = info.platform;
        });
        
        if (info.type === "foryou") {
          forYouAll = forYouAll.concat(items.slice(0, 5));
        } else if (info.type === "series") {
          seriesAll = seriesAll.concat(items.slice(0, 8));
        } else if (info.type === "films") {
          filmsAll = filmsAll.concat(items.slice(0, 8));
        } else if (info.type === "top") {
          topAll = topAll.concat(items.slice(0, 8));
        }
      });
      
      // If no forYou genre results, use series+films as fallback
      if (forYouAll.length === 0) {
        forYouAll = seriesAll.concat(filmsAll);
      }
      
      setForYou(filterDisliked(dedup(forYouAll)).slice(0, 20));
      setSeries(filterDisliked(dedup(seriesAll)).slice(0, 20));
      setFilms(filterDisliked(dedup(filmsAll)).slice(0, 20));
      
      topAll.sort(function(a, b) { return (b.vote_average || 0) - (a.vote_average || 0); });
      setTop(filterDisliked(dedup(topAll)).slice(0, 20));
  ...
  function handleSurprise() {
    ...
    setSurprise(null);
    // Pick a random page (1-5) and random genre from profile
    var profileGenres = getProfileGenres();
    var randomGenre = profileGenres.length > 0 ? profileGenres[Math.floor(Math.random() * profileGenres.length)] : null;
    ...
    discoverTitles(apiKey, randomType, providerIds, randomGenre ? String(randomGenre) : null, randomPage, "popularity.desc").then(function(data) {
      var results = (data && data.results) || [];
      results = results.filter(function(r) { return !profile.disliked.includes(r.id) && r.vote_average >= 6; });
      if (results.length > 0) {
        var pick = results[Math.floor(Math.random() * results.length)];
        pick.media_type = randomType === "serie" ? "tv" : "movie";
      ...
      setTimeout(function() { setSurpriseRevealed(true); }, 800);
            ...
            <div style={{ background: "linear-gradient(135deg, #ff6b3515, #e8439315)", borderRadius: 20, padding: 20, marginBottom: 20, border: "1px solid #ff6b3525", textAlign: "center" }}>
              ...
              {surpriseLoading ? (
                <div>
                  ...
                  <p style={{ fontSize: 15, color: "#c4b8a8", marginTop: 8, fontWeight: 600 }}>Moment... die Magie wirkt...</p>
                </div>
              ) : null}
              {surprise && surpriseRevealed ? (
                <div style={{ animation: "fadeUp 0.5s ease" }}>
                  <p style={{ fontSize: 15, color: "#ff6b35", fontWeight: 700, marginBottom: 12 }}>{surprise._surpriseMsg}</p>
