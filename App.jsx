      ...
      <div onClick={handleExpand} style={{ padding: "12px 14px", display: "flex", alignItems: "center", gap: 12, cursor: "pointer" }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ display: "flex", gap: 5, alignItems: "center", flexWrap: "wrap", marginBottom: 4 }}>
            ...
            {pl ? <span style={{ fontSize: 10, color: pl.color, fontWeight: 700, background: pl.color + "18", padding: "1px 6px", borderRadius: 4 }}>{pl.name}</span> : null}
            {genreIds.slice(0, 3).map(function(gid) {
              var g = GENRES_TMDB[gid];
              return g ? <span key={gid} style={{ fontSize: 9, color: "#666" }}>{g.emoji}{g.name}</span> : null;
            })}
          </div>
          {item.overview ? (
            <p style={{ margin: "0 0 4px", fontSize: 11, color: "#666", lineHeight: 1.4, display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical", overflow: "hidden" }}>
              {item.overview}
            </p>
          ) : null}
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#0a0a0f", color: "#e8e6e1", fontFamily: "'DM Sans', sans-serif", paddingBottom: 85 }}>
      ...
      <div style={{ padding: "16px 18px" }}>
        ...
        {tab === "merkliste" ? (
          <div>
            <h3 style={{ fontSize: 15, fontWeight: 700, marginBottom: 6 }}>❤️ Deine Merkliste</h3>
