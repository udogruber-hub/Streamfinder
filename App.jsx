      ...
      .then(async tmdbItems=>{
        if(tmdbItems.length>0){
          ...
          const fallbackReasons=[
            ...
            "Passt perfekt zu deinem Geschmack.",
          ...
          const formatted=tmdbItems.map((it,i)=>{
            const score=it.vote_average?Math.round(it.vote_average*10)/10:0;
            const aiMatch=aiReasons?.find(a=>titleKey(a.title||"")===titleKey(it.title||it.name||""))||aiReasons?.[i];
            return{
              title:it.title||it.name||"",
              year:(it.release_date||it.first_air_date||"").substring(0,4),
              reason:aiMatch?.reason||fallbackReasons[i]||fallbackReasons[0],
      ...
      {open&&mode!=="swipe"&&(
        <div style={{padding:"0 14px 16px"}}>
          ...
          {!loading&&recs&&recs.map((rec,i)=>{
            ...
            return(
              <SwipeToBlock key={i} onBlock={()=>{
                setRecs(prev=>{
                  const next=prev.filter((_,j)=>j!==i);
                  // Lade neuen Ersatztitel im Hintergrund
                  getTMDBTop5(profile,platform,browseType||"serie").then(fresh=>{
                    const existing=new Set(next.map(r=>r.title));
                    const replacement=fresh.find(it=>!existing.has(it.title||it.name||""));
                    if(replacement){
                      const score=replacement.vote_average?Math.round(replacement.vote_average*10)/10:0;
                      setRecs(cur=>[...(cur||[]),{
                        title:replacement.title||replacement.name||"",
                        year:(replacement.release_date||replacement.first_air_date||"").substring(0,4),
                        reason:"Neu entdeckt — könnte dir gefallen.",
        ...
        <div style={{padding:"0 18px 48px"}}>
          ...
          <PlatformSwipe platform={platform} profile={profile} browseType={browseType} onBlock={t=>onBlock(t)} onLike={it=>onLike(it)} onSelect={onSelect} onRate={onRate||((t,s,g)=>{})} onDone={(freshProfile)=>{
            ...
            getTMDBTop5(freshProfile||profile,platform,browseType||"serie")
              .then(async tmdbItems=>{
                if(tmdbItems.length>0){
                  const fallback=["Passt zu deinem frischen Geschmack.","Perfekt nach dem Swipen.","Dein Profil hat gesprochen.","Läuft gerade gut.","Eine starke Wahl."];
                  let aiReasons=null;
                  try{
                    ...
                    const typeStr=browseType==="serie"?"Serien":"Filme";
                    const system=`Du bist ein witziger Streaming-Experte. Antworte NUR mit JSON-Array, keine Backticks. Format: [{"title":"...","reason":"..."}]`;
                    const msg=`Schreibe für diese ${typeStr} auf ${platform.name} witzige Begründungen.\\nTitel: ${titles}\\nNutzer liebt: ${ctx.top||"nichts"}\\nreason = vollständiger Satz, min 10 Wörter, witzig.`;
                    const text=await callAI([{role:"user",content:msg}],system);
                    aiReasons=JSON.parse(text.replace(/```json|```/g,"").trim());
                  }catch(e){}
                  const formatted=tmdbItems.map((it,i)=>{
                    const aiMatch=aiReasons?.find(a=>titleKey(a.title||"")===titleKey(it.title||it.name||""))||aiReasons?.[i];
                    return{title:it.title||it.name||"",year:(it.release_date||it.first_air_date||"").substring(0,4),reason:aiMatch?.reason||fallback[i]||fallback[0],emoji:it.media_type==="tv"?"📺":"🎬",_tmdbItem:it};
                  });
                  setRecs(formatted);setLoading(false);
