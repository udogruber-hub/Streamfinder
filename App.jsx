...
function MainApp(props) {
  ...
  function loadData() {
    ...
    // Get mood-specific genres (these should DOMINATE "Für dich")
    var moodGenres = getMoodGenres();
    var profileGenres = getProfileGenres();
    
    // For "Für dich": use ONLY mood genres if mood is set, otherwise profile genres
    var forYouGenres = moodGenres.length > 0 ? moodGenres : profileGenres;
    // Use comma (AND) for tighter mood filtering
    var forYouGenreStr = forYouGenres.slice(0, 2).join(",");
    
    // For series/films tabs: use profile genres with OR
    var profileGenreStr = profileGenres.length > 0 ? profileGenres.join("|") : "";

    var promises = [
      // 0: For You series - mood/profile filtered
      discoverTitles(apiKey, "serie", providerIds, forYouGenreStr ? [forYouGenreStr] : [], 1, "popularity.desc"),
      // 1: For You films - mood/profile filtered  
      discoverTitles(apiKey, "film", providerIds, forYouGenreStr ? [forYouGenreStr] : [], 1, "popularity.desc"),
      // 2: Series tab - profile genre filtered
      discoverTitles(apiKey, "serie", providerIds, profileGenreStr ? [profileGenreStr] : [], 1, "popularity.desc"),
      // 3: Films tab - profile genre filtered
      discoverTitles(apiKey, "film", providerIds, profileGenreStr ? [profileGenreStr] : [], 1, "popularity.desc"),
      // 4: Top rated series (no genre filter)
      discoverTitles(apiKey, "serie", providerIds, [], 1, "vote_average.desc"),
      // 5: Top rated films (no genre filter)
    ...
    Promise.all(promises).then(function(results) {
      ...
      var profSeries = (results[2] && results[2].results) || [];
      var profFilms = (results[3] && results[3].results) || [];
      var topSeries = (results[4] && results[4].results) || [];
      var topFilms = (results[5] && results[5].results) || [];

      // Tag media types
      forYouSeries.forEach(function(i) { i.media_type = "tv"; });
      forYouFilms.forEach(function(i) { i.media_type = "movie"; });
      profSeries.forEach(function(i) { i.media_type = "tv"; });
      profFilms.forEach(function(i) { i.media_type = "movie"; });
      ...
      // For you: ONLY mood/profile genre results (no fallback mixing)
      var forYouAll = dedup(forYouSeries.concat(forYouFilms));
      setForYou(filterDisliked(forYouAll).slice(0, 20));

      // Series & Films tabs: profile-based
      setSeries(filterDisliked(dedup(profSeries)).slice(0, 20));
      setFilms(filterDisliked(dedup(profFilms)).slice(0, 20));

      // Top rated
      var topAll = dedup(topSeries.concat(topFilms));
      topAll.sort(function(a, b) { return (b.vote_average || 0) - (a.vote_average || 0); });
      setTop(filterDisliked(topAll).slice(0, 20));
