...
function LedStrip({itemTitle,profile,onRate,genre_ids}){
  ...
  const current=(profile.ratings||{})[key]||0;
  const segments=[
    {v:1,color:"#ef4444",glow:"rgba(239,68,68,0.7)",label:"Nope"},
    {v:3,color:"#f97316",glow:"rgba(249,115,22,0.7)",label:"Okay"},
    {v:5,color:"#4ade80",glow:"rgba(74,222,128,0.7)",label:"Feuer 🔥"},
  ];
  return(
    <div style={{display:"flex",alignItems:"center",gap:5}}>
      <div style={{display:"flex",flex:1,height:18,borderRadius:6,overflow:"hidden",gap:2,background:"rgba(255,255,255,0.04)",padding:2}}>
        {segments.map(seg=>{
          const active=current===seg.v;
          return(
            <button key={seg.v}
              onClick={e=>{e.stopPropagation();e.preventDefault();onRate(itemTitle,current===seg.v?0:seg.v,genre_ids||[]);}}
              style={{flex:1,background:active?seg.color:"transparent",border:"none",borderRadius:4,cursor:"pointer",transition:"all 0.15s",boxShadow:active?"0 0 6px "+seg.glow:"none",display:"flex",alignItems:"center",justifyContent:"center"}}>
              <span style={{fontSize:8,fontWeight:700,color:active?"rgba(0,0,0,0.6)":"rgba(255,255,255,0.18)",pointerEvents:"none",fontFamily:"'DM Sans'"}}>{seg.label}</span>
            </button>
          );
        })}
      </div>
    ...
    <div style={{background:"#12121f",borderRadius:18,overflow:"hidden",border:"1px solid #1e1e30",marginBottom:10}}>
      <div style={{padding:"14px 16px",display:"flex",alignItems:"center",gap:12,cursor:"pointer"}} onClick={handleOpen}>
        ...
        <div style={{flex:1}}>
          ...
          <div style={{fontSize:9,color:"#3a3344",marginTop:2}}>Balken = KI-Match zu deinem Geschmack</div>
        </div>
