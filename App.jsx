...
// AI reason variations - not always "weil du X liebst"
const REASON_TEMPLATES = [
  (title, genre) => \`Passt zu dir wie \${genre} zu einem Regentag.\`,
  (title, genre) => \`Weil dein Streaming-Profil danach schreit.\`,
  (title, genre) => \`Der Algorithmus ist sich bei diesem sicher.\`,
  (title, genre) => \`Wenige kennen es. Du wirst es lieben.\`,
  (title, genre) => \`Dein nächstes Gesprächsthema wartet hier.\`,
  (title, genre) => \`Vertrau uns. Einmal.\`,
  (title, genre) => \`Weil du dir das verdient hast.\`,
];
  ...
  const HELP={
    home:{
      ...
      items:[
        ...
        {icon:"🔄",title:"Empfehlungen aktualisieren",text:"Der Button oben rechts lädt frische Picks. Nach neuen Bewertungen lohnt es sich — der Algorithmus hat dazugelernt."},
...
function PlatformSwipe({platform,profile,browseType,onBlock,onLike,onSelect,onDone,onRate}){
  ...
  // Track how many rated for feedback
  ...
  // Right swipe = 4★ + genre boost (KEIN automatisches Merken)
  function handleRight(item){
    onRate(item.title||item.name||"",4,item.genre_ids||[]);
    setSwipedCount(c=>c+1);
    advance();
  }
  // Left swipe = ausblenden + leichte Genre-Abwertung
  function handleLeft(item){
    onBlock(item.title||item.name||"");
    // Negative genre signal (soft)
    onRate(item.title||item.name||"",1,item.genre_ids||[]);
    setSwipedCount(c=>c+1);
    advance();
  }
  // Skip = neutral, kein Rating
  function handleSkip(){
    setSwipedCount(c=>c+1);
    advance();
  }
  ...
  if(done){
    // Sofort Top 5 anzeigen — kein Button nötig
    onDone();
    return null;
  }
    ...
    getTMDBTop5(profile,platform,browseType||"serie")
      .then(tmdbItems=>{
        if(tmdbItems.length>0){
          // Erstmal mit generischen Texten anzeigen
          const initial=tmdbItems.map(it=>({
          ...
          // KI schreibt witzige Begründungen im Hintergrund
          const ctx=buildCtx(profile);
          ...
          const msg=`Schreibe für diese ${typeStr} auf ${platform.name} je eine kurze witzige Begründung warum dieser Nutzer genau DIESEN Titel schauen sollte.\n\nTitel: ${titles}\n\nNutzer liebt: ${ctx.top||"noch nichts bewertet"}\nLieblingsgenres: ${ctx.topGenres||"alles"}\n\nreason = 1 Satz, witzig und persönlich, 60-100 Zeichen. Nicht immer "weil du X liebst". Sei kreativ — Vergleiche, Prophezeiungen, Warnungen erlaubt.`;
          callAI([{role:"user",content:msg}],system)
            .then(text=>{
              try{
                const aiReasons=JSON.parse(text.replace(/```json|```/g,"").trim());
                setRecs(prev=>prev?prev.map((rec,i)=>{
                  const match=aiReasons.find(a=>titleKey(a.title||"")===titleKey(rec.title))||aiReasons[i];
                  return match?.reason?{...rec,reason:match.reason}:rec;
                }):prev);
              }catch(e){}
            })
            .catch(()=>{
              // KI nicht verfügbar — generische aber bessere Texte
              setRecs(prev=>prev?prev.map((rec,i)=>{
                const score=rec._tmdbItem?.vote_average?Math.round(rec._tmdbItem.vote_average*10)/10:0;
                const reasons=[
                  "Einer der besten auf "+platform.name+" gerade.",
                  "★ "+score+" — spricht für sich.",
                  "Perfekt für heute Abend.",
                  "Läuft gerade heiß bei "+platform.name+".",
                  "Dein Algorithmus hat entschieden.",
                ];
                return{...rec,reason:reasons[i]||reasons[0]};
              }):prev);
            });
        }else{
          setRecs([]);
    ...
    <div style={{background:"#12121f",borderRadius:18,overflow:"hidden",border:"1px solid #1e1e30",marginBottom:10}}>
      ...
      {open&&mode==="swipe"&&(
        <div style={{padding:"0 18px 48px"}}>
          ...
          <PlatformSwipe platform={platform} profile={profile} browseType={browseType} onBlock={t=>onBlock(t)} onLike={it=>onLike(it)} onSelect={onSelect} onRate={onRate||((t,s,g)=>{})} onDone={()=>{setMode("top5");lastTypeRef.current=null;loadTop5Recs();}}/>
        </div>
      )}
    </div>
  );
          ...
          <div style={{padding:"0 18px"}}>
            ...
            {(profile.blocked_titles||[]).length>0&&(
              <div style={{background:"#12121f",borderRadius:14,padding:14,border:"1px solid rgba(239,68,68,0.2)",marginBottom:14}}>
                <p style={{fontSize:11,color:"#ef4444",fontWeight:700,marginBottom:8}}>🚫 Ausgeblendet ({(profile.blocked_titles||[]).length})</p>
