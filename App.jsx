...
// ── Geschmacks-Score — lebendiges Profil ──
function GeschmacksScore({profile}){
  ...
  const milestones=[
    {min:0, max:2, emoji:"🌱", label:"Dein Profil erwacht", sub:"Bewerte ein paar Titel damit wir dich kennenlernen"},
    {min:3, max:9, emoji:"🎯", label:"Wir lernen dich kennen", sub:"Weiter so — deine Empfehlungen werden besser"},
    {min:10, max:24, emoji:"🔥", label:"StreamFinder kennt dich gut", sub:"Deine Empfehlungen sind schon ziemlich persönlich"},
    {min:25, max:49, emoji:"⭐", label:"Du bist ein Stammgast", sub:"Top-Level Empfehlungen — dein Profil ist stark"},
    {min:50, max:999, emoji:"🏆", label:"Experten-Level", sub:"Kaum jemand kennt seinen Geschmack so gut wie du"},
  ];
  ...
  return(
    <div style={{background:"linear-gradient(135deg,rgba(196,169,96,0.08),rgba(255,107,53,0.05))",border:"1px solid rgba(196,169,96,0.15)",borderRadius:16,padding:"14px 16px",marginBottom:16}}>
      <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:10}}>
        ...
        <div style={{flex:1}}>
          ...
          <div style={{fontSize:11,color:"#8a849a"}}>{current.sub}</div>
        </div>
        <div style={{textAlign:"right",flexShrink:0}}>
          <div style={{fontSize:18,fontWeight:900,color:"#c4a960",fontFamily:"'DM Sans'"}}>{total}</div>
          <div style={{fontSize:9,color:"#6a6478"}}>Aktivitäten</div>
        </div>
      </div>
      {topGenres.length>0&&(
        <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:toNext>0?8:0}}>
          {topGenres.map(g=>(
            <span key={g} style={{background:"rgba(196,169,96,0.1)",border:"1px solid rgba(196,169,96,0.2)",borderRadius:20,padding:"3px 10px",fontSize:10,color:"#c4a960",fontWeight:600}}>
              {g}
            </span>
          ))}
        </div>
      ...
      {toNext>0&&(
        <div style={{fontSize:10,color:"#4a4060"}}>
          Noch {toNext} Aktivitäten bis: <span style={{color:"#c4a960"}}>{next.emoji} {next.label}</span>
        </div>
      )}
    </div>
  );
