  ...
  async function loadHomeFeed(){
    ...
    function filterAndScore(items){
      ...
      return items.filter(it=>{
        ...
        if(blockedKeys.has(t))return false;
        const myRating=ratings[t]||0;
        // Filter out anything rated under 3 stars or blocked
        if(myRating>0&&myRating<3)return false;
        if((it.vote_average||0)<5)return false;
        return true;
      }).map(it=>{
        let s=(it.vote_average||0)*2;
        (it.genre_ids||[]).forEach(g=>{s+=(profile.genres?.[g]||0)*1.5;});
        // Boost genres from highly rated titles
        const myRating=ratings[titleKey(it.title||it.name||"")]||0;
        if(myRating>=4)s+=10;
        return{...it,_score:s};
      }).sort((a,b)=>b._score-a._score);
    }
