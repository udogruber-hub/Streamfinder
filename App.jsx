...
function SpotlightTab({profile,cardProps,onSelect}){
  ...
  async function loadFirstRecs(stimmung){
    ...
    try{
      ...
      try{
        const allIds=(profile.platforms||[]).flatMap(id=>PLATFORMS.find(p=>p.id===id)?.tmdbIds||[]);
        // TMDB Fallback — ein Genre nach dem anderen, kein AND-Filter
        const genre=stimmung.genres[0]; // Erstes Genre nehmen
        const results=await Promise.all([
          fetch(TMDB_BASE+"/discover/movie?api_key="+TMDB_API_KEY+"&language=de-DE&sort_by=popularity.desc&vote_count.gte=50&with_genres="+genre+"&page="+Math.ceil(Math.random()*3)).then(r=>r.json()),
          fetch(TMDB_BASE+"/discover/tv?api_key="+TMDB_API_KEY+"&language=de-DE&sort_by=popularity.desc&vote_count.gte=50&with_genres="+genre+"&page="+Math.ceil(Math.random()*3)).then(r=>r.json()),
        ]);
        const fallbackItems=[
          ...(results[0].results||[]).map(it=>({...it,media_type:"movie"})),
          ...(results[1].results||[]).map(it=>({...it,media_type:"tv"})),
        ].filter(it=>it.poster_path&&!shownTitles.current.has(it.id))
...
Kombiniere Stimmung und persönlichen Geschmack.`;
      ...
      try{
        ...
        const res2=await Promise.all([
          fetch(TMDB_BASE+"/discover/movie?api_key="+TMDB_API_KEY+"&language=de-DE&sort_by=popularity.desc&vote_count.gte=50&with_genres="+genre2+"&page="+Math.ceil(Math.random()*3)).then(r=>r.json()),
          fetch(TMDB_BASE+"/discover/tv?api_key="+TMDB_API_KEY+"&language=de-DE&sort_by=popularity.desc&vote_count.gte=50&with_genres="+genre2+"&page="+Math.ceil(Math.random()*3)).then(r=>r.json()),
        ]);
        const items2=[
          ...(res2[0].results||[]).map(it=>({...it,media_type:"movie"})),
          ...(res2[1].results||[]).map(it=>({...it,media_type:"tv"})),
        ].filter(it=>it.poster_path).sort((a,b)=>(b.vote_average||0)-(a.vote_average||0));
      ...
      <div style={{minHeight:"100vh",background:"#09090f",fontFamily:"'DM Sans',sans-serif",display:"flex",flexDirection:"column"}}>
        ...
        {!swipeLoading&&cur&&(
          <div style={{flex:1,display:"flex",flexDirection:"column",padding:"12px 20px 20px",maxWidth:440,margin:"0 auto",width:"100%",minHeight:0}}>
            {/* Poster — Hochformat passt besser für Swipe */}
            <div style={{height:"calc(100vh - 280px)",maxHeight:420,borderRadius:20,overflow:"hidden",position:"relative",background:"#0a0a12",flexShrink:0}}>
              {(cur.poster_path||cur.backdrop_path)
                ?<img src={"https://image.tmdb.org/t/p/w500"+(cur.poster_path||cur.backdrop_path)} alt="" style={{width:"100%",height:"100%",objectFit:"cover",objectPosition:"center center",display:"block"}}/>
