...
Kombiniere Stimmung und persönlichen Geschmack.`;
  ...
  return(
    <div style={{display:"flex",flexDirection:"column",height:"calc(100vh - 120px)"}}>
      ...
      {/* Stimmungs-Empfehlungen — oberhalb der Kacheln */}
      {!activeFeature&&stimmungRecs.length>0&&(
        <div style={{flexShrink:0,padding:"0 12px 8px"}}>
          {stimmungLoading&&<div style={{textAlign:"center",padding:"12px 0"}}><span style={{fontSize:18,animation:"spin 1s linear infinite",display:"inline-block"}}>✨</span></div>}
          <div style={{display:"flex",flexDirection:"column",gap:8}}>
            {stimmungRecs.map((it,i)=>(
      ...
      {!activeFeature&&stimmungLoading&&!stimmungRecs.length&&(
          ...
          <div style={{flexShrink:0,marginBottom:4}}>
            ...
            <div style={{display:"flex",gap:7,overflowX:"auto",paddingBottom:4}}>
              {STIMMUNGEN.map(m=>{
                ...
                return(
                  <button key={m.id} onClick={()=>{setActiveStimmung(m.id===activeStimmung?null:m.id);if(m.id!==activeStimmung)loadStimmungRecs(m);else setStimmungRecs([]);}}
  ...
  return(
    <div style={{minHeight:"100vh",background:"#09090f",color:"#f0ece4",fontFamily:"'DM Sans',sans-serif",paddingBottom:90}}>
      ...
      <div style={{position:"sticky",top:0,zIndex:100,background:"rgba(9,9,15,0.95)",backdropFilter:"blur(20px)",borderBottom:"1px solid #1e1e30",padding:"12px 18px",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
        ...
        <div style={{display:"flex",gap:8}}>
          <button onClick={()=>setShowHelp(true)} style={{width:36,height:36,borderRadius:10,background:"rgba(255,255,255,0.06)",border:"1px solid rgba(255,255,255,0.1)",cursor:"pointer",color:"#b0a8b8",fontSize:16,display:"flex",alignItems:"center",justifyContent:"center"}}>?</button>
          <button onClick={onReset} style={{width:36,height:36,borderRadius:10,background:"rgba(255,255,255,0.06)",border:"1px solid rgba(255,255,255,0.1)",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}}>
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#b0a8b8" strokeWidth="1.8" strokeLinecap="round"><circle cx="12" cy="12" r="3"/><path d="M19.07 4.93a10 10 0 010 14.14M4.93 4.93a10 10 0 000 14.14"/></svg>
          </button>
        </div>
      </div>
