...
function buildCtx(profile){
  ...
  const rated4=Object.entries(ratings).filter(([,v])=>v===4).map(([k])=>titleMap[k]||k).slice(0,6).join(", ");
  const rated2=Object.entries(ratings).filter(([,v])=>v===2).map(([k])=>titleMap[k]||k).slice(0,4).join(", ");
  const rated1=Object.entries(ratings).filter(([,v])=>v===1).map(([k])=>titleMap[k]||k).slice(0,4).join(", ");

  const top=[...Object.entries(ratings).filter(([,v])=>v>=4).map(([k,v])=>(titleMap[k]||k)+"("+v+"★)")].slice(0,8).join(", ");
  const low=[...Object.entries(ratings).filter(([,v])=>v<=2&&v>0).map(([k])=>titleMap[k]||k)].slice(0,5).join(", ");
  ...
  if(rated5)taste+=". Absolut liebt: "+rated5;
  if(rated4)taste+=". Sehr gut bewertet: "+rated4;
  if(rated1||rated2)taste+=". Mag gar nicht: "+([rated1,rated2].filter(Boolean).join(", "));
...
function UniversalSwipe({profile,cardProps,onSelect,onDone}){
  ...
  async function handleDone(){
    ...
    try{
      ...
      const system=`Du bist ein Streaming-Experte der Geschmack tief versteht. Antworte NUR mit JSON-Array, kein Markdown. Format: [{"title":"...","platform":"...","reason":"..."}]. Plattform exakt so wie angegeben.`;
...
BEWERTUNGS-HISTORY:
⭐⭐⭐⭐⭐ Liebt (5★): ${ctx.rated5||"noch keine"}
⭐⭐⭐⭐ Gut (4★): ${ctx.rated4||"noch keine"}
👎 Mag nicht (1-2★): ${[ctx.rated1,ctx.rated2].filter(Boolean).join(", ")||"keine"}
...
- reason = witziger persönlicher Satz auf Deutsch max 12 Wörter`;
      const text=await callAI([{role:"user",content:msg}],system);
