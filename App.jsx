    ...
    <div style={{background:"#12121f",borderRadius:18,overflow:"hidden",border:"1px solid #1e1e30",marginBottom:10}}>
      <div style={{padding:"14px 16px",display:"flex",alignItems:"center",gap:12,cursor:"pointer"}} onClick={handleOpen}>
        ...
        <div style={{flex:1}}>
          ...
          <div style={{fontSize:9,color:"#4a4458",marginTop:2}}>▌= Übereinstimmung mit deinem Verlauf</div>
        </div>
      ...
      {open&&(
        <div style={{padding:"0 14px 16px"}}>
          ...
          {!loading&&recs&&recs.map((rec,i)=>{
            ...
            const scoreColor=score>=8?"#4ade80":score>=7?"#fbbf24":"#fb923c";
            // % Übereinstimmung mit bewertetem Verlauf
            ...
            const matchPct=rec.score||Math.min(95,Math.max(20,Math.round(20+genreMatch*58+ratingBonus*18)));
                    ...
                    <div style={{position:"absolute",inset:0,display:"flex",alignItems:"center",gap:12,padding:"0 14px"}}>
                      ...
                      <div style={{display:"flex",flexDirection:"column",alignItems:"center",flexShrink:0,width:32,gap:2}}>
                        <div style={{width:5,height:36,borderRadius:3,background:"#1e1e30",overflow:"hidden",position:"relative"}}>
                          <div style={{position:"absolute",bottom:0,left:0,right:0,height:matchPct+"%",background:matchColor,borderRadius:3,boxShadow:"0 0 6px "+matchColor+"88",transition:"height 0.4s"}}/>
                        </div>
                        <span style={{fontSize:8,color:matchColor,fontWeight:800,fontFamily:"'DM Sans'",whiteSpace:"nowrap"}}>{matchLabel}</span>
                      </div>
    ...
    try{
      ...
      const platCandidates=await Promise.all(userPlats.map(async(plat,pi)=>{
        ...
        const candidates=all.filter(r=>{
          ...
          const kidGenres=[16,10762,10751]; // Animation, Kids, Family
          const profileHasKids=(profile.genres||{})[16]>0||(profile.genres||{})[10762]>0;
  ...
  return(
    <div style={{position:"relative",minHeight:"85vh",overflow:"hidden"}}>
      ...
      <div style={{position:"relative",zIndex:1,padding:"0 14px",display:"grid",gridTemplateColumns:"1fr 1fr",gridTemplateRows:"auto auto auto",gap:10,marginBottom:10}}>
        ...
        {(()=>{
          const bgImg=profile.watched?.slice(-1)[0]?.backdrop_path||profile.watched?.slice(-1)[0]?.poster_path;
          const bgUrl=bgImg?TMDB_IMG+bgImg:null;
          return(
            <button onClick={()=>setActiveFeature("deepdive")}
              style={{gridColumn:"1",gridRow:"1 / 3",background:tiles[0].gradient,border:"1px solid "+tiles[0].border,borderRadius:20,cursor:"pointer",textAlign:"left",position:"relative",overflow:"hidden",minHeight:200}}>
              {bgUrl&&<img src={bgUrl} alt="" style={{position:"absolute",inset:0,width:"100%",height:"100%",objectFit:"cover",opacity:0.15,filter:"blur(2px)"}}/>}
              <div style={{position:"absolute",inset:0,background:"linear-gradient(135deg,rgba(0,0,0,0.85),rgba(0,0,0,0.5))"}}/>
              <div style={{position:"absolute",top:-30,right:-30,width:120,height:120,background:"radial-gradient(circle,"+tiles[0].color+"30,transparent 70%)",pointerEvents:"none"}}/>
              <div style={{position:"relative",padding:"22px 18px",height:"100%",display:"flex",flexDirection:"column",justifyContent:"space-between"}}>
                <div style={{width:52,height:52,borderRadius:15,background:tiles[0].color+"25",border:"1px solid "+tiles[0].color+"40",display:"flex",alignItems:"center",justifyContent:"center",boxShadow:"0 0 24px "+tiles[0].color+"30"}}>
                  {SpotlightIcons.deepdive(tiles[0].color,28)}
                </div>
                <div>
                  <div style={{fontSize:16,fontWeight:800,color:"#f0ece4",marginBottom:8,lineHeight:1.3}}>{tiles[0].title}</div>
                  <div style={{fontSize:11,color:"#9a94a8",lineHeight:1.5}}>{tiles[0].desc}</div>
                </div>
              </div>
              <div style={{position:"absolute",bottom:16,right:16,width:28,height:28,borderRadius:8,background:tiles[0].color+"20",border:"1px solid "+tiles[0].color+"40",display:"flex",alignItems:"center",justifyContent:"center"}}>
        ...
        {/* Vibe-Meter + Popcorn-Guru — rechts */}
        {[tiles[1],tiles[2]].map((t,ri)=>{
          const bgImg=ri===0?profile.watched?.slice(-3)[1]?.poster_path:profile.watched?.slice(-3)[2]?.poster_path;
          const bgUrl=bgImg?TMDB_IMG+bgImg:null;
          return(
            <button key={t.id} onClick={()=>setActiveFeature(t.id)}
              style={{gridColumn:"2",gridRow:ri===0?"1":"2",background:t.gradient,border:"1px solid "+t.border,borderRadius:20,cursor:"pointer",textAlign:"left",position:"relative",overflow:"hidden"}}>
              {bgUrl&&<img src={bgUrl} alt="" style={{position:"absolute",inset:0,width:"100%",height:"100%",objectFit:"cover",opacity:0.12,filter:"blur(1px)"}}/>}
              <div style={{position:"absolute",inset:0,background:"linear-gradient(135deg,rgba(0,0,0,0.88),rgba(0,0,0,0.55))"}}/>
              <div style={{position:"absolute",top:-20,right:-20,width:80,height:80,background:"radial-gradient(circle,"+t.color+"25,transparent 70%)",pointerEvents:"none"}}/>
              <div style={{position:"relative",padding:"16px 14px"}}>
                <div style={{width:40,height:40,borderRadius:12,background:t.color+"22",border:"1px solid "+t.color+"40",display:"flex",alignItems:"center",justifyContent:"center",marginBottom:10,boxShadow:"0 0 16px "+t.color+"28"}}>
                  {SpotlightIcons[t.id]?.(t.color,21)}
                </div>
                <div style={{fontSize:12,fontWeight:800,color:"#f0ece4",marginBottom:4,lineHeight:1.3}}>{t.title}</div>
                <div style={{fontSize:9,color:"#9a94a8",lineHeight:1.4}}>{t.desc}</div>
              </div>
            </button>
          );
        })}
        ...
        {/* Extras Row - proper 3 columns */}
        <div style={{gridColumn:"1 / -1",gridRow:"3",display:"flex",gap:10}}>
          {extras.map(e=>{
            const extraColors={surprise:"#c4a960",oracle:"#a78bfa",personality:"#e84393"};
            const ec=extraColors[e.id]||"#c4a960";
            return(
              <button key={e.id} onClick={()=>setActiveFeature(e.id)}
                style={{flex:1,background:"rgba(255,255,255,0.03)",border:"1px solid rgba(255,255,255,0.08)",borderRadius:16,padding:"14px 8px",cursor:"pointer",textAlign:"center",position:"relative",overflow:"hidden"}}>
                <div style={{position:"absolute",top:-20,left:"50%",transform:"translateX(-50%)",width:60,height:60,background:"radial-gradient(circle,"+ec+"15,transparent 70%)",pointerEvents:"none"}}/>
                <div style={{width:38,height:38,borderRadius:11,background:ec+"15",border:"1px solid "+ec+"30",display:"flex",alignItems:"center",justifyContent:"center",margin:"0 auto 8px",boxShadow:"0 0 14px "+ec+"20"}}>
                  {SpotlightIcons[iconMap[e.id]]?.(ec,20)}
                </div>
