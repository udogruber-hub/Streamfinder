...
- reason = witziger deutscher Satz max 12 Wörter`;
        ...
        if(liked_sw.length>0){
          try{
            ...
            const seen2=new Set();
            const deduped=combined.filter(it=>{if(!it.id||seen2.has(it.id))return false;seen2.add(it.id);return true;});
            // Verteile auf Plattformen
            kiRecs=userPlats.flatMap(plat=>
              deduped.slice(0,5).map(it=>({
                title:it.title||it.name||"",
                platform:plat.name,
...
function MainApp({profile:initProfile,onReset}){
  ...
  function handleBlock(itemTitle){
    ...
    if(replacement){
      ...
      getSmartRecommendations(profile).then(items=>{
        const existIds=new Set([...heroItems,...feedItems].map(it=>it.id));
        reserveItems.current=items.filter(it=>!existIds.has(it.id)).slice(10);
      }).catch(()=>{});
    }
