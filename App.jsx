...
// ── Stimmungs-Icons SVG ──
const MoodIcons={
  spannung:(c,s=24)=><svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke={c} strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M13 2L3 14h9l-1 8 10-12h-9l1-8z"/></svg>,
  leicht:(c,s=24)=><svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke={c} strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="5"/><path d="M12 1v2M12 21v2M4.22 4.22l1.42 1.42M18.36 18.36l1.42 1.42M1 12h2M21 12h2M4.22 19.78l1.42-1.42M18.36 5.64l1.42-1.42"/></svg>,
  emotional:(c,s=24)=><svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke={c} strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"/></svg>,
  action:(c,s=24)=><svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke={c} strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M12 2c0 6-6 8-6 14a6 6 0 0 0 12 0c0-6-6-8-6-14z"/><path d="M12 12c0 3-2 4-2 7a2 2 0 0 0 4 0c0-3-2-4-2-7z"/></svg>,
  duester:(c,s=24)=><svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke={c} strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M21 12.79A9 9 0 1 1 11.21 3 7 7 0 0 0 21 12.79z"/></svg>,
  komplex:(c,s=24)=><svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke={c} strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M9.09 9a3 3 0 0 1 5.83 1c0 2-3 3-3 3"/><circle cx="12" cy="12" r="10"/><path d="M12 17h.01"/></svg>,
  wahre:(c,s=24)=><svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke={c} strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M2 3h6a4 4 0 0 1 4 4v14a3 3 0 0 0-3-3H2z"/><path d="M22 3h-6a4 4 0 0 0-4 4v14a3 3 0 0 1 3-3h7z"/></svg>,
  fantasy:(c,s=24)=><svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke={c} strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/></svg>,
};

const MOODS_CONFIG=[
  {id:"spannung",label:"Spannung",color:"#ef4444",genres:[53,80,9648],icon:"spannung"},
  {id:"leicht",label:"Leicht & Lustig",color:"#f59e0b",genres:[35,10751,16],icon:"leicht"},
  {id:"emotional",label:"Emotional",color:"#e84393",genres:[18,10749,10402],icon:"emotional"},
  {id:"action",label:"Action",color:"#ff6b35",genres:[28,12,10752],icon:"action"},
  {id:"duester",label:"Düster",color:"#6366f1",genres:[27,9648,53],icon:"duester"},
  {id:"komplex",label:"Komplex",color:"#a78bfa",genres:[878,9648,36],icon:"komplex"},
  {id:"wahre",label:"Wahre Geschichten",color:"#4ade80",genres:[99,36,10770],icon:"wahre"},
  {id:"fantasy",label:"Fantasy & Sci-Fi",color:"#38bdf8",genres:[14,878,12],icon:"fantasy"},
];

// ── Onboarding Component ──
function OnboardingModal({onDone,profile,onProfile}){
  const [step,setStep]=useState(0); // 0=welcome, 1=providers, 2=moods, 3=swipe
  const [selectedProviders,setSelectedProviders]=useState(profile.platforms||[]);
  ...
  async function loadSwipeItems(){
    ...
    try{
      const genres=selectedMoods.flatMap(m=>MOODS_CONFIG.find(mc=>mc.id===m)?.genres||[]);
      const uniqueGenres=[...new Set(genres)];
      const gStr=uniqueGenres.length>0?uniqueGenres.slice(0,3).join("|"):null;
      const pages=[1,2,3,4,5];
      const page=pages[Math.floor(Math.random()*pages.length)];
      const [s,f]=await Promise.all([
        fetch(TMDB_BASE+"/discover/tv?api_key="+TMDB_API_KEY+"&language=de-DE&sort_by=popularity.desc&vote_average.gte=7"+(gStr?"&with_genres="+gStr:"")+"&page="+page).then(r=>r.json()),
        fetch(TMDB_BASE+"/discover/movie?api_key="+TMDB_API_KEY+"&language=de-DE&sort_by=popularity.desc&vote_average.gte=7"+(gStr?"&with_genres="+gStr:"")+"&page="+page).then(r=>r.json()),
      ]);
      const combined=[
        ...(s.results||[]).map(it=>({...it,media_type:"tv"})),
        ...(f.results||[]).map(it=>({...it,media_type:"movie"})),
      ].sort(()=>Math.random()-0.5).slice(0,10);
      setSwipeItems(combined);
    }catch(e){}
    setSwipeLoading(false);
  }

  function handleProviderToggle(id){
    setSelectedProviders(prev=>prev.includes(id)?prev.filter(p=>p!==id):[...prev,id]);
  }

  function handleMoodToggle(id){
  ...
  function handleSwipe(item,liked){
    ...
    if(liked){
      newProfile.ratings={...(newProfile.ratings||{}),[titleKey(item.title||item.name||"")]:5};
      genres.forEach(g=>{newProfile.genres={...(newProfile.genres||{}),[g]:((newProfile.genres||{})[g]||0)+3};});
    }else{
      newProfile.ratings={...(newProfile.ratings||{}),[titleKey(item.title||item.name||"")]:1};
      genres.forEach(g=>{newProfile.genres={...(newProfile.genres||{}),[g]:((newProfile.genres||{})[g]||0)-1};});
    }
    onProfile(newProfile);
    if(swipeIdx>=swipeItems.length-1){setSwipeDone(true);}
    else{setSwipeIdx(i=>i+1);}
  }

  function finishOnboarding(){
    // Save providers + moods to profile
    const moodGenres=selectedMoods.flatMap(m=>MOODS_CONFIG.find(mc=>mc.id===m)?.genres||[]);
    const genreBoosts={};
    moodGenres.forEach(g=>{genreBoosts[g]=(genreBoosts[g]||0)+2;});
    const newProfile={
      ...profile,
      platforms:selectedProviders,
      genres:{...(profile.genres||{}),...genreBoosts},
      onboardingDone:true,
    };
    onProfile(newProfile);
    sSet("profile",newProfile);
    onDone();
  }

  // Welcome
  if(step===0)return(
    <div style={{position:"fixed",inset:0,background:"#09090f",zIndex:1000,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:32}}>
      <div style={{fontSize:64,marginBottom:24}}>🎬</div>
      <h1 style={{fontFamily:"'Instrument Serif',serif",fontSize:36,margin:"0 0 12px",textAlign:"center",background:"linear-gradient(135deg,#c4a960,#f5e090)",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent"}}>StreamFinder</h1>
      <p style={{fontSize:16,color:"#b0a8b8",textAlign:"center",lineHeight:1.6,marginBottom:8}}>Schluss mit endlosem Scrollen.</p>
      <p style={{fontSize:16,color:"#b0a8b8",textAlign:"center",lineHeight:1.6,marginBottom:40}}>In 2 Minuten findest du was du heute Abend schaust.</p>
      <button onClick={()=>setStep(1)} style={{background:"linear-gradient(135deg,#c4a960,#ff6b35)",border:"none",borderRadius:16,padding:"16px 48px",color:"#fff",cursor:"pointer",fontFamily:"'DM Sans'",fontWeight:800,fontSize:17,boxShadow:"0 4px 24px rgba(196,169,96,0.4)"}}>Los geht's →</button>
    </div>
  );

  // Providers
  if(step===1)return(
    <div style={{position:"fixed",inset:0,background:"#09090f",zIndex:1000,overflow:"auto",padding:"40px 24px 100px"}}>
      <div style={{marginBottom:28}}>
        <p style={{fontSize:12,color:"#6a6478",margin:"0 0 6px",letterSpacing:"1px",textTransform:"uppercase"}}>Schritt 1 von 3</p>
        <h2 style={{fontFamily:"'Instrument Serif',serif",fontSize:28,margin:"0 0 8px",color:"#f0ece4"}}>Welche Dienste hast du?</h2>
        <p style={{fontSize:14,color:"#8a849a",margin:0}}>Wähle alle Anbieter die du nutzt.</p>
      </div>
      <div style={{display:"flex",flexDirection:"column",gap:10,marginBottom:32}}>
        {PLATFORMS.map(p=>{
          const sel=selectedProviders.includes(p.id);
          return(
            <button key={p.id} onClick={()=>handleProviderToggle(p.id)}
              style={{display:"flex",alignItems:"center",gap:16,padding:"16px 18px",background:sel?"rgba(196,169,96,0.1)":"rgba(255,255,255,0.03)",border:"1px solid "+(sel?"rgba(196,169,96,0.4)":"rgba(255,255,255,0.08)"),borderRadius:16,cursor:"pointer",textAlign:"left",transition:"all 0.15s"}}>
              <div style={{width:44,height:44,borderRadius:12,background:p.color+"22",border:"1px solid "+p.color+"44",display:"flex",alignItems:"center",justifyContent:"center",fontSize:20,fontWeight:900,color:p.color,flexShrink:0}}>{p.icon}</div>
              <span style={{fontSize:16,fontWeight:700,color:sel?"#f0ece4":"#8a849a",fontFamily:"'DM Sans'"}}>{p.name}</span>
              {sel&&<div style={{marginLeft:"auto",width:22,height:22,borderRadius:"50%",background:"linear-gradient(135deg,#c4a960,#ff6b35)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:12,color:"#fff"}}>✓</div>}
            </button>
          );
        })}
      </div>
      <div style={{position:"fixed",bottom:0,left:0,right:0,padding:"16px 24px",background:"linear-gradient(to top,#09090f,transparent)"}}>
        <button onClick={()=>setStep(2)} disabled={selectedProviders.length===0}
          style={{width:"100%",background:selectedProviders.length>0?"linear-gradient(135deg,#c4a960,#ff6b35)":"rgba(255,255,255,0.08)",border:"none",borderRadius:14,padding:"16px",color:"#fff",cursor:selectedProviders.length>0?"pointer":"default",fontFamily:"'DM Sans'",fontWeight:800,fontSize:16,opacity:selectedProviders.length>0?1:0.5}}>
          Weiter →
        </button>
      </div>
    </div>
  ...
  // Moods
  if(step===2)return(
    <div style={{position:"fixed",inset:0,background:"#09090f",zIndex:1000,overflow:"auto",padding:"40px 24px 120px"}}>
      <div style={{marginBottom:28}}>
        <p style={{fontSize:12,color:"#6a6478",margin:"0 0 6px",letterSpacing:"1px",textTransform:"uppercase"}}>Schritt 2 von 3</p>
        <h2 style={{fontFamily:"'Instrument Serif',serif",fontSize:28,margin:"0 0 8px",color:"#f0ece4"}}>Was magst du?</h2>
        <p style={{fontSize:14,color:"#8a849a",margin:0}}>Wähle alles was dich anspricht — mehreres ist okay.</p>
      </div>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:32}}>
        {MOODS_CONFIG.map(m=>{
          const sel=selectedMoods.includes(m.id);
          return(
            <button key={m.id} onClick={()=>handleMoodToggle(m.id)}
              style={{padding:"20px 14px",background:sel?m.color+"18":"rgba(255,255,255,0.03)",border:"1px solid "+(sel?m.color+"50":"rgba(255,255,255,0.08)"),borderRadius:18,cursor:"pointer",textAlign:"center",transition:"all 0.15s",boxShadow:sel?"0 4px 20px "+m.color+"25":"none"}}>
              <div style={{width:48,height:48,borderRadius:14,background:m.color+"18",border:"1px solid "+(sel?m.color+"50":m.color+"25"),display:"flex",alignItems:"center",justifyContent:"center",margin:"0 auto 10px",boxShadow:sel?"0 0 16px "+m.color+"40":"none"}}>
                {MoodIcons[m.icon]?.(sel?m.color:"#4a4060",26)}
              </div>
              <div style={{fontSize:13,fontWeight:700,color:sel?m.color:"#6a6478",fontFamily:"'DM Sans'"}}>{m.label}</div>
            </button>
          );
        })}
      </div>
      <div style={{position:"fixed",bottom:0,left:0,right:0,padding:"16px 24px",background:"linear-gradient(to top,#09090f,transparent)"}}>
        <button onClick={()=>{setStep(3);loadSwipeItems();}}
          style={{width:"100%",background:"linear-gradient(135deg,#c4a960,#ff6b35)",border:"none",borderRadius:14,padding:"16px",color:"#fff",cursor:"pointer",fontFamily:"'DM Sans'",fontWeight:800,fontSize:16}}>
          {selectedMoods.length>0?"Weiter →":"Überspringen →"}
        </button>
      </div>
    </div>
  );
  ...
  if(step===3){
    ...
    return(
      <div style={{position:"fixed",inset:0,background:"#09090f",zIndex:1000,display:"flex",flexDirection:"column"}}>
        <div style={{padding:"40px 24px 20px"}}>
          <p style={{fontSize:12,color:"#6a6478",margin:"0 0 6px",letterSpacing:"1px",textTransform:"uppercase"}}>Schritt 3 von 3</p>
          <h2 style={{fontFamily:"'Instrument Serif',serif",fontSize:28,margin:"0 0 4px",color:"#f0ece4"}}>Magst du das?</h2>
          <p style={{fontSize:13,color:"#8a849a",margin:0}}>{swipeIdx+1} von {swipeItems.length} · Wir lernen deinen Geschmack</p>
        </div>
        {swipeLoading&&<div style={{flex:1,display:"flex",alignItems:"center",justifyContent:"center"}}><div style={{fontSize:32,animation:"spin 1.5s linear infinite"}}>🎬</div></div>}
        {!swipeLoading&&!swipeDone&&current&&(
          <div style={{flex:1,padding:"0 24px",display:"flex",flexDirection:"column",gap:16}}>
            {/* Card */}
            <div style={{borderRadius:22,overflow:"hidden",position:"relative",flex:1,maxHeight:420}}>
              {current.backdrop_path
                ?<img src={"https://image.tmdb.org/t/p/w780"+current.backdrop_path} alt="" style={{width:"100%",height:"100%",objectFit:"cover"}}/>
                :<div style={{width:"100%",height:"100%",background:"linear-gradient(135deg,#1a1520,#0f1020)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:48}}>🎬</div>
              }
              ...
              <div style={{position:"absolute",bottom:0,left:0,right:0,padding:"20px 20px 16px"}}>
                <div style={{fontSize:11,color:"rgba(255,255,255,0.6)",marginBottom:4}}>{current.media_type==="tv"?"Serie":"Film"} {current.first_air_date||current.release_date?"· "+(current.first_air_date||current.release_date||"").substring(0,4):""}</div>
                <h3 style={{fontFamily:"'Instrument Serif',serif",fontSize:22,color:"#fff",margin:0,textShadow:"0 2px 8px rgba(0,0,0,0.8)"}}>{current.title||current.name}</h3>
              </div>
            </div>
            {/* Buttons */}
            <div style={{display:"flex",gap:12}}>
              <button onClick={()=>handleSwipe(current,false)}
                style={{flex:1,background:"rgba(239,68,68,0.1)",border:"1px solid rgba(239,68,68,0.3)",borderRadius:14,padding:"16px",cursor:"pointer",fontSize:24,display:"flex",alignItems:"center",justifyContent:"center",gap:10}}>
                <span>👎</span><span style={{fontSize:14,color:"#ef4444",fontFamily:"'DM Sans'",fontWeight:700}}>Nein</span>
              </button>
              <button onClick={()=>handleSwipe(current,true)}
                style={{flex:1,background:"rgba(74,222,128,0.1)",border:"1px solid rgba(74,222,128,0.3)",borderRadius:14,padding:"16px",cursor:"pointer",fontSize:24,display:"flex",alignItems:"center",justifyContent:"center",gap:10}}>
                <span>👍</span><span style={{fontSize:14,color:"#4ade80",fontFamily:"'DM Sans'",fontWeight:700}}>Ja!</span>
              </button>
            </div>
            <button onClick={finishOnboarding} style={{background:"transparent",border:"none",color:"#4a4060",cursor:"pointer",fontFamily:"'DM Sans'",fontSize:13,padding:"8px"}}>Überspringen</button>
          </div>
        )}
        {(swipeDone||(!swipeLoading&&swipeItems.length===0))&&(
          <div style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:32}}>
            <div style={{fontSize:48,marginBottom:16}}>🎉</div>
            <h3 style={{fontFamily:"'Instrument Serif',serif",fontSize:26,color:"#f0ece4",margin:"0 0 10px",textAlign:"center"}}>Bereit!</h3>
            <p style={{fontSize:14,color:"#8a849a",textAlign:"center",lineHeight:1.6,marginBottom:32}}>StreamFinder kennt deinen Geschmack jetzt. Deine ersten Empfehlungen warten!</p>
            <button onClick={finishOnboarding} style={{background:"linear-gradient(135deg,#c4a960,#ff6b35)",border:"none",borderRadius:14,padding:"16px 48px",color:"#fff",cursor:"pointer",fontFamily:"'DM Sans'",fontWeight:800,fontSize:16}}>Los geht's! →</button>
          </div>
        )}
      </div>
    );
...
function SpotlightTab({profile,cardProps,onSelect}){
  ...
  useEffect(()=>{
    ...
    const ratingCount=Object.keys(profile.ratings||{}).length;
    const genreCount=Object.keys(profile.genres||{}).filter(k=>(profile.genres||{})[k]>0).length;
    if(ratingCount<1&&genreCount<1)return; // Noch keine Daten
    setInitialLoaded(true);

    // Beste Stimmung aus Profil-Genres ermitteln
    const profileGenres=profile.genres||{};
...
Personalisiere die Empfehlungen — kombiniere Stimmung und Geschmack.`;
      ...
      const recs=JSON.parse(text.replace(/```json|```/g,"").trim());
      const enriched=await Promise.all(recs.map(r=>enrichWithTMDB({title:r.title,type:r.type},profile)));
      setStimmungRecs(enriched.filter(Boolean).map((it,i)=>({...it,_aiReason:recs[i]?.reason})));
    }catch(e){
...
Kombiniere Stimmung und persönlichen Geschmack.`;
  ...
  async function loadMoodRecs(mood){
    if(activeMood===mood.id){setActiveMood(null);setMoodRecs([]);return;}
    setActiveMood(mood.id);setMoodLoading(true);setMoodRecs([]);
    try{
      const allIds=(profile.platforms||[]).flatMap(id=>PLATFORMS.find(p=>p.id===id)?.tmdbIds||[]);
      const gStr=mood.genres.join("|");
      const page=Math.floor(Math.random()*4)+1;
      const [s,f]=await Promise.all([
        fetch(TMDB_BASE+"/discover/tv?api_key="+TMDB_API_KEY+"&language=de-DE&sort_by=vote_average.desc&vote_count.gte=100&vote_average.gte=7.5&with_genres="+gStr+(allIds.length>0?"&with_watch_providers="+allIds.slice(0,3).join("|")+"&watch_region=DE":"")+"&page="+page).then(r=>r.json()),
        fetch(TMDB_BASE+"/discover/movie?api_key="+TMDB_API_KEY+"&language=de-DE&sort_by=vote_average.desc&vote_count.gte=200&vote_average.gte=7.5&with_genres="+gStr+(allIds.length>0?"&with_watch_providers="+allIds.slice(0,3).join("|")+"&watch_region=DE":"")+"&page="+page).then(r=>r.json()),
      ]);
      const watched=new Set((profile.watched||[]).map(w=>w.id));
      const blocked=new Set(profile.blocked_titles||[]);
      const combined=[
        ...(s.results||[]).map(it=>({...it,media_type:"tv"})),
        ...(f.results||[]).map(it=>({...it,media_type:"movie"})),
      ].filter(it=>!watched.has(it.id)&&!blocked.has(titleKey(it.title||it.name||""))).slice(0,5);
      setMoodRecs(combined);
    }catch(e){}
  ...
  return(
    <div style={{display:"flex",flexDirection:"column",height:"calc(100vh - 120px)"}}>
      ...
      {/* Hero Mosaic */}
      <div style={{flex:1,display:"flex",flexDirection:"column",gap:8,padding:"0 12px 8px",minHeight:0,overflowY:"auto"}}>
          ...
          <div style={{flexShrink:0,marginBottom:4}}>
            ...
            <div style={{display:"flex",gap:7,overflowX:"auto",paddingBottom:4}}>
              {MOODS_CONFIG.map(m=>{
                ...
                return(
                  <button key={m.id} onClick={()=>loadMoodRecs(m)}
                    ...
                    <div style={{width:30,height:30,borderRadius:9,background:m.color+"15",display:"flex",alignItems:"center",justifyContent:"center"}}>
                      {MoodIcons[m.icon]?.(active?m.color:"#3a3350",17)}
                    </div>
            ...
            {moodLoading&&<div style={{textAlign:"center",padding:"12px 0"}}><span style={{fontSize:18,animation:"spin 1s linear infinite",display:"inline-block"}}>✨</span></div>}
            {moodRecs.length>0&&!moodLoading&&(
              <div style={{marginTop:10,display:"flex",flexDirection:"column",gap:8}}>
                {moodRecs.map((it,i)=>(
                  <SpotlightCard key={it.id||i} item={it} profile={profile} onSelect={onSelect} onRate={cardProps.onRate} onLike={cardProps.onLike} onWatched={cardProps.onWatched} onBlock={cardProps.onBlock}/>
                ))}
              </div>
            )}
          </div>
        ...
        <button onClick={()=>setActiveFeature("deepdive")}
          style={{flex:2.2,background:"#080810",borderRadius:22,cursor:"pointer",textAlign:"left",position:"relative",overflow:"hidden",border:"none",minHeight:0,display:"flex",flexDirection:"column"}}>
          {/* Vollbild-Bild */}
        ...
        <div style={{flex:1.6,display:"flex",gap:8,minHeight:0}}>
          {tiles.slice(1).map((t,ri)=>(
            <button key={t.id} onClick={()=>setActiveFeature(t.id)}
              style={{flex:1,background:"#080810",borderRadius:20,cursor:"pointer",textAlign:"left",position:"relative",overflow:"hidden",border:"none",minHeight:0,display:"flex",flexDirection:"column"}}>
...
// ── Geschmacks-Score — Langzeitmotivation ──
function GeschmacksScore({profile}){
  const ratingCount=Object.keys(profile.ratings||{}).length;
  const watchedCount=(profile.watched||[]).length;
  const total=ratingCount+watchedCount;

  const milestones=[
    {min:0,    max:24,  emoji:"🌱", label:"Neuling",               sub:"Bewerte deine ersten Titel — wir lernen dich kennen"},
    {min:25,   max:74,  emoji:"🎬", label:"Film-Entdecker",         sub:"Gut! Deine Empfehlungen werden schon persönlicher"},
    {min:75,   max:149, emoji:"🎯", label:"Geschmackskenner",       sub:"StreamFinder versteht was du magst — und was nicht"},
    {min:150,  max:299, emoji:"🔥", label:"Streaming-Profi",        sub:"Deine Top 10 sind jetzt wirklich auf dich zugeschnitten"},
    {min:300,  max:499, emoji:"⭐", label:"Kurator",                sub:"Du hast einen der stärksten Geschmacks-Profile"},
    {min:500,  max:999, emoji:"🏆", label:"Geschmacks-Experte",     sub:"Kaum jemand kennt seinen Geschmack so gut wie du"},
    {min:1000, max:9999,emoji:"👑", label:"StreamFinder Legende",   sub:"Du bist die Referenz — dein Profil ist unschlagbar"},
  ];

  const current=milestones.find(m=>total>=m.min&&total<=m.max)||milestones[milestones.length-1];
  const next=milestones.find(m=>m.min>total);
  const toNext=next?next.min-total:0;
  const progress=next?Math.round(((total-current.min)/(next.min-current.min))*100):100;

  // Dynamischer Nutzwert-Hinweis
  const hint=total<5
    ?"💡 Bewerte 5 Titel → % Übereinstimmung wird freigeschaltet"
    :total<15
    ?"💡 Je mehr du bewertest, desto weniger scrollst du abends"
    :total<30
    ?"💡 Deine Top 10 bei den Anbietern werden jetzt deutlich besser"
    :total<60
    ?"💡 Dein Profil ist stark — StreamFinder kennt deinen Geschmack"
    :"💡 Du bekommst die persönlichsten Empfehlungen die StreamFinder bieten kann";

  const topGenres=Object.entries(profile.genres||{})
    .filter(([,v])=>v>0).sort(([,a],[,b])=>b-a).slice(0,3)
    .map(([g])=>GENRES_TMDB[g]||g);

  return(
    <div style={{background:"linear-gradient(135deg,rgba(196,169,96,0.07),rgba(255,107,53,0.04))",border:"1px solid rgba(196,169,96,0.15)",borderRadius:16,padding:"14px 16px",marginBottom:16}}>
      {/* Header */}
      <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:10}}>
        <div style={{fontSize:26,lineHeight:1}}>{current.emoji}</div>
        <div style={{flex:1}}>
          <div style={{fontSize:14,fontWeight:800,color:"#f0ece4",fontFamily:"'DM Sans'",marginBottom:2}}>{current.label}</div>
          <div style={{fontSize:11,color:"#8a849a",lineHeight:1.4}}>{current.sub}</div>
        </div>
        <div style={{textAlign:"right",flexShrink:0}}>
          <div style={{fontSize:20,fontWeight:900,color:"#c4a960",fontFamily:"'DM Sans'",lineHeight:1}}>{total}</div>
          <div style={{fontSize:9,color:"#6a6478"}}>Aktivitäten</div>
        </div>
      </div>
      {/* Progress bar */}
      {next&&(
        <div style={{marginBottom:8}}>
          <div style={{height:4,borderRadius:2,background:"rgba(255,255,255,0.06)",overflow:"hidden",marginBottom:4}}>
            <div style={{height:"100%",width:progress+"%",background:"linear-gradient(90deg,#c4a960,#ff6b35)",borderRadius:2,transition:"width 0.5s ease"}}/>
          </div>
          <div style={{display:"flex",justifyContent:"space-between"}}>
            <span style={{fontSize:9,color:"#4a4060"}}>{current.emoji} {current.label}</span>
            <span style={{fontSize:9,color:"#4a4060"}}>Noch {toNext} bis {next.emoji} {next.label}</span>
          </div>
        </div>
      )}
      {/* Top Genres */}
      {topGenres.length>0&&(
        <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:8}}>
          {topGenres.map(g=>(
            <span key={g} style={{background:"rgba(196,169,96,0.1)",border:"1px solid rgba(196,169,96,0.15)",borderRadius:20,padding:"3px 10px",fontSize:10,color:"#c4a960",fontWeight:600}}>{g}</span>
          ))}
        </div>
      )}
      {/* Nutzwert-Hinweis */}
      <div style={{fontSize:11,color:"#7a7488",lineHeight:1.5}}>{hint}</div>
    </div>
  );
}

// ── Reaktives Feedback nach Bewertung ──
...
function Onboarding({onComplete}){
  ...
  const STEPS=["intro","platforms","stimmung","swipe"];
  ...
  function handleSkipSwipe(){
    onComplete({platforms,genres:buildGenres(),liked:[],liked_items:[],liked_titles:[],blocked_titles:[],watched:[],ratings:{},languages:["en","de"],onboardingDone:true});
  }

  function handleSwipeDone(ratings,swipeGenres){
    const merged={...buildGenres()};
    Object.entries(swipeGenres).forEach(([g,v])=>{merged[g]=(merged[g]||0)+v;});
    onComplete({platforms,genres:merged,liked:[],liked_items:[],liked_titles:[],blocked_titles:[],watched:[],ratings:Object.fromEntries(Object.entries(ratings).map(([k,v])=>[k,v.stars])),languages:["en","de"],onboardingDone:true});
  }

  return(
    <div style={{minHeight:"100vh",background:"#09090f",fontFamily:"'DM Sans',sans-serif",overflowY:"auto"}}>
      ...
      <div style={{display:"flex",gap:6,justifyContent:"center",padding:"32px 0 0"}}>
        {STEPS.map((_,i)=>(
          <div key={i} style={{width:i===step?28:8,height:8,borderRadius:4,background:i<=step?"linear-gradient(90deg,#c4a960,#ff6b35)":"#1e1e30",transition:"all 0.3s"}}/>
        ))}
      </div>
      <div style={{padding:"24px 24px 40px",maxWidth:480,margin:"0 auto"}}>
        ...
        {cur==="intro"&&(
          <div style={{textAlign:"center",paddingTop:20}}>
            <div style={{width:80,height:80,borderRadius:24,background:"linear-gradient(135deg,#c4a960,#ff6b35)",display:"flex",alignItems:"center",justifyContent:"center",margin:"0 auto 24px",boxShadow:"0 8px 32px rgba(196,169,96,0.4)"}}>
              <svg width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/></svg>
            </div>
            <h1 style={{fontFamily:"'Instrument Serif',serif",fontSize:32,color:"#f0ece4",marginBottom:12,lineHeight:1.1}}>Willkommen bei StreamFinder</h1>
            <p style={{fontSize:14,color:"#b0a8b8",lineHeight:1.7,marginBottom:28}}>Dein persönlicher Streaming-Assistent — findet in Sekunden was du heute Abend schauen willst.</p>
            <div style={{display:"flex",flexDirection:"column",gap:10,marginBottom:32,textAlign:"left"}}>
              {[["⚡","Sofortige Empfehlungen","Basierend auf deinem Geschmack"],["🎯","Immer persönlicher","Je mehr du nutzt desto besser"],["✨","KI-Assistent","Frag einfach was du schauen willst"]].map(([icon,title,sub])=>(
                <div key={title} style={{display:"flex",gap:12,alignItems:"center",background:"rgba(255,255,255,0.03)",borderRadius:12,padding:"12px 14px"}}>
                  <span style={{fontSize:20}}>{icon}</span>
                  <div><div style={{fontSize:13,fontWeight:700,color:"#f0ece4"}}>{title}</div><div style={{fontSize:11,color:"#7a7488"}}>{sub}</div></div>
                </div>
              ))}
            </div>
            <button onClick={()=>setStep(1)} style={{width:"100%",background:"linear-gradient(135deg,#c4a960,#ff6b35)",border:"none",borderRadius:14,padding:"16px",color:"#fff",cursor:"pointer",fontFamily:"'DM Sans'",fontWeight:800,fontSize:15}}>Jetzt starten →</button>
          </div>
        )}

        {cur==="platforms"&&(
          <div>
            <h2 style={{fontFamily:"'Instrument Serif',serif",fontSize:26,color:"#f0ece4",marginBottom:6}}>Welche Anbieter hast du?</h2>
            <p style={{fontSize:13,color:"#b0a8b8",marginBottom:20}}>Wähle alle die du nutzt</p>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:24}}>
              {PLATFORMS.map(p=>{
                const active=platforms.includes(p.id);
                return(
                  <button key={p.id} onClick={()=>setPlatforms(prev=>prev.includes(p.id)?prev.filter(x=>x!==p.id):[...prev,p.id])}
                    style={{background:active?p.color+"22":"#12121f",border:"2px solid "+(active?p.color:"#1e1e30"),borderRadius:14,padding:"14px 12px",cursor:"pointer",display:"flex",alignItems:"center",gap:10,transition:"all 0.2s",textAlign:"left"}}>
                    ...
                    {active&&<span style={{marginLeft:"auto",color:p.color,fontWeight:700}}>✓</span>}
                  </button>
                );
              })}
            </div>
            <button onClick={()=>{if(platforms.length===0)return;setStep(2);}} disabled={platforms.length===0}
              style={{width:"100%",background:"linear-gradient(135deg,#c4a960,#ff6b35)",border:"none",borderRadius:14,padding:"16px",color:"#fff",cursor:platforms.length===0?"not-allowed":"pointer",fontFamily:"'DM Sans'",fontWeight:800,fontSize:15,opacity:platforms.length===0?0.5:1}}>
              Weiter →
            </button>
          </div>
        )}
        ...
        {cur==="stimmung"&&(
          <div>
            <h2 style={{fontFamily:"'Instrument Serif',serif",fontSize:26,color:"#f0ece4",marginBottom:6}}>Was magst du?</h2>
            <p style={{fontSize:13,color:"#b0a8b8",marginBottom:20}}>Wähle alles was dich anspricht</p>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:24}}>
              {STIMMUNGEN.map(s=>{
                ...
                return(
                  <button key={s.id} onClick={()=>setStimmungen(prev=>prev.includes(s.id)?prev.filter(x=>x!==s.id):[...prev,s.id])}
                    style={{background:active?s.color+"18":"#12121f",border:"2px solid "+(active?s.color:"#1e1e30"),borderRadius:16,padding:"16px 12px",cursor:"pointer",textAlign:"center",transition:"all 0.2s",display:"flex",flexDirection:"column",alignItems:"center",gap:8}}>
                    <div style={{width:48,height:48,borderRadius:14,background:active?s.color+"22":"rgba(255,255,255,0.05)",display:"flex",alignItems:"center",justifyContent:"center",boxShadow:active?"0 0 16px "+s.color+"40":"none",transition:"all 0.2s"}}>
                      {StimmungIcons[s.id]?.(active?s.color:"#4a4060",24)}
                    </div>
                    <div>
                      <div style={{fontSize:12,fontWeight:800,color:active?"#f0ece4":"#888",marginBottom:2}}>{s.label}</div>
                      <div style={{fontSize:10,color:active?s.color:"#4a4060"}}>{s.sub}</div>
                    </div>
                  </button>
                );
              })}
            </div>
            <button onClick={()=>{if(stimmungen.length===0)return;setStep(3);}} disabled={stimmungen.length===0}
              style={{width:"100%",background:"linear-gradient(135deg,#c4a960,#ff6b35)",border:"none",borderRadius:14,padding:"16px",color:"#fff",cursor:stimmungen.length===0?"not-allowed":"pointer",fontFamily:"'DM Sans'",fontWeight:800,fontSize:15,opacity:stimmungen.length===0?0.5:1}}>
              Weiter →
            </button>
            <button onClick={()=>setStep(1)} style={{width:"100%",background:"transparent",border:"none",color:"#555",cursor:"pointer",fontFamily:"'DM Sans'",fontSize:13,marginTop:12,padding:8}}>← Zurück</button>
          </div>
        )}

        {cur==="swipe"&&(
          <div>
            <h2 style={{fontFamily:"'Instrument Serif',serif",fontSize:26,color:"#f0ece4",marginBottom:6}}>Zeig uns deinen Geschmack</h2>
            <p style={{fontSize:13,color:"#b0a8b8",marginBottom:16}}>Wische rechts für Interesse, links für nicht mein Ding</p>
            <OnboardingModal
              profile={{platforms,genres:buildGenres(),liked:[],liked_items:[],blocked_titles:[],watched:[],ratings:{},languages:["en","de"],onboardingDone:false}}
              onDone={()=>{}}
              onProfile={(p)=>onComplete({...p,onboardingDone:true})}
            />
            <button onClick={handleSkipSwipe} style={{width:"100%",background:"transparent",border:"1px solid #1e1e30",borderRadius:12,padding:"12px",color:"#555",cursor:"pointer",fontFamily:"'DM Sans'",fontSize:13,marginTop:12}}>Überspringen — direkt zur App</button>
          </div>
        )}
      </div>
    </div>
