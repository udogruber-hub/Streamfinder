      ...
      {open&&mode!=="swipe"&&(
        <div style={{padding:"0 14px 16px"}}>
          ...
          {!loading&&recs&&recs.map((rec,i)=>{
            ...
            return(
              <SwipeToBlock key={rec.title+i} onBlock={()=>{
                const blockedTitle=rec.title;
                setRecs(prev=>{
                  if(!prev)return prev;
                  const next=prev.filter(r=>r.title!==blockedTitle);
                  // Lade Ersatztitel
