      ...
      {open&&(
        <div style={{padding:"0 14px 16px"}}>
          ...
          {!loading&&recs&&recs.map((rec,i)=>{
            ...
            const matchPct=rec.score||Math.min(90,Math.max(35,Math.round(35+(genreScore/maxGenre)*40+(((score||7)-6)/4)*15)));
              ...
              <div key={rec.id||rec.title+i} style={{animation:rec._isNew?"fadeIn 0.5s ease":"none",marginBottom:10}}>
                ...
                <div style={{borderRadius:16,overflow:"hidden",position:"relative",background:"#0d0d18",border:"1px solid #1e1e30"}}>
                  ...
                  <div onClick={e=>e.stopPropagation()} style={{padding:"6px 12px 10px",display:"flex",gap:8,alignItems:"center"}}>
                    ...
                    <button onClick={e=>{e.stopPropagation();if(onWatched&&rec._tmdbItem)onWatched({...rec._tmdbItem,title:rec.title});}}
  ...
  const mainFeatures=[
    {
      ...
      icon:"🔭",
      color:"#c4a960",
      gradient:"linear-gradient(135deg,#1a1525,#0f1020)",
      border:"rgba(196,169,96,0.25)",
      title:"Deep Dive",
      desc:"Starte mit einem Lieblingstitel — wir bauen daraus ein ganzes Entdeckungs-Universum mit ähnlichen Titeln, Regisseuren und versteckten Perlen.",
    },
    {
      id:"vibe",
      icon:"🎛️",
      color:"#e84393",
      gradient:"linear-gradient(135deg,#1a1020,#130818)",
      border:"rgba(232,67,147,0.25)",
      title:"Vibe-Meter",
      desc:"Stelle deine aktuelle Stimmung ein — wie viel Action, wie düster, wie schwer. Wir finden den Titel der genau jetzt passt.",
    },
    {
      id:"bot",
      icon:"💬",
      color:"#4ade80",
      gradient:"linear-gradient(135deg,#0f1a1a,#0d1f1a)",
      border:"rgba(74,222,128,0.25)",
      title:"StreamBot",
      desc:"Stell jede Frage — 'Was läuft auf Netflix wie Breaking Bad?' oder 'Empfiehl mir einen guten Thriller für heute Abend'. Dein persönlicher Experte.",
    },
  ];

  const extras=[
    {id:"surprise",icon:"✨",title:"Überrasch mich",desc:"1 perfekter Titel — kein Scrollen"},
    {id:"oracle",icon:"🔮",title:"Orakel",desc:"Mystische Prophezeiung"},
    {id:"personality",icon:"🎭",title:"Dein Typ",desc:"Dein Streaming-Persönlichkeitstyp"},
  ];

  const featureIcons={
    deepdive:(color)=><svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><circle cx="11" cy="11" r="8"/><path d="m21 21-4.35-4.35"/><path d="M11 8v6M8 11h6"/></svg>,
    vibe:(color)=><svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="1.5" strokeLinecap="round"><path d="M2 12h2l3-8 4 16 3-11 2 3h6"/></svg>,
    bot:(color)=><svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/></svg>,
  };

  return(
    <div style={{minHeight:"85vh",paddingBottom:24,position:"relative",overflow:"hidden"}}>
      {/* Animierte Hintergrund-Orbs */}
      <div style={{position:"absolute",inset:0,pointerEvents:"none",overflow:"hidden",zIndex:0}}>
        <div style={{position:"absolute",top:"5%",left:"10%",width:280,height:280,background:"radial-gradient(circle,rgba(196,169,96,0.12),transparent 70%)",borderRadius:"50%",animation:"orbFloat1 8s ease-in-out infinite"}}/>
        <div style={{position:"absolute",top:"20%",right:"-5%",width:220,height:220,background:"radial-gradient(circle,rgba(232,67,147,0.09),transparent 70%)",borderRadius:"50%",animation:"orbFloat2 11s ease-in-out infinite"}}/>
        <div style={{position:"absolute",bottom:"30%",left:"20%",width:180,height:180,background:"radial-gradient(circle,rgba(74,222,128,0.07),transparent 70%)",borderRadius:"50%",animation:"orbFloat3 9s ease-in-out infinite"}}/>
        <div style={{position:"absolute",bottom:"10%",right:"15%",width:150,height:150,background:"radial-gradient(circle,rgba(255,107,53,0.08),transparent 70%)",borderRadius:"50%",animation:"orbFloat1 13s ease-in-out infinite reverse"}}/>
      </div>

      {/* Header */}
      <div style={{position:"relative",zIndex:1,padding:"24px 20px 20px"}}>
        <div style={{display:"flex",alignItems:"center",gap:14,marginBottom:10}}>
          <div style={{width:54,height:54,borderRadius:16,background:"linear-gradient(135deg,#c4a960,#ff6b35)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:28,boxShadow:"0 6px 28px rgba(196,169,96,0.45),0 0 60px rgba(196,169,96,0.1)",flexShrink:0}}>✨</div>
          <div>
            <h2 style={{fontFamily:"'Instrument Serif',serif",fontSize:34,margin:"0 0 2px",background:"linear-gradient(135deg,#c4a960,#f5e090)",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent",lineHeight:1}}>Spotlight</h2>
            <p style={{fontSize:11,color:"#6a6478",margin:0,letterSpacing:"1px",textTransform:"uppercase"}}>KI-Assistent · Persönlich · Präzise</p>
          </div>
        </div>
        <div style={{background:"rgba(196,169,96,0.05)",borderRadius:14,padding:"11px 16px",border:"1px solid rgba(196,169,96,0.1)"}}>
          <p style={{fontSize:13,color:"#c4b8a8",margin:0,lineHeight:1.6}}>💡 Wähle eine Funktion — je mehr du bewertest, desto besser trifft die KI deinen Geschmack.</p>
        </div>
      </div>

      {/* Haupt-Features */}
      {!activeFeature&&(
        <div style={{position:"relative",zIndex:1,display:"flex",flexDirection:"column",gap:14,marginBottom:28,padding:"0 18px"}}>
          {mainFeatures.map((f,i)=>(
            <button key={f.id} onClick={()=>setActiveFeature(f.id)}
              style={{background:f.gradient,border:"1px solid "+f.border,borderRadius:22,padding:"22px 20px",cursor:"pointer",textAlign:"left",width:"100%",position:"relative",overflow:"hidden",boxShadow:"0 4px 24px rgba(0,0,0,0.3)"}}>
              {/* Corner glow */}
              <div style={{position:"absolute",top:-40,right:-40,width:140,height:140,background:"radial-gradient(circle,"+f.color+"30 0%,transparent 70%)",pointerEvents:"none"}}/>
              <div style={{position:"absolute",bottom:-20,left:-20,width:80,height:80,background:"radial-gradient(circle,"+f.color+"18 0%,transparent 70%)",pointerEvents:"none"}}/>
              <div style={{display:"flex",alignItems:"center",gap:16,position:"relative"}}>
                <div style={{width:58,height:58,borderRadius:16,background:"linear-gradient(135deg,"+f.color+"25,"+f.color+"10)",border:"1px solid "+f.color+"40",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,boxShadow:"0 0 24px "+f.color+"30"}}>
                  {featureIcons[f.id]?.(f.color)||<span style={{fontSize:28}}>{f.icon}</span>}
                </div>
                <div style={{flex:1}}>
                  <div style={{fontSize:17,fontWeight:800,color:"#f0ece4",marginBottom:6,letterSpacing:"-0.2px"}}>{f.title}</div>
                  <div style={{fontSize:12,color:"#8a849a",lineHeight:1.6}}>{f.desc}</div>
                </div>
                <div style={{width:32,height:32,borderRadius:10,background:f.color+"18",border:"1px solid "+f.color+"30",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>
      ...
      {/* Active Feature */}
      {activeFeature&&['deepdive','vibe','bot'].includes(activeFeature)&&(
        <div style={{position:"relative",zIndex:1,marginBottom:20,padding:"0 18px"}}>
          <button onClick={()=>setActiveFeature(null)} style={{background:"transparent",border:"none",color:"#b0a8b8",cursor:"pointer",fontSize:13,fontFamily:"'DM Sans'",fontWeight:600,marginBottom:14,padding:0,display:"flex",alignItems:"center",gap:6}}>
      ...
      {/* Divider */}
      <div style={{position:"relative",zIndex:1,display:"flex",alignItems:"center",gap:10,marginBottom:16,padding:"0 18px"}}>
        <div style={{flex:1,height:1,background:"linear-gradient(90deg,transparent,#2a2340)"}}/>
        <span style={{fontSize:10,color:"#4a4060",fontWeight:700,letterSpacing:"1.5px",textTransform:"uppercase"}}>Extras</span>
        <div style={{flex:1,height:1,background:"linear-gradient(90deg,#2a2340,transparent)"}}/>
      </div>

      {/* Extras */}
      <div style={{position:"relative",zIndex:1,display:"flex",gap:10,marginBottom:16,padding:"0 18px"}}>
        {extras.map(e=>{
          const isActive=activeFeature===e.id;
          const extraColors={surprise:"#c4a960",oracle:"#a78bfa",personality:"#e84393"};
          const ec=extraColors[e.id]||"#c4a960";
          return(
            <button key={e.id} onClick={()=>setActiveFeature(isActive?null:e.id)}
              style={{flex:1,background:isActive?"linear-gradient(135deg,"+ec+"18,"+ec+"08)":"rgba(255,255,255,0.03)",border:"1px solid "+(isActive?ec+"44":"rgba(255,255,255,0.07)"),borderRadius:16,padding:"16px 8px",cursor:"pointer",textAlign:"center",boxShadow:isActive?"0 4px 16px "+ec+"22":"none",transition:"all 0.2s"}}>
              <div style={{fontSize:24,marginBottom:6}}>{e.icon}</div>
              <div style={{fontSize:11,fontWeight:800,color:isActive?ec:"#c4b8c8",marginBottom:3}}>{e.title}</div>
              <div style={{fontSize:9,color:"#4a4060",lineHeight:1.4}}>{e.desc}</div>
            </button>
          );
        })}
      </div>
      ...
      {/* Extra Content */}
      {activeFeature&&['surprise','oracle','personality'].includes(activeFeature)&&(
        <div style={{position:"relative",zIndex:1,padding:"0 18px"}}>
          <button onClick={()=>setActiveFeature(null)} style={{background:"transparent",border:"none",color:"#b0a8b8",cursor:"pointer",fontSize:13,fontFamily:"'DM Sans'",fontWeight:600,marginBottom:12,padding:0,display:"flex",alignItems:"center",gap:6}}>
            ← Zurück
          </button>
