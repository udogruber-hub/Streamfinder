...
function SpotlightTab({profile,cardProps,onSelect}){
  ...
  async function loadStimmungRecs(stimmung){
    ...
    try{
      const ctx=buildCtx(profile);
      const system=`Du bist ein Streaming-Experte. Antworte NUR mit JSON-Array, kein Markdown. Format: [{"title":"...","reason":"...","type":"Serie/Film"}]`;
      const msg=`Empfehle 4 Titel für diese Stimmung: "${stimmung.label}" (${stimmung.sub}). Nutzerprofil: ${ctx.taste||"gemischt"}. Plattformen: ${ctx.platforms}. reason = kurzer persönlicher Satz der den User duzt.`;
      const text=await callAI([{role:"user",content:msg}],system);
      const recs=JSON.parse(text.replace(/\`\`\`json|\`\`\`/g,"").trim());
      const enriched=await Promise.all(recs.map(r=>enrichWithTMDB({title:r.title,type:r.type},profile)));
      setStimmungRecs(enriched.filter(Boolean).map((it,i)=>({...it,_aiReason:recs[i]?.reason})));
    }catch(e){
      // Fallback: TMDB Discover mit Stimmungs-Genres
      try{
        const allIds=(profile.platforms||[]).flatMap(id=>PLATFORMS.find(p=>p.id===id)?.tmdbIds||[]);
      ...
      <div style={{padding:"24px 24px 40px",maxWidth:480,margin:"0 auto"}}>
        ...
        {cur==="swipe"&&(
          <div>
            ...
            <OnboardingSwipe
              profile={{platforms,genres:buildGenres(),liked:[],liked_items:[],blocked_titles:[],watched:[],ratings:{},languages:["en","de"]}}
