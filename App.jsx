...
function PlatformCard({platform,profile,onSelect,onBlock,onLike,onRate,browseType,onWatched,forceOpen}){
  ...
  useEffect(()=>{
    ...
    function onSwipeDone(){setOpen(true);setRecs(null);recsRef.current=null;reserveRef.current=[];setTimeout(()=>{const hasStored=loadStoredRecs();if(!hasStored)loadTMDBRecs();},800);}
...
function UniversalSwipe({profile,cardProps,onSelect,onDone}){
  ...
  async function handleDone(){
    ...
    await Promise.all(platCandidates.map(async({plat,candidates})=>{
      ...
      const swipedLiked=liked_sw.slice(0,8).join(', ');
      const swipedDisliked=disliked_sw.slice(0,5).join(', ');
      const msg=`Wähle 30 Titel für ${plat.name} die zu diesem Nutzer passen. Kandidaten (nur diese): ${titleList}. Hat gerade geliked: ${swipedLiked||'nichts'}. Hat abgelehnt: ${swipedDisliked||'nichts'}. Geschmack: ${ctx.taste||'gemischt'}. Sortiere nach Passung, besten zuerst.`;
      try{
        const text=await callAI([{role:"user",content:msg}],system);
    ...
    // sf_swipe_done nach KI-Call - PlatformCard lädt dann frische Daten
