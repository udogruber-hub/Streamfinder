                  ...
                  Promise.all([
                    ...
                    setRecs(cur=>{
                      ...
                      if(replacement){
                        ...
                        const smartReasons=[
                          `${rScore>=8?"Ein Ausnahmetitel":"Gut bewertet"} — passt zu dem was du magst.`,
                          `Der Algorithmus hat ${rTitle} ausgewählt. Wer sind wir, das zu hinterfragen?`,
                          `${rScore}★ — ${rScore>=8?"Die Zahlen lügen nicht.":"Besser als dein letzter Filmabend."}`,
                          `Frisch entdeckt für dich — könnte dein neues Lieblings${browseType==="serie"?"serie":"film"} werden.`,
                          `Weil dein Profil danach schreit. Oder zumindest flüstert.`,
                        ];
                        const smartReason=smartReasons[Math.floor(Math.random()*smartReasons.length)];
                        return[...cur,{
                          title:rTitle,
        ...
        <div style={{padding:"0 18px 48px"}}>
          ...
          <PlatformSwipe platform={platform} profile={profile} browseType={browseType} onBlock={t=>onBlock(t)} onLike={it=>onLike(it)} onSelect={onSelect} onRate={onRate||((t,s,g)=>{})} onDone={(freshProfile)=>{
            ...
            getTMDBTop5(freshProfile||profile,platform,browseType||"serie")
              .then(async tmdbItems=>{
                if(tmdbItems.length>0){
                  const ctx_s=buildCtx(freshProfile||profile);
                  const topS=ctx_s.top?.split(",")[0]?.trim()||"";
                  const fallback=[
                    topS?`Nachdem du geswiped hast: ${topS}-ähnlich, aber frischer.`:"Frisch ausgewählt nach deinen Swipes.",
                    "Der Algorithmus hat deine Swipes analysiert. Das hier ist das Ergebnis.",
                    "Perfekt für nach dem Stöbern — jetzt weißt du was du willst.",
                    "Wenige kennen ihn. Dein Geschmack schon.",
                    "Eine starke Wahl basierend auf dem was du gerade bewertet hast.",
                  ];
                  let aiReasons=null;
                  try{
