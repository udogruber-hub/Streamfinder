...
Schreibe eine kurze Zusammenfassung des Geschmacks auf Deutsch (1 Satz).`;
  ...
  if(done||idx>=items.length)return(
    <div style={{textAlign:"center",padding:40}}>
      ...
      <p style={{fontSize:12,color:"#b0a8b8",marginBottom:20}}>Bewertungen gespeichert — deine Empfehlungen werden besser.</p>
      <button onClick={handleDone} style={{background:"linear-gradient(135deg,#ff6b35,#e84393)",border:"none",borderRadius:12,padding:"12px 24px",color:"#fff",cursor:"pointer",fontFamily:"'DM Sans'",fontWeight:700,fontSize:14}}>Fertig ✓</button>
    </div>
  );

  function handleRight(){
    const title=current.title||current.name||"";
    localRatings.current[titleKey(title)]={stars:4,genre_ids:current.genre_ids||[]};
    cardProps.onRate(title,4,current.genre_ids||[]);
    setIdx(i=>i+1);
  }
  function handleLeft(){
    const title=current.title||current.name||"";
    localRatings.current[titleKey(title)]={stars:1,genre_ids:current.genre_ids||[]};
    cardProps.onBlock(title);
    setIdx(i=>i+1);
  }
  ...
  return(
    <div>
      ...
      <div style={{position:"relative",height:460}}>
        <SwipeCard key={current.id} item={current} color="#e84393" onSwipeRight={handleRight} onSwipeLeft={handleLeft} onTap={onSelect}/>
      </div>
      {/* Buttons */}
      <div style={{display:"flex",gap:12,justifyContent:"center",alignItems:"center",marginTop:20}}>
        <button onClick={handleLeft} style={{width:58,height:58,borderRadius:29,background:"#12121f",border:"1px solid rgba(239,68,68,0.3)",cursor:"pointer",fontSize:20,display:"flex",alignItems:"center",justifyContent:"center"}}>✕</button>
        ...
        <button onClick={handleRight} style={{width:58,height:58,borderRadius:29,background:"#12121f",border:"1px solid rgba(74,222,128,0.3)",cursor:"pointer",fontSize:20,display:"flex",alignItems:"center",justifyContent:"center"}}>❤️</button>
      </div>
      <p style={{textAlign:"center",fontSize:10,color:"#3a3344",marginTop:10,fontFamily:"'DM Sans'"}}>← Nicht mein Ding &nbsp;·&nbsp; Tippen = Details &nbsp;·&nbsp; ❤️ Merken →</p>
    </div>
  );
}
          ...
          <div style={{background:"linear-gradient(135deg,#1a1525,#0f0e1a)",borderRadius:18,padding:"16px",marginBottom:16,border:"1px solid #2a1f3d"}}>
            ...
            <button onClick={()=>setShowUniversalSwipe(true)}
              ...
              <div style={{flex:1,textAlign:"left"}}>
                ...
                <div style={{fontSize:11,color:"#b0a8b8",marginTop:2}}>Rechts = merken · Links = ausblenden · 1 KI-Call am Ende</div>
              </div>
            ...
            <div style={{position:"fixed",inset:0,background:"#09090f",zIndex:200,overflow:"auto"}}>
              ...
              <div style={{padding:"16px 18px 4px",background:"rgba(232,67,147,0.06)",borderBottom:"1px solid #1e1e30"}}>
                <p style={{fontSize:12,color:"#b0a8b8",margin:0,lineHeight:1.5}}>Wische rechts ❤️ für Titel die dich interessieren, links ✕ für alles andere. Am Ende wählt die KI deine Top 5 pro Anbieter.</p>
              </div>
