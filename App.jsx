...
function MainApp({profile:initProfile,onReset}){
  ...
  async function loadHomeFeed(){
    ...
    try{
      ...
      if(!aiRecs||aiRecs.length===0){setFeedLoading(false);return;}
      const enriched=await Promise.all(aiRecs.map(rec=>enrichWithTMDB(rec,profile)));
      // Filter: remove anything already liked or watched
      ...
      const valid=enriched.filter(it=>{
        if(!it)return false;
        if(it.id&&likedIds.has(it.id))return false;
        if(it.id&&watchedIds.has(it.id))return false;
        const t=titleKey(it.title||it.name||"");
        if(blockedKeys.has(t))return false;
        const myRating=ratings[t]||0;
        if(myRating>0&&myRating<=2)return false;
            ...
            <div style={{padding:"0 18px",marginBottom:14}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
                ...
                <button onClick={doAiRefresh} disabled={aiRefreshing} style={{background:"linear-gradient(135deg,#ff6b3522,#e8439322)",border:"1px solid #ff6b3530",borderRadius:10,padding:"6px 12px",color:"#ff6b35",cursor:"pointer",fontSize:11,fontFamily:"'DM Sans'",fontWeight:700,display:"flex",alignItems:"center",gap:4}}>
                  <span style={{display:"inline-block",animation:aiRefreshing?"spin 1s linear infinite":"none"}}>🔄</span>{aiRefreshing?"KI analysiert…":"KI neu befragen"}
                </button>
              </div>
              <div style={{display:"flex",gap:8,overflowX:"auto",paddingBottom:4}}>
                {MOODS.map(m=>{
