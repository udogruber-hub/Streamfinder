...
async function callAI(messages,systemPrompt){
  ...
  throw new Error(data.error?.message||"KI Fehler");
}
...
async function getAIRecs(profile,platformId,count=3){
  const ctx=buildUserContext(profile);
  const platformName=PLATFORMS.find(p=>p.id===platformId)?.name||platformId;
  const system=`Du bist ein Streaming-Experte. Antworte NUR mit einem JSON-Array, kein Text, keine Backticks. Format: [{"title":"...","year":"...","type":"Film oder Serie","reason":"...","emoji":"..."}]`;
  const prompt=`Empfehle genau ${count} Titel die auf ${platformName} Deutschland verfügbar sind.\nNutzerprofil:\n- Lieblingsgenres: ${ctx.topGenres||"gemischt"}\n- Zuletzt geschaut: ${ctx.watched||"noch nichts"}\n- Gemerkte Titel: ${ctx.liked||"keine"}\nreason = 1 kurzer Satz warum dieser Nutzer das mögen wird.`;
...
async function getAIFeed(profile,prompt){
  const ctx=buildUserContext(profile);
  const system=`Du bist ein persönlicher Streaming-Berater. Antworte auf Deutsch, enthusiastisch und konkret mit 3-5 Empfehlungen. Max 200 Wörter.`;
  const userMsg=`Mein Profil: Genres: ${ctx.topGenres||"gemischt"}. Zuletzt gesehen: ${ctx.watched||"nichts"}. Gemerkt: ${ctx.liked||"nichts"}. Plattformen: ${ctx.platforms}.\n\nFrage: ${prompt}`;
  return callAI([{role:"user",content:userMsg}],system);
}
...
function TitleCard({item,profile,onLike,onThumbsUp,onDislike,onWatched,onSimilar}){
  ...
  function handleExpand(){
    ...
    if(!details){
      setLoadingDet(true);
      getDetails(mediaType,item.id).then(d=>{setDetails(d);setLoadingDet(false);}).catch(()=>setLoadingDet(false));
    }
  }
      ...
      <div onClick={handleExpand} style={{padding:16,display:"flex",alignItems:"flex-start",gap:14,cursor:"pointer"}}>
        ...
        <div style={{flex:1,minWidth:0}}>
          <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:5}}>
            ...
            {isThumbedUp&&<span style={{fontSize:12}}>👍</span>}
      ...
      {expanded&&(
        <div style={{padding:"0 16px 18px",animation:"fadeUp 0.3s ease"}}>
          ...
          <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
            ...
            <button onClick={e=>{e.stopPropagation();onThumbsUp(item);}} style={{padding:"10px 14px",borderRadius:12,background:isThumbedUp?"#4ade8022":"#0d0d18",border:isThumbedUp?"1px solid #4ade8055":"1px solid #1a1a2e",color:isThumbedUp?"#4ade80":"#666",cursor:"pointer",fontSize:14}}>👍</button>
            ...
            <button onClick={e=>{e.stopPropagation();onWatched(item);}} style={{padding:"10px 14px",borderRadius:12,background:isWatched?"#3b82f622":"#0d0d18",border:isWatched?"1px solid #3b82f655":"1px solid #1a1a2e",color:isWatched?"#3b82f6":"#666",cursor:"pointer",fontSize:11,fontFamily:"'DM Sans'",fontWeight:700}}>{isWatched?"✅":"👁"}</button>
            {onSimilar&&<button onClick={e=>{e.stopPropagation();onSimilar(item);}} style={{padding:"10px 14px",borderRadius:12,background:"#B01EFF11",border:"1px solid #e8439333",color:"#B01EFF",cursor:"pointer",fontSize:12,fontFamily:"'DM Sans'",fontWeight:700}}>🔮</button>}
          </div>
        </div>
      )}
    </div>
...
function PlatformCard({platform,profile,onBrowse,onLike,onThumbsUp,onDislike,onWatched,onSimilar}){
  ...
  useEffect(()=>{
    ...
    loaded.current=true;
    getAIRecs(profile,platform.id,3).then(data=>{setRecs(data);setLoading(false);}).catch(()=>{setRecs([]);setLoading(false);});
  },[]);

  return(
    <div style={{background:"linear-gradient(145deg,#1a1525,#151020)",borderRadius:20,overflow:"hidden",border:"1px solid #2a2340",marginBottom:16}}>
      <div style={{padding:"14px 16px",display:"flex",alignItems:"center",gap:12,borderBottom:"1px solid #2a2340"}}>
        <div style={{width:44,height:44,borderRadius:12,background:platform.color+"22",border:"1px solid "+platform.color+"44",display:"flex",alignItems:"center",justifyContent:"center",fontSize:20,fontWeight:900,color:platform.color,flexShrink:0}}>{platform.icon}</div>
        <div style={{flex:1}}>
          <div style={{fontSize:16,fontWeight:800,color:"#f0ece4"}}>{platform.name}</div>
          <div style={{fontSize:11,color:"#555"}}>KI-Empfehlungen für dich</div>
        </div>
        <button onClick={()=>onBrowse(platform)} style={{background:platform.color+"22",border:"1px solid "+platform.color+"44",borderRadius:10,padding:"8px 12px",color:platform.color,cursor:"pointer",fontSize:11,fontFamily:"'DM Sans'",fontWeight:700}}>Alle →</button>
      </div>
      <div style={{padding:"12px 14px"}}>
        {loading&&<div style={{textAlign:"center",padding:16}}><div style={{fontSize:20,animation:"spin 1.5s linear infinite",display:"inline-block"}}>🤖</div><p style={{fontSize:12,color:"#555",marginTop:6}}>KI analysiert...</p></div>}
        {!loading&&recs&&recs.length===0&&<p style={{fontSize:12,color:"#555",textAlign:"center",padding:8}}>Keine Empfehlungen verfügbar.</p>}
        {!loading&&recs&&recs.map((rec,i)=>(
          <div key={i} style={{display:"flex",alignItems:"flex-start",gap:10,padding:"10px 0",borderBottom:i<recs.length-1?"1px solid #1a1a2e":"none"}}>
            <div style={{width:28,height:28,borderRadius:8,background:platform.color+"22",display:"flex",alignItems:"center",justifyContent:"center",fontSize:14,flexShrink:0}}>{rec.emoji||"🎬"}</div>
            <div style={{flex:1,minWidth:0}}>
              <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:3}}>
                ...
                <span style={{fontSize:10,color:"#666",background:"#0d0d18",padding:"2px 6px",borderRadius:5}}>{rec.type}</span>
              </div>
              <p style={{fontSize:12,color:"#8a7e6e",margin:0,lineHeight:1.4,fontStyle:"italic"}}>"{rec.reason}"</p>
            </div>
            <div style={{fontSize:12,color:"#333",fontWeight:800,flexShrink:0}}>#{i+1}</div>
          </div>
        ))}
      </div>
    </div>
  ...
  return(
    <div style={{background:"linear-gradient(145deg,#1a1525,#151020)",borderRadius:20,overflow:"hidden",border:"1px solid #2a2340",marginBottom:16}}>
      <div onClick={()=>setOpen(!open)} style={{padding:14,display:"flex",alignItems:"center",gap:12,cursor:"pointer"}}>
        <div style={{width:44,height:44,borderRadius:12,background:"linear-gradient(135deg,#004E8A22,#FA7D1922)",border:"1px solid #004E8A44",display:"flex",alignItems:"center",justifyContent:"center",fontSize:20,flexShrink:0}}>📡</div>
        <div style={{flex:1}}>
          <div style={{fontSize:16,fontWeight:800,color:"#f0ece4"}}>Öffentlich-Rechtliche</div>
      ...
      {open&&(
        <div style={{padding:"0 12px 12px"}}>
          <div style={{height:1,background:"#2a2340",marginBottom:8}}/>
          {MEDIATHEKEN.map(m=>(
            <a key={m.id} href={m.url} target="_blank" rel="noopener noreferrer" style={{display:"flex",alignItems:"center",gap:10,padding:"10px 8px",borderRadius:12,textDecoration:"none",color:"#c4b8a8"}}>
              <div style={{width:32,height:32,borderRadius:8,background:m.color+"22",border:"1px solid "+m.color+"44",display:"flex",alignItems:"center",justifyContent:"center",fontSize:14,flexShrink:0}}>{m.emoji}</div>
...
function OnboardingScreen({onComplete}){
  ...
  function toggle(id){setPlats(p=>p.includes(id)?p.filter(x=>x!==id):[...p,id]);}
  function finish(){
    if(plats.length===0)return;
    onComplete({platforms:plats,genres:{},liked:[],disliked:[],watched:[],liked_titles:[],thumbsUp:[]});
  }
  return(
    <div style={{minHeight:"100vh",background:"linear-gradient(180deg,#121018,#0d0b14)",color:"#f0ece4",fontFamily:"'DM Sans',sans-serif",display:"flex",flexDirection:"column",padding:24}}>
      ...
      <div style={{flex:1,display:"flex",flexDirection:"column",justifyContent:"center",maxWidth:500,margin:"0 auto",width:"100%"}}>
        ...
        <h1 style={{fontFamily:"'Instrument Serif',serif",fontSize:32,textAlign:"center",margin:"0 0 8px",background:"linear-gradient(135deg,#ff6b35,#e84393)",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent"}}>StreamFinder</h1>
        <p style={{textAlign:"center",color:"#8a7e6e",marginBottom:32,fontSize:15}}>Welche Streaming-Dienste hast du?</p>
        <div style={{display:"flex",flexWrap:"wrap",gap:10,marginBottom:32}}>
          {PLATFORMS.map(p=>{
            const sel=plats.includes(p.id);
            return(
              <button key={p.id} onClick={()=>toggle(p.id)} style={{background:sel?p.color+"22":"#1a1525",border:"2px solid "+(sel?p.color:"#2a2340"),borderRadius:14,padding:"12px 18px",cursor:"pointer",color:sel?p.color:"#6a5e4e",fontFamily:"'DM Sans'",fontWeight:700,fontSize:14,display:"flex",alignItems:"center",gap:8,transition:"all 0.2s",boxShadow:sel?"0 0 20px "+p.color+"33":"none"}}>
                <span style={{fontSize:18}}>{p.icon}</span>{p.name}
              </button>
            );
          })}
          <button onClick={()=>setPlats(["mediatheken"])} style={{background:plats.includes("mediatheken")?"#004E8A22":"#1a1525",border:"2px solid "+(plats.includes("mediatheken")?"#004E8A":"#2a2340"),borderRadius:14,padding:"12px 18px",cursor:"pointer",color:plats.includes("mediatheken")?"#004E8A":"#6a5e4e",fontFamily:"'DM Sans'",fontWeight:700,fontSize:14,display:"flex",alignItems:"center",gap:8}}>
            <span style={{fontSize:18}}>📡</span>Mediatheken
          </button>
        </div>
        <button onClick={finish} disabled={plats.length===0} style={{background:plats.length>0?"linear-gradient(135deg,#ff6b35,#e84393)":"#1a1525",border:"none",borderRadius:16,padding:"16px",color:plats.length>0?"#fff":"#444",cursor:plats.length>0?"pointer":"default",fontFamily:"'DM Sans'",fontWeight:700,fontSize:16,boxShadow:plats.length>0?"0 4px 20px #ff6b3544":"none",transition:"all 0.2s"}}>
          {plats.length>0?"Los geht's →":"Bitte mind. einen Dienst wählen"}
        </button>
      </div>
...
function AITab({profile}){
  ...
  const QUICK=[
    ...
    {label:"Etwas zum Lachen",emoji:"😂"},
    {label:"Gutes Drama das bewegt",emoji:"🎭"},
    {label:"Sci-Fi oder Fantasy",emoji:"🚀"},
    {label:"Ähnlich wie das was ich zuletzt gesehen hab",emoji:"🎯"},
  ];
  async function handleAsk(p){
    const q=p||prompt;if(!q.trim())return;
    setLoading(true);setError("");setResponse("");
    try{const text=await getAIFeed(profile,q);setResponse(text);}
  ...
  return(
    <div style={{padding:"0 18px"}}>
      <div style={{background:"linear-gradient(135deg,#ff6b3512,#e8439312)",border:"1px solid #ff6b3525",borderRadius:20,padding:20,marginBottom:20,textAlign:"center"}}>
        ...
        <h2 style={{fontFamily:"'Instrument Serif',serif",fontSize:22,margin:"0 0 6px",background:"linear-gradient(135deg,#ff6b35,#e84393)",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent"}}>Persönliche KI-Empfehlungen</h2>
        <p style={{fontSize:12,color:"#8a7e6e",margin:0}}>Lernt aus was du schaust · Kennt deinen Geschmack</p>
      </div>
      {(ctx.topGenres||ctx.watched)&&(
        <div style={{background:"#12121f",border:"1px solid #1a1a2e",borderRadius:14,padding:12,marginBottom:16}}>
          <p style={{fontSize:10,color:"#555",fontWeight:700,textTransform:"uppercase",letterSpacing:0.5,marginBottom:8}}>Was ich über dich weiß</p>
          {ctx.topGenres&&<div style={{display:"flex",gap:8,marginBottom:4}}><span style={{fontSize:11,color:"#555",width:70,flexShrink:0}}>Genres</span><span style={{fontSize:11,color:"#c4b8a8"}}>{ctx.topGenres}</span></div>}
          {ctx.watched&&<div style={{display:"flex",gap:8,marginBottom:4}}><span style={{fontSize:11,color:"#555",width:70,flexShrink:0}}>Geschaut</span><span style={{fontSize:11,color:"#c4b8a8"}}>{ctx.watched}</span></div>}
          {ctx.liked&&<div style={{display:"flex",gap:8}}><span style={{fontSize:11,color:"#555",width:70,flexShrink:0}}>Gemerkt</span><span style={{fontSize:11,color:"#c4b8a8"}}>{ctx.liked}</span></div>}
        </div>
      )}
      ...
      <div style={{display:"flex",gap:8,marginBottom:14}}>
        <input value={prompt} onChange={e=>setPrompt(e.target.value)} onKeyDown={e=>e.key==="Enter"&&handleAsk()} placeholder="Eigene Frage…" disabled={loading} style={{flex:1,background:"#12121f",border:"1px solid #2a2a3e",borderRadius:14,padding:"12px 16px",color:"#e8e6e1",fontFamily:"'DM Sans'",fontSize:14,outline:"none"}}/>
      ...
      {loading&&<div style={{background:"#12121f",border:"1px solid #2a2340",borderRadius:16,padding:24,textAlign:"center"}}><div style={{fontSize:28,animation:"spin 1.5s linear infinite",display:"inline-block"}}>🤖</div><p style={{fontSize:13,color:"#8a7e6e",marginTop:8}}>Analysiere deinen Geschmack…</p></div>}
      ...
      {response&&!loading&&(
        <div style={{background:"linear-gradient(145deg,#1a1525,#151020)",border:"1px solid #ff6b3525",borderRadius:16,padding:20,animation:"fadeUp 0.4s ease"}}>
          <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:12}}>
            <span>✦</span>
            <span style={{fontSize:11,color:"#ff6b35",fontWeight:700,letterSpacing:1,textTransform:"uppercase"}}>KI empfiehlt</span>
          </div>
...
function MainApp({profile:initProfile,onReset}){
  ...
  function handleThumbsUp(item){
    updateProfile(p=>{
      const id=item.id;
      const isUp=(p.thumbsUp||[]).includes(id);
    ...
    <div style={{minHeight:"100vh",background:"linear-gradient(180deg,#121018,#0d0b14,#0a0812)",color:"#f0ece4",fontFamily:"'DM Sans',sans-serif",paddingBottom:100}}>
      ...
      {showSettings&&(
        <div style={{margin:"10px 18px",padding:14,background:"#12121f",borderRadius:14,border:"1px solid #1a1a2e"}}>
          <p style={{fontSize:12,color:"#777",margin:"0 0 10px"}}>Profil zurücksetzen und Dienste neu wählen?</p>
      ...
      <div style={{paddingTop:16}}>
        {tab==="platforms"&&(
          <div style={{padding:"0 18px"}}>
            <p style={{fontSize:11,color:"#555",fontWeight:700,textTransform:"uppercase",letterSpacing:0.5,marginBottom:14}}>KI-Empfehlungen für deine Dienste</p>
            {userPlatforms.map(p=><PlatformCard key={p.id} platform={p} profile={profile} onBrowse={pl=>{setFP(pl.id);loadForPlatform(pl.id);setTab("browse");}} {...cardProps}/>)}
        ...
        {tab==="browse"&&(
          <div style={{padding:"0 18px"}}>
            ...
            {searchQuery?(
              ...
              <div>
                {!filterPlatform&&items.length===0&&!loading&&<div style={{textAlign:"center",padding:40,color:"#444"}}><div style={{fontSize:40,marginBottom:10}}>📺</div><p>Wähle oben einen Dienst.</p></div>}
                ...
                {!loading&&items.map(item=><TitleCard key={item.id} item={item} {...cardProps}/>)}
              </div>
            )}
          </div>
        )}
        ...
        {tab==="liked"&&(
          <div style={{padding:"0 18px"}}>
            ...
            {(profile.liked||[]).length===0
              ...
              :items.concat(searchResults).concat(similarItems).filter((item,idx,arr)=>(profile.liked||[]).includes(item.id)&&arr.findIndex(x=>x.id===item.id)===idx).map(item=><TitleCard key={item.id} item={item} {...cardProps}/>)
            }
            {(profile.liked||[]).length>0&&<p style={{fontSize:12,color:"#444",textAlign:"center",marginTop:8}}>{profile.liked.length} gemerkt</p>}
          </div>
        )}
        ...
        {tab==="history"&&(
          <div style={{padding:"0 18px"}}>
            ...
            <p style={{fontSize:12,color:"#555",marginBottom:16}}>Die KI lernt aus deinem Verlauf.</p>
              ...
              :(profile.watched||[]).map(w=>(
                <div key={w.id} style={{background:"#12121f",borderRadius:14,padding:"12px 14px",border:"1px solid #1a1a2e",display:"flex",alignItems:"center",gap:12,marginBottom:8}}>
                  ...
                  <div style={{flex:1,minWidth:0}}>
                    ...
                    <div style={{fontSize:11,color:"#555",marginTop:2}}>{w.media_type==="tv"?"Serie":"Film"} · {w.vote_average?.toFixed(1)}</div>
                  </div>
...
export default function StreamFinder(){
  ...
  if(!profile)return<OnboardingScreen onComplete={handleProfileComplete}/>;
