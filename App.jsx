...
Wähle einen Titel der ihn wirklich überrascht aber trotzdem trifft.`;
    ...
    }catch(e){setSurpriseError("RATE_LIMIT");}
...
Gib ihm einen kreativen Persönlichkeitstyp-Namen (z.B. "Der Noir-Detektiv", "Die Seelen-Forscherin") + lustige Beschreibung + 3 typische Titel.`;
  ...
  if(mode==="surprise")return(
    <div style={{background:"rgba(255,255,255,0.03)",borderRadius:16,padding:20,border:"1px solid rgba(255,255,255,0.07)"}}>
      ...
      {surpriseError==="RATE_LIMIT"&&<RateLimitBanner/>}
      {surpriseResult&&(
        <div>
  ...
  if(mode==="oracle")return(
    <div style={{background:"linear-gradient(135deg,rgba(196,169,96,0.06),rgba(232,67,147,0.04))",borderRadius:16,padding:20,border:"1px solid rgba(196,169,96,0.2)"}}>
      ...
      {oracleError==="RATE_LIMIT"&&<RateLimitBanner/>}
      {oracleResult&&(
...
// ── Rate Limit Banner ──
function RateLimitBanner({onAlternative}){
  return(
    <div style={{background:"linear-gradient(135deg,rgba(196,169,96,0.08),rgba(255,107,53,0.05))",border:"1px solid rgba(196,169,96,0.2)",borderRadius:16,padding:20,textAlign:"center"}}>
      <div style={{fontSize:32,marginBottom:10}}>🌙</div>
      <h3 style={{fontFamily:"'Instrument Serif',serif",fontSize:20,color:"#c4a960",margin:"0 0 8px"}}>Heute Feierabend</h3>
      <p style={{fontSize:13,color:"#b0a8b8",lineHeight:1.6,margin:"0 0 6px"}}>Der KI-Assistent hat sein Tageslimit erreicht.</p>
      <p style={{fontSize:12,color:"#7a7488",lineHeight:1.5,margin:"0 0 16px"}}>StreamFinder nutzt eine kostenlose KI — sie steht täglich ca. 100.000 Anfragen zur Verfügung. Morgen früh ist sie wieder frisch.</p>
      <div style={{background:"rgba(196,169,96,0.06)",borderRadius:12,padding:"12px 14px",marginBottom:14,border:"1px solid rgba(196,169,96,0.1)"}}>
        <p style={{fontSize:12,color:"#c4b8a8",margin:0,lineHeight:1.5}}>💡 <strong>In der Zwischenzeit:</strong> Erkunde die Anbieter-Listen oder starte einen Swipe um neue Empfehlungen zu holen — das funktioniert immer.</p>
      </div>
      {onAlternative&&(
        <button onClick={onAlternative}
          style={{background:"linear-gradient(135deg,#c4a960,#ff6b35)",border:"none",borderRadius:12,padding:"12px 24px",color:"#fff",cursor:"pointer",fontFamily:"'DM Sans'",fontWeight:700,fontSize:14}}>
...
Plattformen: ${ctx.platforms}`;
      ...
      const isLimit=e.message?.includes("429")||e.message?.includes("limit");const msg=isLimit?"RATE_LIMIT":"Fehler — bitte nochmal versuchen.";
...
function SpotlightTab({profile,cardProps,onSelect}){
  ...
  const tiles=[
    {id:"deepdive",title:"Sag mir was mir gefällt…",hint:"Nenn einen Lieblingstitel → wir bauen dein Universum",color:"#c4a960",accentGrad:"linear-gradient(90deg,#c4a960,#ff6b35)"},
