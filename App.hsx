...
async function callAI(messages,systemPrompt){
  try{
    ...
    if(!res.ok){
      ...
      throw new Error("Worker error "+res.status+": "+errText.substring(0,200));
    }
    const data=await res.json();
    ...
    if(data.content&&Array.isArray(data.content)){
      // Anthropic-style response (shouldn't happen but just in case)
...
function PlatformSwipe({platform,profile,browseType,onBlock,onLike,onSelect}){
  ...
  useEffect(()=>{
    ...
    Promise.all([loadWithGenre,loadWithoutGenre,loadPage2]).then(async([r1,r2,r3])=>{
      ...
      const filtered=filterItems(deduped).slice(0,30);
    ...
    <div>
      {showHint&&(
        <div style={{display:"flex",justifyContent:"space-between",marginBottom:12}}>
          <div style={{display:"flex",alignItems:"center",gap:6,background:"#ef444418",borderRadius:10,padding:"6px 12px"}}>
            <span style={{fontSize:16}}>👈</span>
            <span style={{fontSize:11,color:"#ef4444",fontWeight:700}}>Geht gar nicht</span>
          </div>
          <div style={{fontSize:11,color:"#888",alignSelf:"center"}}>Tippen = Details</div>
          <div style={{display:"flex",alignItems:"center",gap:6,background:"#4ade8018",borderRadius:10,padding:"6px 12px"}}>
            <span style={{fontSize:11,color:"#4ade80",fontWeight:700}}>Merken</span>
            <span style={{fontSize:16}}>👉</span>
          </div>
        </div>
      )}
...
function PlatformCard({platform,profile,onSelect,onBlock,onLike,browseType}){
  ...
  function loadTop5Recs(){
    ...
    // Step 1: TMDB sofort (kein KI nötig)
    getTMDBTop5(profile,platform,browseType||"serie").then(tmdbItems=>{
      if(tmdbItems.length>0){
        // Zeige TMDB-Ergebnisse sofort
        setRecs(tmdbItems.map(it=>({
          title:it.title||it.name||"",
          year:(it.release_date||it.first_air_date||"").substring(0,4),
          reason:`★ ${it.vote_average?Math.round(it.vote_average*10)/10:""} — passt zu deinem Geschmack`,
      ...
      // Step 2: KI verfeinert im Hintergrund
      getPlatformTop5(profile,platform.name,browseType||"serie")
        .then(aiItems=>{
          if(aiItems&&aiItems.length>0)setRecs(aiItems);
        })
        .catch(()=>{}); // KI schlägt fehl → TMDB-Ergebnisse bleiben
    }).catch(()=>{
      // Nur KI wenn TMDB fehlschlägt
      getPlatformTop5(profile,platform.name,browseType||"serie")
    ...
    <div style={{background:"#12121f",borderRadius:18,overflow:"hidden",border:"1px solid #1e1e30",marginBottom:10}}>
      ...
      {mode==="top5"&&(
        <div style={{padding:"0 16px 14px"}}>
          ...
          {loading&&<div style={{textAlign:"center",padding:16}}><div style={{fontSize:20,animation:"spin 1.5s linear infinite",display:"inline-block"}}>✨</div><p style={{fontSize:12,color:"#b0a8b8",marginTop:6}}>KI sucht passende Titel…</p></div>}
          {!loading&&recs&&recs.length===0&&(
            <div style={{textAlign:"center",padding:12}}>
              <p style={{fontSize:12,color:"#b0a8b8",marginBottom:10}}>KI konnte keine Titel finden — nochmal versuchen?</p>
          ...
          {!loading&&recs&&recs.map((rec,i)=>{
            ...
            const scoreColor=score>=8?"#4ade80":score>=7?"#fbbf24":"#fb923c";
            return(
              <div key={i} style={{display:"flex",gap:10,padding:"10px 0",borderBottom:i<recs.length-1?"1px solid #1e1e30":"none",alignItems:"flex-start",cursor:"pointer"}}
                onClick={()=>onSelect(tmdb||{title:rec.title,name:rec.title,overview:rec.reason,vote_average:0,genre_ids:[],poster_path:null,media_type:browseType==="serie"?"tv":"movie",release_date:rec.year||""})}>
                {poster
                  ?<img src={poster} alt="" style={{width:36,height:54,borderRadius:8,objectFit:"cover",flexShrink:0}}/>
                  :<div style={{width:36,height:54,borderRadius:8,background:platform.color+"22",display:"flex",alignItems:"center",justifyContent:"center",fontSize:18,flexShrink:0}}>{rec.emoji||"🎬"}</div>
                }
                <div style={{flex:1,minWidth:0}}>
                  <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:2,flexWrap:"wrap"}}>
                    <span style={{fontSize:13,fontWeight:800,color:"#f0ece4"}}>{rec.title}</span>
                    {rec.year&&<span style={{fontSize:10,color:"#b0a8b8",background:"#0d0d18",padding:"2px 5px",borderRadius:4}}>{rec.year}</span>}
...
function MainApp({profile:initProfile,onReset}){
  ...
  async function loadHomeFeed(){
    ...
    const allIds=userPlats.length>0?userPlats.flatMap(p=>p.tmdbIds):[8,9,337]; // fallback Netflix/Prime/Disney
  ...
  function applyMood(mood){
    ...
    setFeedLoading(true);
    Promise.all([
      discoverTitles("serie",allIds,mood.genres.join("|"),1,"popularity.desc"),
      discoverTitles("film",allIds,mood.genres.join("|"),1,"popularity.desc")
    ]).then(([ser,fil])=>{
      const all=[
        ...(ser.results||[]).map(r=>({...r,media_type:"tv"})),
        ...(fil.results||[]).map(r=>({...r,media_type:"movie"}))
      ];
      const seen=new Set();const deduped=[];
      all.forEach(it=>{
        const t=titleKey(it.title||it.name||"");
        const myRating=ratings[t]||0;
        if(!seen.has(it.id)&&!watchedIds.has(it.id)&&!blocked.has(t)&&!(myRating>0&&myRating<=2)){
          ...
          <div>
            <div style={{padding:"0 18px",marginBottom:14}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
                ...
                <button onClick={doAiRefresh} disabled={aiRefreshing} style={{background:"linear-gradient(135deg,#ff6b3522,#e8439322)",border:"1px solid #ff6b3530",borderRadius:10,padding:"6px 14px",color:"#ff6b35",cursor:aiRefreshing?"default":"pointer",fontSize:11,fontFamily:"'DM Sans'",fontWeight:700,display:"flex",alignItems:"center",gap:6,opacity:aiRefreshing?0.7:1}}>
                  ...
                  {aiRefreshing?"KI analysiert…":"Empfehlungen aktualisieren"}
                </button>
              </div>
              <div style={{display:"flex",gap:8,overflowX:"auto",paddingBottom:4}}>
                {MOODS.map(m=>{
            ...
            {feedLoading?(
              <div style={{textAlign:"center",padding:40}}>
                ...
                <p style={{fontSize:14,color:"#f0ece4",fontWeight:700,marginTop:12}}>KI analysiert deinen Geschmack…</p>
                <p style={{fontSize:12,color:"#b0a8b8",marginTop:4}}>Einen Moment — das lohnt sich!</p>
              </div>
            ):(
              <div style={{padding:"0 18px"}}>
                {/* Top 5 KI auf Startseite */}
                {(top5Serie.length>0||top5Film.length>0)&&(
                  <div style={{background:"linear-gradient(135deg,#1a1525,#13101e)",borderRadius:18,padding:"14px 16px",marginBottom:16,border:"1px solid #2a1f3d"}}>
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
                      <p style={{fontSize:11,color:"#ff6b35",fontWeight:700,textTransform:"uppercase",letterSpacing:0.8,margin:0}}>🎯 KI Top 5 — Nur für dich</p>
                      <div style={{display:"flex",gap:6}}>
                        {["serie","film"].map(t=>(
                          <button key={t} onClick={()=>setBrowseType(t)} style={{background:browseType===t?"#ff6b3533":"transparent",border:"1px solid "+(browseType===t?"#ff6b3566":"#2a1f3d"),borderRadius:8,padding:"4px 10px",color:browseType===t?"#ff6b35":"#b0a8b8",cursor:"pointer",fontSize:10,fontFamily:"'DM Sans'",fontWeight:700}}>
                            {t==="serie"?"📺":"🎬"}
                          </button>
                        ))}
                      </div>
                    </div>
                    {(browseType==="serie"?top5Serie:top5Film).map((rec,i)=>{
                      const pl=PLATFORMS.find(p=>p.name===rec.platform)||null;
                      return(
                        <div key={i} style={{display:"flex",gap:10,padding:"8px 0",borderBottom:i<top5Current.length-1?"1px solid #1e1e30":"none",alignItems:"flex-start",cursor:"pointer"}} onClick={()=>setSelectedItem({title:rec.title,name:rec.title,overview:rec.reason,vote_average:0,genre_ids:[],poster_path:null,media_type:browseType==="serie"?"tv":"movie",release_date:rec.year||"",_aiOnly:true})}>
                          <div style={{width:28,height:28,borderRadius:8,background:"#ff6b3522",display:"flex",alignItems:"center",justifyContent:"center",fontSize:14,flexShrink:0}}>{rec.emoji||"🎬"}</div>
                          <div style={{flex:1,minWidth:0}}>
                            <span style={{fontSize:13,fontWeight:800,color:"#f0ece4"}}>{rec.title}</span>
                            {pl&&<span style={{fontSize:10,color:pl.color,fontWeight:700,marginLeft:6}}>{pl.icon}</span>}
                            <p style={{fontSize:11,color:"#a09aaa",margin:"2px 0 0",lineHeight:1.4,fontStyle:"italic"}}>"{rec.reason}"</p>
                          </div>
                          <div style={{fontSize:13,fontWeight:900,color:"#3a3344",flexShrink:0}}>#{i+1}</div>
                        </div>
                      );
                    })}
                    {top5Loading&&<div style={{textAlign:"center",padding:10}}><div style={{fontSize:16,animation:"spin 1.5s linear infinite",display:"inline-block"}}>✨</div><span style={{fontSize:12,color:"#b0a8b8",marginLeft:8}}>KI analysiert…</span></div>}
                  </div>
                )}
              ...
              <div>
                ...
                <div style={{background:"linear-gradient(135deg,#1a1525,#13101e)",borderRadius:18,padding:"14px 16px",marginBottom:16,border:"1px solid #2a1f3d"}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
                    <p style={{fontSize:11,color:"#ff6b35",fontWeight:700,textTransform:"uppercase",letterSpacing:0.8,margin:0}}>🎯 Top 5 — Für dich</p>
                    <button onClick={loadTop5} disabled={top5Loading} style={{background:"#ff6b3522",border:"1px solid #ff6b3544",borderRadius:8,padding:"5px 10px",color:"#ff6b35",cursor:"pointer",fontSize:11,fontFamily:"'DM Sans'",fontWeight:700,display:"flex",alignItems:"center",gap:4}}>
                      <span style={{display:"inline-block",animation:top5Loading?"spin 1s linear infinite":"none"}}>🔄</span>
                      {top5Loading?"Lädt…":"Aktualisieren"}
                    </button>
                  </div>
                  {top5Loading?(
                    <div style={{textAlign:"center",padding:10}}><div style={{fontSize:20,animation:"spin 1.5s linear infinite",display:"inline-block"}}>✨</div><p style={{fontSize:12,color:"#b0a8b8",marginTop:6}}>Lade passende Titel…</p></div>
                  ):top5Current.length===0?(
                    <div style={{textAlign:"center",padding:10}}>
                      <p style={{fontSize:12,color:"#b0a8b8",marginBottom:8}}>Noch keine Daten — tippe Aktualisieren!</p>
                      <button onClick={loadTop5} style={{background:"#ff6b3522",border:"1px solid #ff6b3544",borderRadius:8,padding:"6px 14px",color:"#ff6b35",cursor:"pointer",fontSize:12,fontFamily:"'DM Sans'",fontWeight:700}}>🔄 Laden</button>
                    </div>
                    ...
                    const pl=PLATFORMS.find(p=>p.name===rec.platform)||null;
                    return(
                      <div key={i} style={{display:"flex",gap:10,padding:"10px 0",borderBottom:i<top5Current.length-1?"1px solid #1e1e30":"none",alignItems:"flex-start",cursor:"pointer"}}
                        onClick={()=>setSelectedItem(tmdb||{title:rec.title,name:rec.title,overview:rec.reason,vote_average:0,genre_ids:[],poster_path:null,media_type:browseType==="serie"?"tv":"movie",release_date:rec.year||""})}>
                        {poster
                          ?<img src={poster} alt="" style={{width:36,height:54,borderRadius:8,objectFit:"cover",flexShrink:0}}/>
                          :<div style={{width:36,height:54,borderRadius:8,background:"#ff6b3522",display:"flex",alignItems:"center",justifyContent:"center",fontSize:18,flexShrink:0}}>{rec.emoji||"🎬"}</div>
                        }
                        <div style={{flex:1,minWidth:0}}>
                          <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:2,flexWrap:"wrap"}}>
                            <span style={{fontSize:13,fontWeight:800,color:"#f0ece4"}}>{rec.title}</span>
