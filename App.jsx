...
function PlatformCard({platform,profile,onSelect,onBlock,onLike,onRate,browseType,onWatched}){
  ...
  async function loadTMDBRecs(){
    ...
    try{
      ...
      if(top5.length>0){
        const watchedGoodG=new Set((profile.watched||[])
          .filter(w=>(profile.ratings||{})[titleKey(w.title||"")]>=3)
          .flatMap(w=>w.genre_ids||[]));
        const formatted=top5.slice(0,10).map(it=>{
          const titleGenres=it.genre_ids||[];
          const matchingG=titleGenres.filter(g=>watchedGoodG.has(g)).length;
          const genreMatch=titleGenres.length>0?matchingG/Math.min(titleGenres.length,3):0;
          const ratingBonus=((it.vote_average||7)-6.5)/5;
          const calcScore=Math.min(95,Math.max(20,Math.round(20+genreMatch*58+ratingBonus*18)));
          return{
            id:it.id,
  ...
  async function replaceRec(removedTitle){
    ...
    setRecs(prev=>{
      ...
      if(repl){
        ...
        const watchedGoodG=new Set((profile.watched||[]).filter(w=>(profile.ratings||{})[titleKey(w.title||"")]>=3).flatMap(w=>w.genre_ids||[]));
    ...
    <div style={{background:"#12121f",borderRadius:18,overflow:"hidden",border:"1px solid #1e1e30",marginBottom:10}}>
      <div style={{padding:"14px 16px",display:"flex",alignItems:"center",gap:12,cursor:"pointer"}} onClick={handleOpen}>
        ...
        <div style={{flex:1}}>
          ...
          <div style={{fontSize:9,color:"#4a4458",marginTop:2,cursor:"pointer"}} title="Der Balken zeigt wie gut dieser Titel zu deinem Geschmack passt">▌% = Übereinstimmung mit deinem Geschmack</div>
        </div>
...
Plattformen: ${ctx.platforms}`;
      ...
      const q2=text.match(/\*\*([^*]{2,50})\*\*/g)||[];
      const q3=text.match(/^\d+[\.\)]\s+(.{2,50})$/gm)||[];
      const allMatches=[...q1,...q2,...q3];
      const titles=[...new Set(allMatches.map(t=>t.replace(/["„“”„*]/g,"").replace(/^\d+[\.\)]\s+/,"").trim()).filter(t=>t.length>2&&t.length<60))].slice(0,6);
