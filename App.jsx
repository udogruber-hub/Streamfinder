...
function PlatformCard({platform,profile,onSelect,onBlock,onLike,onRate,browseType}){
  ...
  // Ersetze Titel — immer auf 5 auffüllen
  async function replaceRec(removedTitle){
    const key=titleKey(removedTitle);
    // Erst aus Liste entfernen
    let currentRecs=[];
    setRecs(prev=>{
      currentRecs=prev?prev.filter(r=>titleKey(r.title)!==key):[];
      storeRecs(currentRecs);
      return currentRecs;
    });
    // Ersatz finden aus Reserve
    const reserve=reserveRef.current;
    const blocked=new Set(profile.blocked_titles||[]);
    const watched=new Set((profile.watched||[]).map(w=>w.id));
    const currentIds=new Set(currentRecs.map(r=>r.id).filter(Boolean));
    const replacement=reserve.find(r=>{
      const t=titleKey(r.title||r.name||"");
      return t!==key&&!blocked.has(t)&&!watched.has(r.id)&&!currentIds.has(r.id);
    });
    if(replacement){
      ...
      // Reserve leer → frische TMDB-Titel laden
      try{
        const fresh=await getTMDBTop5(profile,platform,browseType||"serie");
        const currentTitlesSet=new Set(currentRecs.map(r=>titleKey(r.title)));
        const newItems=fresh.filter(it=>{
          const t=titleKey(it.title||it.name||"");
          return t!==key&&!currentTitlesSet.has(t)&&!blocked.has(t)&&!watched.has(it.id);
        });
        if(newItems.length>0){
          ...
          const newRec={
            ...
            reason:first.overview?first.overview.substring(0,140)+(first.overview.length>140?"…":""):"",
            emoji:first.media_type==="tv"?"📺":"🎬",
            _tmdbItem:first,
            _isNew:true,
          };
          setRecs(prev=>{
