...
async function enrichWithTMDB(rec,profile){
  try{
    const isTV=rec.type==="Serie"||rec.type==="tv"||rec.type==="serie"||rec.media_type==="tv";
    const isMovie=rec.type==="Film"||rec.type==="movie"||rec.media_type==="movie";
    // Wenn Typ bekannt → direkt suchen, sonst beide versuchen
    const searchTypes=isTV?["tv"]:isMovie?["movie"]:["tv","movie"];
    for(const mt of searchTypes){
      const res=await fetch(
        TMDB_BASE+"/search/"+mt+"?api_key="+TMDB_API_KEY+
        "&language=en-US&query="+encodeURIComponent(rec.title)+
        (rec.year?"&first_air_date_year="+rec.year:"")
      ).then(r=>r.json());
      const results=res.results||[];
      const exact=results.find(r=>(r.title||r.name||"").toLowerCase().trim()===rec.title.toLowerCase().trim());
      const found=exact||results[0];
      if(found)return{...found,media_type:mt,_aiReason:rec.reason,_platform:rec.platform};
    }
    return null;
  }catch{return null;}
}
          ...
          if(platItems.length>0){
            await Promise.all(["serie","film"].map(async bt=>{
              ...
              try{
                ...
                if(valid.length>0){
                  const formatted=valid.slice(0,5).map(it=>({
                    id:it.id,
                    title:it.title||it.name||"",
                    year:(it.release_date||it.first_air_date||"").substring(0,4),
                  ...
                  localStorage.setItem("sf_plat_"+plat.id+"_"+bt,JSON.stringify(formatted));
                }
              }catch(e){}
            }));
          }
