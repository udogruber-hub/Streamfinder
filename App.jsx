...
function HelpModal({onClose,activeTab}){
  ...
  const HELP={
    spotlight:{
      ...
      intro:"Hier wohnt die KI. Wähle ein Genre für sofortige Empfehlungen — oder nutze die drei KI-Funktionen für noch persönlichere Picks.",
      items:[
        {icon:"🎬",title:"Genre wählen",text:"Tippe eine der 12 Kategorie-Kacheln — Action, Thriller, Sci-Fi, Horror... StreamFinder lädt sofort passende Empfehlungen. Nochmal tippen = Auswahl aufheben."},
        {icon:"📦",title:"Alle Genres",text:"Tippe 'Alle anzeigen' um alle 12 Genres zu sehen. Jede Kachel hat ein Filmbild als Hintergrund."},
        {icon:"👈",title:"Empfehlungen wegwischen",text:"Nach links wischen blendet einen Titel aus und lädt automatisch einen neuen nach — immer aus demselben Genre."},
        {icon:"🔍",title:"Sag mir was mir gefällt",text:"Nenn einen Titel den du liebst — die KI findet sofort ähnliche Empfehlungen. Probiere: Breaking Bad, Inception, Peaky Blinders."},
        {icon:"🎚️",title:"Vibe-Meter",text:"3 Regler: Action & Spannung · Leichtigkeit · Dunkel & Ernst. Die KI findet den exakt passenden Titel für deine Stimmung."},
        {icon:"🍿",title:"Popcorn-Guru",text:"Stell jede Frage — er hat alle Antworten. 'Was läuft auf Netflix wie Breaking Bad?' oder 'Guter Thriller für heute Abend?'"},
        {icon:"✨",title:"Überrasch mich",text:"1 Klick. 1 perfekter Titel. Die KI wählt basierend auf deinem Profil — kein Scrollen nötig."},
        ...
        {icon:"🎭",title:"Welcher Filmtyp bin ich?",text:"Die KI analysiert dein Profil: Der Spannungs-Junkie, Die Seelen-Forscherin, Der Lach-Philosoph..."},
        {icon:"🎯",title:"Geschmack verfeinern",text:"Tippe den Button unten im Spotlight — swipe durch Titel mit Ja / Nope / Kenne ich nicht. Danach werden alle Empfehlungen neu berechnet."},
      ]
    },
    browse:{
      title:"🔍 Erkunden",
      intro:"Alle deine Streaming-Anbieter auf einen Blick — Top 10 Titel sortiert nach deinem persönlichen Geschmack.",
      items:[
        {icon:"📺",title:"Anbieter antippen",text:"Tippe Netflix, Prime, HBO etc. an um deine persönlichen Top 10 zu laden. Sortiert nach % Übereinstimmung mit deinem Geschmack."},
        {icon:"🔄",title:"Persönliche Empfehlungen",text:"Tippe den Button oben — swipe durch Titel. Danach berechnet die KI deine Top 10 pro Anbieter neu."},
        {icon:"✓",title:"Gesehen",text:"Tippe 'Gesehen' direkt in der Liste — der Titel verschwindet und wird durch einen neuen ersetzt."},
        {icon:"🔴🟡🟢",title:"Bewerten",text:"3 LED-Punkte rechts: Nope · Ok · Top. Direkt tippbar — kein Extra-Schritt nötig."},
        {icon:"👈",title:"Ausblenden",text:"Titel nach links wischen blendet ihn dauerhaft aus und lädt sofort einen neuen nach."},
        {icon:"🔍",title:"Schnellsuche",text:"Oben in Erkunden nach jedem Titel suchen — mit Bewertung, Beschreibung und direktem Zugriff auf alle Funktionen."},
      ]
    },
    liked:{
      title:"❤️ Watchlist",
      intro:"Alle Titel die du noch sehen möchtest — auf einen Blick.",
      items:[
        {icon:"🤍",title:"Titel merken",text:"Tippe das Herz-Symbol bei jedem Titel um ihn auf die Watchlist zu setzen. Funktioniert aus allen Tabs."},
        {icon:"❤️",title:"Bereits gemerkt",text:"Rotes Herz = gemerkt. Nochmal tippen = entfernen. Liste bleibt gespeichert auch nach App-Neustart."},
        {icon:"✓",title:"Gesehen markieren",text:"Tippe 'Gesehen' um einen Watchlist-Titel abzuhaken — er wandert in den Verlauf."},
        {icon:"💡",title:"Tipp",text:"Watchlist-Titel beeinflussen deine Empfehlungen. Je mehr du merkst und bewertest, desto persönlicher wird StreamFinder."},
      ]
    },
    profile:{
      title:"👤 Mein Profil",
      intro:"Dein persönliches Streaming-Profil — Statistiken, Geschmack und Einstellungen.",
      items:[
        {icon:"📊",title:"Statistiken",text:"Wie viele Titel du bewertet, gesehen und gemerkt hast — dein persönlicher Streaming-Überblick."},
        {icon:"🎯",title:"Dein Geschmack",text:"Die Genres die du am liebsten schaust — automatisch berechnet aus deinen Bewertungen und Swipes."},
        {icon:"🔄",title:"Geschmack verfeinern",text:"Starte einen neuen Swipe um frische Daten zu sammeln und deine Empfehlungen zu verbessern."},
        {icon:"📺",title:"Meine Anbieter",text:"Deine ausgewählten Streaming-Dienste sind im Profil sichtbar."},
        {icon:"🗑️",title:"Profil zurücksetzen",text:"Löscht alle Daten und startet das Onboarding neu. Ganz unten im Profil."},
      ]
    },
  };
...
function SwipeCard({item,color,onSwipeRight,onSwipeLeft,onTap}){
  ...
  return(
    <div
      ...
      {(poster||backdrop)?<img src={poster||backdrop} alt="" onLoad={e=>e.target.style.opacity=1} style={{width:"100%",height:"100%",objectFit:"cover",pointerEvents:"none",opacity:0,transition:"opacity 0.3s ease"}}/>
...
function PlatformCard({platform,profile,onSelect,onBlock,onLike,onRate,browseType,onWatched}){
  ...
  async function replaceRec(removedTitle){
    ...
    setRecs(prev=>{
      ...
      if(repl){
        ...
        const profileGenres=profile.genres||{};
        const maxB=Math.max(...Object.values(profileGenres).filter(v=>v>0),1);
        const tg=repl.genre_ids||[];
        const boost=tg.reduce((s,g)=>s+(profileGenres[g]||0),0);
        const gm=Math.min(1,boost/(maxB*2));
        const rb=((repl.vote_average||7)-6.5)/5;
        const calcScore=Math.min(92,Math.max(25,Math.round(25+gm*52+rb*18)));
        const newItem={id:repl.id,title:repl.title||repl.name||"",year:(repl.release_date||repl.first_air_date||"").substring(0,4),reason:repl.overview||"",emoji:repl.media_type==="tv"?"📺":"🎬",score:calcScore,_tmdbItem:repl,_isNew:true};
        const next=[...filtered,newItem].sort((a,b)=>(b.score||0)-(a.score||0)).slice(0,10);
        storeRecs(next);return next;
      }
      const next=filtered.slice(0,10);storeRecs(next);
      Promise.all([discoverTitles("serie",platform.tmdbIds,null,Math.floor(Math.random()*5)+1,"vote_average.desc",null),discoverTitles("film",platform.tmdbIds,null,Math.floor(Math.random()*5)+1,"vote_average.desc",null)])
        .then(([s,f])=>{
          const candidates=[...(s.results||[]).map(it=>({...it,media_type:"tv"})),...(f.results||[]).map(it=>({...it,media_type:"movie"}))]
            .filter(it=>{const t=titleKey(it.title||it.name||"");return !existIds.has(it.id)&&!existTitles.has(t)&&!blocked.has(t)&&!watched.has(it.id)&&(it.vote_average||0)>=6.5;});
          if(candidates.length>0)reserveRef.current=[...reserveRef.current,...candidates.slice(0,10)];
        }).catch(()=>{});
      return next;
    });
  }
  async function handleOpen(){
    ...
    if(next&&!recs){const hasStored=loadStoredRecs();if(!hasStored)await loadTMDBRecs();}
  }
  useEffect(()=>{
  ...
  useEffect(()=>{
    ...
    function onSwipeDone(){setOpen(true);setTimeout(()=>{const hasStored=loadStoredRecs();if(!hasStored)loadTMDBRecs();},800);}
    ...
    return()=>{window.removeEventListener("sf_watched",onWatched);window.removeEventListener("sf_blocked",onBlocked);window.removeEventListener("sf_swipe_done",onSwipeDone);window.removeEventListener("sf_rated",onRated);};
  },[]);
  return(
...
function UniversalSwipe({profile,cardProps,onSelect,onDone}){
  ...
  async function handleDone(){
    ...
    try{
      const platCandidates=await Promise.all(userPlats.map(async(plat,pi)=>{
        const page1=(pi%5)+1;const page2=((pi*2+3)%6)+1;
        const [s1,s2,f1,f2]=await Promise.all([
      ...
      await Promise.all(platCandidates.map(async({plat,candidates})=>{
        ...
        const system=`Du bist ein Streaming-Experte. Antworte NUR mit JSON-Array, kein Markdown. Format: [{"title":"...","reason":"...","score":85}].`;
        const msg=`Wähle die 10 besten Titel für diesen Nutzer:\nKANDIDATEN (alle auf ${plat.name} DE): ${titles}\nPROFIL: ${ctx.taste||"gemischt"}\n🟢 Top: ${ctx.rated5||"keine"}\n🔴 Nope: ${ctx.rated2||"keine"}\n❤️ Gemocht: ${liked_sw.join(", ")||"nichts"}\nGESEHEN: ${ctx.watched||"nichts"}\nNur aus Kandidaten wählen! reason = witziger Satz Deutsch. score = 40-95.`;
        try{
          const text=await callAI([{role:"user",content:msg}],system);
          ...
          if(result.length<10){
            ...
            const fill=unique.filter(c=>!existIds.has(c.id)).slice(0,10-result.length).map(c=>({id:c.id,title:c.title||c.name||"",year:(c.release_date||c.first_air_date||"").substring(0,4),reason:c.overview?c.overview.substring(0,160):"",emoji:c.media_type==="tv"?"📺":"🎬",_tmdbItem:c}));
            result.push(...fill);
          }
          result.forEach(r=>globalUsedIds.add(r.id));
          if(result.length>0)localStorage.setItem("sf_plat_"+plat.id,JSON.stringify(result));
        }catch(e){
          const fallback=unique.slice(0,10).map(c=>({id:c.id,title:c.title||c.name||"",year:(c.release_date||c.first_air_date||"").substring(0,4),reason:c.overview?c.overview.substring(0,160):"",emoji:c.media_type==="tv"?"📺":"🎬",_tmdbItem:c}));
      ...
      await Promise.all(userPlats.map(async(plat,pi)=>{
        try{
          const [s,f]=await Promise.all([discoverTitles("serie",plat.tmdbIds,genreStr,(pi%4)+1,"vote_average.desc",langFilter),discoverTitles("film",plat.tmdbIds,genreStr,(pi%4)+1,"vote_average.desc",langFilter)]);
          const all=[...(s.results||[]).map(r=>({...r,media_type:"tv"})),...(f.results||[]).map(r=>({...r,media_type:"movie"}))];
          const items2=all.filter(it=>!watched.has(it.id)&&!globalUsedIds.has(it.id)&&(it.vote_average||0)>=7).slice(0,10).map(it=>({id:it.id,title:it.title||it.name||"",year:(it.release_date||it.first_air_date||"").substring(0,4),reason:it.overview?it.overview.substring(0,160):"",emoji:it.media_type==="tv"?"📺":"🎬",_tmdbItem:it}));
          items2.forEach(r=>globalUsedIds.add(r.id));
          if(items2.length>0)localStorage.setItem("sf_plat_"+plat.id,JSON.stringify(items2));
        }catch(e2){}
      }));
    }
    setTimeout(()=>{
  ...
  if(done||idx>=items.length)return(
    <div style={{textAlign:"center",padding:40}}>
      {processing?(<><div style={{fontSize:32,animation:"spin 1.5s linear infinite",display:"inline-block",marginBottom:12}}>✨</div><p style={{fontSize:14,color:"#b0a8b8"}}>Empfehlungen werden angepasst…</p></>):(
        <>
...
const STIMMUNGEN=[
  {id:"action",    label:"Action",        color:"#ff6b35", genres:[28],    tmdbImg:"https://image.tmdb.org/t/p/w500/9n2tJBplPbgR2ca05hS5CKXwP2c.jpg"},
  {id:"comedy",    label:"Komödie",       color:"#f59e0b", genres:[35],    tmdbImg:"https://image.tmdb.org/t/p/w500/bQXAqRx2Fgc46uCVWgoPz5L5Dtr.jpg"},
  {id:"thriller",  label:"Thriller",      color:"#ef4444", genres:[53],    tmdbImg:"https://image.tmdb.org/t/p/w500/44immBwzhDVyjn87b3x3l9mlhAD.jpg"},
  {id:"drama",     label:"Drama",         color:"#e84393", genres:[18],    tmdbImg:"https://image.tmdb.org/t/p/w500/3bhkrj58Vtu7enYsLeMMovMAKAg.jpg"},
  {id:"horror",    label:"Horror",        color:"#6366f1", genres:[27],    tmdbImg:"https://image.tmdb.org/t/p/w500/wTnV3PCVW5O92JMrFvvrRcV39RU.jpg"},
  {id:"scifi",     label:"Sci-Fi",        color:"#a78bfa", genres:[878],   tmdbImg:"https://image.tmdb.org/t/p/w500/d5NXSklpcvkD173vtvzrmOd4UPq.jpg"},
  {id:"fantasy",   label:"Fantasy",       color:"#4ade80", genres:[14],    tmdbImg:"https://image.tmdb.org/t/p/w500/56v2KjBlU4XaOv9rVYEQypROD7P.jpg"},
  {id:"mystery",   label:"Mystery",       color:"#22d3ee", genres:[9648],  tmdbImg:"https://image.tmdb.org/t/p/w500/gMJngTNfaqCSCqGD4y8lVMZXyfI.jpg"},
  {id:"doku",      label:"Doku",          color:"#34d399", genres:[99],    tmdbImg:"https://image.tmdb.org/t/p/w500/z1p34vh7dEOnLDmyCrlUVLuoDzd.jpg"},
  {id:"romance",   label:"Romantik",      color:"#f472b6", genres:[10749], tmdbImg:"https://image.tmdb.org/t/p/w500/hSaH9tt67bozo9K50sbH0s4YjEc.jpg"},
  {id:"krimi",     label:"Krimi",         color:"#fb923c", genres:[80],    tmdbImg:"https://image.tmdb.org/t/p/w500/ggFHVNu6YYI5L9pCfOacjizRGt.jpg"},
  {id:"abenteuer", label:"Abenteuer",     color:"#60a5fa", genres:[12],    tmdbImg:"https://image.tmdb.org/t/p/w500/gEU2QniE6E77NI6lCU6MxlNBvIx.jpg"},
];
...
function SpotlightTab({profile,cardProps,onSelect}){
  ...
  useEffect(()=>{
    ...
    window.addEventListener("sf_watched",onWatched);window.addEventListener("sf_rated",onRated);
    return()=>{window.removeEventListener("sf_watched",onWatched);window.removeEventListener("sf_rated",onRated);};
  },[]);
  ...
  async function loadFirstRecs(stimmung){
    ...
    try{
      ...
      const msg=`Empfehle 15 verschiedene Titel:\nStimmung: "${stimmung.label}"\nMag gerne: ${likedTitles||"noch unbekannt"}\nMag nicht: ${dislikedTitles||"noch unbekannt"}\nLieblingsgenres: ${ctx.topGenres||"gemischt"}\nPlattformen: ${ctx.platforms}\nAlle Titel müssen unterschiedlich sein — kein Titel doppelt.`;
      const text=await callAI([{role:"user",content:msg}],system);
      const recs=JSON.parse(text.replace(/```json|```/g,"").trim());
      const enriched=await Promise.all(recs.map(r=>enrichWithTMDB({title:r.title,type:r.type},profile)));
      const valid=enriched.filter(Boolean).filter(it=>!shownTitles.current.has(it.id)).map((it,i)=>({...it,_aiReason:recs[i]?.reason}));
      valid.forEach(it=>shownTitles.current.add(it.id));
      const toShow=valid.slice(0,3);
      recReserve.current=valid.slice(3);
      stimmungCache.current[stimmung.id]={recs:toShow,reserve:valid.slice(3)};
      setStimmungRecs(toShow);
    }catch(e){
      try{
        const genre=stimmung.genres[0];const page=Math.ceil(Math.random()*5);
        const deP=(profile.platforms||[]).flatMap(id=>PLATFORMS.find(p=>p.id===id)?.tmdbIds||[]).join("|")||"8|9|337|350";
        const [fMovies,fTv]=await Promise.all([
          fetch(TMDB_BASE+"/discover/movie?api_key="+TMDB_API_KEY+"&language=de-DE&sort_by=vote_average.desc&vote_count.gte=100&with_genres="+genre+"&watch_region=DE&with_watch_providers="+deP+"&with_watch_monetization_types=flatrate&page="+page).then(r=>r.json()),
          fetch(TMDB_BASE+"/discover/tv?api_key="+TMDB_API_KEY+"&language=de-DE&sort_by=vote_average.desc&vote_count.gte=50&with_genres="+genre+"&watch_region=DE&with_watch_providers="+deP+"&with_watch_monetization_types=flatrate&page="+page).then(r=>r.json()),
        ]);
        const fallbackItems=[...(fMovies.results||[]).map(it=>({...it,media_type:"movie"})),...(fTv.results||[]).map(it=>({...it,media_type:"tv"}))]
          .filter(it=>it.poster_path&&!shownTitles.current.has(it.id)&&(it.genre_ids||[]).includes(genre)).sort((a,b)=>(b.vote_average||0)-(a.vote_average||0));
        fallbackItems.forEach(it=>shownTitles.current.add(it.id));
        recReserve.current=fallbackItems.slice(3);
        stimmungCache.current[stimmung.id]={recs:fallbackItems.slice(0,3),reserve:fallbackItems.slice(3)};
        setStimmungRecs(fallbackItems.slice(0,3));
      }catch{}
    }
  ...
  async function loadStimmungRecs(stimmung){
    if(stimmungCache.current[stimmung.id]){
      ...
      recReserve.current=[...cached.reserve];setStimmungRecs(cached.recs);return;
    }
    setStimmungLoading(true);setStimmungRecs([]);
    try{
      const ctx=buildCtx(profile);
        ...
        <div style={{flexShrink:0,marginBottom:4}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:8,overflow:"hidden",maxHeight:genreExpanded?"500px":"88px",transition:"max-height 0.35s ease"}}>
            {STIMMUNGEN.map(m=>{
              ...
              return(
                <button key={m.id} onClick={()=>{if(m.id===activeStimmung){setActiveStimmung(null);setStimmungRecs([]);return;}setActiveStimmung(m.id);loadStimmungRecs(m);}}
                  ...
                  <img src={m.tmdbImg} alt="" style={{position:"absolute",inset:0,width:"100%",height:"100%",objectFit:"cover",objectPosition:"center 20%",opacity:active?0.7:0.4,transition:"opacity 0.25s"}} onError={e=>{e.target.style.display="none";}}/>
              ...
              <div style={{display:"flex",flexDirection:"column",gap:8}}>
                ...
                {stimmungRecs.slice(0,3).map((it,i)=>(
                  <SwipeToBlock key={it.id||i} onBlock={async()=>{
                    const blockedId=it.id;shownTitles.current.add(blockedId||titleKey(it.title||it.name||""));
                    setStimmungRecs(prev=>{
                      const filtered=prev.filter((_,j)=>j!==i);let next=null;
                      if(recReserve.current.length>0){next=recReserve.current.shift();shownTitles.current.add(next.id);}
                      const newRecs=next?[...filtered,next]:filtered;
                      if(activeStimmung&&stimmungCache.current[activeStimmung]){stimmungCache.current[activeStimmung]={recs:newRecs,reserve:[...recReserve.current]};}
                      return newRecs;
                    });
                    if(recReserve.current.length===0){
                      try{
                        const st=STIMMUNGEN.find(s=>s.id===activeStimmung);if(!st)return;
                        const genre=st.genres[0];const page=Math.floor(Math.random()*8)+1;
                        const [rm,rt]=await Promise.all([
                          fetch(TMDB_BASE+"/discover/movie?api_key="+TMDB_API_KEY+"&language=de-DE&sort_by=popularity.desc&vote_count.gte=50&with_genres="+genre+"&page="+page).then(r=>r.json()),
                          fetch(TMDB_BASE+"/discover/tv?api_key="+TMDB_API_KEY+"&language=de-DE&sort_by=popularity.desc&vote_count.gte=50&with_genres="+genre+"&page="+page).then(r=>r.json()),
                        ]);
                        const fresh=[...(rm.results||[]).map(it=>({...it,media_type:"movie"})),...(rt.results||[]).map(it=>({...it,media_type:"tv"}))]
                          .filter(it=>it.poster_path&&!shownTitles.current.has(it.id)&&(it.genre_ids||[]).includes(genre));
                        fresh.slice(0,5).forEach(it=>shownTitles.current.add(it.id));
                        recReserve.current=fresh.slice(1,5);
                        if(fresh.length>0){setStimmungRecs(prev=>{const withNew=[...prev,fresh[0]];if(activeStimmung&&stimmungCache.current[activeStimmung]){stimmungCache.current[activeStimmung]={recs:withNew,reserve:[...recReserve.current]};}return withNew;});}
                      }catch(e){}
                    }
                  }}>
...
function BrowseTab({profile,cardProps,onSelect,onSwipe,onWatched}){
  ...
  useEffect(()=>{
    ...
    return()=>{window.removeEventListener("sf_open_swipe",onOpenSwipe);window.removeEventListener("sf_open_swipe_inner",onOpenSwipe);};
  },[]);
  ...
  if(showUniversal){
    return(
      <div style={{padding:"0 18px 24px"}}>
        ...
        <UniversalSwipe profile={profile} cardProps={cardProps} onSelect={onSelect} onDone={()=>{setShowUniversal(false);onSwipe&&onSwipe();}}/>
      </div>
    );
  }
...
function Onboarding({onDone}){
  ...
  // Cinematic background images per step
  const BG_IMGS=[
    "https://image.tmdb.org/t/p/original/44immBwzhDVyjn87b3x3l9mlhAD.jpg", // thriller
    "https://image.tmdb.org/t/p/original/9n2tJBplPbgR2ca05hS5CKXwP2c.jpg", // action
    "https://image.tmdb.org/t/p/original/56v2KjBlU4XaOv9rVYEQypROD7P.jpg", // fantasy
    "https://image.tmdb.org/t/p/original/3bhkrj58Vtu7enYsLeMMovMAKAg.jpg", // drama
  ];
  ...
  function buildProfile(){
    const genres={};
    selStimmungen.forEach(sid=>{
      const s=STIMMUNGEN.find(x=>x.id===sid);
      if(s)s.genres.forEach(g=>{genres[g]=(genres[g]||0)+5;});
    });
      ...
      <div style={{minHeight:"100vh",background:"#09090f",display:"flex",flexDirection:"column"}}>
        <div style={{padding:"20px 20px 0",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
          <div style={{display:"flex",alignItems:"center",gap:8}}>
            <div style={{width:34,height:34,borderRadius:10,background:"linear-gradient(135deg,#7c3aed,#e84393)",display:"flex",alignItems:"center",justifyContent:"center"}}>
              <svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polygon points="23 7 16 12 23 17 23 7"/><rect x="1" y="5" width="15" height="14" rx="2" ry="2"/></svg>
            </div>
            <span style={{fontFamily:"'Instrument Serif',serif",fontSize:18,background:"linear-gradient(135deg,#c4a960,#f5e090)",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent"}}>StreamFinder</span>
          </div>
          <button onClick={finish} style={{background:"none",border:"none",color:"#555",cursor:"pointer",fontSize:13,fontFamily:"'DM Sans'"}}>Überspringen →</button>
        </div>
        <div style={{padding:"16px 20px 8px"}}>
          <h2 style={{fontFamily:"'Instrument Serif',serif",fontSize:26,color:"#f0ece4",margin:"0 0 4px"}}>Erster Eindruck 🎴</h2>
          <p style={{fontSize:13,color:"#7a7488",margin:0}}>Swipe kurz durch Titel — StreamFinder lernt deinen Geschmack sofort.</p>
        </div>
  ...
  const bg=BG_IMGS[step]||BG_IMGS[0];

  return(
    <div style={{minHeight:"100vh",display:"flex",flexDirection:"column",position:"relative",overflow:"hidden",background:"#09090f"}}>
      {/* Cinematic background */}
      <img key={bg} src={bg} alt="" onLoad={()=>setBgLoaded(true)}
        style={{position:"fixed",inset:0,width:"100%",height:"100%",objectFit:"cover",objectPosition:"center 20%",opacity:bgLoaded?0.35:0,transition:"opacity 0.8s ease",zIndex:0,pointerEvents:"none"}}/>
      {/* Gradient overlay */}
      <div style={{position:"fixed",inset:0,zIndex:1,pointerEvents:"none",background:"linear-gradient(180deg,rgba(9,9,15,0.5) 0%,rgba(9,9,15,0.2) 30%,rgba(9,9,15,0.75) 70%,rgba(9,9,15,0.98) 100%)"}}/>
      ...
      <div style={{position:"relative",zIndex:2,flex:1,display:"flex",flexDirection:"column"}}>
        ...
        {step===0&&(
          <div style={{flex:1,display:"flex",flexDirection:"column",justifyContent:"flex-end",padding:"0 24px 0"}}>
            <div style={{marginBottom:32}}>
              <div style={{display:"inline-flex",alignItems:"center",gap:10,background:"rgba(255,255,255,0.08)",backdropFilter:"blur(20px)",borderRadius:30,padding:"8px 16px 8px 10px",border:"1px solid rgba(255,255,255,0.12)",marginBottom:20}}>
                <div style={{width:32,height:32,borderRadius:10,background:"linear-gradient(135deg,#7c3aed,#e84393)",display:"flex",alignItems:"center",justifyContent:"center"}}>
                  <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polygon points="23 7 16 12 23 17 23 7"/><rect x="1" y="5" width="15" height="14" rx="2" ry="2"/></svg>
                </div>
                <span style={{fontSize:15,fontWeight:700,color:"#f0ece4",fontFamily:"'DM Sans'"}}>StreamFinder</span>
              </div>
              <h1 style={{fontFamily:"'Instrument Serif',serif",fontSize:52,margin:"0 0 12px",color:"#fff",lineHeight:1.05,textShadow:"0 4px 32px rgba(0,0,0,0.8)"}}>Dein Kino.<br/>Jeden Abend.</h1>
              <p style={{fontSize:17,color:"rgba(255,255,255,0.7)",lineHeight:1.6,margin:"0 0 28px",textShadow:"0 2px 12px rgba(0,0,0,0.8)"}}>KI findet den perfekten Titel — für deine Stimmung, deine Plattformen, deinen Geschmack.</p>
              <div style={{display:"flex",gap:10,marginBottom:16}}>
                {[{icon:"✨",text:"KI-Picks"},{icon:"🎴",text:"Swipe"},{icon:"📺",text:"Alle Anbieter"}].map((f,i)=>(
                  <div key={i} style={{flex:1,background:"rgba(255,255,255,0.1)",backdropFilter:"blur(20px)",borderRadius:14,padding:"10px 8px",textAlign:"center",border:"1px solid rgba(255,255,255,0.15)"}}>
                    <div style={{fontSize:20,marginBottom:4}}>{f.icon}</div>
                    <div style={{fontSize:11,color:"rgba(255,255,255,0.8)",fontWeight:700,fontFamily:"'DM Sans'"}}>{f.text}</div>
                  </div>
                ))}
              </div>
            </div>
        ...
        {/* Step 1: Anbieter */}
        {step===1&&(
          <div style={{flex:1,display:"flex",flexDirection:"column",justifyContent:"flex-end",padding:"0 22px 0"}}>
            <div style={{marginBottom:24}}>
              <p style={{fontSize:11,color:"#c4a960",fontWeight:700,textTransform:"uppercase",letterSpacing:2,margin:"0 0 8px"}}>Schritt 1 von 2</p>
              <h2 style={{fontFamily:"'Instrument Serif',serif",fontSize:36,color:"#fff",margin:"0 0 6px",lineHeight:1.1,textShadow:"0 4px 24px rgba(0,0,0,0.8)"}}>Deine Anbieter</h2>
              <p style={{fontSize:14,color:"rgba(255,255,255,0.6)",margin:"0 0 18px"}}>Welche Abos hast du gerade?</p>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
                {PLATFORMS.map(p=>{
                  const sel=selPlats.includes(p.id);
                  return(
                    <button key={p.id} onClick={()=>togglePlat(p.id)}
                      style={{background:sel?p.color+"33":"rgba(255,255,255,0.07)",backdropFilter:"blur(20px)",border:"2px solid "+(sel?p.color:"rgba(255,255,255,0.1)"),borderRadius:16,padding:"14px 12px",cursor:"pointer",textAlign:"center",transition:"all 0.2s",boxShadow:sel?"0 0 24px "+p.color+"60":"none",position:"relative"}}>
                      {sel&&<div style={{position:"absolute",top:6,right:6,width:18,height:18,borderRadius:9,background:p.color,display:"flex",alignItems:"center",justifyContent:"center"}}>
                        <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth="3"><polyline points="20 6 9 17 4 12"/></svg>
                      </div>}
                      <div style={{fontSize:24,fontWeight:900,color:sel?p.color:"rgba(255,255,255,0.7)",marginBottom:6}}>{p.icon}</div>
                      <div style={{fontSize:13,fontWeight:700,color:sel?p.color:"rgba(255,255,255,0.6)"}}>{p.name}</div>
                    </button>
                  );
                })}
              </div>
        ...
        {step===2&&(
          <div style={{flex:1,display:"flex",flexDirection:"column",justifyContent:"flex-end",padding:"0 22px 0"}}>
            <div style={{marginBottom:24}}>
              <p style={{fontSize:11,color:"#c4a960",fontWeight:700,textTransform:"uppercase",letterSpacing:2,margin:"0 0 8px"}}>Schritt 2 von 2</p>
              <h2 style={{fontFamily:"'Instrument Serif',serif",fontSize:36,color:"#fff",margin:"0 0 6px",lineHeight:1.1,textShadow:"0 4px 24px rgba(0,0,0,0.8)"}}>Was liebst du?</h2>
              <p style={{fontSize:14,color:"rgba(255,255,255,0.6)",margin:"0 0 16px"}}>Wähle deine Lieblingsgenres</p>
              <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:8}}>
                {STIMMUNGEN.map(m=>{
                  const sel=selStimmungen.includes(m.id);
                  return(
                    <button key={m.id} onClick={()=>toggleStimmung(m.id)}
                      style={{position:"relative",borderRadius:14,overflow:"hidden",cursor:"pointer",border:"2px solid "+(sel?m.color:"transparent"),transition:"all 0.2s",height:80,background:"rgba(0,0,0,0.3)",boxShadow:sel?"0 0 20px "+m.color+"60":"none",padding:0}}>
                      <img src={m.tmdbImg} alt="" style={{position:"absolute",inset:0,width:"100%",height:"100%",objectFit:"cover",objectPosition:"center 20%",opacity:sel?0.8:0.4,transition:"opacity 0.2s"}} onError={e=>{e.target.style.display="none";}}/>
                      <div style={{position:"absolute",inset:0,background:sel?"linear-gradient(to bottom,transparent 20%,"+m.color+"90)":"linear-gradient(to bottom,rgba(0,0,0,0.1),rgba(0,0,0,0.75))"}}/>
                      {sel&&<div style={{position:"absolute",top:5,right:5,width:16,height:16,borderRadius:8,background:m.color,display:"flex",alignItems:"center",justifyContent:"center"}}>
                        <svg width="8" height="8" viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth="3.5"><polyline points="20 6 9 17 4 12"/></svg>
                      </div>}
                      <div style={{position:"absolute",bottom:0,left:0,right:0,padding:"5px 5px 7px",textAlign:"center"}}>
                        <span style={{fontSize:11,fontWeight:700,color:"#fff",fontFamily:"'DM Sans'",lineHeight:1.2,textShadow:"0 1px 6px rgba(0,0,0,0.9)",display:"block"}}>{m.label}</span>
                      </div>
                    </button>
                  );
                })}
        ...
        {step===3&&(
          <div style={{flex:1,display:"flex",flexDirection:"column",justifyContent:"flex-end",padding:"0 24px 0"}}>
            <div style={{marginBottom:24}}>
              ...
              <h2 style={{fontFamily:"'Instrument Serif',serif",fontSize:40,color:"#fff",margin:"0 0 10px",lineHeight:1.1,textShadow:"0 4px 24px rgba(0,0,0,0.8)",textAlign:"center"}}>Bereit!</h2>
              <div style={{background:"rgba(255,255,255,0.08)",backdropFilter:"blur(20px)",borderRadius:18,padding:"16px 18px",border:"1px solid rgba(255,255,255,0.12)",marginBottom:14}}>
                <div style={{display:"flex",gap:16,justifyContent:"center"}}>
                  <div style={{textAlign:"center"}}><div style={{fontSize:24,fontWeight:900,color:"#f0ece4"}}>{selPlats.length}</div><div style={{fontSize:11,color:"rgba(255,255,255,0.5)"}}>Anbieter</div></div>
                  <div style={{width:1,background:"rgba(255,255,255,0.1)"}}/>
                  <div style={{textAlign:"center"}}><div style={{fontSize:24,fontWeight:900,color:"#f0ece4"}}>{selStimmungen.length}</div><div style={{fontSize:11,color:"rgba(255,255,255,0.5)"}}>Genres</div></div>
                  <div style={{width:1,background:"rgba(255,255,255,0.1)"}}/>
                  <div style={{textAlign:"center"}}><div style={{fontSize:24,fontWeight:900,color:"#4ade80"}}>✓</div><div style={{fontSize:11,color:"rgba(255,255,255,0.5)"}}>KI bereit</div></div>
                </div>
              </div>
              <p style={{fontSize:13,color:"rgba(255,255,255,0.45)",textAlign:"center",lineHeight:1.5}}>Tipp: Ein kurzer Swipe macht die ersten Empfehlungen noch besser.</p>
            </div>
          </div>
        )}

        {/* Buttons */}
        <div style={{padding:"12px 22px",paddingBottom:"calc(env(safe-area-inset-bottom) + 16px)",display:"flex",gap:10,flexShrink:0}}>
          {step>0&&<button onClick={()=>setStep(s=>s-1)}
            style={{width:52,height:56,borderRadius:16,background:"rgba(255,255,255,0.1)",backdropFilter:"blur(20px)",border:"1px solid rgba(255,255,255,0.15)",cursor:"pointer",color:"#fff",fontSize:20,flexShrink:0}}>←</button>}
          {step<3&&<button onClick={()=>setStep(s=>s+1)}
            disabled={step===1&&selPlats.length===0}
            style={{flex:1,background:step===0?"linear-gradient(135deg,#7c3aed,#e84393)":"linear-gradient(135deg,#ff6b35,#e84393)",border:"none",borderRadius:16,padding:"16px",cursor:"pointer",color:"#fff",fontFamily:"'DM Sans'",fontWeight:800,fontSize:17,opacity:step===1&&selPlats.length===0?0.4:1,boxShadow:"0 8px 32px rgba(232,67,147,0.5)"}}>
            {step===0?"Loslegen →":"Weiter →"}
          </button>}
          {step===3&&<>
            <button onClick={finish} style={{flex:1,background:"rgba(255,255,255,0.1)",backdropFilter:"blur(20px)",border:"1px solid rgba(255,255,255,0.15)",borderRadius:16,padding:"16px",cursor:"pointer",color:"rgba(255,255,255,0.7)",fontFamily:"'DM Sans'",fontWeight:700,fontSize:14}}>Direkt starten</button>
            <button onClick={goToSwipe} style={{flex:2,background:"linear-gradient(135deg,#c4a960,#ff6b35)",border:"none",borderRadius:16,padding:"16px",cursor:"pointer",color:"#fff",fontFamily:"'DM Sans'",fontWeight:800,fontSize:15,boxShadow:"0 8px 32px rgba(196,169,96,0.5)"}}>🎴 Titel entdecken →</button>
          </>}
        </div>
        ...
        <div style={{display:"flex",gap:5,justifyContent:"center",paddingBottom:20}}>
          {[0,1,2,3].map(i=>(
            <div key={i} style={{width:i===step?24:7,height:7,borderRadius:4,background:i===step?"#e84393":i<step?"rgba(232,67,147,0.4)":"rgba(255,255,255,0.15)",transition:"all 0.3s"}}/>
          ))}
        </div>
      </div>
    </div>
    ...
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.85)",zIndex:300,display:"flex",alignItems:"flex-end"}} onClick={onClose}>
      <div onClick={e=>e.stopPropagation()} style={{width:"100%",maxHeight:"90vh",overflowY:"auto",background:"#0d0d1a",borderRadius:"24px 24px 0 0",paddingBottom:40}}>
        ...
        <div style={{padding:"16px 20px"}}>
          ...
          <button onClick={()=>window.dispatchEvent(new CustomEvent("sf_open_swipe"))} style={{width:"100%",background:"rgba(196,169,96,0.08)",border:"1px solid rgba(196,169,96,0.2)",borderRadius:14,padding:12,cursor:"pointer",color:"#c4a960",fontFamily:"'DM Sans'",fontWeight:700,fontSize:13,marginBottom:16}}>
            🎯 Geschmack verfeinern
          </button>
          {!showReset?(
...
function MainApp({initialProfile}){
  ...
  return(
    <div style={{maxWidth:720,margin:"0 auto",minHeight:"100vh",background:"#09090f",position:"relative",fontFamily:"'DM Sans',sans-serif"}}>
      <style>{`
      ...
      <div style={{position:"sticky",top:0,zIndex:100,background:"rgba(9,9,15,0.92)",backdropFilter:"blur(20px)",borderBottom:"1px solid rgba(255,255,255,0.05)",padding:"10px 18px",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
        <div style={{display:"flex",alignItems:"center",gap:8}}>
          ...
          <span style={{fontFamily:"'Instrument Serif',serif",fontSize:20,background:"linear-gradient(135deg,#c4a960,#f5e090)",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent",fontWeight:400}}>StreamFinder</span>
        </div>
        <div style={{display:"flex",gap:6}}>
          <button onClick={()=>setShowHelp(true)} style={{width:34,height:34,borderRadius:10,background:"rgba(255,255,255,0.05)",border:"1px solid rgba(255,255,255,0.1)",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",color:"#b0a8b8",fontSize:16,fontWeight:700}}>?</button>
      ...
      <div style={{paddingBottom:80}}>
        {activeTab==="spotlight"&&<SpotlightTab profile={profile} cardProps={cardProps} onSelect={setSelectedItem}/>}
        {activeTab==="browse"&&<BrowseTab profile={profile} cardProps={cardProps} onSelect={setSelectedItem} onWatched={handleWatched}/>}
      ...
      <div style={{position:"fixed",bottom:0,left:"50%",transform:"translateX(-50%)",width:"100%",maxWidth:720,background:"rgba(9,9,15,0.97)",backdropFilter:"blur(24px)",borderTop:"1px solid rgba(255,255,255,0.07)",zIndex:100,padding:"6px 0 calc(env(safe-area-inset-bottom) + 6px)"}}>
