...
async function getPlatformTop3(profile,platformName){
  const ctx=buildCtx(profile);
  const system=`Du bist ein Streaming-Experte. Antworte NUR mit JSON-Array, keine Backticks. Format: [{"title":"...","year":"...","type":"Film oder Serie","reason":"...","emoji":"..."}]`;
  const msg=`Empfehle genau 3 Titel die JETZT auf ${platformName} Deutschland verfügbar sind.\nHoch bewertet vom Nutzer: ${ctx.topRated||"noch nichts"}.\nGesehen: ${ctx.watched||"nichts"}.\nLieblingsgenres: ${ctx.topGenres||"gemischt"}.\nreason = 1 kurzer witziger Satz warum.`;
  const text=await callAI([{role:"user",content:msg}],system);
  try{return JSON.parse(text.replace(/```json|```/g,"").trim());}catch{return[];}
}
      ...
      <div style={{background:"linear-gradient(180deg,#13121f,#09090f)",minHeight:"100vh",maxWidth:560,margin:"0 auto"}} onClick={e=>e.stopPropagation()}>
        ...
        <div style={{padding:"0 20px 40px"}}>
          <div style={{display:"flex",gap:14,alignItems:"flex-end",marginTop:-50,marginBottom:16}}>
            ...
            <div style={{flex:1,paddingBottom:4}}>
              ...
              <div style={{display:"flex",gap:6,alignItems:"center",flexWrap:"wrap"}}>
                {year&&<span style={{fontSize:12,color:"#555"}}>{year}</span>}
          ...
          <div style={{background:"#12121f",borderRadius:14,padding:"14px 16px",marginBottom:14,display:"flex",alignItems:"center",justifyContent:"space-between",border:"1px solid #1e1e30"}}>
            <div>
              <div style={{fontSize:12,color:"#6a5e70",fontWeight:600}}>Deine Bewertung</div>
          ...
          {(item.genre_ids||[]).length>0&&(
            <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:16}}>
              {(item.genre_ids||[]).map(gid=>GENRES_TMDB[gid]?<span key={gid} style={{fontSize:12,color:"#8a7e90",background:"#1a1a2e",padding:"4px 10px",borderRadius:8}}>{GENRE_EMOJI[gid]} {GENRES_TMDB[gid]}</span>:null)}
            </div>
          )}
          ...
          {details?.credits?.cast?.length>0&&(
            <div style={{marginBottom:16}}>
              <p style={{fontSize:11,color:"#555",fontWeight:700,textTransform:"uppercase",letterSpacing:0.5,marginBottom:8}}>Besetzung</p>
              <div style={{display:"flex",gap:8,overflowX:"auto",paddingBottom:4}}>
                {details.credits.cast.slice(0,8).map(c=>(
                  <div key={c.id} style={{flexShrink:0,textAlign:"center",width:60}}>
                    ...
                    <div style={{fontSize:10,color:"#6a5e70",lineHeight:1.2}}>{c.name}</div>
                  </div>
                ))}
              </div>
            </div>
          ...
          {details?.["watch/providers"]?.results?.DE?.flatrate?.length>0&&(
            <div style={{marginBottom:16}}>
              <p style={{fontSize:11,color:"#555",fontWeight:700,textTransform:"uppercase",letterSpacing:0.5,marginBottom:8}}>Verfügbar bei</p>
        ...
        <div style={{display:"flex",gap:12,alignItems:"flex-start"}}>
          ...
          <div style={{flex:1,minWidth:0}}>
            ...
            <div style={{display:"flex",gap:5,flexWrap:"wrap",marginBottom:6}}>
              ...
              {(item.genre_ids||[]).slice(0,2).map(gid=>GENRES_TMDB[gid]?<span key={gid} style={{fontSize:11,color:"#6a5e70"}}>{GENRE_EMOJI[gid]} {GENRES_TMDB[gid]}</span>:null)}
            </div>
...
function PlatformCard({platform,profile,onBrowse,onSelect}){
  ...
  function handleToggle(){
    ...
    if(next&&!loaded.current){
      ...
      getPlatformTop3(profile,platform.name).then(d=>{setRecs(d);setLoading(false);}).catch(()=>{setRecs([]);setLoading(false);});
    }
  }
    ...
    <div style={{background:"#12121f",borderRadius:18,overflow:"hidden",border:"1px solid #1e1e30",marginBottom:10}}>
      <div style={{padding:"14px 16px",display:"flex",alignItems:"center",gap:12}}>
        ...
        <div style={{flex:1}}>
          ...
          <div style={{fontSize:11,color:"#555"}}>Top 3 nur für dich</div>
        </div>
      ...
      {open&&(
        <div style={{padding:"0 16px 14px",animation:"fadeUp 0.3s ease"}}>
          ...
          {loading&&<div style={{textAlign:"center",padding:12}}><div style={{fontSize:20,animation:"spin 1.5s linear infinite",display:"inline-block"}}>✨</div><p style={{fontSize:12,color:"#555",marginTop:6}}>Analysiere deinen Geschmack…</p></div>}
          {!loading&&recs&&recs.length===0&&<p style={{fontSize:12,color:"#555",textAlign:"center"}}>Noch zu wenig Daten.</p>}
    ...
    <div style={{background:"#12121f",borderRadius:18,overflow:"hidden",border:"1px solid #1e1e30",marginBottom:16}}>
      <div onClick={()=>setOpen(!open)} style={{padding:14,display:"flex",alignItems:"center",gap:12,cursor:"pointer"}}>
        ...
        <div style={{flex:1}}>
          ...
          <div style={{fontSize:11,color:"#555"}}>ARD · ZDF · Arte · BR · 3sat · funk — kostenlos!</div>
        </div>
      ...
      {open&&(
        <div style={{padding:"0 14px 14px"}}>
          ...
          {searching&&<p style={{fontSize:12,color:"#555",textAlign:"center",marginBottom:8}}>Suche…</p>}
                ...
                <a key={i} href={item.url_video||item.url_website||"#"} target="_blank" rel="noopener noreferrer" style={{display:"flex",alignItems:"flex-start",gap:10,padding:"8px",borderRadius:10,textDecoration:"none",color:"#c4b8c8",marginBottom:4,background:"#0d0d18"}}>
                  <div style={{flex:1,minWidth:0}}>
                    ...
                    <div style={{display:"flex",gap:6}}>
                      ...
                      {item.duration&&<span style={{fontSize:10,color:"#555"}}>{formatDuration(item.duration)}</span>}
                    </div>
                    {item.description&&<p style={{fontSize:11,color:"#6a5e70",margin:"3px 0 0",lineHeight:1.4,display:"-webkit-box",WebkitLineClamp:2,WebkitBoxOrient:"vertical",overflow:"hidden"}}>{item.description}</p>}
                  </div>
            ...
            {MEDIATHEKEN.map(m=>(
              <button key={m.id} onClick={()=>loadChannel(m)} style={{background:activeChannel?.id===m.id?m.color+"22":"#0d0d18",border:"1px solid "+(activeChannel?.id===m.id?m.color+"55":"#1e1e30"),borderRadius:12,padding:"10px 12px",cursor:"pointer",display:"flex",alignItems:"center",gap:8,transition:"all 0.2s"}}>
                ...
                <div style={{textAlign:"left"}}>
                  ...
                  <div style={{fontSize:10,color:"#555"}}>Aktuelle Inhalte</div>
                </div>
              </button>
            ))}
          </div>
          {/* Channel items */}
          {channelLoading&&<div style={{textAlign:"center",padding:12}}><div style={{fontSize:20,animation:"spin 1.5s linear infinite",display:"inline-block"}}>📡</div><p style={{fontSize:12,color:"#555",marginTop:4}}>Lade aktuelle Inhalte…</p></div>}
          {!channelLoading&&activeChannel&&channelItems.length>0&&(
            <div>
              <p style={{fontSize:11,color:"#555",fontWeight:700,textTransform:"uppercase",letterSpacing:0.5,marginBottom:8}}>Aktuell bei {activeChannel.name}</p>
              {channelItems.map((item,i)=>(
                <a key={i} href={item.url_video||item.url_website||activeChannel.url} target="_blank" rel="noopener noreferrer" style={{display:"flex",alignItems:"flex-start",gap:10,padding:"10px 8px",borderRadius:12,textDecoration:"none",color:"#c4b8c8",borderBottom:i<channelItems.length-1?"1px solid #1e1e30":"none"}}>
                  ...
                  <div style={{flex:1,minWidth:0}}>
                    ...
                    {item.description&&<p style={{fontSize:11,color:"#6a5e70",margin:0,lineHeight:1.4,display:"-webkit-box",WebkitLineClamp:2,WebkitBoxOrient:"vertical",overflow:"hidden"}}>{item.description}</p>}
                  </div>
          ...
          {!channelLoading&&activeChannel&&channelItems.length===0&&<p style={{fontSize:12,color:"#555",textAlign:"center"}}>Keine Inhalte gefunden.</p>}
        </div>
      )}
    </div>
  );
        ...
        {step===0&&(
          <div style={{animation:"fadeUp 0.4s ease"}}>
            <div style={{textAlign:"center",marginBottom:20}}>
              ...
              <p style={{color:"#6a5e70",fontSize:15,margin:0}}>Kein Bullshit. Nur gute Picks. 🎯</p>
            </div>
            <p style={{color:"#6a5e70",marginBottom:16,fontSize:14,textAlign:"center"}}>Welche Dienste hast du? (Alle die zutreffen)</p>
            ...
            <button onClick={()=>setPlats(s=>s.includes("mediatheken")?s.filter(x=>x!=="mediatheken"):[...s,"mediatheken"])} style={{width:"100%",background:plats.includes("mediatheken")?"linear-gradient(135deg,#004E8A33,#FA7D1922)":"#12121f",border:"1px solid "+(plats.includes("mediatheken")?"#004E8A88":"#1e1e30"),borderRadius:16,padding:"13px 16px",cursor:"pointer",display:"flex",alignItems:"center",gap:12,marginBottom:28,transition:"all 0.2s"}}>
              ...
              <div style={{textAlign:"left"}}><div style={{fontSize:12,fontWeight:700,color:plats.includes("mediatheken")?"#f0ece4":"#6a5e70"}}>Öffentlich-Rechtliche</div><div style={{fontSize:10,color:"#555"}}>ARD · ZDF · Arte — komplett kostenlos!</div></div>
      ...
      <div style={{padding:"20px 18px 0",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
        <div style={{display:"flex",alignItems:"center",gap:10}}>
          ...
          <div>
            ...
            <p style={{fontSize:10,color:"#444",margin:0,fontStyle:"italic"}}>{homeQuip}</p>
          </div>
        </div>
      ...
      <div style={{paddingTop:14}}>
          ...
          <div>
            ...
            <div style={{padding:"0 18px",marginBottom:14}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
                <p style={{fontSize:11,color:"#555",fontWeight:700,textTransform:"uppercase",letterSpacing:0.8,margin:0}}>Stimmung — {day}?</p>
                ...
                <div style={{display:"flex",gap:10,overflowX:"auto",paddingBottom:4}}>
                  {aiRecs.map((rec,i)=>{
                    ...
                    return(<div key={i} style={{flexShrink:0,width:150,background:"linear-gradient(145deg,#1a1525,#130f1e)",border:"1px solid "+(pl?pl.color+"33":"#2a1f3d"),borderRadius:16,padding:14,cursor:"pointer"}} onClick={()=>setSelectedItem({title:rec.title,name:rec.title,overview:rec.reason,vote_average:0,genre_ids:[],poster_path:null,media_type:rec.type==="Serie"?"tv":"movie",release_date:rec.year||""})}>
                      ...
                      {rec.year&&<div style={{fontSize:10,color:"#555",marginBottom:4}}>{rec.year} · {rec.type}</div>}
            ...
            {feedLoading?(
              <div style={{textAlign:"center",padding:40}}><div style={{fontSize:28,animation:"spin 2s linear infinite",display:"inline-block"}}>🍿</div><p style={{fontSize:13,color:"#555",marginTop:8}}>Einen Moment… schauen was gut ist.</p></div>
            ):(
              <div style={{padding:"0 18px"}}>
                {heroItems.length>0&&(<div style={{marginBottom:10}}><p style={{fontSize:11,color:"#555",fontWeight:700,textTransform:"uppercase",letterSpacing:0.8,marginBottom:10}}>Top-Picks</p>{heroItems.map(item=><HeroCard key={item.id} item={item} {...cardProps}/>)}</div>)}
              ...
              <div>
                ...
                <div style={{background:"linear-gradient(135deg,#1a1525,#13101e)",borderRadius:18,padding:"14px 16px",marginBottom:16,border:"1px solid #2a1f3d"}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
                    ...
                    <button onClick={loadTop5} style={{background:"transparent",border:"none",color:"#555",cursor:"pointer",fontSize:12,fontFamily:"'DM Sans'"}}>🔄</button>
                  </div>
                  {top5Loading?(
                    <div style={{textAlign:"center",padding:10}}><div style={{fontSize:20,animation:"spin 1.5s linear infinite",display:"inline-block"}}>✨</div><p style={{fontSize:12,color:"#555",marginTop:6}}>Analysiere Geschmack…</p></div>
                ...
                {userPlatforms.map(p=><PlatformCard key={p.id} platform={p} profile={profile} onBrowse={pl=>loadBrowse(pl)} onSelect={setSelectedItem}/>)}
        ...
        {tab==="fun"&&(
          <div style={{padding:"0 18px"}}>
            ...
            <p style={{fontSize:12,color:"#555",marginBottom:20}}>Weil normales Suchen so langweilig ist.</p>

            {/* Überrasch mich + Orakel */}
            <div style={{background:"linear-gradient(135deg,#1a1525,#13101e)",borderRadius:20,padding:20,marginBottom:16,border:"1px solid #2a1f3d",textAlign:"center"}}>
              ...
              <p style={{fontSize:12,color:"#6a5e70",marginBottom:14}}>Du kannst dich nicht entscheiden? Kein Problem. Wir entscheiden. Du schaust. Ende der Diskussion.</p>
              ...
              {surpriseLoading&&<div><div style={{fontSize:32,animation:"spin 1s linear infinite",display:"inline-block"}}>🎲</div><p style={{fontSize:13,color:"#8a7e90",marginTop:8}}>Würfelt… die Spannung ist kaum auszuhalten.</p></div>}
              {surpriseData&&!surpriseLoading&&(
                <div style={{animation:"fadeUp 0.5s ease"}}>
                  <p style={{fontSize:14,color:"#ff6b35",fontWeight:800,marginBottom:4}}>{rnd(SURPRISE_MSGS)}</p>
                  <div style={{background:"#0d0d18",borderRadius:14,padding:"14px",marginBottom:12,textAlign:"left",border:"1px solid #2a1f3d"}}>
                    ...
                    <p style={{fontSize:12,color:"#8a7e90",fontStyle:"italic",lineHeight:1.6,margin:"0 0 8px"}}><span style={{color:"#e84393",fontWeight:700}}>{rnd(ORACLE_INTROS)}</span><br/>"{surpriseData.prophecy}"</p>
                  </div>
                  <div style={{display:"flex",gap:8"}}>
                    <button onClick={()=>setSelectedItem({title:surpriseData.title,name:surpriseData.title,overview:surpriseData.prophecy,vote_average:0,genre_ids:[],poster_path:null,media_type:"movie",release_date:surpriseData.year||""})} style={{flex:1,background:"#1a1a2e",border:"1px solid #2a2340",borderRadius:12,padding:"10px",color:"#888",cursor:"pointer",fontFamily:"'DM Sans'",fontWeight:700,fontSize:12}}>Details</button>
            ...
            <div style={{background:"linear-gradient(135deg,#0f1a1a,#0d1f1a)",borderRadius:20,padding:20,border:"1px solid #1f3d2a"}}>
              ...
              <p style={{fontSize:12,color:"#6a5e70",marginBottom:14,textAlign:"center"}}>Wir analysieren deine Bewertungen und enthüllen wer du wirklich bist. Bitte nicht erschrecken.</p>
              ...
              {personalityLoading&&<div style={{textAlign:"center"}}><div style={{fontSize:28,animation:"spin 1.5s linear infinite",display:"inline-block"}}>🧠</div><p style={{fontSize:12,color:"#555",marginTop:8}}>Analysiere Filmgeschmack… bitte ehrlich bleiben.</p></div>}
              {personality&&!personalityLoading&&(
                <div style={{animation:"fadeUp 0.5s ease"}}>
        ...
        {tab==="history"&&(
          <div style={{padding:"0 18px"}}>
            ...
            <p style={{fontSize:12,color:"#555",marginBottom:16}}>Je mehr du markierst, desto besser die Empfehlungen. Der Algorithmus beobachtet. Er urteilt. Er empfiehlt.</p>
            {(profile.watched||[]).length===0
              ...
              :(profile.watched||[]).map(w=>(
                <div key={w.id} style={{background:"#12121f",borderRadius:14,padding:"12px",border:"1px solid #1e1e30",display:"flex",alignItems:"center",gap:12,marginBottom:8,cursor:"pointer"}} onClick={()=>setSelectedItem(w)}>
                  ...
                  <button onClick={e=>{e.stopPropagation();handleWatched(w);}} style={{padding:"7px 10px",borderRadius:10,background:"#1a1a2e",border:"1px solid #2a2340",color:"#555",cursor:"pointer",fontSize:12}}>✕</button>
                </div>
              ))
            }
            {(()=>{
