...
function PlatformCard({platform,profile,onSelect,onBlock,onLike,browseType}){
  ...
  function loadTop5Recs(){
    ...
    // TMDB sofort laden — Premium Cards
    getTMDBTop5(profile,platform,browseType||"serie").then(tmdbItems=>{
      if(tmdbItems.length>0){
        const formatted=tmdbItems.map(it=>({
          ...
          reason:"Passt zu deinem Geschmack",
        ...
        // KI verbessert nur die Begründungen — ersetzt NICHT die Karten
        getPlatformTop5(profile,platform.name,browseType||"serie")
          .then(aiItems=>{
            if(!aiItems||aiItems.length===0)return;
            // Nur Begründungen aus KI übernehmen, Bilder bleiben von TMDB
            setRecs(prev=>{
              if(!prev)return prev;
              return prev.map((rec,i)=>{
                const aiMatch=aiItems.find(a=>
                  titleKey(a.title||"")===titleKey(rec.title||"")
                )||aiItems[i];
                if(aiMatch?.reason){
                  return{...rec,reason:aiMatch.reason};
                }
                return rec;
              });
            });
