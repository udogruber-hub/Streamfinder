...
function LedStrip({itemTitle,profile,onRate,genre_ids}){
  ...
  const segments=[
    {v:1,color:"#ef4444",glow:"rgba(239,68,68,0.7)",label:"Nein"},
    {v:3,color:"#f97316",glow:"rgba(249,115,22,0.7)",label:"Ok"},
    {v:5,color:"#4ade80",glow:"rgba(74,222,128,0.7)",label:"Top"},
  ];
  ...
  return(
    <div ref={stripRef}
      ...
      onMouseUp={e=>{e.stopPropagation();handleEnd(e.clientX);}}
      onMouseLeave={e=>{if(dragging)handleEnd(e.clientX);setDragging(false);}}
      ...
      onTouchEnd={e=>{e.stopPropagation();handleEnd(e.changedTouches[0].clientX);}}
      style={{display:"flex",borderRadius:20,overflow:"hidden",height:28,cursor:"pointer",userSelect:"none",touchAction:"none"}}>
      {segments.map((seg,i)=>{
        const active=current===seg.v;
        const isPast=current>0&&seg.v<=current;
        return(
          <div key={seg.v} style={{
            flex:1,
            background:isPast?seg.color:active?seg.color:"rgba(255,255,255,0.06)",
            display:"flex",alignItems:"center",justifyContent:"center",
            boxShadow:active?"0 0 12px "+seg.glow:"none",
            transition:"background 0.15s, box-shadow 0.15s",
            borderLeft:i>0?"1px solid rgba(0,0,0,0.3)":"none",
          }}>
            <span style={{fontSize:10,fontWeight:700,color:isPast?"rgba(0,0,0,0.7)":"rgba(255,255,255,0.3)",pointerEvents:"none"}}>
              {seg.label}
            </span>
          </div>
        );
