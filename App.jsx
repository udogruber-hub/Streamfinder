...
// ── Platform Swipe Mode ──
function PlatformSwipe({platform,profile,browseType,onBlock,onLike,onSelect,onDone}){
  ...
  if(done)return(
    <div style={{textAlign:"center",padding:20,color:"#b0a8b8"}}>
      <div style={{fontSize:28,marginBottom:6}}>🎉</div>
      <p style={{fontWeight:700,marginBottom:4}}>Alle durch!</p>
      <p style={{fontSize:11,marginBottom:14,color:"#666"}}>Bewertungen gespeichert.</p>
      <button onClick={()=>onDone()} style={{background:"linear-gradient(135deg,#ff6b35,#e84393)",border:"none",borderRadius:12,padding:"10px 20px",color:"#fff",cursor:"pointer",fontFamily:"'DM Sans'",fontWeight:700,fontSize:13}}>
        Top 5 anzeigen →
      </button>
    </div>
  );
    ...
    <div style={{background:"#12121f",borderRadius:18,overflow:"hidden",border:"1px solid #1e1e30",marginBottom:10}}>
      ...
      {open&&mode==="swipe"&&(
        <div style={{padding:"0 18px 48px"}}>
          ...
          <PlatformSwipe platform={platform} profile={profile} browseType={browseType} onBlock={t=>onBlock(t)} onLike={it=>onLike(it)} onSelect={onSelect} onDone={()=>{setMode("top5");lastTypeRef.current=null;loadTop5Recs();}}/>
        </div>
      )}
    </div>
  );
