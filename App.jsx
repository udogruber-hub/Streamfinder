...
function PlatformCard({platform,profile,onSelect,onBlock,onLike,onRate,browseType}){
  ...
  useEffect(()=>{
    ...
    // Nach Universal Swipe → aufklappen + neu laden mit frischen Daten
    function onSwipeDone(e){
      setOpen(true);
      lastTypeRef.current=null;
      setRecs(null);
      // Kurze Pause damit Profil gespeichert ist
      setTimeout(()=>loadTop5Recs(),500);
    }
  ...
  return(
    <div style={{background:"#12121f",borderRadius:18,overflow:"hidden",border:"1px solid #1e1e30",marginBottom:10}}>
      ...
      <div style={{padding:"14px 16px",display:"flex",alignItems:"center",gap:12,cursor:"pointer"}} onClick={()=>{const next=!open;setOpen(next);if(next&&!recs)loadTop5Recs();}}>
...
function UniversalSwipe({profile,cardProps,onSelect,onDone}){
  ...
  async function handleDone(){
    setDone(true);
    // Profil updaten mit Swipe-Ergebnissen
    const liked_sw=Object.entries(localRatings.current).filter(([,v])=>v.stars>=4).map(([k])=>k).join(", ");
    const disliked_sw=Object.entries(localRatings.current).filter(([,v])=>v.stars<=2).map(([k])=>k).join(", ");
    if(liked_sw||disliked_sw){
      try{
        const ctx=buildCtx(profile);
        const system=`Antworte NUR mit JSON-Array, kein Markdown. Format: [{"title":"...","reason":"..."}]`;
        const msg=`Nutzer hat geswiped:
❤️ GEMOCHT: ${liked_sw||"nichts"}
✕ ABGELEHNT: ${disliked_sw||"nichts"}

Schreibe eine kurze Zusammenfassung des Geschmacks auf Deutsch (1 Satz).`;
        await callAI([{role:"user",content:msg}],system);
      }catch(e){}
    }
  ...
  // Bug-Fix: Capture current item at time of swipe, not via closure
  function handleRight(item){
  ...
  async function handleDone(){
    ...
    // KI wählt Top 5 Titel + schreibt witzige Sätze
    if(liked_sw.length>0||disliked_sw.length>0){
      try{
        const ctx=buildCtx(profile);
        const userPlats=PLATFORMS.filter(p=>profile.platforms.includes(p.id)).map(p=>p.name).join(", ");
        const system=`Du bist ein Streaming-Experte. Antworte NUR mit JSON-Array, kein Markdown. Format: [{"title":"...","reason":"...","platform":"..."}]`;
        const msg=`Empfehle 5 Serien oder Filme basierend auf diesen Swipe-Ergebnissen:\n❤️ GEMOCHT: ${liked_sw.join(", ")||"nichts"}\n✕ ABGELEHNT: ${disliked_sw.join(", ")||"nichts"}\nWatchlist: ${ctx.watchlist||"leer"}\nPlattformen: ${userPlats||ctx.platforms}\n\nRegeln:\n- Orientiere dich STARK an den gemochten Titeln\n- Vermeide alles Ähnliche zu den abgelehnten\n- Nur verfügbar auf den Plattformen des Nutzers\n- reason = witziger deutscher Satz (max 12 Wörter)`;
        const text=await callAI([{role:"user",content:msg}],system);
        const recs=JSON.parse(text.replace(/```json|```/g,"").trim());
        // Speichere KI-Empfehlungen für spätere Anzeige
        if(Array.isArray(recs)&&recs.length>0){
          sessionStorage.setItem("sf_swipe_recs",JSON.stringify(recs));
        }
      }catch(e){}
    }
    // Signal: Profil hat sich verändert → alles neu laden
    // Warte kurz damit Profil-Updates in localStorage sind
    setTimeout(()=>{
      window.dispatchEvent(new CustomEvent("sf_swipe_done",{detail:{
        liked:liked_sw,
        disliked:disliked_sw,
      }}));
      onDone();
    },600);
  }
...
function MainApp({profile:initProfile,onReset}){
  ...
  // Nach Universal Swipe → Startseite neu laden
  useEffect(()=>{
