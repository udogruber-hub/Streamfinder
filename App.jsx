              ...
              <div style={{display:"flex",flexDirection:"column",gap:8}}>
                ...
                {stimmungRecs.slice(0,3).map((it,i)=>(
                  <SwipeToBlock key={it.id||i} onBlock={async()=>{
                    ...
                    // Aus Reserve nehmen wenn möglich
                    if(recReserve.current.length>0){
                      const next=recReserve.current.shift();
                      shownTitles.current.add(next.id);
                      setStimmungRecs(prev=>[...prev.filter((_,j)=>j!==i),next]);
                      return;
                    }
                    // Sonst neu laden
                    ...
                    try{
                      ...
                      if(fresh.length>0){
                        shownTitles.current.add(fresh[0].id);
                        setStimmungRecs(prev=>[...prev,{...fresh[0],media_type:"movie"}]);
                      }
                    }catch(e){}
                  }}>
