    ...
    <div style={{background:"#12121f",borderRadius:18,overflow:"hidden",border:"1px solid #1e1e30",marginBottom:10}}>
      <div style={{padding:"14px 16px",display:"flex",alignItems:"center",gap:12,cursor:"pointer"}} onClick={handleOpen}>
        ...
        <div style={{flex:1}}>
          ...
          <div style={{fontSize:9,color:"#4a4458",marginTop:2,cursor:"pointer"}} title="Der Balken zeigt wie gut dieser Titel zu deinem Geschmack passt — basierend auf deinen Bewertungen und gesehenen Titeln."}>▌% = Übereinstimmung mit deinem Geschmack ℹ️</div>
        </div>
...
Plattformen: ${ctx.platforms}`;
      ...
      // Extrahiere Titel-Empfehlungen aus Antwort (Zeilen mit Anführungszeichen oder Nummerierung)
      const titleMatches=text.match(/["„]([^"„""]+)[""]/g)||[];
      const titles=titleMatches.map(t=>t.replace(/["„""]/g,"").trim()).filter(t=>t.length>2&&t.length<60).slice(0,4);
