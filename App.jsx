...
function BrowseFeature({mode,profile,cardProps,onSelect}){
  ...
  async function startDeepDive(titleStr){
    setDDSearchRes([]);setDDLoading(true);setDDResults([]);
    try{
      const ctx=buildCtx(profile);
      const system=`Du bist ein Streaming-Experte. Antworte NUR mit JSON-Array, kein Markdown. Format: [{"title":"...","reason":"...","type":"Serie/Film"}]`;
      const msg=`Erstelle 8 personalisierte Empfehlungen basierend auf "${titleStr}".\n\nNutzerprofil:\n${ctx.taste||"noch aufbauend"}\nLiebt: ${ctx.rated5||"noch nichts"}\nPlattformen: ${ctx.platforms}\n\nFinde Titel die thematisch ähnlich sind aber weniger bekannt. reason = witziger Satz Deutsch warum dieser Titel passt.`;
      const text=await callAI([{role:"user",content:msg}],system);
    ...
    <div>
      ...
      {ddResults.length>0&&(
        <div style={{display:"flex",flexDirection:"column",gap:8}}>
          {ddResults[0]?._rateLimit?<p style={{color:"#b0a8b8",fontSize:13,textAlign:"center",padding:12}}>🌙 Heute ausgebucht — morgen wieder verfügbar.</p>:ddResults.map((it,i)=><TitleCard key={it.id||i} item={it} profile={profile} onSelect={onSelect} onRate={cardProps.onRate} onLike={cardProps.onLike} onWatched={cardProps.onWatched} onBlock={cardProps.onBlock}/>)}
        </div>
      )}
    </div>
  );
  ...
  if(mode==="vibe")return(
    <div>
      ...
      {vibeResults.length>0&&(
        <div style={{display:"flex",flexDirection:"column",gap:8}}>
          {vibeResults[0]?._rateLimit?<p style={{color:"#b0a8b8",fontSize:13,textAlign:"center",padding:12}}>🌙 Heute ausgebucht — morgen wieder verfügbar.</p>:vibeResults.map((it,i)=><TitleCard key={it.id||i} item={it} profile={profile} onSelect={onSelect} onRate={cardProps.onRate} onLike={cardProps.onLike} onWatched={cardProps.onWatched} onBlock={cardProps.onBlock}/>)}
        </div>
      )}
    </div>
  );
