...
function getAIRecommendations(profile, mood, type) {
  var liked = (profile.watched || []).map(function(w) { return w.title; });
  var lovedTitles = (profile.lovedTitles || []);
  var hatedTitles = (profile.hatedTitles || []);
  var allLiked = lovedTitles.concat(liked).filter(function(t, i, a) { return a.indexOf(t) === i; });
  
  var thumbsUpGenres = [];
  Object.entries(profile.genres || {}).forEach(function(e) {
    if (e[1] > 2) {
      var g = GENRES_TMDB[parseInt(e[0])];
      if (g) thumbsUpGenres.push(g.name);
    }
  });
  var platforms = (profile.platforms || []).map(function(pid) {
    var p = PLATFORMS.find(function(x) { return x.id === pid; });
    return p ? p.name : pid;
  }).join(", ");

  var tasteInfo = profile.tasteProfile ? "Geschmacksprofil: " + profile.tasteProfile + ". " : "";
  
  var typeText = "";
  if (type === "serie") typeText = "Empfehle NUR Serien, keine Filme. ";
  if (type === "film") typeText = "Empfehle NUR Filme, keine Serien. ";

  var prompt = "Du bist ein Streaming-Experte. Empfehle genau 8 " + (type === "serie" ? "Serien" : type === "film" ? "Filme" : "Filme und Serien") + " die aktuell auf diesen Streaming-Diensten in Deutschland verfügbar sind: " + platforms + ". " +
    tasteInfo +
    typeText +
    (allLiked.length > 0 ? "Der User mag: " + allLiked.join(", ") + ". " : "") +
    (hatedTitles.length > 0 ? "Der User mag NICHT: " + hatedTitles.join(", ") + ". " : "") +
    (thumbsUpGenres.length > 0 ? "Bevorzugte Genres: " + thumbsUpGenres.join(", ") + ". " : "") +
    "Empfehle keine Titel die der User schon kennt. " +
    "Antworte NUR mit einem JSON-Array. Jedes Element hat: title (exakter Titel), year (Erscheinungsjahr als Zahl), type (\"movie\" oder \"tv\"), reason (1 Satz auf Deutsch warum der User das mögen wird), imdb (IMDb-Bewertung als Zahl z.B. 8.3). Kein anderer Text, nur das JSON-Array.";

  return askClaude(prompt).then(function(text) {
    try {
...
function getAISurprise(profile) {
  var liked = (profile.watched || []).map(function(w) { return w.title; });
  var lovedTitles = (profile.lovedTitles || []);
  var hatedTitles = (profile.hatedTitles || []);
  var allLiked = lovedTitles.concat(liked).filter(function(t, i, a) { return a.indexOf(t) === i; });
  ...
  var prompt = "Du bist ein witziger Streaming-Berater mit Humor. Der User kann sich nicht entscheiden. " +
    tasteInfo +
    "Verfügbare Dienste: " + platforms + ". " +
    (allLiked.length > 0 ? "Mag: " + allLiked.join(", ") + ". " : "") +
    (hatedTitles.length > 0 ? "Mag NICHT: " + hatedTitles.join(", ") + ". " : "") +
    (thumbsUpGenres.length > 0 ? "Genres: " + thumbsUpGenres.join(", ") + ". " : "") +
    "Empfehle EINEN einzigen Titel der perfekt passt und den der User wahrscheinlich noch nicht kennt. Antworte NUR mit JSON: {\"title\": \"...\", \"year\": 2024, \"type\": \"movie\" oder \"tv\", \"reason\": \"lustiger Spruch auf Deutsch warum genau DAS heute Abend geschaut werden muss (2-3 Sätze, locker und witzig)\", \"tagline\": \"kurzer witziger Einzeiler\", \"imdb\": 8.1}. Nur JSON, kein anderer Text.";
  ...
  return askClaude(prompt).then(function(text) {
    try {
      ...
      console.error("AI surprise parse error:", e, text);
      return null;
    }
  });
}
...
function MainApp(props) {
  ...
  function lookupTitlesOnTMDB(aiResults) {
    // Search each AI-recommended title on TMDB to get poster, score, etc.
    var lookupPromises = aiResults.map(function(rec) {
      return searchTitles(apiKey, rec.title + " " + rec.year).then(function(data) {
        var results = (data && data.results) || [];
        // Find best match
        var match = results.find(function(r) {
          return (r.media_type === rec.type) && (r.title === rec.title || r.name === rec.title || (r.title || r.name || "").toLowerCase().indexOf(rec.title.toLowerCase()) !== -1);
        });
        if (!match && results.length > 0) {
          match = results.find(function(r) { return r.media_type === rec.type; });
        }
        if (!match && results.length > 0) {
          match = results[0];
        }
        if (match) {
          match.media_type = match.media_type || rec.type;
          match._aiReason = rec.reason;
          match._imdb = rec.imdb || null;
          return match;
        }
        return null;
      }).catch(function() { return null; });
    });
    return Promise.all(lookupPromises).then(function(items) {
  ...
  function loadData() {
    ...
    var aiPromises = [
      getAIRecommendations(profile, null, null),      // For You (mixed)
      getAIRecommendations(profile, null, "serie"),    // Series
      getAIRecommendations(profile, null, "film"),     // Films
    ];

    Promise.all(aiPromises).then(function(aiResults) {
      var forYouAI = aiResults[0] || [];
      var seriesAI = aiResults[1] || [];
      var filmsAI = aiResults[2] || [];

      // Look up all on TMDB for posters, scores, etc.
      return Promise.all([
        lookupTitlesOnTMDB(forYouAI),
        lookupTitlesOnTMDB(seriesAI),
        lookupTitlesOnTMDB(filmsAI),
      ]);
      ...
      setForYou(filterDisliked(tmdbResults[0]).slice(0, 20));
      setSeries(filterDisliked(tmdbResults[1]).slice(0, 20));
      setFilms(filterDisliked(tmdbResults[2]).slice(0, 20));
      setTop([]); // Not needed with AI

      setLoading(false);
    }).catch(function(err) {
      console.error("AI/TMDB error:", err);
      // Fallback to basic TMDB discover
      fallbackLoadData();
    });
  }
  ...
  function fallbackLoadData() {
    ...
    var promises = [
      discoverTitles(apiKey, "serie", providerIds, null, 1, "popularity.desc"),
      discoverTitles(apiKey, "film", providerIds, null, 1, "popularity.desc"),
    ];
    Promise.all(promises).then(function(results) {
      var s = (results[0] && results[0].results) || [];
      s.forEach(function(i) { i.media_type = "tv"; });
      var f = (results[1] && results[1].results) || [];
      f.forEach(function(i) { i.media_type = "movie"; });
      setForYou(s.concat(f).slice(0, 20));
      setSeries(s.slice(0, 20));
      setFilms(f.slice(0, 20));
      setLoading(false);
  ...
  function handleSurprise() {
    ...
    getAISurprise(profile).then(function(aiResult) {
      if (!aiResult) { setSurpriseLoading(false); return; }
      
      // Look up on TMDB
      searchTitles(apiKey, aiResult.title + " " + aiResult.year).then(function(data) {
        var results = (data && data.results) || [];
        var match = results.find(function(r) { return r.media_type === aiResult.type; });
        if (!match && results.length > 0) match = results[0];
        
        if (match) {
          match.media_type = match.media_type || aiResult.type;
          match._surpriseMsg = aiResult.tagline || "🍿 Heute Abend schaust du...";
          match._aiReason = aiResult.reason;
          if (aiResult.imdb) match._imdb = aiResult.imdb;
        }
        
        setSurprise(match || { 
          title: aiResult.title, 
          name: aiResult.title,
          media_type: aiResult.type,
          _surpriseMsg: aiResult.tagline || "🍿 Heute Abend schaust du...",
          _aiReason: aiResult.reason,
          _imdb: aiResult.imdb || null,
          vote_average: 0,
          id: Math.random(),
          genre_ids: []
        });
    ...
    if (loading) {
      return (
        <div style={{ textAlign: "center", padding: 40 }}>
          ...
          <p style={{ fontSize: 15, color: "#c4b8a8", fontWeight: 600 }}>KI analysiert deinen Geschmack...</p>
          <p style={{ fontSize: 12, color: "#6a5e4e" }}>Durchsucht alle Streaming-Dienste</p>
        </div>
      );
    }
    if (items.length === 0) {
            ...
            <div style={{ background: "linear-gradient(135deg, #ff6b3515, #e8439315)", borderRadius: 20, padding: 20, marginBottom: 20, border: "1px solid #ff6b3525", textAlign: "center" }}>
              ...
              {surpriseLoading ? (
                <div>
                  ...
                  <p style={{ fontSize: 15, color: "#c4b8a8", marginTop: 8, fontWeight: 600 }}>KI grübelt für dich...</p>
                  <p style={{ fontSize: 12, color: "#6a5e4e" }}>Analysiert deinen Geschmack</p>
                </div>
              ) : null}
