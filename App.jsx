...
function Onboarding({onComplete}){
  ...
  async function loadSwipeItems(selectedPlats){
    ...
    const sortOptions=["popularity.desc","vote_average.desc","primary_release_date.desc"];
    const randomSort=sortOptions[Math.floor(Math.random()*sortOptions.length)];
    try{
      const [t1,t2,d1,d2,d3,d4]=await Promise.all([
        getTrending("tv"),
        getTrending("movie"),
        discoverTitles("serie",useIds,null,randomPage,"popularity.desc"),
        discoverTitles("film",useIds,null,randomPage,"popularity.desc"),
        discoverTitles("serie",useIds,null,randomPage+1,randomSort),
        discoverTitles("film",useIds,null,randomPage+1,randomSort),
      ]);
      const all=[
        ...
        ...(d3.results||[]).map(r=>({...r,media_type:"tv"})),
        ...(d4.results||[]).map(r=>({...r,media_type:"movie"})),
      ];
      const seen=new Set();
