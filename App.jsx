...
function PlatformCard({platform,profile,onSelect,onBlock,onLike,onRate,browseType}){
  ...
  useEffect(()=>{
    ...
    async function onSwipeDone(e){
      setOpen(true);
      const kiRecs=e.detail?.kiRecs||JSON.parse(localStorage.getItem("sf_ki_recs")||"[]");
      // Filter KI-Empfehlungen für diese Plattform
          ...
          if(valid.length>0){
            setRecs(valid.map(it=>({
              ...
              reason:platRecs.find(r=>titleKey(r.title)===titleKey(it.title||it.name||""))?.reason||
                     (it.overview?it.overview.substring(0,120)+"…":""),
              emoji:it.media_type==="tv"?"📺":"🎬",
              _tmdbItem:it,
            })));
            setLoading(false);
  ...
  async function handleDone(){
    ...
    if(liked_sw.length>0||disliked_sw.length>0){
      try{
        ...
        const msg=`Empfehle je 3-5 Titel pro Plattform basierend auf diesen Swipes:
