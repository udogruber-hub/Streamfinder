...
const GROQ_API_KEY = sGet("sf_groq_key") || "";

async function callClaude(messages,systemPrompt){
  const key = sGet("sf_groq_key");
  if(!key) throw new Error("Kein Groq API Key — bitte im ⚙️ Setup eintragen");
    ...
    <div style={{minHeight:"100vh",background:"linear-gradient(180deg,#121018,#0d0b14,#0a0812)",color:"#f0ece4",fontFamily:"'DM Sans',sans-serif",paddingBottom:100}}>
      ...
      {showSettings&&(
        <div style={{margin:"10px 18px",padding:14,background:"#12121f",borderRadius:14,border:"1px solid #1a1a2e"}}>
          <p style={{fontSize:12,color:"#ff6b35",fontWeight:700,margin:"0 0 8px"}}>🤖 Groq API Key</p>
          <input
            defaultValue={sGet("sf_groq_key")||""} 
            placeholder="gsk_..."
            onChange={e=>{sSet("sf_groq_key",e.target.value.trim());}}
            style={{width:"100%",background:"#0d0d18",border:"1px solid #2a2a3e",borderRadius:10,padding:"10px 12px",color:"#e8e6e1",fontFamily:"monospace",fontSize:12,outline:"none",marginBottom:10}}
          />
          <p style={{fontSize:11,color:"#555",marginBottom:10}}>Kostenlos auf groq.com — kein Cloudflare nötig!</p>
          <p style={{fontSize:12,color:"#777",margin:"0 0 10px"}}>Profil zurücksetzen?</p>
