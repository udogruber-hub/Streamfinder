  ...
  async function handleDone(){
    ...
    try{
      ...
      await Promise.all(platCandidates.map(async({plat,candidates})=>{
        ...
        const system=`Du bist ein Streaming-Experte. Antworte NUR mit JSON-Array, kein Markdown. Format: [{"title":"...","reason":"...","score":85}]. reason immer mit "du" — persönlich, nicht generisch.`;
        const msg=`Wähle die 10 besten Titel für diesen Nutzer:\nKANDIDATEN (alle auf ${plat.name} DE): ${titles}\nPROFIL: ${ctx.taste||"gemischt"}\n🟢 Top: ${ctx.rated5||"keine"}\n🔴 Nope: ${ctx.rated2||"keine"}\n❤️ Gemocht: ${liked_sw.join(", ")||"nichts"}\nGESEHEN: ${ctx.watched||"nichts"}\nNur aus Kandidaten wählen! reason = witziger Satz mit "du" — persönlich, nicht generisch. score = 40-95.`;
        try{
          const text=await callAI([{role:"user",content:msg}],system);
...
function SpotlightTab({profile,cardProps,onSelect}){
  ...
  async function loadFirstRecs(stimmung){
    ...
    try{
      ...
      const system=`Du bist ein Streaming-Experte. Antworte NUR mit JSON-Array, kein Markdown. Format: [{"title":"...","reason":"...","type":"Serie/Film"}]. reason = kurzer persönlicher Satz mit "du" — direkt, nicht generisch.`;
...
function Onboarding({onDone}){
  ...
  if(step===0){
    // Zufälliges düsteres Kinobild
    const heroImgs=[
      "https://image.tmdb.org/t/p/original/9faGSFi5jam6pDWGNd0p8JztGJY.jpg",  // Ozark
      "https://image.tmdb.org/t/p/original/ggFHVNu6YYI5L9pCfOacjizRGt.jpg",   // Breaking Bad
      "https://image.tmdb.org/t/p/original/suaEOtk1N1sgg2MTM7oZd2cfVp3.jpg",  // Mindhunter
      "https://image.tmdb.org/t/p/original/xMKPuCcKFCEpUCP4YFfuKAIrfWY.jpg",  // Blade Runner
      "https://image.tmdb.org/t/p/original/rqbCbjB19amtOtFQbb3K2lgm2zv.jpg",  // The Dark Knight
      "https://image.tmdb.org/t/p/original/vL5LR6WdxWPjLPFRLe133jXWsh5.jpg",  // The Godfather
    ];
    ...
    return(
      <div style={{minHeight:"100vh",background:"#09090f",display:"flex",flexDirection:"column",position:"relative",overflow:"hidden"}}>
        <img src={heroImg} alt="" style={{position:"absolute",inset:0,width:"100%",height:"100%",objectFit:"cover",objectPosition:"center 20%"}} onError={e=>{e.target.style.display="none";}}/>
        {/* Starkes, dramatisches Overlay */}
        <div style={{position:"absolute",inset:0,background:"linear-gradient(to bottom,rgba(0,0,0,0.5) 0%,rgba(0,0,0,0.2) 25%,rgba(9,9,15,0.75) 60%,#09090f 90%)"}}/>
        {/* Lila Glow oben links */}
        <div style={{position:"absolute",top:"-5%",left:"-5%",width:"50%",height:"45%",borderRadius:"50%",background:"radial-gradient(circle,rgba(124,58,237,0.25) 0%,transparent 70%)",pointerEvents:"none"}}/>
        {/* Pink Glow unten rechts */}
        <div style={{position:"absolute",bottom:"15%",right:"-10%",width:"40%",height:"35%",borderRadius:"50%",background:"radial-gradient(circle,rgba(232,67,147,0.2) 0%,transparent 70%)",pointerEvents:"none"}}/>
        {/* Content — alles unten */}
        <div style={{position:"relative",zIndex:1,marginTop:"auto",padding:"0 32px 56px"}}>
          <h1 style={{fontFamily:"'Instrument Serif',serif",fontSize:52,lineHeight:1.05,color:"#fff",margin:"0 0 16px",textShadow:"0 4px 32px rgba(0,0,0,0.8)",whiteSpace:"pre-line"}}>{sp.h}</h1>
          <p style={{fontSize:17,color:"rgba(255,255,255,0.6)",lineHeight:1.5,margin:"0 0 36px",fontFamily:"'DM Sans'",fontWeight:400}}>{sp.s}</p>
          <button onClick={()=>setStep(1)} style={{width:"100%",background:"linear-gradient(135deg,#7c3aed,#e84393)",border:"none",borderRadius:20,padding:"20px",color:"#fff",fontFamily:"'DM Sans'",fontWeight:800,fontSize:18,cursor:"pointer",boxShadow:"0 12px 48px rgba(124,58,237,0.6)",letterSpacing:"0.2px"}}>
            Loslegen →
          </button>
        </div>
      </div>
