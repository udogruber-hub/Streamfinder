...
function PlatformCard({platform,profile,onSelect,onBlock,onLike,onRate,browseType,onWatched,forceOpen}){
    ...
    try{
      ...
      if(stored){
        ...
        if(parsed&&parsed.active&&parsed.active.length>0){
          const watched3=new Set((profile.watched||[]).map(w=>w.id));
          const blocked3=new Set(profile.blocked_titles||[]);
          const active=parsed.active.filter(r=>!watched3.has(r.id)&&!blocked3.has(titleKey(r.title||"")));
          const reserve=(parsed.reserve||[]).filter(r=>!watched3.has(r.id)&&!blocked3.has(titleKey(r.title||"")));
  ...
  async function loadTMDBRecs(){
    ...
    try{
      ...
      const GOOD_LANGS=new Set(["de","en","fr","it","es"]);
      const seen=new Set();
      const filtered=all.filter(r=>{
        ...
        // Sprach-Filter + kein Anime
        if(r.original_language&&!GOOD_LANGS.has(r.original_language))return false;
      ...
      if(needMore>0){
        ...
        const filler=all.filter(r=>!existIds.has(r.id)&&!watched.has(r.id)&&GOOD_LANGS.has(r.original_language||"en")&&!(r.genre_ids||[]).includes(16)&&(r.vote_average||0)>=6.0).slice(0,needMore);
        top10.push(...filler);
      }
  ...
  async function replaceRec(removedTitle){
    ...
    setRecs(prev=>{
      ...
      if(reserveRef.current.length>0){
        const curIds=new Set((recsRef.current||[]).map(r=>r.id));
        const watched2=new Set((profile.watched||[]).map(w=>w.id));
        let next=null;
        while(reserveRef.current.length>0){
          const candidate=reserveRef.current.shift();
          if(!curIds.has(candidate.id)&&!watched2.has(candidate.id)){next=candidate;break;}
        }
  ...
  async function handleDone(){
    ...
    const platCandidates=await Promise.all(userPlats.map(async(plat,pi)=>{
      try{
        ...
        // Similar-Titles: nur die Genre-IDs extrahieren als Boost-Signal
        // Anbieter-Verfügbarkeit wird NUR über Discover sichergestellt
        const similarGenreBoost={};
        const similarTitleNames=[];
        similarPool.forEach(r=>{
          const ok=["de","en","fr","it","es"].includes(r.original_language||"en");
          if(!ok||(r.genre_ids||[]).includes(16))return;
          // Genre-Boost aus Similar-Titeln
          (r.genre_ids||[]).forEach(g=>{similarGenreBoost[g]=(similarGenreBoost[g]||0)+2;});
          if(r._similarTo)similarTitleNames.push(r.title||r.name||"");
        });

        // NUR Discover-Kandidaten (haben echten Anbieter-Filter via watch_region=DE)
        const seen=new Set();
        const allCandidates=[...genreCandidates].filter(r=>{
          if(seen.has(r.id))return false;seen.add(r.id);
          ...
          // Score: Similar-Genre-Boost + Profil-Genre-Match + Rating
          let sa=(a.vote_average||0)*2;
          let sb=(b.vote_average||0)*2;
          (a.genre_ids||[]).forEach(g=>{
            sa+=(mergedGenres[g]||0)*2;
            sa+=(similarGenreBoost[g]||0)*3; // Boost wenn ähnlich zu gelikten
          });
          (b.genre_ids||[]).forEach(g=>{
            sb+=(mergedGenres[g]||0)*2;
            sb+=(similarGenreBoost[g]||0)*3;
          });
          return sb-sa;
        });
