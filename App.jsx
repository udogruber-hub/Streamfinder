...
function BrowseTab({profile,cardProps,onSelect,onSwipe,onWatched}){
  ...
  useEffect(()=>{
    ...
    return()=>{window.removeEventListener("sf_open_swipe_inner",onOpenSwipe);};
  },[]);
...
function Onboarding({onDone}){
  ...
  if(step===0){
    return(
      <div style={{minHeight:"100vh",background:"#09090f",display:"flex",flexDirection:"column",position:"relative",overflow:"hidden"}}>
        <img src="https://image.tmdb.org/t/p/original/9faGSFi5jam6pDWGNd0p8JztGJY.jpg" alt="" style={{position:"absolute",inset:0,width:"100%",height:"100%",objectFit:"cover",objectPosition:"center 20%"}} onError={e=>{e.target.style.display="none";}}/>
        <div style={{position:"absolute",inset:0,background:"linear-gradient(to bottom,rgba(0,0,0,0.3) 0%,rgba(9,9,15,0.85) 65%,#09090f 100%)"}}/>
        <div style={{position:"absolute",top:"-10%",left:"50%",transform:"translateX(-50%)",width:"80%",height:"40%",borderRadius:"50%",background:"radial-gradient(circle,rgba(124,58,237,0.35) 0%,transparent 70%)",pointerEvents:"none"}}/>
        <div style={{position:"relative",zIndex:1,padding:"54px 28px 0",display:"flex",justifyContent:"center",alignItems:"center",gap:12}}>
          <div style={{width:46,height:46,borderRadius:15,background:"linear-gradient(135deg,#7c3aed,#e84393)",display:"flex",alignItems:"center",justifyContent:"center",boxShadow:"0 8px 28px rgba(124,58,237,0.6)"}}>
            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><polygon points="23 7 16 12 23 17 23 7"/><rect x="1" y="5" width="15" height="14" rx="2" ry="2"/></svg>
          </div>
          <span style={{fontFamily:"'Instrument Serif',serif",fontSize:28,color:"#fff",letterSpacing:"-0.5px",textShadow:"0 2px 12px rgba(0,0,0,0.5)"}}>StreamFinder</span>
        </div>
        <div style={{position:"relative",zIndex:1,marginTop:"auto",padding:"0 28px 52px"}}>
          <h1 style={{fontFamily:"'Instrument Serif',serif",fontSize:46,lineHeight:1.08,color:"#fff",margin:"0 0 14px",textShadow:"0 2px 24px rgba(0,0,0,0.6)"}}>Dein persönlicher<br/>Streaming-Guide.</h1>
          <p style={{fontSize:16,color:"rgba(255,255,255,0.65)",lineHeight:1.55,margin:"0 0 24px",fontFamily:"'DM Sans'"}}>KI-Empfehlungen die wirklich passen.<br/>Kein endloses Scrollen mehr.</p>
          <div style={{display:"flex",gap:8,flexWrap:"wrap",marginBottom:28}}>
            {["✨ KI-Empfehlungen","🎴 Swipe & Entdecken","🎭 Dein Filmtyp"].map(t=>(
              <div key={t} style={{background:"rgba(255,255,255,0.1)",backdropFilter:"blur(16px)",WebkitBackdropFilter:"blur(16px)",border:"1px solid rgba(255,255,255,0.2)",borderRadius:22,padding:"7px 16px",fontSize:13,color:"rgba(255,255,255,0.92)",fontWeight:600,fontFamily:"'DM Sans'"}}>{t}</div>
            ))}
          </div>
          <button onClick={()=>setStep(1)} style={{width:"100%",background:"linear-gradient(135deg,#7c3aed,#e84393)",border:"none",borderRadius:20,padding:"19px",color:"#fff",fontFamily:"'DM Sans'",fontWeight:800,fontSize:18,cursor:"pointer",boxShadow:"0 10px 40px rgba(124,58,237,0.55)"}}>Jetzt starten →</button>
          <p style={{textAlign:"center",fontSize:12,color:"rgba(255,255,255,0.22)",marginTop:13,fontFamily:"'DM Sans'"}}>Kostenlos · Keine Registrierung</p>
        </div>
      </div>
    );
  }
    ...
    return(
      <div style={{minHeight:"100vh",background:"linear-gradient(180deg,#09090f,#0d0d1a)",display:"flex",flexDirection:"column"}}>
        <div style={{padding:"48px 24px 20px",textAlign:"center"}}>
          <div style={{width:56,height:56,borderRadius:18,background:"linear-gradient(135deg,#7c3aed,#e84393)",display:"flex",alignItems:"center",justifyContent:"center",margin:"0 auto 14px",boxShadow:"0 8px 28px rgba(124,58,237,0.5)"}}>
            <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><polygon points="23 7 16 12 23 17 23 7"/><rect x="1" y="5" width="15" height="14" rx="2" ry="2"/></svg>
          </div>
          <h1 style={{fontFamily:"'Instrument Serif',serif",fontSize:30,margin:"0 0 4px",background:"linear-gradient(135deg,#c4a960,#f5e090)",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent"}}>StreamFinder</h1>
          <p style={{fontSize:11,color:"#5a5468",margin:0,letterSpacing:"1.5px",textTransform:"uppercase"}}>Schritt 1 von 2</p>
        </div>
        <div style={{flex:1,padding:"0 20px",overflowY:"auto"}}>
          <h2 style={{fontFamily:"'Instrument Serif',serif",fontSize:26,color:"#f0ece4",margin:"0 0 6px"}}>Deine Anbieter</h2>
          <p style={{fontSize:13,color:"#7a7488",marginBottom:18}}>Welche Streaming-Dienste nutzt du?</p>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
            {PLATFORMS.map(p=>{
              const sel=selPlats.includes(p.id);
              return(<button key={p.id} onClick={()=>togglePlat(p.id)} style={{background:sel?p.color+"22":"rgba(255,255,255,0.04)",border:"2px solid "+(sel?p.color:"rgba(255,255,255,0.08)"),borderRadius:16,padding:"16px 12px",cursor:"pointer",textAlign:"center",transition:"all 0.2s",boxShadow:sel?"0 0 24px "+p.color+"44":"none",position:"relative"}}>
                {sel&&<div style={{position:"absolute",top:8,right:8,width:20,height:20,borderRadius:10,background:p.color,display:"flex",alignItems:"center",justifyContent:"center"}}><svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth="3.5"><polyline points="20 6 9 17 4 12"/></svg></div>}
                <div style={{fontSize:24,fontWeight:900,color:p.color,marginBottom:6}}>{p.icon}</div>
        ...
        <div style={{padding:"20px 20px 16px",display:"flex",gap:10}}>
          ...
          <button onClick={()=>setStep(2)} disabled={selPlats.length===0} style={{flex:1,background:"linear-gradient(135deg,#ff6b35,#e84393)",border:"none",borderRadius:14,padding:"14px",cursor:"pointer",color:"#fff",fontFamily:"'DM Sans'",fontWeight:800,fontSize:15,opacity:selPlats.length===0?0.4:1}}>Weiter →</button>
        </div>
