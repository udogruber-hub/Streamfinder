    ...
    <div style={{background:"#12121f",borderRadius:18,overflow:"hidden",border:"1px solid #1e1e30",marginBottom:10}}>
      ...
      {open&&mode!=="swipe"&&(
        <div style={{padding:"0 14px 16px"}}>
          ...
          {!loading&&recs&&recs.length>0&&(
            <p style={{fontSize:10,color:"#3a3a4a",margin:"0 0 10px",textAlign:"right",letterSpacing:"0.3px"}}>← wischen zum Ausblenden</p>
          )}
          {!loading&&recs&&recs.map((rec,i)=>{
            ...
            return(
              <SwipeToBlock key={i} onBlock={()=>{onBlock(rec.title);setRecs(prev=>prev.filter((_,j)=>j!==i));}}>
              <div style={{borderRadius:16,overflow:"hidden",marginBottom:10,position:"relative",background:"#0d0d18",border:"1px solid #1e1e30"}}>
        ...
        {tab==="home"&&(
          <div>
                  ...
                  return(
                    <button key={m.id} onClick={()=>applyMood(m)}
                      style={{
                        ...
                        border:"1px solid "+(active?m.color+"88":m.color+"33"),
                        borderRadius:8,padding:"6px 14px",cursor:"pointer",
                        color:active?m.color:"#888",
                        fontFamily:"'DM Sans'",fontWeight:active?700:500,
            ...
            {feedLoading?(
              ...
              <div style={{padding:"0 18px"}}>
                {heroItems.length>0&&<div style={{marginBottom:10}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}><p style={{fontSize:11,color:"#b0a8b8",fontWeight:700,textTransform:"uppercase",letterSpacing:0.8,margin:0}}>Top-Picks</p><span style={{fontSize:10,color:"#3a3a4a",letterSpacing:"0.3px"}}>← wischen zum Ausblenden</span></div>{heroItems.map(it=><SwipeToBlock key={it.id} onBlock={()=>cardProps.onBlock(it.title||it.name||"")}><HeroCard item={it} {...cardProps}/></SwipeToBlock>)}</div>}
