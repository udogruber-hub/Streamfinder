...
function TitleCard(props) {
  ...
  return (
    <div style={{
      background: "linear-gradient(145deg, #14142a, #0f0f1e)", borderRadius: 20, overflow: "hidden",
      border: expanded ? "1px solid " + (pl ? pl.color : "#B01EFF") + "44" : "1px solid #1e1e35",
      ...
      <div onClick={handleExpand} style={{ padding: "16px", display: "flex", alignItems: "flex-start", gap: 14, cursor: "pointer" }}>
        {rank !== undefined ? (
          <span style={{ width: 28, height: 28, borderRadius: 8, flexShrink: 0, background: rank < 3 ? "linear-gradient(135deg, #f5c518, #ff8c00)" : "#1a1a2e", border: "none", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13, fontWeight: 900, color: rank < 3 ? "#000" : "#555", marginTop: 8 }}>{rank + 1}</span>
        ) : null}
        {poster ? (
          <img src={poster} alt="" style={{ width: 56, height: 84, borderRadius: 12, objectFit: "cover", flexShrink: 0, background: "#1a1a2e", boxShadow: "0 4px 12px #00000066" }} />
        ) : (
          <div style={{ width: 56, height: 84, borderRadius: 12, flexShrink: 0, background: "linear-gradient(135deg, #1a1a2e, #252540)", display: "flex", alignItems: "center", justifyContent: "center", color: "#444", fontSize: 24 }}>🎬</div>
        )}
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 5 }}>
            <h3 style={{ margin: 0, fontSize: 17, fontWeight: 800, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", letterSpacing: "-0.3px" }}>{title}</h3>
            {isLiked ? <span style={{ fontSize: 13 }}>❤️</span> : null}
          </div>
          <div style={{ display: "flex", gap: 6, alignItems: "center", flexWrap: "wrap", marginBottom: 6 }}>
            <span style={{ fontSize: 12, color: "#aaa", background: "#1e1e35", padding: "3px 10px", borderRadius: 8, fontWeight: 700 }}>
              {mediaType === "tv" ? "📺 Serie" : "🎬 Film"}
            </span>
            {year ? <span style={{ fontSize: 12, color: "#666", fontWeight: 600 }}>{year}</span> : null}
          ...
          <div style={{ display: "flex", gap: 5, flexWrap: "wrap", marginBottom: 6 }}>
            {genreIds.slice(0, 3).map(function(gid) {
              ...
              return g ? <span key={gid} style={{ fontSize: 11, color: "#888", fontWeight: 500 }}>{g.emoji} {g.name}</span> : null;
            })}
          </div>
          {item.overview ? (
            <p style={{ margin: "0 0 6px", fontSize: 13, color: "#777", lineHeight: 1.5, display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical", overflow: "hidden" }}>
              {item.overview}
            </p>
          ) : null}
          <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
            ...
            {item.vote_count ? <span style={{ fontSize: 11, color: "#555" }}>({item.vote_count > 1000 ? Math.round(item.vote_count / 1000) + "K" : item.vote_count})</span> : null}
          </div>
        </div>
        <div style={{ color: "#3a3a55", fontSize: 16, flexShrink: 0, transition: "transform 0.3s", transform: expanded ? "rotate(180deg)" : "rotate(0)", marginTop: 8 }}>▾</div>
      </div>
...
function MainApp(props) {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "linear-gradient(180deg, #0a0a14, #08081a)", color: "#e8e6e1", fontFamily: "'DM Sans', sans-serif", paddingBottom: 100 }}>
      <link href="https://fonts.googleapis.com/css2?family=DM+Sans:wght@400;500;700&family=Instrument+Serif&display=swap" rel="stylesheet" />

      {/* Header */}
      <div style={{ padding: "20px 20px 0", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <h1 style={{ fontFamily: "'Instrument Serif', serif", fontSize: 30, margin: 0, background: "linear-gradient(135deg, #E50914, #B01EFF)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", letterSpacing: "-0.5px" }}>StreamFinder</h1>
        <button onClick={function() { setSS(!showSettings); }} style={{ background: "linear-gradient(135deg, #14142a, #1e1e35)", border: "1px solid #2a2a40", borderRadius: 12, width: 42, height: 42, cursor: "pointer", color: "#888", fontSize: 18, display: "flex", alignItems: "center", justifyContent: "center" }}>⚙️</button>
      </div>
            ...
            {MOODS.map(function(m) {
              return (
                <button key={m.id} onClick={function() { changeMood(m.id); }} style={{
                  background: mood === m.id ? "linear-gradient(135deg, #E5091428, #B01EFF28)" : "linear-gradient(135deg, #14142a, #1e1e35)",
                  border: mood === m.id ? "1px solid #B01EFF55" : "1px solid #1e1e35",
                  borderRadius: 14, padding: "10px 16px", cursor: "pointer",
                  color: mood === m.id ? "#fff" : "#666",
      ...
      <div style={{ padding: "16px 18px" }}>
        {tab === "foryou" ? (
          <div>
            <h3 style={{ fontSize: 20, fontWeight: 800, marginBottom: 14 }}>✨ Empfohlen für dich</h3>
      ...
      {/* Bottom Nav */}
      <div style={{ position: "fixed", bottom: 0, left: 0, right: 0, background: "linear-gradient(180deg, transparent, #0a0a0f 20%)", backdropFilter: "blur(20px)", borderTop: "1px solid #1e1e35", display: "flex", padding: "10px 8px 16px", zIndex: 100 }}>
        {tabs.map(function(t) {
          var isActive = tab === t.id;
          return (
            <button key={t.id} onClick={function() { setTab(t.id); }} style={{
              flex: 1, background: isActive ? "linear-gradient(135deg, #E5091418, #B01EFF18)" : "transparent",
              border: isActive ? "1px solid #B01EFF33" : "1px solid transparent",
              borderRadius: 14, cursor: "pointer", display: "flex", flexDirection: "column",
              alignItems: "center", gap: 3, padding: "8px 4px",
              color: isActive ? "#B01EFF" : "#555",
              transition: "all 0.2s",
            }}>
