...
function SpotlightTab({profile,cardProps,onSelect}){
  ...
  const stimmungCache=useRef({}); // Cache pro Stimmung

  // Auto-load beim ersten Öffnen — kombiniert Swipe + Stimmung
  // Remove from recs when watched or rated badly
  useEffect(()=>{
    function onWatched(e){
      ...
      setStimmungRecs(prev=>{
        ...
        if(filtered.length<prev.length&&recReserve.current.length>0){
  ...
  async function loadFirstRecs(stimmung){
    ...
    // Init shown titles from watched + disliked
    shownTitles.current=new Set([
      ...(profile.watched||[]).map(w=>w.id),
    ]);
    ...
    try{
      ...
      const valid=enriched.filter(Boolean)
        .filter(it=>!shownTitles.current.has(it.id))
        .map((it,i)=>({...it,_aiReason:recs[i]?.reason}));
      ...
      try{
        const allIds=(profile.platforms||[]).flatMap(id=>PLATFORMS.find(p=>p.id===id)?.tmdbIds||[]);
        // TMDB Fallback — striktes Genre-Filtering
        const genre=stimmung.genres[0];
        const page=Math.ceil(Math.random()*5);
        const [fMovies,fTv]=await Promise.all([
          fetch(TMDB_BASE+"/discover/movie?api_key="+TMDB_API_KEY+"&language=de-DE&sort_by=vote_average.desc&vote_count.gte=100&with_genres="+genre+"&page="+page).then(r=>r.json()),
          fetch(TMDB_BASE+"/discover/tv?api_key="+TMDB_API_KEY+"&language=de-DE&sort_by=vote_average.desc&vote_count.gte=50&with_genres="+genre+"&page="+page).then(r=>r.json()),
        ]);
        const fallbackItems=[
          ...(fMovies.results||[]).map(it=>({...it,media_type:"movie"})),
          ...(fTv.results||[]).map(it=>({...it,media_type:"tv"})),
        ].filter(it=>
          it.poster_path&&
          !shownTitles.current.has(it.id)&&
          // Sicherstellen dass Genre wirklich passt
          (it.genre_ids||[]).includes(genre)
...
Kombiniere Stimmung und persönlichen Geschmack.`;
      ...
      try{
        const genre2=stimmung.genres[0];
        const page2=Math.ceil(Math.random()*5);
        const [res2m,res2t]=await Promise.all([
          fetch(TMDB_BASE+"/discover/movie?api_key="+TMDB_API_KEY+"&language=de-DE&sort_by=vote_average.desc&vote_count.gte=100&with_genres="+genre2+"&page="+page2).then(r=>r.json()),
          fetch(TMDB_BASE+"/discover/tv?api_key="+TMDB_API_KEY+"&language=de-DE&sort_by=vote_average.desc&vote_count.gte=50&with_genres="+genre2+"&page="+page2).then(r=>r.json()),
        ]);
        const items2=[
          ...(res2m.results||[]).map(it=>({...it,media_type:"movie"})),
          ...(res2t.results||[]).map(it=>({...it,media_type:"tv"})),
        ].filter(it=>it.poster_path&&(it.genre_ids||[]).includes(genre2))
         .sort((a,b)=>(b.vote_average||0)-(a.vote_average||0));
        const toShow2=items2.slice(0,3);
        recReserve.current=items2.slice(3);
        stimmungCache.current[stimmung.id]={recs:toShow2,reserve:items2.slice(3)};
        setStimmungRecs(toShow2);
      }catch{}
    }
    setStimmungLoading(false);
  }
...
{/* Stimmungsleiste — Ausklappbar */}
        {!activeFeature&&(
          <div style={{flexShrink:0,marginBottom:4}}>
            {/* Header mit Toggle */}
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:10}}>
              <p style={{fontSize:10,color:"#5a5468",margin:0,letterSpacing:"0.8px",textTransform:"uppercase"}}>Was schaust du heute?</p>
              <button onClick={()=>setGenreExpanded(v=>!v)}
                style={{background:"rgba(255,255,255,0.05)",border:"1px solid rgba(255,255,255,0.08)",borderRadius:8,padding:"4px 10px",cursor:"pointer",display:"flex",alignItems:"center",gap:5}}>
            ...
            {/* Grid — 2 Zeilen sichtbar, rest ausklappbar */}
              ...
              {STIMMUNGEN.map(m=>{
                ...
                return(
                  <button key={m.id} onClick={()=>{
                    ...
                    <img src={m.tmdbImg} alt="" style={{position:"absolute",inset:0,width:"100%",height:"100%",objectFit:"cover",opacity:active?0.7:0.4,transition:"opacity 0.25s"}}
              ...
              <div style={{display:"flex",flexDirection:"column",gap:8}}>
                ...
                {stimmungRecs.slice(0,3).map((it,i)=>(
                  <SwipeToBlock key={it.id||i} onBlock={async()=>{
                    const blockedId=it.id;
                    shownTitles.current.add(blockedId||titleKey(it.title||it.name||""));
                    setStimmungRecs(prev=>{
                      const filtered=prev.filter((_,j)=>j!==i);
                      let next=null;
                      // Aus Reserve nehmen
                      if(recReserve.current.length>0){
                        next=recReserve.current.shift();
                        shownTitles.current.add(next.id);
                      }
                      const newRecs=next?[...filtered,next]:filtered;
                      // Cache aktualisieren
                      if(activeStimmung&&stimmungCache.current[activeStimmung]){
                        stimmungCache.current[activeStimmung]={
                          recs:newRecs,
                          reserve:[...recReserve.current]
                        };
                      }
                      return newRecs;
                    });
                    // Wenn Reserve leer — TMDB Fallback mit Genre-Check
                    if(recReserve.current.length===0){
                      try{
                        ...
                        const fresh=[
                          ...(rm.results||[]).map(it=>({...it,media_type:"movie"})),
                          ...(rt.results||[]).map(it=>({...it,media_type:"tv"})),
                        ].filter(it=>
                          it.poster_path&&
                          !shownTitles.current.has(it.id)&&
                          (it.genre_ids||[]).includes(genre) // Genre-Check!
                        );
                        fresh.slice(0,5).forEach(it=>shownTitles.current.add(it.id));
                        recReserve.current=fresh.slice(1,5);
                        if(fresh.length>0){
                          setStimmungRecs(prev=>{
                            const withNew=[...prev,fresh[0]];
                            if(activeStimmung&&stimmungCache.current[activeStimmung]){
                              stimmungCache.current[activeStimmung]={recs:withNew,reserve:[...recReserve.current]};
                            }
                            return withNew;
                          });
                        }
...
const STIMMUNGEN=[
  {id:"action",    label:"Action",          color:"#ff6b35", genres:[28],    tmdbImg:"https://image.tmdb.org/t/p/w780/8Y43POKjjKDGI9MH89NW0NAzzp8.jpg"}, // Mad Max
  {id:"comedy",    label:"Komödie",         color:"#f59e0b", genres:[35],    tmdbImg:"https://image.tmdb.org/t/p/w780/zAHthUkAe2p1EVh7ezJIXuCB3dT.jpg"}, // Superbad
  {id:"thriller",  label:"Thriller",        color:"#ef4444", genres:[53],    tmdbImg:"https://image.tmdb.org/t/p/w780/pB8BM7pdSp6B6Ih7QZ4DrQ3PmJK.jpg"}, // No Country
  {id:"drama",     label:"Drama",           color:"#e84393", genres:[18],    tmdbImg:"https://image.tmdb.org/t/p/w780/3bhkrj58Vtu7enYsLeMMovMAKAg.jpg"}, // Shawshank
  {id:"horror",    label:"Horror",          color:"#6366f1", genres:[27],    tmdbImg:"https://image.tmdb.org/t/p/w780/ArZ5bAkOSBHFYYQH4UrVQm4KVOM.jpg"}, // The Shining
  {id:"scifi",     label:"Sci-Fi",          color:"#a78bfa", genres:[878],   tmdbImg:"https://image.tmdb.org/t/p/w780/xJHokMbljvjADYdit5fK5VQsXEG.jpg"}, // Interstellar
  {id:"fantasy",   label:"Fantasy",         color:"#4ade80", genres:[14],    tmdbImg:"https://image.tmdb.org/t/p/w780/56v2KjBlU4XaOv9rVYEQypROD7P.jpg"}, // LOTR
  {id:"mystery",   label:"Mystery",         color:"#22d3ee", genres:[9648],  tmdbImg:"https://image.tmdb.org/t/p/w780/cWUOv3H7YFwvKeaQhoAQTLLpo9Z.jpg"}, // Gone Girl
  {id:"doku",      label:"Dokumentation",   color:"#34d399", genres:[99],    tmdbImg:"https://image.tmdb.org/t/p/w780/2zl7HBCkmqZXYnXNWVTqX7oFkbb.jpg"}, // Free Solo
  {id:"romance",   label:"Romantik",        color:"#f472b6", genres:[10749], tmdbImg:"https://image.tmdb.org/t/p/w780/rSPw7tgCH9c6NqICZef4kZjFOQ5.jpg"}, // La La Land
  {id:"krimi",     label:"Krimi",           color:"#fb923c", genres:[80],    tmdbImg:"https://image.tmdb.org/t/p/w780/d5NXSklpcvkD173vtvzrmOd4UPq.jpg"}, // The Godfather
  {id:"abenteuer", label:"Abenteuer",       color:"#60a5fa", genres:[12],    tmdbImg:"https://image.tmdb.org/t/p/w780/gEU2QniE6E77NI6lCU6MxlNBvIx.jpg"}, // Indiana Jones
];
  ...
  if(step===2)return(
    <div style={{minHeight:"100vh",background:"#09090f",fontFamily:"'DM Sans',sans-serif",padding:"48px 24px 32px",overflowY:"auto"}}>
      ...
      <div style={{maxWidth:440,margin:"0 auto"}}>
        ...
        <button onClick={async()=>{if(stimmungen.length===0)return;await loadSwipeItems();setStep(3);}} disabled={stimmungen.length===0}
      ...
      <div style={{minHeight:"100vh",background:"#09090f",fontFamily:"'DM Sans',sans-serif",display:"flex",flexDirection:"column"}}>
        ...
        {!swipeLoading&&cur&&(
          <div style={{flex:1,display:"flex",flexDirection:"column",padding:"12px 20px 20px",maxWidth:440,margin:"0 auto",width:"100%",minHeight:0}}>
            {/* Swipe Card — Backdrop Querformat, kein Abschneiden */}
            <div style={{borderRadius:20,overflow:"hidden",position:"relative",background:"#0a0a12",flexShrink:0,aspectRatio:"16/9"}}>
              {cur.backdrop_path
                ?<img src={"https://image.tmdb.org/t/p/w780"+cur.backdrop_path} alt="" style={{width:"100%",height:"100%",objectFit:"contain",display:"block",background:"#0a0a12"}}/>
                :cur.poster_path
                  ?<img src={"https://image.tmdb.org/t/p/w500"+cur.poster_path} alt="" style={{width:"100%",height:"100%",objectFit:"contain",display:"block",background:"#0a0a12"}}/>
            ...
            <button onClick={()=>finishOnboarding(swipeRatings,swipeGenres)}
...
function MainApp({profile,onReset}){
  ...
  useEffect(()=>{
    function onOpenSwipe(){setShowSwipeModal(true);}
