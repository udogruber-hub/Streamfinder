...
function HelpModal({onClose,activeTab}){
  ...
  const HELP={
    home:{
      title:"✨ Für dich",
      intro:"Deine persönliche Startseite. Je mehr du bewertest, desto besser die Empfehlungen.",
      items:[
        {icon:"🎭",title:"Stimmung wählen",text:"Tippe eine Stimmungskategorie an — die Empfehlungen passen sich sofort an. Nochmal tippen = zurück zu deinen Picks."},
        {icon:"🔄",title:"Empfehlungen aktualisieren",text:"Der Button oben rechts lädt frische Picks basierend auf deinen aktuellen Bewertungen. Je öfter du bewertest, desto besser."},
        {icon:"⭐",title:"Sterne bewerten",text:"1-2★ = nie wieder empfehlen · 3★ = okay · 4-5★ = mehr davon! Der Algorithmus merkt sich alles."},
        {icon:"✕",title:"Interessiert mich nicht",text:"Tippe den Button und der Titel verschwindet sofort — und kommt nie wieder als Empfehlung."},
        {icon:"👁",title:"Als gesehen markieren",text:"Markiere was du schon gesehen hast — diese Titel werden nicht mehr empfohlen. Im Verlauf-Tab siehst du alles."},
        {icon:"🤍",title:"Merken",text:"Was dich interessiert aber noch nicht gesehen ist. Alles in der Merkliste gesammelt."},
      ]
    },
    browse:{
      title:"🔍 Entdecken",
      intro:"Drei Wege zum perfekten Titel — alle nutzen dein Profil.",
      items:[
        {icon:"📺",title:"Anbieter durchstöbern",text:"Top 5 passende Titel pro Plattform — nach deinen Bewertungen sortiert. Plus Swipe-Modus: rechts = merken, links = nicht interessant."},
        {icon:"🔭",title:"Deep Dive",text:"Gib einen Lieblingstitel ein — und entdecke sein ganzes Universum. Gleicher Regisseur, ähnliche Stimmung, verborgene Perlen."},
        {icon:"🎛️",title:"Vibe-Meter",text:"Drei Regler: Qualität, Komplexität, Stimmung. Einstellen, absenden — 4 Picks die exakt zu deinem heutigen Vibe passen."},
        {icon:"💬",title:"StreamBot",text:"Einfach tippen was du willst: 'Wie Breaking Bad aber lustiger' oder 'Kurzfilm für heute Abend'. Der Bot antwortet mit konkreten Picks."},
        {icon:"🔄",title:"Aktualisieren",text:"Jeder Bereich hat einen Aktualisierungsbutton — tippe ihn für frische Empfehlungen basierend auf deinem aktuellen Profil."},
      ]
    },
    fun:{
      title:"🎲 Spaß-Ecke",
      intro:"Wenn normales Suchen zu langweilig ist.",
      items:[
        {icon:"🎲",title:"Überrasch mich!",text:"Du kannst dich nicht entscheiden? Wir entscheiden. Die KI wählt einen Titel basierend auf deinem Profil — mit dramatischer Begründung."},
        {icon:"🔮",title:"Das Orakel",text:"Unabhängig vom Würfel. Das Orakel wählt etwas Überraschendes — mystisch begründet. Manchmal trifft es den Nagel auf den Kopf."},
        {icon:"🧠",title:"Dein Streaming-Typ",text:"Die KI analysiert deine Bewertungen und gibt dir deinen persönlichen Streaming-Persönlichkeitstyp. Mit Schwächen. Ehrlich."},
        {icon:"💡",title:"Tipp",text:"Je mehr du bewertet hast, desto treffsicherer werden alle drei Features. Mit leerem Profil rät die KI nur."},
      ]
    },
    liked:{
      title:"❤️ Merkliste",
      intro:"Alles was dich interessiert — auf einen Blick.",
      items:[
        {icon:"🤍",title:"Titel merken",text:"Bei jedem Titel auf 'Merken' tippen. Der Titel landet hier — auch ohne ihn gesehen zu haben."},
        {icon:"❤️",title:"Gemerkt",text:"Rote Herzen zeigen bereits gemerkte Titel an. Nochmal tippen = entfernen."},
        {icon:"💡",title:"Tipp",text:"Die Merkliste ist deine Watchlist. Sie beeinflusst keine Empfehlungen — dafür nutze Sterne-Bewertungen und 'Als gesehen'."},
      ]
    },
    history:{
      title:"📋 Verlauf & Profil",
      intro:"Dein Streaming-Gedächtnis und Geschmacksprofil.",
      items:[
        {icon:"✅",title:"Gesehene Titel",text:"Alle Titel die du als gesehen markiert hast. Tippe ✕ um einen zu entfernen. Gesehene Titel werden nicht mehr empfohlen."},
        {icon:"🚫",title:"Blockierte Titel",text:"Alle 'Nicht mein Ding' Entscheidungen. Tippe auf einen um ihn zu entsperren."},
        {icon:"📊",title:"Geschmacksprofil",text:"Der Balken-Graph zeigt deine Top-Genres basierend auf allem was du bewertet und gesehen hast. Je voller, desto besser die Empfehlungen."},
        {icon:"💡",title:"Tipp",text:"Mehr bewerten = besseres Profil = bessere Empfehlungen. Das Profil wird bei jeder Bewertung automatisch aktualisiert."},
      ]
    },
  };
    ...
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.92)",zIndex:200,overflowY:"auto"}} onClick={onClose}>
      <div style={{background:"linear-gradient(180deg,#13121f,#09090f)",minHeight:"100vh",maxWidth:560,margin:"0 auto",paddingBottom:100}} onClick={e=>e.stopPropagation()}>
        ...
        <div style={{padding:"0 20px"}}>
          ...
          <button onClick={()=>{onBlock(title);onClose();}} style={{width:"100%",background:"transparent",border:"none",color:"#555",cursor:"pointer",fontFamily:"'DM Sans'",fontWeight:500,fontSize:12,padding:"6px 0",display:"flex",alignItems:"center",justifyContent:"center",gap:5}}>
            <span>✕</span> 👻 Nicht mein Ding
          </button>
    ...
    <div style={{borderRadius:24,overflow:"hidden",border:"1px solid #1e1e30",marginBottom:14,cursor:"pointer"}} onClick={()=>onSelect(item)}>
      ...
      <div style={{background:"linear-gradient(145deg,#13121f,#0f0e1a)",padding:"14px 16px"}}>
        ...
        <div style={{display:"flex",gap:6,marginTop:12,flexWrap:"wrap"}}>
          ...
          <button onClick={e=>{e.stopPropagation();onBlock(title);}} style={{padding:"9px 10px",borderRadius:12,background:"#1a1a2e",border:"1px solid #2a2340",color:"#555",cursor:"pointer",fontFamily:"'DM Sans'",fontWeight:500,fontSize:11,display:"flex",alignItems:"center",gap:4,whiteSpace:"nowrap"}}>👻 Nicht mein Ding</button>
        </div>
      </div>
    </div>
  );
...
function PlatformSwipe({platform,profile,browseType,onBlock,onLike,onSelect}){
  ...
  useEffect(()=>{
    ...
    const rs=sorts[Math.floor(Math.random()*sorts.length)];

    const loadWithGenre=genreStr
      ?discoverTitles(browseType,platform.tmdbIds,genreStr,rp1,"popularity.desc")
      :Promise.resolve({results:[]});
    const loadWithoutGenre=discoverTitles(browseType,platform.tmdbIds,null,rp1,rs);
    const loadPage2=discoverTitles(browseType,platform.tmdbIds,null,rp2,"vote_average.desc");
    ...
    Promise.all([loadWithGenre,loadWithoutGenre,loadPage2]).then(async([r1,r2,r3])=>{
      ...
      const filtered=filterItems(deduped).slice(0,20);
...
function PlatformCard({platform,profile,onSelect,onBlock,onLike,browseType}){
  ...
  function handleRefresh(){
    if(mode==="top5"){
      lastTypeRef.current=null; // Reset so it reloads
      loadTop5Recs();
    }
    if(mode==="swipe"){
      // Reset swipe state by toggling mode
      setMode(null);
      setTimeout(()=>setMode("swipe"),50);
    }
  }

  const REFRESH_LABELS=["🔮 Neue Picks","✨ Frisch laden","🎲 Nochmal","🔄 Aktualisieren"];
  const refreshLabel=REFRESH_LABELS[Math.floor(Math.random()*REFRESH_LABELS.length)];

  // Auto-load top5 on first render
  useEffect(()=>{
  ...
  return(
    <div style={{background:"#12121f",borderRadius:18,overflow:"hidden",border:"1px solid #1e1e30",marginBottom:10}}>
      {/* Header — klickbar */}
      <div style={{padding:"14px 16px",display:"flex",alignItems:"center",gap:12,cursor:"pointer"}} onClick={()=>handleMode("top5")}>
        ...
        <div style={{flex:1}}>
          ...
          <div style={{fontSize:11,color:"#b0a8b8"}}>Tippen für Empfehlungen</div>
        </div>
        <div style={{display:"flex",gap:5}} onClick={e=>e.stopPropagation()}>
          <button onClick={()=>handleMode("top5")} style={{background:mode==="top5"?"rgba(255,107,53,0.13)":"#1a1a2e",border:mode==="top5"?"1px solid rgba(255,107,53,0.27)":"1px solid #2a2340",borderRadius:10,padding:"7px 8px",color:mode==="top5"?"#ff6b35":"#b0a8b8",cursor:"pointer",fontSize:11,fontFamily:"'DM Sans'",fontWeight:700}}>🎯 Top 5</button>
          <button onClick={()=>handleMode("swipe")} style={{background:mode==="swipe"?"rgba(232,67,147,0.13)":"#1a1a2e",border:mode==="swipe"?"1px solid rgba(232,67,147,0.27)":"1px solid #2a2340",borderRadius:10,padding:"7px 8px",color:mode==="swipe"?"#e84393":"#b0a8b8",cursor:"pointer",fontSize:11,fontFamily:"'DM Sans'",fontWeight:700}}>💘 Swipe</button>
          <button onClick={e=>{e.stopPropagation();handleRefresh();}} style={{background:"#1a1a2e",border:"1px solid #2a2340",borderRadius:10,padding:"7px 8px",color:"#888",cursor:"pointer",fontSize:11}}>🔄</button>
        </div>
      </div>
      {mode==="top5"&&(
        <div style={{padding:"0 14px 16px"}}>
          ...
          {!loading&&recs&&recs.map((rec,i)=>{
            ...
            return(
              <div key={i} style={{borderRadius:16,overflow:"hidden",marginBottom:10,position:"relative",background:"#0d0d18",border:"1px solid #1e1e30"}}>
                {/* Main card - clickable for details */}
                <div onClick={()=>onSelect(tmdb||{title:rec.title,name:rec.title,overview:rec.reason,vote_average:0,genre_ids:[],poster_path:null,media_type:browseType==="serie"?"tv":"movie",release_date:rec.year||""})}
                  ...
                  <div style={{position:"absolute",inset:0,display:"flex",alignItems:"center",gap:12,padding:"0 14px"}}>
                    ...
                    {poster?<img src={poster} alt="" style={{width:44,height:66,borderRadius:8,objectFit:"cover",flexShrink:0,border:"1px solid rgba(255,255,255,0.08)",boxShadow:"0 4px 12px #000"}}/>:
                    ...
                    <div style={{flex:1,minWidth:0}}>
                      ...
                      {rec.reason&&<p style={{fontSize:11,color:"#a09aaa",margin:0,lineHeight:1.3,fontStyle:"italic",display:"-webkit-box",WebkitLineClamp:1,WebkitBoxOrient:"vertical",overflow:"hidden"}}>"{rec.reason}"</p>}
                    </div>
                  </div>
                </div>
                {/* Swipe hint + Nicht mein Ding */}
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"4px 14px 6px",borderTop:"1px solid rgba(255,255,255,0.04)"}}>
                  <span style={{fontSize:9,color:"#333",fontFamily:"'DM Sans'"}}>← wischen = nicht mein Ding</span>
                  <button onClick={e=>{e.stopPropagation();onBlock(rec.title);setRecs(prev=>prev.filter((_,j)=>j!==i));}}
                    style={{background:"transparent",border:"none",color:"#444",cursor:"pointer",fontFamily:"'DM Sans'",fontSize:10,padding:"2px 0",display:"flex",alignItems:"center",gap:3}}>
                    👻 Nicht mein Ding
                  </button>
                </div>
              </div>
...
const MEDIATHEK_CHANNELS=[
  {id:"ard",  name:"ARD Mediathek", color:"#004E8A", icon:"📡", channel:"ARD",  url:"https://www.ardmediathek.de"},
  {id:"zdf",  name:"ZDF Mediathek", color:"#FA7D19", icon:"📺", channel:"ZDF",  url:"https://www.zdf.de"},
  {id:"arte", name:"Arte",          color:"#C8102E", icon:"🎨", channel:"ARTE", url:"https://www.arte.tv/de/"},
  {id:"br",   name:"BR Mediathek",  color:"#009FE3", icon:"📡", channel:"BR",   url:"https://www.br.de/mediathek"},
  {id:"3sat", name:"3sat",          color:"#666",    icon:"3️⃣", channel:"3sat", url:"https://www.3sat.de"},
  {id:"mdr",  name:"MDR Mediathek", color:"#006633", icon:"📡", channel:"MDR",  url:"https://www.mdr.de/mediathek"},
];

function MediathekChannelCard({ch,profile,onSelect,onBlock}){
  const [items,setItems]=useState([]);
  const [loading,setLoading]=useState(false);
  ...
  async function load(){
    ...
    setLoading(true);
    try{
      const results=await getMediathekChannel(ch.channel);
      // Filter: min 20min, kein Trailer/Teaser
      const filtered=results.filter(r=>
        (r.duration||0)>1200&&
        !/(trailer|teaser|vorschau|clip|kurz)/i.test(r.title)
      ).slice(0,8);
      setItems(filtered);
    }catch(e){setItems([]);}
    setLoading(false);
  ...
  return(
    <div style={{background:"#12121f",borderRadius:16,overflow:"hidden",border:"1px solid #1e1e30",marginBottom:8}}>
      <div onClick={handleOpen} style={{padding:"14px 16px",display:"flex",alignItems:"center",gap:12,cursor:"pointer"}}>
        <div style={{width:40,height:40,borderRadius:10,background:ch.color+"22",border:"1px solid "+ch.color+"44",display:"flex",alignItems:"center",justifyContent:"center",fontSize:16,flexShrink:0}}>{ch.icon}</div>
        <div style={{flex:1}}>
          <div style={{fontSize:14,fontWeight:800,color:"#f0ece4"}}>{ch.name}</div>
          <div style={{fontSize:10,color:"#b0a8b8"}}>Aktuelle Sendungen</div>
        </div>
        <div style={{color:"#3a3344",fontSize:14,transition:"transform 0.3s",transform:open?"rotate(180deg)":"rotate(0deg)"}}>▾</div>
      </div>

      {open&&(
        <div style={{borderTop:"1px solid #1e1e30",padding:"10px 14px 14px"}}>
          {loading&&<div style={{textAlign:"center",padding:12}}><div style={{fontSize:18,animation:"spin 1.5s linear infinite",display:"inline-block"}}>📡</div><p style={{fontSize:11,color:"#b0a8b8",marginTop:6}}>Lade aktuelle Sendungen…</p></div>}
          {!loading&&items.length===0&&(
            <div style={{textAlign:"center",padding:12}}>
              <p style={{fontSize:12,color:"#b0a8b8",marginBottom:8}}>Keine aktuellen Sendungen gefunden</p>
              <a href={ch.url} target="_blank" rel="noopener noreferrer"
                style={{background:ch.color+"22",border:"1px solid "+ch.color+"44",borderRadius:10,padding:"8px 14px",color:ch.color,textDecoration:"none",fontSize:12,fontWeight:700,display:"inline-block"}}>
                Zur {ch.name} →
              </a>
            </div>
          )}
          {!loading&&items.map((item,i)=>{
            const dur=item.duration?Math.round(item.duration/60)+"min":"";
            const date=item.timestamp?new Date(item.timestamp*1000).toLocaleDateString("de-DE",{day:"2-digit",month:"2-digit"}):"";
            return(
              <div key={i} style={{marginBottom:8}}>
                <div onClick={()=>onSelect({
                    id:item.id||i,title:item.title||"",name:item.title||"",
                    overview:item.description||item.topic||"",
                    vote_average:0,genre_ids:[99],poster_path:null,
                    media_type:"tv",_mediathekUrl:item.url_video_hd||item.url_video||item.url,
                    _mediathekThumb:item.url_thumbnail,_channel:ch.name,
                  })}
                  style={{background:"#0d0d18",borderRadius:12,padding:"10px 12px",cursor:"pointer",border:"1px solid #1e1e30",display:"flex",gap:10,alignItems:"flex-start"}}>
                  {item.url_thumbnail?(
                    <img src={item.url_thumbnail} alt="" style={{width:80,height:50,borderRadius:8,objectFit:"cover",flexShrink:0}}/>
                  ):(
                    <div style={{width:80,height:50,borderRadius:8,background:ch.color+"22",flexShrink:0,display:"flex",alignItems:"center",justifyContent:"center",fontSize:20}}>{ch.icon}</div>
                  )}
                  <div style={{flex:1,minWidth:0}}>
                    <div style={{fontSize:12,fontWeight:700,color:"#f0ece4",marginBottom:2,lineHeight:1.3}}>{item.title}</div>
                    <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
                      {item.topic&&<span style={{fontSize:10,color:ch.color,fontWeight:600}}>{item.topic}</span>}
                      {dur&&<span style={{fontSize:10,color:"#888"}}>{dur}</span>}
                      {date&&<span style={{fontSize:10,color:"#888"}}>{date}</span>}
                    </div>
                  </div>
                </div>
                <button onClick={()=>onBlock(item.title||"")}
                  style={{background:"transparent",border:"none",color:"#444",cursor:"pointer",fontSize:10,fontFamily:"'DM Sans'",padding:"3px 12px",display:"block",width:"100%",textAlign:"left"}}>
                  👻 Nicht mein Ding
                </button>
              </div>
            );
          ...
          {!loading&&items.length>0&&(
            <a href={ch.url} target="_blank" rel="noopener noreferrer"
              style={{display:"block",textAlign:"center",marginTop:4,color:ch.color,fontSize:11,fontWeight:700,textDecoration:"none",opacity:0.7}}>
              Mehr auf {ch.name} →
            </a>
          )}
        </div>
...
function MediathekenSection({profile,onSelect,onBlock,onLike,browseType}){
  ...
  return(
    <div style={{marginBottom:10}}>
      <button onClick={()=>setOpen(!open)} style={{width:"100%",background:"linear-gradient(135deg,rgba(0,78,138,0.13),rgba(250,125,25,0.13))",border:"1px solid rgba(0,78,138,0.27)",borderRadius:18,padding:"14px 16px",cursor:"pointer",display:"flex",alignItems:"center",gap:12}}>
        <div style={{width:44,height:44,borderRadius:12,background:"linear-gradient(135deg,rgba(0,78,138,0.2),rgba(250,125,25,0.13))",display:"flex",alignItems:"center",justifyContent:"center",fontSize:20,flexShrink:0}}>📡</div>
      ...
      {open&&(
        <div style={{paddingTop:8}}>
          {MEDIATHEK_CHANNELS.map(ch=>(
            <MediathekChannelCard key={ch.id} ch={ch} profile={profile} onSelect={onSelect} onBlock={onBlock}/>
          ))}
        </div>
      )}
    </div>
        ...
        {step===0&&(
          <div style={{animation:"fadeUp 0.4s ease"}}>
            ...
            <button onClick={()=>setPlats(s=>s.includes("mediatheken")?s.filter(x=>x!=="mediatheken"):[...s,"mediatheken"])}
              style={{width:"100%",background:plats.includes("mediatheken")?"linear-gradient(135deg,rgba(0,78,138,0.2),rgba(250,125,25,0.13))":"#12121f",border:"1px solid "+(plats.includes("mediatheken")?"rgba(0,78,138,0.53)":"#1e1e30"),borderRadius:16,padding:"13px 16px",cursor:"pointer",display:"flex",alignItems:"center",gap:12,marginBottom:24,transition:"all 0.2s"}}>
                    ...
                    return(
                      <div key={i} onClick={()=>onSelect(it)}
                        ...
                        <div style={{flex:1,minWidth:0}}>
                          ...
                          <button onClick={e=>{e.stopPropagation();cardProps.onBlock(it.title||it.name||"");}} style={{background:"transparent",border:"none",color:"#555",cursor:"pointer",fontSize:10,fontFamily:"'DM Sans'",padding:"4px 0 0",display:"flex",alignItems:"center",gap:3}}>👻 Nicht mein Ding</button>
                        </div>
                      </div>
                    );
                  })}
...
function MainApp({profile:initProfile,onReset}){
  ...
  function applyMood(mood){
    ...
    const genreStr=mood.genres.join("|"); // OR — mindestens ein Genre muss passen
    setFeedLoading(true);
    Promise.all([
      discoverTitles("serie",allIds,genreStr,1,"vote_average.desc"),
      discoverTitles("film",allIds,genreStr,1,"vote_average.desc"),
    ...
    <div style={{minHeight:"100vh",background:"#09090f",color:"#f0ece4",fontFamily:"'DM Sans',sans-serif",paddingBottom:90}}>
      ...
      {showSettings&&(
        <div style={{margin:"10px 18px",padding:14,background:"#12121f",borderRadius:14,border:"1px solid #1e1e30"}}>
          ...
          <button onClick={onReset} style={{background:"rgba(229,9,20,0.09)",border:"1px solid rgba(229,9,20,0.27)",borderRadius:10,padding:"9px 18px",color:"#E50914",cursor:"pointer",fontFamily:"'DM Sans'",fontWeight:600,fontSize:12}}>🔄 Reset</button>
        </div>
      )}
          ...
          <div>
            <div style={{padding:"0 18px",marginBottom:14}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
                ...
                <button onClick={doAiRefresh} disabled={aiRefreshing} style={{background:"linear-gradient(135deg,rgba(255,107,53,0.13),rgba(232,67,147,0.13))",border:"1px solid rgba(255,107,53,0.19)",borderRadius:10,padding:"6px 14px",color:"#ff6b35",cursor:aiRefreshing?"default":"pointer",fontSize:11,fontFamily:"'DM Sans'",fontWeight:700,display:"flex",alignItems:"center",gap:6,opacity:aiRefreshing?0.7:1}}>
        ...
        {tab==="fun"&&(
          <div style={{padding:"0 18px"}}>
            ...
            <div style={{background:"linear-gradient(135deg,#1a1525,#13101e)",borderRadius:20,padding:20,marginBottom:16,border:"1px solid #2a1f3d",textAlign:"center"}}>
              ...
              {surpriseError&&!surpriseLoading&&<div style={{background:"rgba(239,68,68,0.09)",border:"1px solid rgba(239,68,68,0.27)",borderRadius:12,padding:12,marginBottom:10}}><p style={{fontSize:12,color:"#ef4444",margin:"0 0 8px"}}>⚠️ {surpriseError}</p><button onClick={doSurprise} style={{background:"rgba(255,107,53,0.13)",border:"1px solid rgba(255,107,53,0.27)",borderRadius:10,padding:"8px 16px",color:"#ff6b35",cursor:"pointer",fontSize:12,fontFamily:"'DM Sans'",fontWeight:700}}>🔄 Nochmal versuchen</button></div>}
            ...
            <div style={{background:"linear-gradient(135deg,#1a1020,#130818)",borderRadius:20,padding:20,marginBottom:16,border:"1px solid #3d1f5a",textAlign:"center"}}>
              ...
              {oracleError&&!oracleLoading&&<div style={{background:"rgba(124,58,237,0.09)",border:"1px solid rgba(124,58,237,0.27)",borderRadius:12,padding:12,marginBottom:10}}><p style={{fontSize:12,color:"#a78bfa",margin:"0 0 8px"}}>⚠️ {oracleError}</p><button onClick={doOracle} style={{background:"rgba(124,58,237,0.13)",border:"1px solid rgba(124,58,237,0.27)",borderRadius:10,padding:"8px 16px",color:"#a78bfa",cursor:"pointer",fontSize:12,fontFamily:"'DM Sans'",fontWeight:700}}>🔄 Nochmal versuchen</button></div>}
        ...
        {tab==="history"&&(
          <div style={{padding:"0 18px"}}>
            ...
            {(profile.blocked_titles||[]).length>0&&(
              <div style={{background:"#12121f",borderRadius:14,padding:14,border:"1px solid rgba(239,68,68,0.2)",marginBottom:14}}>
                ...
                <div style={{display:"flex",flexWrap:"wrap",gap:6}}>
                  {(profile.blocked_titles||[]).map(t=>(
                    <button key={t} onClick={()=>handleBlock(t)} style={{background:"rgba(239,68,68,0.09)",border:"1px solid rgba(239,68,68,0.27)",borderRadius:8,padding:"4px 10px",color:"#ef4444",cursor:"pointer",fontSize:11,fontFamily:"'DM Sans'"}}>{t} ✕</button>
                  ))}
                </div>
              </div>
            )}
