...
function SpotlightTab({profile,cardProps,onSelect}){
  ...
  // Lade TMDB-Bilder für Kacheln beim ersten Öffnen
  useEffect(()=>{
    const ratings=profile.ratings||{};
    const watched=profile.watched||[];
    // Top bewertete Titel mit Bildern bevorzugen
    const topRated=watched
      .filter(w=>(ratings[titleKey(w.title||"")]||0)>=3)
      .slice(0,6);
    const toLoad=topRated.length>=3?topRated:watched.slice(0,6);
    if(toLoad.length===0)return;
    // Pro Kachel einen anderen Titel
    Promise.all(toLoad.slice(0,3).map(async(w,i)=>{
      try{
        const res=await fetch(TMDB_BASE+"/search/multi?api_key="+TMDB_API_KEY+"&query="+encodeURIComponent(w.title||"")).then(r=>r.json());
        const found=(res.results||[])[0];
        if(found?.backdrop_path)return TMDB_IMG+found.backdrop_path;
        if(found?.poster_path)return TMDB_IMG+found.poster_path;
        return null;
      }catch{return null;}
    })).then(imgs=>setBgImgs(imgs));
  },[]);

  const tiles=[
    {id:"deepdive",title:"Sag mir was mir gefällt…",hint:"Nenn uns einen Lieblingstitel → wir bauen dein persönliches Universum",color:"#c4a960",gradient:"linear-gradient(160deg,rgba(20,18,10,0.92),rgba(15,14,20,0.8))",border:"rgba(196,169,96,0.2)"},
    {id:"vibe",title:"Vibe-Meter",hint:"Düster & intensiv? Leicht & lustig? Wir finden den Titel für deine Stimmung",color:"#e84393",gradient:"linear-gradient(160deg,rgba(20,10,18,0.92),rgba(15,8,20,0.8))",border:"rgba(232,67,147,0.2)"},
    {id:"bot",title:"Frag den Popcorn-Guru",hint:"Stell jede Frage — dein persönlicher Streaming-Experte antwortet",color:"#4ade80",gradient:"linear-gradient(160deg,rgba(10,20,15,0.92),rgba(8,15,12,0.8))",border:"rgba(74,222,128,0.2)"},
  ];
  ...
  return(
    <div style={{display:"flex",flexDirection:"column",height:"calc(100vh - 130px)",overflow:"hidden"}}>
      ...
      <div style={{flexShrink:0,padding:"14px 18px 10px",display:"flex",alignItems:"center",gap:12}}>
        <div style={{width:44,height:44,borderRadius:13,background:"linear-gradient(135deg,#c4a960,#ff6b35)",display:"flex",alignItems:"center",justifyContent:"center",boxShadow:"0 4px 18px rgba(196,169,96,0.4)",flexShrink:0}}>
      ...
      {/* Mosaic — flex füllt automatisch den Rest */}
      <div style={{flex:1,display:"flex",flexDirection:"column",gap:8,padding:"0 12px 8px",minHeight:0}}>

        {/* Row 1 — Deep Dive, volle Breite */}
        <button onClick={()=>setActiveFeature("deepdive")}
          style={{flex:"2",background:tiles[0].gradient,border:"1px solid "+tiles[0].border,borderRadius:20,cursor:"pointer",textAlign:"left",position:"relative",overflow:"hidden",minHeight:0}}>
          {bgImgs[0]&&<img src={bgImgs[0]} alt="" style={{position:"absolute",inset:0,width:"100%",height:"100%",objectFit:"cover",opacity:0.25,transition:"opacity 0.5s"}}/>}
          <div style={{position:"absolute",inset:0,background:"linear-gradient(135deg,rgba(0,0,0,0.88) 0%,rgba(0,0,0,0.45) 100%)"}}/>
          <div style={{position:"absolute",top:-40,right:-40,width:160,height:160,background:"radial-gradient(circle,"+tiles[0].color+"25,transparent 70%)",pointerEvents:"none"}}/>
          <div style={{position:"relative",padding:"18px 18px 14px",height:"100%",display:"flex",flexDirection:"column",justifyContent:"space-between"}}>
            <div style={{width:46,height:46,borderRadius:13,background:tiles[0].color+"20",border:"1px solid "+tiles[0].color+"40",display:"flex",alignItems:"center",justifyContent:"center",boxShadow:"0 0 20px "+tiles[0].color+"30"}}>
              {SpotlightIcons.deepdive(tiles[0].color,24)}
            </div>
            <div>
              <div style={{fontSize:17,fontWeight:800,color:"#f0ece4",marginBottom:5,letterSpacing:"-0.2px"}}>{tiles[0].title}</div>
              <div style={{fontSize:12,color:"rgba(255,255,255,0.55)",lineHeight:1.5}}>{tiles[0].hint}</div>
            </div>
          </div>
          <div style={{position:"absolute",bottom:14,right:14,width:30,height:30,borderRadius:9,background:tiles[0].color+"20",border:"1px solid "+tiles[0].color+"40",display:"flex",alignItems:"center",justifyContent:"center"}}>
            <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke={tiles[0].color} strokeWidth="2.5" strokeLinecap="round"><polyline points="9 18 15 12 9 6"/></svg>
          </div>
        </button>

        {/* Row 2 — Vibe + Guru, gleich groß */}
        <div style={{flex:"1.5",display:"flex",gap:8,minHeight:0}}>
          {tiles.slice(1).map((t,ri)=>(
            <button key={t.id} onClick={()=>setActiveFeature(t.id)}
              style={{flex:1,background:t.gradient,border:"1px solid "+t.border,borderRadius:20,cursor:"pointer",textAlign:"left",position:"relative",overflow:"hidden",minHeight:0}}>
              {bgImgs[ri+1]&&<img src={bgImgs[ri+1]} alt="" style={{position:"absolute",inset:0,width:"100%",height:"100%",objectFit:"cover",opacity:0.2,transition:"opacity 0.5s"}}/>}
              <div style={{position:"absolute",inset:0,background:"linear-gradient(135deg,rgba(0,0,0,0.9),rgba(0,0,0,0.55))"}}/>
              ...
              <div style={{position:"relative",padding:"14px 12px",height:"100%",display:"flex",flexDirection:"column",justifyContent:"space-between"}}>
                <div style={{width:40,height:40,borderRadius:12,background:t.color+"22",border:"1px solid "+t.color+"40",display:"flex",alignItems:"center",justifyContent:"center",boxShadow:"0 0 16px "+t.color+"28"}}>
                  {SpotlightIcons[t.id]?.(t.color,21)}
                </div>
                <div>
                  <div style={{fontSize:13,fontWeight:800,color:"#f0ece4",marginBottom:4,lineHeight:1.2}}>{t.title}</div>
                  <div style={{fontSize:10,color:"rgba(255,255,255,0.45)",lineHeight:1.4}}>{t.hint}</div>
                </div>
              </div>
            </button>
          ))}
        ...
        <div style={{flex:"1",display:"flex",gap:8,minHeight:0}}>
          {extras.map(e=>(
            <button key={e.id} onClick={()=>setActiveFeature(e.id)}
              style={{flex:1,background:"rgba(255,255,255,0.03)",border:"1px solid rgba(255,255,255,0.08)",borderRadius:18,cursor:"pointer",textAlign:"center",position:"relative",overflow:"hidden",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:6,padding:"10px 6px"}}>
              <div style={{position:"absolute",top:-15,left:"50%",transform:"translateX(-50%)",width:70,height:70,background:"radial-gradient(circle,"+e.color+"12,transparent 70%)",pointerEvents:"none"}}/>
              <div style={{width:38,height:38,borderRadius:11,background:e.color+"15",border:"1px solid "+e.color+"28",display:"flex",alignItems:"center",justifyContent:"center",boxShadow:"0 0 12px "+e.color+"18"}}>
                {SpotlightIcons[e.icon]?.(e.color,20)}
              </div>
              <div style={{fontSize:10,fontWeight:800,color:"#d0c8d8",lineHeight:1.2,textAlign:"center"}}>{e.title}</div>
              <div style={{fontSize:8,color:"rgba(255,255,255,0.28)",lineHeight:1.3,textAlign:"center"}}>{e.hint}</div>
            </button>
          ))}
        </div>
